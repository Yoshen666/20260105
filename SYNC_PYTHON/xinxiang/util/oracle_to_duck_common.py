...
from xinxiang.util import my_oracle, my_date, my_cmder, my_duck
import pandas as pd


def sync_oracle_to_duck_by_csv_operate(etl_name,
                                       source_table,
                                       target_table,
                                       query_sql,
    ...
    try:
        oracle_to_duck_csv_operate(conn, etl_name, source_table, target_table, query_sql,
                                   target_path=target_path, create_table_sql=create_table_sql)
...
def sync_oracle_to_duck_by_csv(etl_name,
    ...
    conn = my_oracle.oracle_get_connection()
    try:
        oracle_to_duck_csv(conn, etl_name, source_table, target_table, query_sql,
                           target_path=target_path, create_table_sql=create_table_sql)
    except Exception as e:
        logging.info(">>> Retry to Sync {source_table} to {target_table}".format(source_table=source_table,
                                                                                target_table=target_table))
...
# 所有的字段都转成varchar，因为有很多数字类型的存在问题
def sync_oracle_to_duck_by_csv_varchar(etl_name,
                               source_table,
                               target_table,
                               query_sql,
                               target_path=config.g_mem_sync_result_path,
    ...
    conn = my_oracle.oracle_get_connection()
    try:
        oracle_to_duck_csv_varchar(conn, etl_name, source_table, target_table, query_sql,
                           target_path=target_path, create_table_sql=create_table_sql)
    except Exception as e:
        logging.info(">>> Retry to Sync {source_table} to {target_table}".format(source_table=source_table,
                                                                                 target_table=target_table))
...
def oracle_to_duck_csv(conn,
    ...
    _csv_file_name = target_table_name + "_" + my_date.date_time_second_short_str() + ".csv"
    ...
    target_csv_file = os.path.join(target_path, target_table_name, _csv_file_name)  # 最终文件在inprocess目录上层
    ...
    try:
        ...
        # 检查Oracle中是否存在数据
        if not my_oracle.ExecuteViewToTable(conn, source_table):
            return

        # 创建DuckDB文件
        duck_db_cursor = duckdb.connect(in_process_db_file)
        duck_db_cursor.execute(create_table_sql)
        # 读取Oracle中的数据
        start = datetime.now()
        ...
        count_sql = """
               select count(1) as rn from read_csv_auto('{target_csv_file}', delim='[@@]', header=True)
               """.format(target_table=target_table, target_csv_file=target_csv_file)
        # 防止sqluload出现数据为0，并保存版本的情况
        if not my_duck.doubleCheckView(duck_db_cursor, count_sql, conn, source_table):
            return
        ...
        if os.path.exists(target_csv_file):
            os.remove(target_csv_file)
        my_oracle.EndCleanUpAndLog(conn=conn, ETLProcName=etl_name, ETLStartTime=_now)

# 所有导出的字段都转成String类型
def oracle_to_duck_csv_varchar(conn,
                       etl_name,
                       source_table,
                       target_table,
                       query_sql,
    ...
    try:
        ...
        insert_sql = """
        insert into {target_table} select * from read_csv_auto('{target_csv_file}', delim='[@@]', header=True, all_varchar=True)
        """.format(target_table=target_table, target_csv_file=target_csv_file)
...
def oracle_to_duck_csv_operate(conn,
                               etl_name,
                               source_table,
                               target_table,
                               query_sql,
    ...
    try:
        ...
        insert_sql = """
        insert into {target_table} select * from read_csv('{target_csv_file}', delim='[@@]', header=True,columns= 
        """.format(target_table=target_table, target_csv_file=target_csv_file) + """{'PRODSPEC_ID':'VARCHAR','PR_OP_NO':'VARCHAR','PR_UNIT_ID':'VARCHAR','OP_NO_1':'VARCHAR','UNIT_ID_1':'VARCHAR','OP_NO_2':'VARCHAR','UNIT_ID_2':'VARCHAR','MASK_NM':'VARCHAR','PA_FLAG':'VARCHAR','JOB_NO':'VARCHAR','MAX_EFCT_TIME':'VARCHAR(20)','REMAIN_DAY':'VARCHAR','FOURTH_VAL_FLG':'VARCHAR','ERR_FLG':'VARCHAR'})"""
        duck_db_cursor.execute(insert_sql)
        duck_db_cursor.close()
...
def _base_oracle_to_duck(conn, etl_name, source_table, target_table, where_start_sql=None, target_path=config.g_mem_sync_result_path, create_table_sql=None):
    ...
    try:
        ...
        duck_db_cursor.sql("SET memory_limit ='40GB'")
        # 读取Oracle中的数据
        start = datetime.now()
        ...
        # 如果数据为0，则还需要看下skip表里是否配置
        if result_count == 0:
            if not my_oracle.ExecuteViewToSpecialTable(conn, source_table):
                return
        ...
        for col, data_type in pd_result.dtypes.items():
            if "datetime64[ns]" == str(data_type):
                pd_result[col].fillna('', inplace=True)
