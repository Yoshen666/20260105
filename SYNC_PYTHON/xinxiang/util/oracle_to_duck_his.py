...
def delete_over_three_version(table_name, curr_file_name):
    path = ""
    sorted_file_names = []
                ...
                if len(_sorted) > 3:
                    for item in _sorted[0:-3]:
                        if os.path.exists(os.path.join(path, item)):
                            ...
                            # 当前最新的文件不能删除
                            if os.path.join(path, item) == curr_file_name:
                                continue
...
def delete_over_three_version_masterandsalve(table_name, curr_file_name, g_mem_sync_result_path, g_mem_etl_output_path):
    path = ""
    sorted_file_names = []
    for i in range(len(g_mem_etl_output_path)):
        try:
            if os.path.isdir(os.path.join(g_mem_sync_result_path[i], table_name)):
                path = os.path.join(g_mem_sync_result_path[i], table_name)
                file_names = os.listdir(os.path.join(g_mem_sync_result_path[i], table_name))
                if file_names.__contains__('inprocess'):
                    file_names.remove('inprocess')
                for file_name in file_names:
                    if file_name.endswith('.db.wal'):
                    ...
                    if file_name.endswith('.csv'):
                        file_names.remove(file_name)
                sorted_file_names = sorted(file_names)
            elif os.path.isdir(os.path.join(g_mem_etl_output_path[i], table_name)):
                path = os.path.join(g_mem_etl_output_path[i], table_name)
                file_names = os.listdir(os.path.join(g_mem_etl_output_path[i], table_name))
                if file_names.__contains__('inprocess'):
                    file_names.remove('inprocess')
                for file_name in file_names:
                    if file_name.endswith('.db.wal'):
