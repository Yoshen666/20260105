...
def ExecuteViewToSpecialTable(conn, tableName=""):

    # 如果还没有的话直接报错,但必须在v_setting_view_nullskip外
...
def SaveAlarmLogDataForTmp(conn, etl_proc_name, alarm_code, exception):
    if config.g_debug_mode:
        conn = oracle_get_connection_local()
    hostname = socket.gethostname()
    hostAddress = socket.gethostbyname(hostname)
    msg = etl_proc_name + ' JOB异常： ' + exception
    job_msg = etl_proc_name + ' JOB:' + exception + ", HostName : " + hostname + ", HostAddress : " + hostAddress
    # print(my_runner.judge_main_server(conn))
    if not my_runner.judge_main_server(conn):
        sql = """
                insert into alarm_send_log
                                   ...
                                   select s.code,
                                     ...
                                     from set_alarm_code_mapping s
                                    where s.code =  '{alarm_code}'
                """.format(job_msg=job_msg, alarm_code=alarm_code, msg=msg)
    else:
        sql = """
                insert into alarm_send_log
                                   (code,
