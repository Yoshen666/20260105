...
from xinxiang.util import my_postgres, my_date, my_oracle

import pandas as pd

def export_to_test_APS_ETL_STATIC_CONS(table_name, file_name, id):
    ...
    postgres_table_define = """etl_static_cons(parentid,rule_name, action, toolg_id, tool_id, ch_id, ppid, rule_seq, 
                                                      rls_attr1, prod_id, layer, stage, remark, update_time)
                                        """
...
def export_to_test_APS_ETL_MERGE_CONS(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_DEMAND(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
    ...
    postgres_table_define = """etl_demand(parentid,toolg_id,prodg_id,prodg_tech,prod_id,plan_id,step_id,tool_id,lot_type,   
                                                      ...
                                                      target_time_from,target_time,target_mode,update_time,control_flag,wip_attr2)
                                        """
    my_postgres.copy_duckdb_to_postgres_for_test_pg(uuid=uuid,
                                                    duckdb=duck_db_memory,
                                                    table_name_in_duckdb=table_name,
...
def export_to_test_APS_ETL_FLOW(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
    ...
    postgres_table_define = """etl_flow(parentid, prodg_id, prodg_tech,prod_id,plan_no,plan_id, step_id,layer,stage,toolg_id,recipe,reticle_group,step_type,toolg_type,process_time,
                                                    cycle_time,capability,sgs_flag,update_time,sgs_group,batch_name, batch_target_flag,multi_group,multi_target_flag, SKIP_FLAG,
...
def export_to_test_APS_ETL_HOLD_RLS(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_LOT_OP_HIST(table_name, file_name):
    ...
    pg_conn = my_postgres.decide_postgres_db_connection(export_to_pg_prod_test=True,
                                                        pg_table_name="etl_lot_op_hist")
    sql_key_in_pg = """
                                SELECT DISTINCT LOT_ID, TOOLG_ID, TOOL_ID, PROD_ID,PLAN_ID, TRACK_IN
                                FROM yth.etl_lot_op_hist
                                where track_in >= timestamp '{current_time}' - INTERVAL '5 day'
    ...
    select_sql_in_duck = """select  lot_id, 
                                                ...
                                                update_time,
                                                CASE WHEN FORMBATCH_TIME='' THEN NULL ELSE FORMBATCH_TIME END AS FORMBATCH_TIME 
                                          FROM APS_ETL_LOT_OP_HIST t
                                          WHERE track_in >= DATE_TRUNC('second', CAST('{current_time}' AS TIMESTAMP)) - INTERVAL '4 day'
                                          AND NOT EXISTS (
                                                SELECT 1 
    ...
    postgres_table_define = """etl_lot_op_hist(lot_id, toolg_id, tool_id, prod_id, plan_id,step_id,lot_type, pty,shr_flag,layer, stage,recipe,ppid,
                                                                        qty,reticle_id,arrival,job_prepare,track_in,process_start,update_time,FORMBATCH_TIME)
                                              """
    my_postgres.copy_duckdb_to_postgres_for_test_pg(uuid=uuid,
                                                    duckdb=duck_db_memory,
                                                    table_name_in_duckdb=table_name,
...
def export_to_test_APS_ETL_LOTHISTORY(table_name, file_name):
    ...
    sql_key_in_pg = """
                                ...
                                where track_out >= timestamp '{current_time}' - INTERVAL '5 day'
                            """.format(current_time=current_time)
    pg_exist_result = pd.read_sql(sql_key_in_pg, pg_conn)
    print(pg_exist_result)
    ...
    select_sql_in_duck = """select  lot_id, toolg_id, tool_id, prod_id, plan_id,
                                            ...
                                            WHERE CAST(track_out AS TIMESTAMP) >= DATE_TRUNC('second', CAST('{current_time}' AS TIMESTAMP)) - INTERVAL '4 day'
                                    AND NOT EXISTS(
                                        select 1 
                                        from pg_exist_result pd
                                        where t.LOT_ID = pd.LOT_ID
    ...
    postgres_table_define = """etl_lothistory(lot_id, toolg_id, tool_id, prod_id, plan_id,step_id, prodg_id,prodg_tech,lot_type, pty,shr_flag,layer, stage,recipe,ppid,
                                                                      ...
                                                                      backup_flag,update_time, scanner_start,scanner_end, recipe_setup, ch_set,wip_attr2 )
                                                     """
    my_postgres.copy_duckdb_to_postgres_for_test_pg(uuid=uuid,
                                                    duckdb=duck_db_memory,
                                                    table_name_in_duckdb=table_name,
...
def export_to_test_APS_ETL_MASK_HISTORY(table_name, file_name):
    ...
    sql_key_in_pg = """
                    ...
                    where event_time >= timestamp '{current_time}' - INTERVAL '5 day'
    ...
    select_sql_in_duck = """select 
                        ...
                        WHERE event_time >= DATE_TRUNC('second', CAST('{current_time}' AS TIMESTAMP)) - INTERVAL '4 day'
                        AND NOT EXISTS(
                            select 1 
                            from pg_exist_result pd
...
def export_to_test_APS_ETL_MASK_INFO(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_MONITOR(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_PIRUN(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_QTIME_HOLD(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_QTIME_SPEC(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_RLS(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
    ...
    postgres_table_define = """etl_rls(parentid,lot_id,target_step_id,target_plan_id,toolg_id,tool_id,recipe,ppid,reticle_id,ch_set,
                                                   ...
                                                   other_min_batch_size,recipe_setup,target_OPE_NO,rls_attr1)
                                        """
    my_postgres.copy_duckdb_to_postgres_for_test_pg(uuid=uuid,
                                                    duckdb=duck_db_memory,
                                                    table_name_in_duckdb=table_name,
...
def export_to_test_APS_ETL_SAMPLING_LOT(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_SEASON_RULE(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
    ...
    select_sql_in_duck = """select  CASE WHEN parentid='' THEN NULL ELSE parentid END AS parentid,
                                                ...
                                                CASE WHEN rule_name='' THEN NULL ELSE rule_name END AS rule_name,
                                                CASE WHEN reserved_lp='' THEN NULL ELSE reserved_lp END AS reserved_lp,
                                                CASE WHEN ORG_SETUP_FLAG IS NULL THEN '0' ELSE ORG_SETUP_FLAG END AS ORG_SETUP_FLAG 
    ...
    postgres_table_define = """etl_season_rule( parentid,toolg_id,tool_id,ch_id,seq,process_time,lot_type,recipe_last,recipe_last_mode,
                                                            ...
                                                            stage_next_mode,consec_unit,consec_cnt,consec_cnt_mode,rule_name,reserved_lp,org_setup_flag)
                                        """
    my_postgres.copy_duckdb_to_postgres_for_test_pg(uuid=uuid,
                                                    duckdb=duck_db_memory,
                                                    table_name_in_duckdb=table_name,
...
def export_to_test_APS_ETL_SGS_RLS(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
    ...
    select_sql_in_duck = """select  CASE WHEN parentid='' THEN NULL ELSE parentid END AS parentid,
                                                ...
                                                CASE WHEN stage='' THEN NULL ELSE stage END AS stage,
                                                CASE WHEN tool_id='' THEN NULL ELSE tool_id END AS tool_id,
                                                CASE WHEN create_time='' THEN NULL ELSE create_time END AS create_time,
                                                CASE WHEN pass_flg_prc='' THEN NULL ELSE pass_flg_prc END AS pass_flg_prc,
                                                CASE WHEN prc_update_time='' THEN NULL ELSE prc_update_time END AS prc_update_time,
                                                CASE WHEN pass_flg_mfg='' THEN NULL ELSE pass_flg_mfg END AS pass_flg_mfg,
                                                CASE WHEN mfg_update_time='' THEN NULL ELSE mfg_update_time END AS mfg_update_time,
                                                CASE WHEN update_time='' THEN NULL ELSE update_time END AS update_time 
                                            from APS_ETL_SGS_RLS
                                            """
...
def export_to_test_APS_ETL_SGS_RULE(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
    ...
    select_sql_in_duck = """select  parentid,
                                                CASE WHEN tool_id='' THEN NULL ELSE tool_id END AS tool_id,
                                                CASE WHEN prodg_id='' THEN NULL ELSE prodg_id END AS prodg_id,
                                                CASE WHEN from_layer='' THEN NULL ELSE from_layer END AS from_layer,
                                                CASE WHEN from_stage='' THEN NULL ELSE from_stage END AS from_stage,
                                                CASE WHEN to_layer='' THEN NULL ELSE to_layer END AS to_layer,
                                                CASE WHEN to_stage='' THEN NULL ELSE to_stage END AS to_stage,
                                                CASE WHEN update_time='' THEN NULL ELSE update_time END AS update_time   
                                                from APS_ETL_SGS_RULE
                                     """
...
def export_to_test_APS_ETL_SIZE_CONTROL(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_TOOL(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_TOOL_DOWN(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_TRANSPORT(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_VC_RECORD(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_WIP(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_WAFER_START(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_PPID_INFO(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
...
def export_to_test_APS_ETL_PRI_WIP(table_name, file_name, id):
    if config.g_debug_mode:
        print("copy to test postgres db : {}".format(table_name))
    logging.info("copy to test postgres db : {}".format(table_name))
