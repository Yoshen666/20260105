...
from testcase import r_2 as aps_etl_rls_new_5m_helper_02, r_1 as aps_etl_rls_new_5m_helper_01
...
def execute():
    ...
    try:
        ...
        pd_APS_TMP_ETL_RLS_APC_PILOT_RCP1 = None

        with concurrent.futures.ThreadPoolExecutor() as ex:
            if os.path.exists(r'D:\dddd.db'):
                os.remove(r'D:\dddd.db')
            duck_db_temp = duckdb.connect(r'D:\dddd.db')
            duck_db_temp.close()
            duck_db_memory.execute("attach 'D:\dddd.db' as dddd ")
            duck_db_memory.execute("""create table dddd.APS_TMP_ETL_RLS_EQPRCP_RESULT as select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_RESULT""".format(tempdb=my_duck.get_temp_table_mark()))
            duck_db_memory.execute(
            ...
            duck_db_temp = duckdb.connect(r'D:\dddd.db')  # 78

            ex_getRcpInhibit = ex.submit(aps_etl_rls_new_5m_helper_02.getRcpInhibit,
                                         duck_db_temp,
                                         uuid,
                                         current_time,
                                         oracle_conn,
            ...
            ex_mixed = ex.submit(aps_etl_rls_new_5m_helper_02.mixed,  # 100
                                 duck_db_memory,
                                 uuid,
                                 current_time,
                                 oracle_conn,
                                 ETL_Proc_Name,
                                 None)
            pd_APS_TMP_ETL_RLS_APC_PILOT_RCP1 = ex_getRcpInhibit.result()
            duck_db_temp.close()
            ex_mixed_result = ex_mixed.result()

        # ----------------------------------------------------------------------------
        print(pd_APS_TMP_ETL_RLS_APC_PILOT_RCP1)
        print("eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
        end_time = time.time()
        print("Mix+Pandas执行时间", str(end_time - start_time) + "秒")
        # --------------------------------------------------------------------------------------------------------------
        ...
        aps_etl_rls_new_5m_helper_02.InsertRlsTempData2Temp(duck_db_memory=duck_db_memory,
                                                            ...
                                                            ETL_Proc_Name=ETL_Proc_Name,
                                                            used_table_dict=used_table_list,
                                                            pd_result=pd_APS_TMP_ETL_RLS_APC_PILOT_RCP1)
