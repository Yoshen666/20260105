...
# SQL变换
	...
	DuckDB: SP.END_TIME BETWEEN DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '30 day' AND DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day'
	这个一定要用'second' 否则会有问题
	...
	ORACLE: HS.CLAIM_TIME >= TRUNC(SYSDATE,'HH24')-1/24

	DUCKDB: HS.CLAIM_TIME >= date_add(CURRENT_TIMESTAMP, INTERVAL '-1 hour')
	
	百分位、中位数
	median(cast(RECOVER_TIME as decimal))  as RECOVER_TIME1,
    quantile_cont(cast(RECOVER_TIME as decimal),0.5) as RECOVER_TIME2,
    quantile_cont(cast(RECOVER_TIME as decimal),0.8) as RECOVER_TIME
    PERCENTILE_DISC(0.5) WITHIN GROUP(ORDER BY PT)  AS MAX_50  
    MEDIAN(CAST(RECOVER_TIME AS DECIMAL)) OVER(PARTITION BY PA_TYPE )||||||| .r2417
	DUCKDB: HS.CLAIM_TIME >= date_add(CURRENT_TIMESTAMP, INTERVAL '-1 hour')=======
	DUCKDB: HS.CLAIM_TIME >= date_add(CURRENT_TIMESTAMP, INTERVAL '-1 hour')
	...
	#-------------------------------------
	create table SMY_SYNC_PD_RECIPE_EQP_ALL
    ( 
    ...
    insert into SMY_SYNC_PD_RECIPE_EQP_ALL(
        PRODSPEC_ID, 
        PD_ID, 
        details
    )
    select 
        PRODSPEC_ID, 
        PD_ID, 
        list({'eqp_id': eqp_id,'lcrecipe_id': lcrecipe_id,'physical_recipe_id': physical_recipe_id,'m_recipe': m_recipe,'recipe_id_c': recipe_id_c})
    from APS_SYNC_PD_RECIPE_EQP_ALL
    group by PRODSPEC_ID,PD_ID
    
    select count(1) from SMY_SYNC_PD_RECIPE_EQP_ALL
    
    select PRODSPEC_ID, PD_ID ,unnest(details)->>'eqp_id' as eqp_id
    from SMY_SYNC_PD_RECIPE_EQP_ALL
    where PRODSPEC_ID='AFKNK501N.0F01'
