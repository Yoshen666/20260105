...
def execute():
    ...
    file_APS_TMP_ETL_RLS_EQPRCP_RESULT = None
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1 = None
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2 = None
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4 = None
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5 = None
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6 = None
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7 = None
    ...
    try:
        ...
        temp_process_db_file = os.path.join(config.g_mem_etl_output_path, target_table,
                                            target_table + "_" + uuid + '.db')

        file_APS_TMP_ETL_RLS_EQPRCP_RESULT =  os.path.join(config.g_mem_etl_output_path, target_table,
                                            "APS_TMP_ETL_RLS_EQPRCP_RESULT" + "_" + uuid + '.parquet')
        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1 = os.path.join(config.g_mem_etl_output_path, target_table,
                                                          "APS_TMP_ETL_RLS_EQPRCP_INHIBIT1" + "_" + uuid + '.parquet')
        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2 = os.path.join(config.g_mem_etl_output_path, target_table,
                                                            "APS_TMP_ETL_RLS_EQPRCP_INHIBIT2" + "_" + uuid + '.parquet')
        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4 = os.path.join(config.g_mem_etl_output_path, target_table,
                                                            "APS_TMP_ETL_RLS_EQPRCP_INHIBIT4" + "_" + uuid + '.parquet')
        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5 = os.path.join(config.g_mem_etl_output_path, target_table,
                                                            "APS_TMP_ETL_RLS_EQPRCP_INHIBIT5" + "_" + uuid + '.parquet')
        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6 = os.path.join(config.g_mem_etl_output_path, target_table,
                                                            "APS_TMP_ETL_RLS_EQPRCP_INHIBIT6" + "_" + uuid + '.parquet')
        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7 = os.path.join(config.g_mem_etl_output_path, target_table,
                                                            "APS_TMP_ETL_RLS_EQPRCP_INHIBIT7" + "_" + uuid + '.parquet')

        sql = """copy (select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_RESULT) to '{path}' (FORMAT 'parquet')""".format(
            tempdb=my_duck.get_temp_table_mark(),
            path=file_APS_TMP_ETL_RLS_EQPRCP_RESULT
        )
        duck_db_memory.execute(sql)
        sql = """copy (select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT1) to '{path}' (FORMAT 'parquet')""".format(
            tempdb=my_duck.get_temp_table_mark(),
            path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1
        )
        duck_db_memory.execute(sql)
        sql = """copy (select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT2) to '{path}' (FORMAT 'parquet')""".format(
            tempdb=my_duck.get_temp_table_mark(),
            path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2
        )
        duck_db_memory.execute(sql)
        sql = """copy (select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT4) to '{path}' (FORMAT 'parquet')""".format(
            tempdb=my_duck.get_temp_table_mark(),
            path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4
        )
        duck_db_memory.execute(sql)
        sql = """copy (select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT5) to '{path}' (FORMAT 'parquet')""".format(
            tempdb=my_duck.get_temp_table_mark(),
            path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5
        )
        duck_db_memory.execute(sql)
        sql = """copy (select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT6) to '{path}' (FORMAT 'parquet')""".format(
            tempdb=my_duck.get_temp_table_mark(),
            path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6
        )
        duck_db_memory.execute(sql)
        sql = """copy (select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT7) to '{path}' (FORMAT 'parquet')""".format(
            tempdb=my_duck.get_temp_table_mark(),
            path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7
        )
        duck_db_memory.execute(sql)
        ...
        # duck_db_temp.close()
        # duck_db_memory.execute(
        #     "attach '{temp_process_db_file}' as dddd ".format(temp_process_db_file=temp_process_db_file))
        # duck_db_memory.execute(
        #     """create table dddd.APS_TMP_ETL_RLS_EQPRCP_RESULT as select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_RESULT""".format(
        #         tempdb=my_duck.get_temp_table_mark()))
        # duck_db_memory.execute(
        #     """create table dddd.APS_TMP_ETL_RLS_EQPRCP_INHIBIT1 as select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT1""".format(
        #         tempdb=my_duck.get_temp_table_mark()))
        # duck_db_memory.execute(
        #     """create table dddd.APS_TMP_ETL_RLS_EQPRCP_INHIBIT2 as select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT2""".format(
        #         tempdb=my_duck.get_temp_table_mark()))
        # duck_db_memory.execute(
        #     """create table dddd.APS_TMP_ETL_RLS_EQPRCP_INHIBIT4 as select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT4""".format(
        #         tempdb=my_duck.get_temp_table_mark()))
        # duck_db_memory.execute(
        #     """create table dddd.APS_TMP_ETL_RLS_EQPRCP_INHIBIT5 as select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT5""".format(
        #         tempdb=my_duck.get_temp_table_mark()))
        # duck_db_memory.execute(
        #     """create table dddd.APS_TMP_ETL_RLS_EQPRCP_INHIBIT6 as select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT6""".format(
        #         tempdb=my_duck.get_temp_table_mark()))
        # duck_db_memory.execute(
        #     """create table dddd.APS_TMP_ETL_RLS_EQPRCP_INHIBIT7 as select * from {tempdb}APS_TMP_ETL_RLS_EQPRCP_INHIBIT7""".format(
        #         tempdb=my_duck.get_temp_table_mark()))
        # duck_db_memory.execute("""DETACH dddd""")
        # duck_db_memory.sql("VACUUM")
        print("---------------------------------------")
        path = os.path.dirname(os.path.abspath(__file__))
        print("------------------------------", path)
        sub_process = subprocess.Popen(["python", os.path.join(path, "aps_etl_rls_5m_sub.py"),
                                        temp_process_db_file,
                                        file_APS_TMP_ETL_RLS_EQPRCP_RESULT,
                                        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1,
                                        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2,
                                        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4,
                                        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5,
                                        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6,
                                        file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7])
        ...
        try_db = None
        try:
            duckdb.connect(temp_process_db_file)
        except Exception as f_open1:
            time.sleep(3)
            try:
                duckdb.connect(temp_process_db_file)
            except Exception as f_open2:
                time.sleep(3)
                try:
                    duckdb.connect(temp_process_db_file)
                except Exception as f_open3:
                    time.sleep(3)
                    raise f_open3
        finally:
            if try_db:
        ...
        aps_etl_rls_new_5m_helper_02.InsertRlsTempData2Temp(duck_db_memory=duck_db_memory,
                                                            ...
                                                            ETL_Proc_Name=ETL_Proc_Name,
                                                            used_table_dict=used_table_list,
                                                            pd_result=temp_process_db_file)
        ...
        if os.path.exists(temp_process_db_file):
            os.remove(temp_process_db_file)
        if os.path.exists(file_APS_TMP_ETL_RLS_EQPRCP_RESULT):
            os.remove(file_APS_TMP_ETL_RLS_EQPRCP_RESULT)
        if os.path.exists(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1):
            os.remove(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1)
        if os.path.exists(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2):
            os.remove(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2)
        if os.path.exists(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4):
            os.remove(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4)
        if os.path.exists(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5):
            os.remove(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5)
        if os.path.exists(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6):
            os.remove(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6)
        if os.path.exists(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7):
            os.remove(file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7)
        gc.collect()  # 内存释放
