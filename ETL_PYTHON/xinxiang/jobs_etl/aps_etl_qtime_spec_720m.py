...
def getGradeDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         ...
         SELECT CASE WHEN CUSTOM_FIELD = 'A' THEN '1'
                     WHEN CUSTOM_FIELD = 'B' THEN '2'
                     WHEN CUSTOM_FIELD = 'C' THEN '3'
                     WHEN CUSTOM_FIELD = 'D' THEN '4'
                END AS CUSTOM_FIELD,
                PRODSPEC_ID,FLOW_KEY,FROM_OPE_NO,TO_OPE_NO                                                                  
         FROM GARDE_DATA                                                                                                                                      
         WHERE RNT = 1                                                                          
    """.format(uuid=uuid, current_time=current_time,tempdb=my_duck.get_temp_table_mark())
