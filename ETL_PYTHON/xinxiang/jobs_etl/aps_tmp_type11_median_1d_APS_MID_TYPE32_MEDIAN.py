...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
           INSERT  /*+ append */  INTO APS_MID_TYPE32_MEDIAN(                             
         PARENTID,TOOLG_ID,PROCESS_TIME, UPDATE_TIME, PARTCODE                                                                    
        )                                                                                   
        WITH TYPE32_DATA AS(                                                                                                                                                                                                                           
               SELECT T.TOOLG_ID,                                                                                                            
                   ...
                   ROW_NUMBER() OVER (PARTITION BY TOOLG_ID ORDER BY T.PROCESS_START DESC) AS RN                       
               FROM APS_TR_TYPE32.APS_TR_TYPE32 T  
               where BATCH_FLAG = 'Y'                                                                                                                             
        ),                                                                                                                           
        MEDAIN_DATA AS (                                                                                                                                                              
                SELECT TOOLG_ID,RN, 
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_TYPE32_MEDIAN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_TYPE32_MEDIAN")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_TYPE11_MEDIAN_1D_APS_MID_TYPE32_MEDIAN"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_TYPE32_MEDIAN"
    used_table_list = ['APS_TR_TYPE32']
    target_table_sql = """
    create table {}APS_MID_TYPE32_MEDIAN
    (
        parentid     VARCHAR(64),
        toolg_id     VARCHAR(64),
    ...
    try:
        ...
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, "XETL051")
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
