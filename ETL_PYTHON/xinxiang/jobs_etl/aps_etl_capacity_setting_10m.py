...
def GetCapatitySettingSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO APS_ETL_CAPACITY_SETTING(                    
      PARENTID, TOOLG_ID, SCHE_TOOLG_ID, COE, UPDATE_TIME, PARTCODE                                                        
    )  
    SELECT '{uuid}' AS PARENTID,  
            EQP_G,SCHE_EQP_G,
            COALESCE(SETTING_VALUE, DEFAULT_VAL) AS COE,
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_ETL_CAPACITY_SETTING",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_CAPACITY_SETTING")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_CAPACITY_SETTING_10M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_ETL_CAPACITY_SETTING"
    used_table_list = ['APS_SYNC_RTD_CAPACITY_SETTING']
    target_table_sql = """
        create table {}APS_ETL_CAPACITY_SETTING
        (
    ...
    try:
        ...
        ################################################################################################################
        # 获取WaferStart信息
        GetCapatitySettingSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        ...
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            select_sql_in_duck = """select parentid,
                                     ...
                                     from APS_ETL_CAPACITY_SETTING
                                 """
            postgres_table_define = """etl_capacity_setting(parentid,toolg_id,sche_toolg_id,coe,update_time)
                                    """
            my_postgres.copy_duckdb_to_postgres(uuid=uuid,
                                                duckdb=duck_db_memory,
                                                table_name_in_duckdb=target_table,
                                                table_name_in_pg="etl_capacity_setting",  # 要小写
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_CAPACITY_SETTING_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
