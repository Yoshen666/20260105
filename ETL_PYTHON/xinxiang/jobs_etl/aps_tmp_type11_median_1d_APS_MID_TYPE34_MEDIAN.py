...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT  /*+ append */  INTO APS_MID_IDLE_DUMMY34_MEDIAN(                             
         PARENTID, PROCESS_TIME, TOOLG_ID, PRODID, CASEID, UPDATE_TIME, PARTCODE                                                                     
        )                                                                                  
        WITH TOOL_DATA AS (                                                              
             SELECT EQP_ID,                                                              
        ...
        COMPLETE_DATA AS (                                                                      
             SELECT D.TOOLG_ID, C.PRODID, C.CASEID,                                                            
             ...
             ROW_NUMBER() OVER (PARTITION BY D.TOOLG_ID, C.PRODID, C.CASEID  ORDER BY PROCESS_START_TIME DESC) AS RN     
        ...
        SOURE_DATA AS (                                                                         
             SELECT DISTINCT 
                    QUANTILE_CONT(CAST(PROCESS_TIME AS DECIMAL),0.5) OVER(PARTITION BY TOOLG_ID,PRODID,CASEID) AS PROCESS_TIME,                                
                    TOOLG_ID,PRODID,CASEID,RN                                                                
            FROM COMPLETE_DATA                                                           
        ),                                                                               
        GROUP_DATA AS (                                                                         
            SELECT                                                                                   
                   MAX(PROCESS_TIME) AS PROCESS_TIME,                                           
                   TOOLG_ID,PRODID,CASEID, MAX(RN) AS RN                                                     
            FROM SOURE_DATA                                                              
            GROUP BY TOOLG_ID,PRODID,CASEID                                                           
        )                                                                                
        SELECT  '{uuid}',                                                 
               PROCESS_TIME,                                                             
               TOOLG_ID, PRODID, CASEID,                                                                
               '{current_time}',                                                   
               ''                                                     
        FROM GROUP_DATA                                                                  
        --WHERE RN > 10                                                                                                                                                                                                                                                                                                                                           
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_IDLE_DUMMY34_MEDIAN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_IDLE_DUMMY34_MEDIAN")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_TYPE11_MEDIAN_1D_APS_MID_TYPE34_MEDIAN"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_IDLE_DUMMY34_MEDIAN"
    used_table_list = ['APS_SYNC_ETL_TOOL', 'APS_SYNC_OCS_AUTODUMMY_COMPLETE']
    target_table_sql = """
    create table {}APS_MID_IDLE_DUMMY34_MEDIAN
    (
        parentid     VARCHAR(64),
        toolg_id     VARCHAR(64),
        prodid       VARCHAR(64),
