...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
           INSERT INTO APS_ETL_SGS_RULE
                 (PARENTID,TOOL_ID,PRODG_ID,FROM_LAYER,FROM_STAGE,TO_LAYER,TO_STAGE,UPDATE_TIME,PARTCODE)
                 SELECT '{uuid}',
                        REPLACE(Y.EQP_ID, '*', '%') TOOL_ID,
                        REPLACE(Y.PRODG, '*', '%') PRODG_ID,
                        SUBSTRING(Y.START_OPE, 1, position('.' in Y.START_OPE) - 1) FROM_LAYER,
                        SUBSTRING(Y.START_OPE, position('.' in Y.START_OPE) + 1) FROM_STAGE,
                        SUBSTRING(Y.TARGET_OPE, 1, position('.' in Y.TARGET_OPE) - 1) TO_LAYER,
                        REPLACE(SUBSTRING(Y.TARGET_OPE, position('.' in Y.TARGET_OPE) + 1),
                                '*',
                                '%') TO_STAGE,
                       '{current_time}',   
                        ''
                   FROM APS_SYNC_RTD_ID_OPE_CONFIG.APS_SYNC_RTD_ID_OPE_CONFIG Y                                                                                                                                                                                                                                
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_SGS_RULE_720M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str();
    uuid = my_oracle.UUID()

    target_table = "APS_ETL_SGS_RULE"
    used_table_list = ['APS_SYNC_RTD_ID_OPE_CONFIG']
    target_table_sql = """
        create table {}APS_ETL_SGS_RULE
        (
            parentid    VARCHAR(32) not null,
            tool_id     VARCHAR(64),
            prodg_id    VARCHAR(64),
            from_layer  VARCHAR(64),
    ...
    try:
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_SGS_RULE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
