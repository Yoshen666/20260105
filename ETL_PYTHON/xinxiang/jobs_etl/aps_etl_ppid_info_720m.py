import gc
import logging
import os

from xinxiang import config
from xinxiang.util import my_duck, my_oracle, my_date, cons_error_code, cons, my_postgres, my_runner


def create_temp_table(duck_db):
    table_type = ""
    if not config.g_debug_mode:
        table_type = 'TEMP'
    '''
    这里在内存中创建使用到的临时表
    '''

    sql = """
        create {table_type} table APS_TMP_ETL_PPID_INFO_RECIPE_VALUE
        (
            EQP_G           VARCHAR(64), 
            ED_PARAMETER_ID VARCHAR(64), 
            RECIPE          VARCHAR(64), 
            ADD_TYPE        VARCHAR(64), 
            ADD_VAL         VARCHAR(64), 
            REPORT_TIME     VARCHAR(64), 
            UPDATE_TIME     VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)

def GetRecipeValueSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PPID_INFO_RECIPE_VALUE (
            EQP_G, ED_PARAMETER_ID, RECIPE, ADD_TYPE, ADD_VAL, REPORT_TIME, UPDATE_TIME)  
    WITH VALUE_DATA AS (                                                          
        SELECT ROW_NUMBER() OVER(PARTITION BY V.EQP_G,V.ED_PARAMETER_ID,V.RECIPE,V.ADD_TYPE ORDER BY V.UPDATE_TIME DESC) RNT,
               V.EQP_G,V.ED_PARAMETER_ID,V.RECIPE,V.ADD_TYPE,V.ADD_VAL,V.REPORT_TIME,V.UPDATE_TIME
        FROM APS_SYNC_EQP_RECIPE_ED_ADD_VALUE.APS_SYNC_EQP_RECIPE_ED_ADD_VALUE v                                 
    )                                                                                  
    SELECT V.EQP_G,V.ED_PARAMETER_ID,V.RECIPE,V.ADD_TYPE,V.ADD_VAL,V.REPORT_TIME,V.UPDATE_TIME                                                  
    FROM VALUE_DATA V                                                                                  
    WHERE RNT = 1                                                                                                               
       """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PPID_INFO_RECIPE_VALUE",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PPID_INFO_RECIPE_VALUE")

# ①膜厚值取PPID最后一位为数字，向前推4位，如果都是数字/10；如果不是数字则不放入PPID
def PpidInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
          INSERT  /*+ append */  INTO APS_ETL_PPID_INFO (                                                                        
        PARENTID,
            TOOLG_ID,
            TOOL_ID,
            CH_ID,
            SEQ,
            RECIPE,
            PPID,
            PROD_ID,
            PLAN_ID,
            STAGE,
            COAT_LBOUND,
            COAT_UBOUND,
            COAT_PER_WAFER,
            COAT_PER_CHAMBER,
            CLEAN_TIME,
            CLEAN_MON_TIME,
            UPDATE_TIME,
            PARTCODE                                                                                                            
        )                                                                                                                              
         WITH SOUCE_PPID_DATA AS (                                                                                                    
             SELECT L.EQP_ID,T.EQP_G AS TOOLG_ID,                                                                                      
                     SUBSTRING( (SUBSTRING(L.M_RECIPE, 1, LENGTH(L.M_RECIPE)- POSITION('.' in reverse(L.M_RECIPE)))),   
                     LENGTH(( SUBSTRING(L.M_RECIPE, 1, LENGTH(L.M_RECIPE)- POSITION('.' in reverse(L.M_RECIPE)))) ) 
                     -POSITION('.' IN reverse(( SUBSTRING(L.M_RECIPE, 1, LENGTH(L.M_RECIPE)- POSITION('.' in reverse(L.M_RECIPE)))) ) ) +2) AS PPID                                      
             FROM APS_SYNC_PD_RECIPE_EQP_MAIN.APS_SYNC_PD_RECIPE_EQP_MAIN L, APS_SYNC_ETL_TOOL.APS_SYNC_ETL_TOOL T                       
             WHERE T.EQP_ID = L.EQP_ID                                                                                                
             AND T.REAL_MODULE ='DIFF'                                                                                                                                               
             AND POSITION('F' IN L.EQP_ID) = 1                                                                                                
             GROUP BY L.EQP_ID,T.EQP_G,L.M_RECIPE                                                                                     
         ),                                                                                                                           
         PPID_DATA AS (                                                                                                               
             SELECT EQP_ID AS TOOL_ID,TOOLG_ID,                                                                                       
                    PPID, '' AS CH_SET,                                                                                               
                    substring(regexp_extract(R.PPID,'([0-9,A-Z])(\d)(\d)(\d)[A-Z]*$'),1,4) AS M_PPID
             FROM SOUCE_PPID_DATA R                                                                                                   
             GROUP BY R.EQP_ID,R.TOOLG_ID,R.PPID                                                                                      
         )                                                                                                                            
         SELECT '{uuid}' AS PARENTID,                                                                                    
                R.TOOLG_ID,                                                                                                           
                R.TOOL_ID,                                                                                                            
                -- R.CH_SET AS CH_ID,
                CASE WHEN R.CH_SET='' THEN NULL ELSE R.CH_SET END AS CH_ID,                                                                                                    
                '1' AS SEQ,                                                                                                           
                NULL AS RECIPE,                                                                                                         
                R.PPID,                                                                                                               
                NULL AS PROD_ID,                                                                                                        
                NULL AS PLAN_ID,                                                                                                        
                NULL AS STAGE,                                                                                                          
                NULL AS COAT_LBOUND,                                                                                                    
                NULL AS COAT_UBOUND,                                                                                                    
                NULL AS COAT_PER_WAFER,                                                                                                 
                (CAST(R.M_PPID AS DECIMAL(18,2))/10 ) AS COAT_PER_CHAMBER,                                                                                   
                NULL AS CLEAN_TIME,                                                                                                     
                NULL AS CLEAN_MON_TIME,                                                                                                 
                CAST('{current_time}' AS TIMESTAMP) AS UPDATE_TIME,                                                                                
                '' AS PARTCODE                                                                     
         FROM PPID_DATA R                                                                                                     
         WHERE REGEXP_MATCHES(R.M_PPID,'^[0-9]+[0-9]$')                                                                                                                                                                                                                                         
    """.format(uuid=uuid, current_time=current_time)
    # print(sql)

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PPID_INFO",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PPID_INFO")
    # ①全区非炉管参数厚度资料
    sql = """
        INSERT  /*+ append */  INTO APS_ETL_PPID_INFO(                                           
          PARENTID,TOOLG_ID,TOOL_ID,SEQ,PPID,COAT_PER_WAFER,COAT_PER_CHAMBER,PARAMETER,UPDATE_TIME, PARTCODE                                                                                   
         )                                                                                                 
          WITH PIRUN_DATA AS (                                                                             
              SELECT DISTINCT E.EQP_G, T.SPEC_UNIT, T.DC_ITEM_NAME                                         
              FROM APS_SYNC_TMS_EQP_PARAMETER.APS_SYNC_TMS_EQP_PARAMETER T                                    
              INNER JOIN (SELECT DISTINCT EQP_G,EQP_ID FROM APS_SYNC_ETL_TOOL.APS_SYNC_ETL_TOOL
                     WHERE HOST_EQP_FLAG  = 'Y' ) E     
              ON T.EQUIPMENT_NO = E.EQP_ID                                                                 
              WHERE T.SPEC_UNIT NOT IN ('Months','Days','Pieces')                                          
              AND T.EQUIPMENT_NO NOT LIKE 'F%'                                                             
              AND T.DC_ITEM_VALUE IS NOT NULL  AND T.DC_ITEM_VALUE <> ''                                                                                             
          )                                                                                                
          SELECT '{uuid}' AS PARENTID,                                                        
                 ED.EQP_G AS TOOLG_ID,                                                                     
                 '' AS TOOL_ID,                                                                            
                 '1' AS SEQ,                                                                               
                 ED.RECIPE AS PPID,                                                                        
                 CASE WHEN ED.ADD_TYPE = 'PIECES' THEN CAST(ED.ADD_VAL AS DECIMAL(18,4)) END AS COAT_PER_WAFER,                   
                 CASE WHEN ED.ADD_TYPE = 'BATCH' THEN CAST(ED.ADD_VAL AS DECIMAL(18,4)) END AS COAT_PER_CHAMBER,                  
                 ED.ED_PARAMETER_ID AS PARAMETER,                                                          
                 CAST('{current_time}' AS TIMESTAMP) AS UPDATE_TIME,                                                    
                  '' AS PARTCODE                                         
          FROM {tempdb}APS_TMP_ETL_PPID_INFO_RECIPE_VALUE ED                                 
          INNER JOIN PIRUN_DATA T1                                                                         
          ON ED.EQP_G = T1.EQP_G                                                                           
          AND ED.ED_PARAMETER_ID = T1.DC_ITEM_NAME                                                                                                                                                                                                                                         
       """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    # print(sql)

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PPID_INFO",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PPID_INFO")


def execute():
    ###############################################################
    ### 以下参数必须定义
    ### ETL_Proc_Name    : ETL 名称
    ### current_time     ：请直接拷贝
    ### current_time_short ：请直接拷贝
    ### uuid             ：请直接拷贝
    ### target_table     : 该ETL输出表名
    ### used_table_list  : 该ETL使用到的，参考到的表名(中间表不算)
    ### target_table_sql ： 该ETL输出表定义SQL
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_PPID_INFO_720M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_ETL_PPID_INFO"
    used_table_list = ['APS_SYNC_PD_RECIPE_EQP_MAIN',
                       'APS_SYNC_ETL_TOOL',
                       'APS_SYNC_TMS_EQP_PARAMETER',
                       'APS_SYNC_EQP_RECIPE_ED_ADD_VALUE']
    target_table_sql = """
    create table {}APS_ETL_PPID_INFO
    (
        parentid         VARCHAR(60) not null,
        toolg_id         VARCHAR(60) not null,
        tool_id          VARCHAR(60) not null,
        ch_id            VARCHAR(60),
        seq              VARCHAR(60) not null,
        recipe           VARCHAR(60),
        ppid             VARCHAR(60) not null,
        prod_id          VARCHAR(60),
        plan_id          VARCHAR(60),
        stage            VARCHAR(60),
        coat_lbound      DECIMAL,
        coat_ubound      DECIMAL,
        coat_per_wafer   DECIMAL(18,4),
        coat_per_chamber DECIMAL,
        clean_time       DECIMAL,
        clean_mon_time   DECIMAL,
        update_time      TIMESTAMP not null,
        partcode         VARCHAR(60) not null,
        parameter        VARCHAR(64)
    )
    """.format("")  # 注意:这里一定要这么写 [create table 表名] => [create table {}表名]
    target_db_file = my_duck.get_target_file_name(target_table, current_time_short)

    # -------------------------- 内存模式改成文件模式
    _temp_db_path = os.path.join(config.g_mem_speed_etl_output_path, target_table, "inprocess")
    if not os.path.exists(_temp_db_path):
        os.makedirs(_temp_db_path)

    temp_db_file = os.path.join(_temp_db_path, target_table + "_" + current_time_short + "_temp.db")
    # 处理中文件
    in_process_db_file = os.path.join(_temp_db_path, target_table + "_" + current_time_short + ".db")
    # 结果文件
    target_db_file = os.path.join(config.g_mem_etl_output_path, target_table,
                                  target_table + "_" + current_time_short + ".db")
    # --------------------------

    oracle_conn = None
    try:
        oracle_conn = my_oracle.oracle_get_connection()
        # 开始日志
        my_oracle.StartCleanUpAndLog(oracle_conn, ETL_Proc_Name, current_time)
        # -------------------------- 内存模式改成文件模式
        # 创建DuckDB
        duck_db_memory = my_duck.create_duckdb_in_file(_temp_db_path, in_process_db_file, target_table_sql)
        duck_db_memory.sql('SET threads TO 4')
        if not os.path.exists(os.path.join(config.g_mem_speed_etl_output_path, 'duck_temp', uuid)):
            os.makedirs(os.path.join(config.g_mem_speed_etl_output_path, 'duck_temp', uuid))
        duck_db_memory.execute(
            "SET temp_directory='{}'".format(os.path.join(config.g_mem_speed_etl_output_path, 'duck_temp', uuid)))

        if not config.g_debug_mode:
            create_temp_table(duck_db_memory)
        else:
            duck_db_temp = my_duck.create_duckdb_for_temp_table(_temp_db_path, temp_db_file)
            # 创建Temp表
            create_temp_table(duck_db_temp)
            duck_db_temp.commit()
            duck_db_temp.close()
            my_duck.attach_temp_db_write_able(duck_db_memory, "TEMPDB", temp_db_file)
        # --------------------------
        # Attach用到的表
        my_duck.attach_used_table(oracle_conn, duck_db_memory, used_table_list)
        ################################################################################################################
        ## 以下为业务逻辑
        ################################################################################################################
        GetRecipeValueSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        PpidInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        ################################################################################################################
        ## 写入log
        ################################################################################################################
        # 写版本号
        my_oracle.Monitor_HandlingVerControl(oracle_conn, uuid, target_table, target_db_file, current_time_short)

        ################################################################################################################
        ## 以上为业务逻辑
        ################################################################################################################
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            select_sql_in_duck = """select CASE WHEN seq='' THEN NULL ELSE seq END AS seq,
                                           CASE WHEN ppid='' THEN NULL ELSE ppid END AS ppid,
                                           parentid,
                                           CASE WHEN toolg_id='' THEN NULL ELSE toolg_id END AS toolg_id,
                                           CASE WHEN tool_id='' THEN NULL ELSE tool_id END AS tool_id,
                                           CASE WHEN ch_id='' THEN NULL ELSE ch_id END AS ch_id,
                                           CASE WHEN prod_id='' THEN NULL ELSE prod_id END AS prod_id,
                                           CASE WHEN plan_id='' THEN NULL ELSE plan_id END AS plan_id,
                                           CASE WHEN stage='' THEN NULL ELSE stage END AS stage,
                                           coat_lbound,coat_ubound,coat_per_wafer,
                                           coat_per_chamber,clean_time,clean_mon_time, update_time, 
                                           CASE WHEN parameter='' THEN NULL ELSE parameter END AS parameter 
                                           from APS_ETL_PPID_INFO
                                      """
            postgres_table_define = """etl_ppid_info(seq,ppid, parentid,toolg_id,tool_id,ch_id,prod_id, plan_id,stage,coat_lbound,coat_ubound,coat_per_wafer,
                                                     coat_per_chamber,clean_time,clean_mon_time, update_time, parameter)
                                                     """
            my_postgres.copy_duckdb_to_postgres(uuid=uuid,
                                                duckdb=duck_db_memory,
                                                table_name_in_duckdb=target_table,
                                                table_name_in_pg="etl_ppid_info",  # 要小写
                                                select_sql_in_duck=select_sql_in_duck,
                                                postgres_table_define=postgres_table_define,
                                                oracle_conn=oracle_conn,
                                                ETL_Proc_Name=ETL_Proc_Name)
        # 导出到目标文件中
        target_db_file = my_duck.export_result_duck_file_and_close_duck_db_memory2(duck_db_memory,
                                                                                   in_process_db_file=in_process_db_file,
                                                                                   target_table=target_table,
                                                                                   current_time=current_time_short)

        # 写版本号
        my_oracle.HandlingVerControl(oracle_conn, uuid, target_table, target_db_file, current_time_short)
        # 加入PG执行结束的时间更新
        my_oracle.Update_PG_HandlingVerControl(oracle_conn, uuid, target_table, target_db_file, current_time_short)
        # 写完成日志
        my_oracle.EndCleanUpAndLog(oracle_conn, ETL_Proc_Name, current_time)
    except Exception as e:
        logging.exception("{ETL_Proc_Name} 處理出錯 : {e}".format(ETL_Proc_Name=ETL_Proc_Name, e=e))
        # 导出到目标文件中
        my_duck.export_result_duck_file_and_close_duck_db_memory2(duck_db_memory,
                                                                  in_process_db_file=in_process_db_file,
                                                                  target_table=target_table,
                                                                  current_time=current_time_short)
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_PPID_INFO_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
        # 删除TMP目录:LQN:2023/08/21
        if os.path.exists(os.path.join(config.g_mem_etl_output_path, 'duck_temp', uuid)):
            os.remove(os.path.join(config.g_mem_etl_output_path, 'duck_temp', uuid))
        if os.path.exists(temp_db_file) and not config.g_debug_mode:
            os.remove(temp_db_file)
        gc.collect()  # 内存释放


if __name__ == '__main__':
    # 单JOB测试用
    print("start")
    execute()

