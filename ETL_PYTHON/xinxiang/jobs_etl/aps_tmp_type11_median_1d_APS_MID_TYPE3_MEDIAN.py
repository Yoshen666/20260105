...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
          INSERT  /*+ append */  INTO APS_MID_IDLE_DUMMY11_MEDIAN(                             
        ...
        COMPLETE_DATA AS (                                                                      
            ...
            -- ROUND((C.PROCESS_END_TIME - C.PROCESS_START_TIME)*24,3) AS PROCESS_TIME,   
             ...
             ROW_NUMBER() OVER (PARTITION BY TOOLID,TOOLG_ID  ORDER BY PROCESS_START_TIME DESC) AS RN       
        ...
        SOURE_DATA AS (                                                                         
            SELECT DISTINCT  
                   ---MEDIAN(CAST(PROCESS_TIME AS DECIMAL)) OVER(PARTITION BY TOOLID,TOOLG_ID ) AS PROCESS_TIME,
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_IDLE_DUMMY11_MEDIAN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_IDLE_DUMMY11_MEDIAN")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_TYPE11_MEDIAN_1D_APS_MID_TYPE3_MEDIAN"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_IDLE_DUMMY11_MEDIAN"
    used_table_list = ['APS_SYNC_ETL_TOOL', 'APS_SYNC_OCS_AUTODUMMY_COMPLETE']
    target_table_sql = """
    create table {}APS_MID_IDLE_DUMMY11_MEDIAN
    (
        parentid     VARCHAR(64),
        toolg_id     VARCHAR(64),
        tool_id      VARCHAR(64),
