...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT  /* append */  INTO APS_MID_TYPE31_MEDIAN(                             
         PARENTID,TOOLG_ID,PLAN_ID,TOOL_ID,PROCESS_TIME, UPDATE_TIME, PARTCODE                                                                       
         )                                                                                     
         WITH TYPE31_DATA AS(                                                                                                                                                                                                    
                SELECT T.TOOLG_ID,T.TOOL_ID,T.PLAN_ID,                                                                                                                       
                    ...
                    ROUND((extract(epoch from(CAST(T.PROCESS_END AS TIMESTAMP) 
                     - (CAST(T.PROCESS_START AS TIMESTAMP))))) / 3600.0, 2) AS PROCESS_TIME,                                             
                    ROW_NUMBER() OVER (PARTITION BY TOOLG_ID,PLAN_ID,TOOL_ID ORDER BY T.PROCESS_START DESC) AS RN                           
                FROM APS_TR_TYPE31.APS_TR_TYPE31 T 
                where BATCH_FLAG = 'Y'                                                                                                                                         
         ),                                                                                                                          
         MEDAIN_DATA AS (                                                                                                                                                              
                SELECT TOOLG_ID,PLAN_ID,TOOL_ID,RN,                                                                                         
                --MEDIAN(CAST(PROCESS_TIME AS DECIMAL)) OVER(PARTITION BY TOOLG_ID,PLAN_ID,TOOL_ID ) AS PROCESS_TIME
                QUANTILE_CONT(CAST(PROCESS_TIME AS DECIMAL),0.5) OVER(PARTITION BY TOOLG_ID,PLAN_ID,TOOL_ID ) AS PROCESS_TIME  
                FROM TYPE31_DATA                                                                                                        
         ),                                                                                                                                        
         RESULT_DATE AS (                                                                                                                   
               SELECT TOOLG_ID,PLAN_ID,TOOL_ID,                                                                                             
                      MAX(RN) AS RN,                                                                                                        
                      MAX(PROCESS_TIME) AS PROCESS_TIME                                                                                     
               FROM MEDAIN_DATA                                                                                                             
               GROUP BY TOOLG_ID,PLAN_ID,TOOL_ID                                                                                           
         )                                                                                                                                  
         SELECT  '{uuid}',                                                                                                   
                 TOOLG_ID,PLAN_ID,TOOL_ID,PROCESS_TIME,                                                                                      
                 '{current_time}',                                                                   
                 ''                                                                                                 
         FROM RESULT_DATE                                                                                                                   
         WHERE RN > 2                                                                                                                                                                                                                
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_TYPE31_MEDIAN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_TYPE31_MEDIAN")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_TYPE11_MEDIAN_1D_APS_MID_TYPE31_MEDIAN"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_TYPE31_MEDIAN"
    used_table_list = ['APS_TR_TYPE31']
    target_table_sql = """
    create table {}APS_MID_TYPE31_MEDIAN
    (
        parentid     VARCHAR(64),
        toolg_id     VARCHAR(64),
        plan_id      VARCHAR(64),
    ...
    try:
        ...
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, "XETL050")
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
