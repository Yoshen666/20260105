import gc
import logging
import os

from xinxiang import config
from xinxiang.util import my_duck, my_oracle, my_date, cons_error_code, cons, my_postgres, my_runner


def create_temp_table(duck_db):
    table_type = ""
    if not config.g_debug_mode:
        table_type = 'TEMP'
    '''
    这里在内存中创建使用到的临时表
    '''
    sql = """
    create {table_type} table APS_TMP_ETL_PIRUN
    (
        toolg_id        VARCHAR(60),
        tool_id         VARCHAR(60),
        ch_id           VARCHAR(60),
        prod_id         VARCHAR(60),
        plan_id         VARCHAR(60),
        step_id         VARCHAR(60),
        recipe          VARCHAR(60),
        ppid            VARCHAR(60),
        layer           VARCHAR(60),
        stage           VARCHAR(60),
        tech            VARCHAR(60),
        customer        VARCHAR(60),
        lot_id          VARCHAR(60),
        reticle_id      VARCHAR(60),
        pre_tool_id     VARCHAR(60),
        rule_name       VARCHAR(60),
        remaining_count VARCHAR(60),
        remaining_time  VARCHAR(60),
        recover_time    VARCHAR(60),
        valid_recipe    VARCHAR(60),
        valid_time      VARCHAR(60),
        hold_time       VARCHAR(60),
        valid_lot       VARCHAR(60),
        split_qty       VARCHAR(60),
        pick_pilot_mode VARCHAR(1)
    )
    """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
    create {table_type} table APS_TMP_ETL_PIRUN1
    (
        toolg_id        VARCHAR(60),
        tool_id         VARCHAR(60),
        ch_id           VARCHAR(60),
        prod_id         VARCHAR(60),
        plan_id         VARCHAR(60),
        step_id         VARCHAR(60),
        recipe          VARCHAR(60),
        ppid            VARCHAR(60),
        layer           VARCHAR(60),
        stage           VARCHAR(60),
        tech            VARCHAR(60),
        customer        VARCHAR(60),
        lot_id          VARCHAR(60),
        reticle_id      VARCHAR(60),
        pre_tool_id     VARCHAR(60),
        rule_name       VARCHAR(60),
        remaining_count VARCHAR(60),
        remaining_time  VARCHAR(60),
        recover_time    VARCHAR(60),
        valid_recipe    VARCHAR(60),
        valid_time      VARCHAR(60),
        hold_time       VARCHAR(60),
        valid_lot       VARCHAR(60),
        split_qty       VARCHAR(60)
    )
    """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
    create {table_type} table APS_TMP_ETL_PIRUN2
    (
        toolg_id        VARCHAR(60),
        tool_id         VARCHAR(60),
        ch_id           VARCHAR(60),
        prod_id         VARCHAR(60),
        plan_id         VARCHAR(60),
        step_id         VARCHAR(60),
        recipe          VARCHAR(60),
        ppid            VARCHAR(60),
        layer           VARCHAR(60),
        stage           VARCHAR(60),
        tech            VARCHAR(60),
        customer        VARCHAR(60),
        lot_id          VARCHAR(60),
        reticle_id      VARCHAR(60),
        pre_tool_id     VARCHAR(60),
        rule_name       VARCHAR(60),
        remaining_count VARCHAR(60),
        remaining_time  VARCHAR(60),
        recover_time    VARCHAR(60),
        valid_recipe    VARCHAR(60),
        valid_time      VARCHAR(60),
        hold_time       VARCHAR(60),
        valid_lot       VARCHAR(60),
        split_qty       VARCHAR(60)
    )
    """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
    create {table_type} table APS_TMP_ETL_PIRUN3
    (
        toolg_id        VARCHAR(60),
        tool_id         VARCHAR(60),
        ch_id           VARCHAR(60),
        prod_id         VARCHAR(60),
        plan_id         VARCHAR(60),
        step_id         VARCHAR(60),
        recipe          VARCHAR(60),
        ppid            VARCHAR(60),
        layer           VARCHAR(60),
        stage           VARCHAR(60),
        tech            VARCHAR(60),
        customer        VARCHAR(60),
        lot_id          VARCHAR(60),
        reticle_id      VARCHAR(60),
        pre_tool_id     VARCHAR(60),
        rule_name       VARCHAR(60),
        remaining_count VARCHAR(60),
        remaining_time  VARCHAR(60),
        recover_time    VARCHAR(60),
        valid_recipe    VARCHAR(60),
        valid_time      VARCHAR(60),
        hold_time       VARCHAR(60),
        valid_lot       VARCHAR(60),
        split_qty       VARCHAR(60)
    )
    """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN4
     (
        toolg_id        VARCHAR(60),
        tool_id         VARCHAR(60),
        ch_id           VARCHAR(60),
        prod_id         VARCHAR(60),
        plan_id         VARCHAR(60),
        step_id         VARCHAR(60),
        recipe          VARCHAR(60),
        ppid            VARCHAR(60),
        layer           VARCHAR(60),
        stage           VARCHAR(60),
        tech            VARCHAR(60),
        customer        VARCHAR(60),
        lot_id          VARCHAR(60),
        reticle_id      VARCHAR(60),
        pre_tool_id     VARCHAR(60),
        rule_name       VARCHAR(60),
        remaining_count VARCHAR(60),
        remaining_time  VARCHAR(60),
        recover_time    VARCHAR(60),
        valid_recipe    VARCHAR(60),
        valid_time      VARCHAR(60),
        hold_time       VARCHAR(60),
        valid_lot       VARCHAR(60),
        split_qty       VARCHAR(60)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN5
     (
        toolg_id        VARCHAR(60),
        tool_id         VARCHAR(60),
        ch_id           VARCHAR(60),
        prod_id         VARCHAR(60),
        plan_id         VARCHAR(60),
        step_id         VARCHAR(60),
        recipe          VARCHAR(60),
        ppid            VARCHAR(60),
        layer           VARCHAR(60),
        stage           VARCHAR(60),
        tech            VARCHAR(60),
        customer        VARCHAR(60),
        lot_id          VARCHAR(60),
        reticle_id      VARCHAR(60),
        pre_tool_id     VARCHAR(60),
        rule_name       VARCHAR(60),
        remaining_count VARCHAR(60),
        remaining_time  VARCHAR(60),
        recover_time    VARCHAR(60),
        valid_recipe    VARCHAR(60),
        valid_time      VARCHAR(60),
        hold_time       VARCHAR(60),
        valid_lot       VARCHAR(60),
        split_qty       VARCHAR(60)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN7
     (
        toolg_id        VARCHAR(60),
        tool_id         VARCHAR(60),
        ch_id           VARCHAR(60),
        prod_id         VARCHAR(60),
        plan_id         VARCHAR(60),
        step_id         VARCHAR(60),
        recipe          VARCHAR(60),
        ppid            VARCHAR(60),
        layer           VARCHAR(60),
        stage           VARCHAR(60),
        tech            VARCHAR(60),
        customer        VARCHAR(60),
        lot_id          VARCHAR(60),
        reticle_id      VARCHAR(60),
        pre_tool_id     VARCHAR(60),
        rule_name       VARCHAR(60),
        remaining_count VARCHAR(60),
        remaining_time  VARCHAR(60),
        recover_time    VARCHAR(60),
        valid_recipe    VARCHAR(60),
        valid_time      VARCHAR(60),
        hold_time       VARCHAR(60),
        valid_lot       VARCHAR(60),
        split_qty       VARCHAR(60)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN_CSFRINHIBIT
     (
        productid    VARCHAR(60),
        routeid      VARCHAR(60),
        openo        VARCHAR(60),
        equipmentid  VARCHAR(60),
        pass_flg_prc VARCHAR(60),
        pass_flg_mfg VARCHAR(60),
        prc_upt_user VARCHAR(60),
        update_time  VARCHAR(60)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN_PB1
     (
        pr_unit_id   VARCHAR(60),
        prodspec_id  VARCHAR(60),
        pr_op_no     VARCHAR(60),
        mask         VARCHAR(60),
        pb_flag      VARCHAR(60),
        remain_day   VARCHAR(60),
        max_efec_day VARCHAR(60),
        split_count  VARCHAR(60),
        FOURTH_VAL_FLG   VARCHAR(60),
        ERR_FLG   VARCHAR(60)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN_PA1
     (
        pr_unit_id    VARCHAR(60),
        prodspec_id   VARCHAR(60),
        pr_op_no      VARCHAR(60),
        mask_nm       VARCHAR(60),
        op_no_1       VARCHAR(60),
        unit_id_1     VARCHAR(60),
        op_no_2       VARCHAR(60),
        unit_id_2     VARCHAR(60),
        pa_flag       VARCHAR(60),
        remain_day    VARCHAR(60),
        max_efct_time VARCHAR(60),
        split_count   VARCHAR(60),
        FOURTH_VAL_FLG   VARCHAR(60),
        ERR_FLG   VARCHAR(60)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN_TEST_RUN_RECIPE
     (
        m_recipe   VARCHAR(64),
        module     VARCHAR(64),
        toolid     VARCHAR(64),
        expiretime VARCHAR(64),
        jobgroupid VARCHAR(64),
        jobspecid  VARCHAR(64),
        lifetime   VARCHAR(64),
        subname    VARCHAR(64),
        eqp_g      VARCHAR(64),
        state      VARCHAR(64)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN_TEST_RUN_T21
     (
        toolg_id VARCHAR(64),
        tool_id  VARCHAR(64),
        ppid     VARCHAR(64),
        pt       VARCHAR(64),
        MODULE_ID VARCHAR(64)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN_TEST_RUN_T22
     (
        toolg_id VARCHAR(64),
        ppid     VARCHAR(64),
        pt       VARCHAR(64),
        MODULE_ID VARCHAR(64)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN_TEST_RUN_T221
     (
        toolg_id VARCHAR(64),
        tool_id  VARCHAR(64),
        ch_cnt   VARCHAR(64),
        pt       VARCHAR(64),
        MODULE_ID VARCHAR(64)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
     create {table_type} table APS_TMP_ETL_PIRUN_RTDRUN_PSTART
     (
        eqp_id       VARCHAR(64),
        ope_no       VARCHAR(64),
        prodspec_id  VARCHAR(64),
        claim_time   VARCHAR(64),
        ope_category VARCHAR(64),
        lot_id       VARCHAR(64),
        ctrl_job     VARCHAR(64)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
    create {table_type} table APS_TMP_ETL_PIRUN_RTDRUN_COUNT
    (
        eqp_id    VARCHAR(64),
        layer     VARCHAR(64),
        pd_id     VARCHAR(64),
        lot_count VARCHAR(64)
    )
    """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_RTDRUN_LOTCOUNT
        (
            eqp_id VARCHAR(64),
            layer  VARCHAR(64),
            pd_id  VARCHAR(64),
            rn     VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_RTDRUN_PS
        (
            eqp_id       VARCHAR(64),
            ope_no       VARCHAR(64),
            prodspec_id  VARCHAR(64),
            claim_time   VARCHAR(64),
            ope_category VARCHAR(64),
            lot_id       VARCHAR(64),
            ctrl_job     VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_RTDRUN_PE
        (
            eqp_id       VARCHAR(64),
            ope_no       VARCHAR(64),
            prodspec_id  VARCHAR(64),
            claim_time   VARCHAR(64),
            ope_category VARCHAR(64),
            lot_id       VARCHAR(64),
            ctrl_job     VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_TOOL
        (
            tool_id       VARCHAR(64),
            toolg_id      VARCHAR(64),
            module        VARCHAR(64),
            host_eqp_flag VARCHAR(64),
            eqp_chamber_flag VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_FOUR_CSB
            (
                productid    VARCHAR(64),
                routeid      VARCHAR(64),
                openo        VARCHAR(64),
                equipmentid  VARCHAR(64),
                pass_flg_prc VARCHAR(64),
                chipbody     VARCHAR(64)
            )
            """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_INFO
            (
                apc_type  VARCHAR(64),
                prod_id   VARCHAR(64),
                lot_id    VARCHAR(64),
                c_op_no   VARCHAR(64),
                c_unit_id VARCHAR(64),
                op_no_1   VARCHAR(64),
                unit_id_1 VARCHAR(64),
                op_no_2   VARCHAR(64),
                unit_id_2 VARCHAR(64),
                mask_nm   VARCHAR(64)
            )
            """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_HIST
            (
                apc_type   VARCHAR(64),
                lot_id     VARCHAR(64),
                c_op_no    VARCHAR(64),
                c_unit_id  VARCHAR(64),
                op_no_1    VARCHAR(64),
                unit_id_1  VARCHAR(64),
                op_no_2    VARCHAR(64),
                unit_id_2  VARCHAR(64),
                mask_nm    VARCHAR(64),
                start_date VARCHAR(64),
                del_date   VARCHAR(64)
            )
                """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_RECOVER_TIME
            (
                apc_type     VARCHAR(64),
                prodspec_id  VARCHAR(64),
                c_op_no      VARCHAR(64),
                c_unit_id    VARCHAR(64),
                lot_id       VARCHAR(64),
                op_no_1      VARCHAR(64),
                unit_id_1    VARCHAR(64),
                op_no_2      VARCHAR(64),
                unit_id_2    VARCHAR(64),
                mask_nm      VARCHAR(64),
                start_date   VARCHAR(64),
                recover_time VARCHAR(64)
            )
                """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_RECOVER_TIME_8
            (
                toolg_id     VARCHAR(64),
                recover_time VARCHAR(64),
                pa_type      VARCHAR(32),
                pb_type      VARCHAR(32)
            )
                """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_REWINFO
            (
                productid   VARCHAR(64),
                routeid     VARCHAR(64),
                openo       VARCHAR(64),
                equipmentid VARCHAR(64)
            )
                """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_NOTREWINFO
            (
                productid   VARCHAR(64),
                openo       VARCHAR(64),
                equipmentid VARCHAR(64)
            )
            """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_CSFINFO
            (
                productid    VARCHAR(64),
                routeid      VARCHAR(64),
                openo        VARCHAR(64),
                equipmentid  VARCHAR(64),
                pass_flg_prc VARCHAR(64),
                update_time  VARCHAR(64),
                chipbody     VARCHAR(64)
            )
                """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_RESULT
            (
                productid    VARCHAR(64),
                routeid      VARCHAR(64),
                openo        VARCHAR(64),
                equipmentid  VARCHAR(64),
                pass_flg_prc VARCHAR(64)
            )
                """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_SOURCE
            (
                productid    VARCHAR(64),
                routeid      VARCHAR(64),
                openo        VARCHAR(64),
                equipmentid  VARCHAR(64),
                pass_flg_prc VARCHAR(64),
                sgs_first_op VARCHAR(64),
                rn           VARCHAR(64)
            )
                """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_RESULT
            (
                productid   VARCHAR(64),
                routeid     VARCHAR(64),
                openo       VARCHAR(64),
                equipmentid VARCHAR(64)
            )
                """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_APS_WIP
            (
                plan_id  VARCHAR(64),
                prod_id  VARCHAR(64),
                ope_no   VARCHAR(64),
                chipbody VARCHAR(64)
            )
                """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_PATH_SETTING
            (
                use_flag_type VARCHAR(64),
                rs_hold_time  VARCHAR(64),
                tool_id       VARCHAR(64),
                toolg_id      VARCHAR(64)
            )
                    """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_PURGE_DATA
            (
                toolg_id     VARCHAR(64),
                equipment_id VARCHAR(64),
                val          decimal,
                report_time  VARCHAR(64),
                rtn          decimal,
                last_val     decimal
            )
                    """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_PURGE_MAX_BATCH
            (
                equipment_id VARCHAR(64),
                batch_spec   decimal,
                rtn2         decimal
            )
                    """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
         create {table_type} table APS_TMP_ETL_PIRUN_PURGE_EQP_BATCH
             (
                equipment_id VARCHAR(64),
                val          decimal,
                rtn          decimal
             )
                     """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
       create {table_type} table APS_TMP_ETL_PIRUN_PURGE_DEF_BATCH
           (
                eqp_g        VARCHAR(64),
                eqp_id       VARCHAR(64),
                unit_dc_item VARCHAR(64),
                unit_spec    decimal
           )
                       """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
         create {table_type} table APS_TMP_ETL_PIRUN_PURGE_LAST
             (
                start_time VARCHAR(64),
                eqp_id     VARCHAR(64),
                rtn        DECIMAL
             )
                         """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_RECIPE_ADD
            (
                  toolg_id       VARCHAR(64),
                  tool_id        VARCHAR(64),
                  ch_id          VARCHAR(64),
                  rule_name      VARCHAR(64),
                  count_unit     VARCHAR(64),
                  current_count  DECIMAL,
                  lsl            DECIMAL,
                  usl            DECIMAL,
                  process_time   DECIMAL,
                  parameter      VARCHAR(64),
                  all_ch_flag    VARCHAR(2),
                  clean_mon_time DECIMAL,
                  module         VARCHAR(64),
                  clean_group    VARCHAR(64),
                  clean_fix_flag VARCHAR(1)
            )
                        """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_ETL_PIRUN_RECIPE_ADD_GROUP
            (
                  toolg_id       VARCHAR(64),
                  tool_id        VARCHAR(64),
                  ch_id          VARCHAR(64),
                  rule_name      VARCHAR(64),
                  count_unit     VARCHAR(64),
                  current_count  DECIMAL,
                  lsl            DECIMAL,
                  usl            DECIMAL,
                  process_time   DECIMAL,
                  parameter      VARCHAR(64),
                  all_ch_flag    VARCHAR(2),
                  clean_mon_time DECIMAL,
                  module         VARCHAR(64),
                  clean_group    VARCHAR(64),
                  clean_fix_flag VARCHAR(1)
            )
            """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_PIRUN_PAD_DATA
            (
                  toolg_id       VARCHAR(64),
                  lsl            DECIMAL,
                  parameter      VARCHAR(64),
                  count_unit     VARCHAR(64),
                  ch_id          VARCHAR(64),
                  tool_id        VARCHAR(64)
            )
            """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_PIRUN_PT_PER_WAFER
            (
                  toolg_id       VARCHAR(64),
                  process_time_per_wafer     DECIMAL(18,4),
                  parameter      VARCHAR(64)
            )
            """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_PIRUN_MIN_LSL_DATA
            (
                  tool_id  VARCHAR(64), 
                  ch_id    VARCHAR(64), 
                  lsl_h    DECIMAL, 
                  lsl_p    DECIMAL
            )
            """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
    create {table_type} table APS_TMP_PIRUN_ATP_QTIME_TASK
        (
              task_no  VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
    create {table_type} table APS_TMP_PIRUN_ATP_PRE_LOT
        (
              task_no  VARCHAR(64),
              lot_no  VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
    create {table_type} table APS_TMP_PIRUN_V1
        (
            parentid        VARCHAR(60),
            toolg_id        VARCHAR(60) not null,
            tool_id         VARCHAR(60) not null,
            ch_id           VARCHAR(60),
            prod_id         VARCHAR(60),
            plan_id         VARCHAR(60),
            step_id         VARCHAR(60),
            recipe          VARCHAR(60),
            ppid            VARCHAR(60),
            layer           VARCHAR(60),
            stage           VARCHAR(60),
            tech            VARCHAR(60),
            customer        VARCHAR(60),
            lot_id          VARCHAR(60),
            reticle_id      VARCHAR(60),
            pre_tool_id     VARCHAR(60),
            rule_name       VARCHAR(60) not null,
            remaining_count VARCHAR(60),
            remaining_time  VARCHAR(60),
            recover_time    VARCHAR(60),
            valid_recipe    VARCHAR(60),
            valid_time      VARCHAR(60),
            update_time     VARCHAR(60) not null,
            hold_time       VARCHAR(60),
            partcode        VARCHAR(60) not null,
            valid_lot       VARCHAR(60),
            count_unit      VARCHAR(60),
            current_count   VARCHAR(60),
            lsl             VARCHAR(60),
            usl             VARCHAR(60),
            process_time    VARCHAR(60),
            job_id          VARCHAR(64),
            split_qty       VARCHAR(60),
            parameter       VARCHAR(64),
            all_ch_flag     VARCHAR(2),
            valid_count     DECIMAL,
            prodg_id        VARCHAR(64),
            clean_acc_flag  VARCHAR(1),
            clean_mon_time  DECIMAL,
            clean_group     VARCHAR(64),
            clean_fix_flag  VARCHAR(1),
            pick_pilot_mode VARCHAR(1),
            wip_attr3       VARCHAR(64),
            RLS_attr3       VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
    create {table_type} table APS_TMP_PIRUN_TT
    (
        parentid        VARCHAR(60),
        toolg_id        VARCHAR(60) not null,
        tool_id         VARCHAR(60) not null,
        ch_id           VARCHAR(60),
        prod_id         VARCHAR(60),
        plan_id         VARCHAR(60),
        step_id         VARCHAR(60),
        recipe          VARCHAR(60),
        ppid            VARCHAR(60),
        layer           VARCHAR(60),
        stage           VARCHAR(60),
        tech            VARCHAR(60),
        customer        VARCHAR(60),
        lot_id          VARCHAR(60),
        reticle_id      VARCHAR(60),
        pre_tool_id     VARCHAR(60),
        rule_name       VARCHAR(60) not null,
        remaining_count VARCHAR(60),
        remaining_time  VARCHAR(60),
        recover_time    VARCHAR(60),
        valid_recipe    VARCHAR(60),
        valid_time      VARCHAR(60),
        update_time     VARCHAR(60) not null,
        hold_time       VARCHAR(60),
        partcode        VARCHAR(60),
        valid_lot       VARCHAR(60),
        count_unit      VARCHAR(60),
        current_count   VARCHAR(60),
        lsl             VARCHAR(60),
        usl             VARCHAR(60),
        process_time    VARCHAR(60),
        job_id          VARCHAR(64),
        split_qty       VARCHAR(60),
        parameter       VARCHAR(64),
        all_ch_flag     VARCHAR(2),
        valid_count     DECIMAL,
        prodg_id        VARCHAR(64),
        clean_acc_flag  VARCHAR(1),
        clean_mon_time  DECIMAL,
        clean_group     VARCHAR(64),
        clean_fix_flag  VARCHAR(1),
        pick_pilot_mode VARCHAR(1),
        wip_attr3       VARCHAR(64),
        rtn             INT,
        COAT_FIX_BOUND  VARCHAR(64),
        valid_ppid      VARCHAR(64),
        rls_attr1       VARCHAR(64),
        rls_attr3       VARCHAR(64)

    )
    """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
    create {table_type} table APS_TMP_PIRUN_TT_PAD_PM
    (
        toolg_id VARCHAR(64),
        parameter VARCHAR(64),
        tool_id  VARCHAR(64),
        ch_id    VARCHAR(64),
        clean_group VARCHAR(64),
        count_unit  VARCHAR(64),
        lsl         VARCHAR(64)
    )
    """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
    create {table_type} table APS_TMP_PIRUN_TT_PPID_INFO
    (
        toolg_id VARCHAR(64),
        parameter VARCHAR(64),
        process_time_per_wafer  DECIMAL
    )
    """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_PIRUN_TT_PAD_PIECES
        (
            tool_id VARCHAR(64),
            ch_id VARCHAR(64),
            clean_group VARCHAR(64),
            lsl_p       VARCHAR(60)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
    create {table_type} table APS_TMP_PIRUN_TT_COAT_HOURS
    (
        parameter VARCHAR(64),
        toolg_id VARCHAR(64),
        rtn       INT,
        process_time_per_wafer   decimal
    )
    """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
     CREATE {table_type} TABLE APS_TMP_RTD_PREFER_INHIBIT_LOT
     (
        toolg_id        VARCHAR(60),
        tool_id         VARCHAR(60),
        prod_id         VARCHAR(60),
        prodg_id        VARCHAR(60),
        layer           VARCHAR(60),
        stage           VARCHAR(60),
        rule_name       VARCHAR(60),
        count_unit      VARCHAR(60),
        current_count   VARCHAR(60),
        lsl             VARCHAR(60),
        usl             VARCHAR(60),
        process_time    VARCHAR(60),
        parameter       VARCHAR(60),
        pick_pilot_mode VARCHAR(60),
        wip_attr3       VARCHAR(60)
     )
     """.format(table_type=table_type)
    duck_db.sql(sql)


def getetApsEtlWipSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_APS_WIP(                                          
            PLAN_ID, PROD_ID, OPE_NO, CHIPBODY                                                       
            )                                                                         
            SELECT DISTINCT SUBSTRING(W.TARGET_PLAN_ID, 1, POSITION('.' IN W.TARGET_PLAN_ID)-1) AS PLAN_ID,                                    
                W.TARGET_PROD_ID AS PROD_ID,                                             
                W.TARGET_OPE_NO AS OPE_NO,                                              
                SUBSTRING(W.TARGET_PROD_ID, 1, POSITION('.' IN W.TARGET_PROD_ID)-1) AS CHIPBODY 
            FROM APS_ETL_WIP.APS_ETL_WIP W                                                                                                                                                   
        """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_APS_WIP",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_APS_WIP")


def GetCsfrinhibitSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_CSFRINHIBIT(      
            PRODUCTID, ROUTEID, OPENO, EQUIPMENTID, PASS_FLG_PRC, PASS_FLG_MFG, PRC_UPT_USER, UPDATE_TIME                                                 
         )                                                                     
          SELECT DISTINCT V.PRODUCTID,V.ROUTEID,                                
                 V.OPENO,V.EQUIPMENTID,                                         
                 V.PASS_FLG_PRC,V.PASS_FLG_MFG,                                 
                 V.PRC_UPT_USER, V.PRC_UPT_TIME as UPDATE_TIME                                   
          FROM APS_TR_CSFRINHIBIT.APS_TR_CSFRINHIBIT V,{tempdb}APS_TMP_ETL_PIRUN_APS_WIP W               
          WHERE                        
           W.PROD_ID = V.PRODUCTID                                          
          AND W.PLAN_ID = V.ROUTEID                                            
          AND W.OPE_NO = V.OPENO                                               
          AND (POSITION('F' IN V.EQUIPMENTID) = 1 OR POSITION('W' IN V.EQUIPMENTID) = 1 OR POSITION('P' IN V.EQUIPMENTID) = 1)                        
        """.format(current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_CSFRINHIBIT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_CSFRINHIBIT")


def GetApcPb1DataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_PB1(          
        PR_UNIT_ID, PRODSPEC_ID, PR_OP_NO, MASK, PB_FLAG, REMAIN_DAY, MAX_EFEC_DAY, SPLIT_COUNT,
        FOURTH_VAL_FLG,ERR_FLG                                                         
        )                                                                      
         SELECT PB.PR_UNIT_ID,PB.PRODSPEC_ID,                                  
               PB.PR_OP_NO,PB.MASK,PB.PB_FLAG,                                 
               PB.REMAIN_DAY,PB.MAX_EFEC_DAY,SC.SPLIT_COUNT,  
               PB.FOURTH_VAL_FLG,
               PB.ERR_FLG                    
         FROM APS_SYNC_SCHE_APC_PB.APS_SYNC_SCHE_APC_PB PB                   
         LEFT JOIN APS_SYNC_APC_SPLIT_WAFER.APS_SYNC_APC_SPLIT_WAFER SC          
         ON SC.JOB_NO = PB.JOB_NO    
         WHERE PB.PRODSPEC_ID IN (                                          
               SELECT DISTINCT W.PRODSPEC_ID                                   
               FROM APS_SYNC_WIP.APS_SYNC_WIP W  )                                                                                                                               
    """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_PB1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_PB1")


def GetApcPa1DataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_PA1(          
             PR_UNIT_ID, PRODSPEC_ID, PR_OP_NO, MASK_NM, OP_NO_1, UNIT_ID_1, OP_NO_2, UNIT_ID_2,
             PA_FLAG, REMAIN_DAY, MAX_EFCT_TIME, SPLIT_COUNT  ,FOURTH_VAL_FLG,ERR_FLG                                                       
             )                                                                      
              SELECT PA.PR_UNIT_ID,PA.PRODSPEC_ID,                                  
                    PA.PR_OP_NO,PA.MASK_NM,PA.OP_NO_1,                              
                    PA.UNIT_ID_1,PA.OP_NO_2,                                        
                    PA.UNIT_ID_2,PA.PA_FLAG,                                        
                    PA.REMAIN_DAY,PA.MAX_EFCT_TIME,SC.SPLIT_COUNT,
                    PA.FOURTH_VAL_FLG,
                    PA.ERR_FLG                
              FROM APS_SYNC_SCHE_APC_PA.APS_SYNC_SCHE_APC_PA PA                  
              LEFT JOIN APS_SYNC_APC_SPLIT_WAFER.APS_SYNC_APC_SPLIT_WAFER SC            
              ON SC.JOB_NO = PA.JOB_NO     
                --modify by xiecheng for version up      
              WHERE PA.PRODSPEC_ID IN (                                           
                   SELECT DISTINCT W.PRODSPEC_ID                                    
                   FROM APS_SYNC_WIP.APS_SYNC_WIP W )                                                                                               
    """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_PA1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_PA1")


def GetEtlToolDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_TOOL(         
                TOOL_ID, TOOLG_ID, MODULE, HOST_EQP_FLAG,EQP_CHAMBER_FLAG                                                         
           )                                                                     
            SELECT EQP_ID AS TOOL_ID,MAX(EQP_G) AS TOOLG_ID,                      
                   MAX(REAL_MODULE) AS MODULE, MAX(HOST_EQP_FLAG) AS HOST_EQP_FLAG,
                   MAX(EQP_CHAMBER_FLAG) AS EQP_CHAMBER_FLAG 
            FROM APS_SYNC_ETL_TOOL.APS_SYNC_ETL_TOOL                                            
            GROUP BY EQP_ID                                                                                                                                                                                        
     """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_TOOL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_TOOL")


def getPirunRunInfo(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_INFO(                                                   
            APC_TYPE,PROD_ID,LOT_ID,C_OP_NO,C_UNIT_ID,OP_NO_1,UNIT_ID_1,OP_NO_2,UNIT_ID_2,MASK_NM                                                                            
       )                                                                                        
        SELECT T.APC_TYPE,                                                                      
              MAX(B.PRODSPEC_ID) AS PROD_ID,                                                    
              REPLACE(T.LOT_KEY_NO,'.','') AS LOT_ID,                                           
              T.C_OP_NO,T.C_UNIT_ID,                                                            
              COALESCE(T.OP_NO_1,'*') AS OP_NO_1,COALESCE(T.UNIT_ID_1,'*') AS UNIT_ID_1,                  
              COALESCE(T.OP_NO_2,'*') AS OP_NO_2,COALESCE(T.UNIT_ID_2,'*') AS UNIT_ID_2,                  
              COALESCE(T.MASK_NM,'*') AS MASK_NM                                                     
        FROM APS_SYNC_RSPILOT_RUNING.APS_SYNC_RSPILOT_RUNING T                                  
        INNER JOIN APS_SYNC_WIP.APS_SYNC_WIP B                                   
        ON REPLACE(T.LOT_KEY_NO,'.','') = B.LOT_ID         
        WHERE T.APC_TYPE IN ('PA','PB')                    
        GROUP BY T.APC_TYPE,                                                                    
              T.LOT_KEY_NO,                                                                     
              T.C_OP_NO,T.C_UNIT_ID,                                                            
              T.OP_NO_1,T.UNIT_ID_1,                                                            
              T.OP_NO_2,T.UNIT_ID_2,                                                            
              T.MASK_NM                                                                             
    """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_INFO",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_INFO")


def getPirunHistData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_HIST(                                                    
        APC_TYPE,LOT_ID,C_OP_NO,C_UNIT_ID,OP_NO_1,UNIT_ID_1, OP_NO_2, UNIT_ID_2, MASK_NM, START_DATE, DEL_DATE                                                                           
        )                                                                                         
         SELECT T.APC_TYPE,                                                                       
                REPLACE(T.LOT_KEY_NO,'.','') AS LOT_ID,                                           
                T.C_OP_NO,T.C_UNIT_ID,                                                            
                T.OP_NO_1,T.UNIT_ID_1,                                                            
                T.OP_NO_2,T.UNIT_ID_2,                                                            
                T.MASK_NM,                                                                        
                T.START_DATE,T.DEL_DATE                                                           
         FROM APS_MID_RSPILOT_RUN_HS_VIEW.APS_MID_RSPILOT_RUN_HS_VIEW T                                  
         WHERE T.START_DATE >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '30 day'                                                          
         AND T.STATUS='COMP'                                                                                                                                                                                              
        """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_HIST",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_HIST")


def getRecoverTimeByLotHisData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_RECOVER_TIME(                                                   
              APC_TYPE,PRODSPEC_ID,C_OP_NO,C_UNIT_ID,LOT_ID,OP_NO_1,
              UNIT_ID_1,OP_NO_2,UNIT_ID_2,MASK_NM,START_DATE,RECOVER_TIME
              )                                                                                        
               SELECT A.APC_TYPE,MAX(B.PRODSPEC_ID) AS PRODSPEC_ID,                                                   
                      MAX(A.C_OP_NO) AS C_OP_NO,                                                                      
                      MAX(A.C_UNIT_ID) AS C_UNIT_ID,                                                                  
                      A.LOT_ID,                                                                                       
                      COALESCE(MAX(A.OP_NO_1),'*') AS OP_NO_1,                                                             
                      COALESCE(MAX(A.UNIT_ID_1),'*') AS UNIT_ID_1,                                                         
                      COALESCE(MAX(A.OP_NO_2),'*') AS OP_NO_2,                                                             
                      COALESCE(MAX(A.UNIT_ID_2),'*') AS UNIT_ID_2,                                                         
                      COALESCE(MAX(A.MASK_NM), '*') AS MASK_NM,                                                            
                      A.START_DATE,                                                                                   
                      -- ROUND(CEIL((MAX(B.CLAIM_TIME) - MIN(B.CLAIM_TIME))*24*60*60)/3600,2) AS RECOVER_TIME   
                      ROUND((extract(epoch from(MAX(CAST(B.CLAIM_TIME AS TIMESTAMP)) - MIN(CAST(B.CLAIM_TIME AS TIMESTAMP))))) / 3600.0, 2) AS RECOVER_TIME            
               FROM {tempdb}APS_TMP_ETL_PIRUN_HIST A                                                                         
               LEFT JOIN APS_TMP_LOTHISTORY_VIEW.APS_TMP_LOTHISTORY_VIEW B                                                
               ON A.LOT_ID = B.LOT_ID                                                                                 
               AND CAST(B.CLAIM_TIME AS TIMESTAMP) >= CAST(A.START_DATE AS TIMESTAMP)                                                                       
               AND CAST(B.CLAIM_TIME AS TIMESTAMP) <= CAST(A.DEL_DATE AS TIMESTAMP)  
               --modify by xiecheng for version up
               AND B.CLAIM_TIME >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '30 day'                                                                        
               GROUP BY A.APC_TYPE,A.LOT_ID, A.START_DATE                                                                                   
    """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_RECOVER_TIME",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_RECOVER_TIME")


def getRecoverTimeByMedianData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    '''sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_RECOVER_TIME_8(
             TOOLG_ID,RECOVER_TIME,PA_TYPE,PB_TYPE)
              WITH RECOVER_ALL_DATE AS (
                  SELECT TL.TOOLG_ID,
                         'APC_PA' AS PA_TYPE,
                         '' AS PB_TYPE,
                         Z.RECOVER_TIME
                  FROM  {tempdb}APS_TMP_ETL_PIRUN_RECOVER_TIME Z,  {tempdb}APS_TMP_ETL_PIRUN_PA1  PA, {tempdb}APS_TMP_ETL_PIRUN_TOOL TL
                  WHERE Z.APC_TYPE='PA'
                        AND Z.PRODSPEC_ID = PA.PRODSPEC_ID AND Z.C_OP_NO = PA.PR_OP_NO
                        AND Z.C_UNIT_ID = PA.PR_UNIT_ID
                        AND PA.PR_UNIT_ID = TL.TOOL_ID
                       -- AND (DECODE(Z.MASK_NM, '*', '1', Z.MASK_NM) = DECODE(Z.MASK_NM, '*', '1', PA.MASK_NM)
                       -- OR DECODE(PA.MASK_NM, '*', '1', Z.MASK_NM) = DECODE(PA.MASK_NM, '*', '1', PA.MASK_NM))

                           and ( ((Z.MASK_NM <> '*' AND Z.MASK_NM = PA.MASK_NM) OR (Z.MASK_NM = '*' and 1=1)) OR
                          ((PA.MASK_NM <> '*' AND Z.MASK_NM =  PA.MASK_NM) OR (PA.MASK_NM  = '*' and 1=1)) )

                        --AND (DECODE(Z.OP_NO_1, '*', '1', Z.OP_NO_1) = DECODE(Z.OP_NO_1, '*', '1', PA.OP_NO_1)
                        --OR DECODE(PA.OP_NO_1, '*', '1', PA.OP_NO_1) = DECODE(PA.OP_NO_1, '*', '1', Z.OP_NO_1))

                          and ( ((Z.OP_NO_1 <> '*' AND Z.OP_NO_1 = PA.OP_NO_1) OR (Z.OP_NO_1 = '*' and 1=1)) OR
                          ((PA.OP_NO_1 <> '*' AND PA.OP_NO_1 =  Z.OP_NO_1) OR (PA.OP_NO_1  = '*' and 1=1)) )

                       -- AND (DECODE(Z.UNIT_ID_1, '*', '1', Z.UNIT_ID_1) = DECODE(Z.UNIT_ID_1, '*', '1', PA.UNIT_ID_1)
                       -- OR DECODE(PA.UNIT_ID_1, '*', '1', PA.UNIT_ID_1) = DECODE(PA.UNIT_ID_1, '*', '1', Z.UNIT_ID_1))

                        and ( ((Z.UNIT_ID_1 <> '*' AND Z.UNIT_ID_1 = PA.UNIT_ID_1) OR (Z.UNIT_ID_1 = '*' and 1=1)) OR
                          ((PA.UNIT_ID_1 <> '*' AND PA.UNIT_ID_1 =  Z.UNIT_ID_1) OR (PA.UNIT_ID_1  = '*' and 1=1)) )

                       -- AND (DECODE(Z.OP_NO_2, '*', '1', Z.OP_NO_2) = DECODE(Z.OP_NO_2, '*', '1', PA.OP_NO_2)
                        --OR DECODE(PA.OP_NO_2, '*', '1', PA.OP_NO_2) = DECODE(PA.OP_NO_2, '*', '1', Z.OP_NO_2))

                         and ( ((Z.OP_NO_2 <> '*' AND Z.OP_NO_2 = PA.OP_NO_2) OR (Z.OP_NO_2 = '*' and 1=1)) OR
                          ((PA.OP_NO_2 <> '*' AND PA.OP_NO_2 =  Z.OP_NO_2) OR (PA.OP_NO_2  = '*' and 1=1)) )

                       -- AND (DECODE(Z.UNIT_ID_2, '*', '1', Z.UNIT_ID_2) = DECODE(Z.UNIT_ID_2, '*', '1', PA.MASK_NM)
                       -- OR DECODE(PA.UNIT_ID_2, '*', '1', PA.UNIT_ID_2) = DECODE(PA.UNIT_ID_2, '*', '1', Z.UNIT_ID_2))
                          and ( ((Z.UNIT_ID_2 <> '*' AND Z.UNIT_ID_2 = PA.MASK_NM) OR (Z.UNIT_ID_2 = '*' and 1=1)) OR
                          ((PA.UNIT_ID_2 <> '*' AND PA.UNIT_ID_2 =  Z.UNIT_ID_2) OR (PA.UNIT_ID_2  = '*' and 1=1)) )

                  UNION ALL
                  SELECT TL.TOOLG_ID,
                         '' AS PA_TYPE,
                         'APC_PB' AS PB_TYPE,
                         Z.RECOVER_TIME
                  FROM  {tempdb}APS_TMP_ETL_PIRUN_RECOVER_TIME Z, {tempdb}APS_TMP_ETL_PIRUN_PB1 PB, {tempdb}APS_TMP_ETL_PIRUN_TOOL TL
                  WHERE Z.APC_TYPE='PB'
                        AND Z.PRODSPEC_ID = PB.PRODSPEC_ID
                        AND Z.C_OP_NO = PB.PR_OP_NO
                        AND PB.PR_UNIT_ID = TL.TOOL_ID
                        --AND (DECODE(PB.MASK, '*', '1', PB.MASK) = DECODE(PB.MASK, '*', '1', Z.MASK_NM)
                       -- OR DECODE(Z.MASK_NM, '*', '1', Z.MASK_NM) = DECODE(Z.MASK_NM, '*', '1', PB.MASK))
                        and ( ((PB.MASK <> '*' AND PB.MASK = Z.MASK_NM) OR (PB.MASK = '*' and 1=1)) OR
                          ((Z.MASK_NM <> '*' AND Z.MASK_NM = PB.MASK) OR (Z.MASK_NM = '*' and 1=1)) )
                        AND Z.C_UNIT_ID = PB.PR_UNIT_ID
              )
              SELECT
                     TOOLG_ID,
                     --PERCENTILE_CONT(0.8) WITHIN GROUP(ORDER BY RECOVER_TIME) AS RECOVER_TIME,
                      quantile_cont(cast(RECOVER_TIME as decimal),0.8) AS RECOVER_TIME,
                     'APC_PA' AS PA_TYPE,
                     'APC_PB' AS PB_TYPE
                FROM RECOVER_ALL_DATE
                WHERE RECOVER_TIME IS NOT NULL
                GROUP BY TOOLG_ID
              """.format(tempdb=my_duck.get_temp_table_mark())'''

    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_RECOVER_TIME_8(
             TOOLG_ID, RECOVER_TIME, PA_TYPE, PB_TYPE)

        -- PA 的中位數恢復時間 (按 TOOLG_ID)
        WITH PA_DATA AS (
            SELECT 
                   TL.TOOLG_ID,
                   H.APC_TYPE,
                   ROUND((extract(epoch from(H.DEL_DATE - H.START_DATE)) / 3600.0), 2) AS RECOVER_TIME
            FROM APS_MID_RSPILOT_RUN_HS_VIEW.APS_MID_RSPILOT_RUN_HS_VIEW H
            LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL TL
            ON H.C_UNIT_ID = TL.TOOL_ID
            WHERE H.STATUS = 'COMP'
              AND H.APC_TYPE = 'PA'
              AND H.START_DATE >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '30 day'
              AND (extract(epoch from(H.DEL_DATE - H.START_DATE)) / 3600.0) > 0
        ),PB_DATA AS (
            SELECT 
                   TL.TOOLG_ID,
                   H.APC_TYPE,
                   ROUND((extract(epoch from(H.DEL_DATE - H.START_DATE)) / 3600.0), 2) AS RECOVER_TIME
            FROM APS_MID_RSPILOT_RUN_HS_VIEW.APS_MID_RSPILOT_RUN_HS_VIEW H
            LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL TL
            ON H.C_UNIT_ID = TL.TOOL_ID
            WHERE H.STATUS = 'COMP'
              AND H.APC_TYPE = 'PB'
              AND H.START_DATE >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '30 day'
              AND (extract(epoch from(H.DEL_DATE - H.START_DATE)) / 3600.0) > 0
        )

        SELECT 
               TOOLG_ID,
               quantile_cont(cast(RECOVER_TIME as decimal), 0.5) AS RECOVER_TIME,
               'APC_PA' AS PA_TYPE,
               '' AS PB_TYPE
        FROM PA_DATA
        GROUP BY TOOLG_ID
        UNION ALL
        SELECT 
               TOOLG_ID,
               quantile_cont(cast(RECOVER_TIME as decimal), 0.5) AS RECOVER_TIME,
               '' AS PA_TYPE,
               'APC_PB' AS PB_TYPE
        FROM PB_DATA
        GROUP BY TOOLG_ID
        """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_RECOVER_TIME_8",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_RECOVER_TIME_8")


def GetPiRunSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN(                                                                                      
         TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
         STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
         REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY ,  PICK_PILOT_MODE                                                                                                                                
         )                                                                                                                                               
          WITH VALID_LOT_INFO AS (                                                                                                                           
              SELECT DISTINCT                                                                                                                           
                    REPLACE(SV.LOT_KEY_NO,'.','') AS LOT_ID,                                                                                            
                    SV.OP_NO,                                                                                                                           
                    SV.UNIT_ID, SV.APC_TYPE                                                                                                             
             FROM APS_SYNC_SCHE_APC_LOT_SPECIAL_VALUE.APS_SYNC_SCHE_APC_LOT_SPECIAL_VALUE SV                                                                                                       
             INNER JOIN APS_SYNC_WIP.APS_SYNC_WIP W                                                                                      
             ON  REPLACE(SV.LOT_KEY_NO,'.','') = W.LOT_ID                                                          
             WHERE  SV.APC_TYPE IN ('PA','PB')                                                                     
          )                                                                                                                                             
           SELECT DISTINCT TL.TOOLG_ID,                                                                                                                 
           AA.PR_UNIT_ID AS TOOL_ID,                                                                                                                    
           '' AS CH_ID, AA.PRODSPEC_ID AS PROD_ID,                                                                                                      
           '' AS PLAN_ID, '' AS STEP_ID,                                                                                                                
           '' AS RECIPE, '' AS PPID,                                                                                                                    
           SUBSTRING(AA.PR_OP_NO, 1, POSITION('.' IN AA.PR_OP_NO)-1) AS LAYER,                                                                                
           SUBSTRING(AA.PR_OP_NO, POSITION('.' IN AA.PR_OP_NO)+1) AS STAGE,                                                                                   
           '' AS TECH, '' AS CUSTOMER,                                                                                                                  
           PI.LOT_ID, 
           --DECODE(PA.MASK_NM,'*','',PA.MASK_NM) AS RETICLE_ID,    
           (CASE WHEN AA.MASK_NM = '*' THEN '' ELSE AA.MASK_NM END) AS RETICLE_ID,                                                                             
           CASE WHEN (AA.UNIT_ID_1 NOT IN ('*') AND AA.UNIT_ID_2 NOT IN ('*')) THEN COALESCE(AA.UNIT_ID_1,'') || ';' || COALESCE(AA.UNIT_ID_2,'') ELSE                            
             (CASE WHEN AA.UNIT_ID_1<>'*' THEN AA.UNIT_ID_1 ELSE (CASE WHEN AA.UNIT_ID_2 = '*' THEN '' ELSE AA.UNIT_ID_2 END) END)                                          
           END AS PRE_TOOL_ID,                                                                                                                          
           'APC_PA' AS RULE_NAME,                                                                                                                       
           '0' AS REMAINING_COUNT, 
           CASE WHEN (AA.FOURTH_VAL_FLG = 2 AND AA.ERR_FLG is null AND AA.REMAIN_DAY = -1 AND AA.PA_FLAG = 'Y') THEN -9999 --2025/04/29 v 
                WHEN (AA.PA_FLAG='Y' AND CAST(AA.REMAIN_DAY AS DECIMAL) <=0) THEN 9999 
                ELSE CAST(cast(AA.REMAIN_DAY as decimal)*24 AS DECIMAL(18,2)) END  AS REMAINING_TIME,
           Z.RECOVER_TIME, '' AS VALID_RECIPE,                                                                                                          
           --ROUND(PA.MAX_EFCT_TIME * 24,2) AS VALID_TIME, 
           CAST(cast(AA.MAX_EFCT_TIME as decimal) *24 AS DECIMAL(18,2)) AS VALID_TIME,                                                                                                
           '' AS HOLD_TIME,                                                                                                                             
           LI.LOT_ID AS VALID_ID, AA.SPLIT_COUNT,
           CASE WHEN (AA.FOURTH_VAL_FLG = 2 AND AA.ERR_FLG is null AND AA.REMAIN_DAY = -1 AND AA.PA_FLAG = 'Y') THEN 0
                ELSE 1
                END AS PICK_PILOT_MODE 
           FROM {tempdb}APS_TMP_ETL_PIRUN_PA1 AA
           INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL TL                                                                                                    
           ON TL.TOOL_ID = AA.PR_UNIT_ID                                                                                                                
           LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_RECOVER_TIME_8 Z                                                                                                                  
           ON Z.PA_TYPE='APC_PA'         
           AND Z.TOOLG_ID = TL.TOOLG_ID                                                                                                               
           LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_INFO PI                                                                                                                      
           ON PI.APC_TYPE='PA'                                                                                                                          
              AND PI.PROD_ID = AA.PRODSPEC_ID AND PI.C_OP_NO = AA.PR_OP_NO                                                                              
              AND PI.C_UNIT_ID = AA.PR_UNIT_ID   

              --AND (DECODE(PI.MASK_NM, '*', '1', PI.MASK_NM) = DECODE(PI.MASK_NM, '*', '1', PA.MASK_NM)                                                  
            --  OR DECODE(PA.MASK_NM, '*', '1', PI.MASK_NM) = DECODE(PA.MASK_NM, '*', '1', PA.MASK_NM))  
                  and ( ((PI.MASK_NM <> '*' AND PI.MASK_NM = AA.MASK_NM) OR (PI.MASK_NM = '*' and 1=1)) OR 
                      ((AA.MASK_NM <> '*' AND PI.MASK_NM =  AA.MASK_NM) OR (AA.MASK_NM  = '*' and 1=1)) )      


             -- AND (DECODE(PI.OP_NO_1, '*', '1', PI.OP_NO_1) = DECODE(PI.OP_NO_1, '*', '1', PA.OP_NO_1)                                                  
            --  OR DECODE(PA.OP_NO_1, '*', '1', PA.OP_NO_1) = DECODE(PA.OP_NO_1, '*', '1', PI.OP_NO_1))                                                               
               and ( ((PI.OP_NO_1 <> '*' AND PI.OP_NO_1 = AA.OP_NO_1) OR (PI.OP_NO_1 = '*' and 1=1)) OR 
                   ((AA.OP_NO_1 <> '*' AND AA.OP_NO_1 =  PI.OP_NO_1) OR (AA.OP_NO_1  = '*' and 1=1)) )  

            -- AND (DECODE(PI.UNIT_ID_1, '*', '1', PI.UNIT_ID_1) = DECODE(PI.UNIT_ID_1, '*', '1', PA.UNIT_ID_1)                                        
            --  OR DECODE(PA.UNIT_ID_1, '*', '1', PA.UNIT_ID_1) = DECODE(PA.UNIT_ID_1, '*', '1', PI.UNIT_ID_1))                                           
              and ( ((PI.UNIT_ID_1 <> '*' AND PI.UNIT_ID_1 = AA.UNIT_ID_1) OR (PI.UNIT_ID_1 = '*' and 1=1)) OR 
               ((AA.UNIT_ID_1 <> '*' AND AA.UNIT_ID_1 =  PI.UNIT_ID_1) OR (AA.UNIT_ID_1  = '*' and 1=1)) )  

            --  AND (DECODE(PI.OP_NO_2, '*', '1', PI.OP_NO_2) = DECODE(PI.OP_NO_2, '*', '1', PA.OP_NO_2)                                                
            --  OR DECODE(PA.OP_NO_2, '*', '1', PA.OP_NO_2) = DECODE(PA.OP_NO_2, '*', '1', PI.OP_NO_2))                                                   

           and ( ((PI.OP_NO_2 <> '*' AND PI.OP_NO_2 = AA.OP_NO_2) OR (PI.OP_NO_2 = '*' and 1=1)) OR 
               ((AA.OP_NO_2 <> '*' AND AA.OP_NO_2 =  PI.OP_NO_2) OR (AA.OP_NO_2  = '*' and 1=1)) ) 

            --  AND (DECODE(PI.UNIT_ID_2, '*', '1', PI.UNIT_ID_2) = DECODE(PI.UNIT_ID_2, '*', '1', PA.MASK_NM)                                          
            --  OR DECODE(PA.UNIT_ID_2, '*', '1', PA.UNIT_ID_2) = DECODE(PA.UNIT_ID_2, '*', '1', PI.UNIT_ID_2))   

             and ( ((PI.UNIT_ID_2 <> '*' AND PI.UNIT_ID_2 = AA.MASK_NM) OR (PI.UNIT_ID_2 = '*' and 1=1)) OR 
               ((AA.UNIT_ID_2 <> '*' AND AA.UNIT_ID_2 =  PI.UNIT_ID_2) OR (AA.UNIT_ID_2  = '*' and 1=1)) ) 

           LEFT JOIN VALID_LOT_INFO LI                                                                                                                 
              ON LI.OP_NO = AA.PR_OP_NO                                                                                                                 
              AND LI.UNIT_ID = AA.PR_UNIT_ID                                                                                                            
              AND LI.APC_TYPE = 'PA'                                                                                                                    
           WHERE 1=1                                                                                                                                    
           AND AA.REMAIN_DAY IS NOT NULL AND AA.REMAIN_DAY <> ''                                                                                                               
           AND AA.MAX_EFCT_TIME IS NOT NULL AND AA.MAX_EFCT_TIME <> ''                                                                                                            
          UNION ALL                                                                                                                                     
           SELECT DISTINCT TL.TOOLG_ID,                                                                                                                 
           PB.PR_UNIT_ID AS TOOL_ID,                                                                                                                    
           '' AS CH_ID, PB.PRODSPEC_ID AS PROD_ID,                                                                                                      
           '' AS PLAN_ID, '' AS STEP_ID,                                                                                                                
           '' AS RECIPE, '' AS PPID,                                                                                                                    
           SUBSTRING(PB.PR_OP_NO, 1, POSITION('.' IN PB.PR_OP_NO)-1) AS LAYER,                                                                                
           SUBSTRING(PB.PR_OP_NO, POSITION('.' IN PB.PR_OP_NO)+1) AS STAGE,                                                                                   
           '' AS TECH, '' AS CUSTOMER,                                                                                                                  
           PI.LOT_ID, 
           -- DECODE(PB.MASK,'*','',PB.MASK) AS RETICLE_ID,   
           (CASE WHEN PB.MASK = '*' THEN '' ELSE PB.MASK END)  AS RETICLE_ID,                                                                                 
           '' AS PRE_TOOL_ID,                                                                                                                           
           'APC_PB' AS RULE_NAME,                                                                                                                       
           '0' AS REMAINING_COUNT, 
           CASE 
               WHEN (PB.FOURTH_VAL_FLG = 2 AND PB.ERR_FLG is null AND PB.REMAIN_DAY = -1 AND PB.PB_FLAG = 'Y') THEN -9999 --2025/04/29 v
               WHEN (PB.PB_FLAG = 'Y' AND CAST(PB.REMAIN_DAY AS decimal) <=0) THEN 9999 
               ELSE CAST(cast(PB.REMAIN_DAY as decimal) *24 AS DECIMAL(18,2)) END  AS REMAINING_TIME,
           Z.RECOVER_TIME, '' AS VALID_RECIPE,                                                                                                          
           --ROUND(PB.MAX_EFEC_DAY * 24,2) AS VALID_TIME,
           CAST(cast(PB.MAX_EFEC_DAY as decimal) *24 AS DECIMAL(18,2)) AS VALID_TIME,                                                                                      
           '' AS HOLD_TIME,                                                                                                                             
           LI.LOT_ID AS VALID_ID, PB.SPLIT_COUNT ,
          CASE WHEN (PB.FOURTH_VAL_FLG = 2 AND PB.ERR_FLG is null AND PB.REMAIN_DAY = -1 AND PB.PB_FLAG = 'Y') THEN 0
                ELSE 1
                END AS PICK_PILOT_MODE                                                                                                     
           FROM  {tempdb}APS_TMP_ETL_PIRUN_PB1  PB                                                                                                       
           INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL TL                                                                                                    
           ON TL.TOOL_ID = PB.PR_UNIT_ID                                                                                                                
           LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_RECOVER_TIME_8 Z                                                                                                                  
           ON Z.PB_TYPE='APC_PB'
           AND Z.TOOLG_ID = TL.TOOLG_ID                                                                                                                        
           LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_INFO PI                                                                                             
           ON PI.APC_TYPE='PB'                                                                                                                          
           AND PI.PROD_ID = PB.PRODSPEC_ID                                                                                                           
           AND PI.C_OP_NO = PB.PR_OP_NO                                                                                                              
           -- AND (DECODE(PB.MASK, '*', '1', PI.MASK_NM) = DECODE(PB.MASK, '*', '1', PB.MASK)                                                           
           -- OR DECODE(PI.MASK_NM, '*', '1', PI.MASK_NM) = DECODE(PI.MASK_NM, '*', '1', PB.MASK))   
           and ( ((PB.MASK <> '*' AND PI.MASK_NM = PB.MASK) OR (PB.MASK = '*' and 1=1)) OR 
            ((PI.MASK_NM <> '*' AND PI.MASK_NM =  PB.MASK) OR (PI.MASK_NM  = '*' and 1=1)) )                                      
           AND PI.C_UNIT_ID = PB.PR_UNIT_ID                                                                                                          
           LEFT JOIN VALID_LOT_INFO LI                                                                                                                  
              ON LI.OP_NO = PB.PR_OP_NO                                                                                                                 
              AND LI.UNIT_ID = PB.PR_UNIT_ID                                                                                                            
              AND LI.APC_TYPE = 'PB'                                                                                                                    
           WHERE 1=1                                                                                                                                    
           AND PB.REMAIN_DAY IS NOT NULL AND PB.REMAIN_DAY <> ''                                                                                                                
           AND PB.MAX_EFEC_DAY IS NOT NULL AND PB.MAX_EFEC_DAY <> ''                                                                                                                                                               
       """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN")


def GetUserFlagIsActiveSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_PATH_SETTING(                  
             USE_FLAG_TYPE, RS_HOLD_TIME, TOOL_ID, TOOLG_ID                                                       
             )                                                                    
              SELECT S.USE_FLAG_TYPE,S.RS_HOLD_TIME,                              
                     T.TOOL_ID,T.TOOLG_ID                                         
              FROM APS_SYNC_UMT_PATH_CONVERT_SETTING.APS_SYNC_UMT_PATH_CONVERT_SETTING S,   
              {tempdb}APS_TMP_ETL_PIRUN_TOOL T                                     
              WHERE S.EQP_G = T.TOOLG_ID AND S.ACTIVE_FLAG = 'Y'                             
              AND S.MODULE = T.MODULE                                                                         
          """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_PATH_SETTING",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_PATH_SETTING")


def GetFirstRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
       INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN1(                     
          TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER, 
          STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT, 
          REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY                                                                   
         )                                                                                
          SELECT TL.TOOLG_ID,                                                             
              CT.EQUIPMENTID AS TOOL_ID,                                                  
              '' AS CH_ID,                                                                
              CT.PRODUCTID AS PROD_ID,                                                    
              CT.ROUTEID || '.%' AS PLAN_ID,                                              
              '' AS STEP_ID,                                                              
              '' AS RECIPE,                                                               
              '' AS PPID,                                                                 
              SUBSTRING(CT.OPENO, 1, POSITION('.' IN CT.OPENO)-1) AS LAYER,                                                                      
              SUBSTRING(CT.OPENO, POSITION('.' IN CT.OPENO) +1) AS STAGE,                        
              '' AS TECH,                                                                 
              '' AS CUSTOMER,                                                             
              '' AS LOT_ID,                                                               
              '' AS RETICLE_ID,                                                           
              '' AS PRE_TOOL_ID,                                                          
              TL.USE_FLAG_TYPE AS RULE_NAME,                                              
              '' AS REMAINING_COUNT,                                                      
              '-1' AS REMAINING_TIME,                                                     
              '' AS RECOVE_TIME,                                                          
              '' AS VALID_RECIPE,                                                         
              '' AS VALID_IMTE,                                                           
              TL.RS_HOLD_TIME AS HOLD_TIME,                                               
              '' AS VALID_LOT,                                                            
              '' AS SPLIT_QTY                                                             
              FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRINHIBIT CT                                     
              INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_PATH_SETTING TL                          
              ON TL.TOOL_ID = CT.EQUIPMENTID                                              
              AND TL.USE_FLAG_TYPE = 'Userflag(1)'  
              WHERE CT.PASS_FLG_PRC='N'                                                   
              AND CT.prc_upt_user ='SPC'                                                                                                                                   
          """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN1")


def GetSecondRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN2(                       
          TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER, 
          STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT, 
          REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY                                                                    
         )                                                                                                                                               
          SELECT TL.TOOLG_ID,                                                              
              CT.EQUIPMENTID AS TOOL_ID,                                                   
              '' AS CH_ID,                                                                 
              CT.PRODUCTID AS PROD_ID,                                                     
              CT.ROUTEID || '.%' AS PLAN_ID,                                               
              '' AS STEP_ID,                                                               
              '' AS RECIPE,                                                                
              '' AS PPID,                                                                  
              SUBSTRING(CT.OPENO, 1, POSITION('.' IN CT.OPENO)-1) AS LAYER,                                                                      
              SUBSTRING(CT.OPENO, POSITION('.' IN CT.OPENO) +1) AS STAGE,                       
              '' AS TECH,                                                                  
              '' AS CUSTOMER,                                                              
              '' AS LOT_ID,                                                                
              '' AS RETICLE_ID,                                                            
              '' AS PRE_TOOL_ID,                                                           
              TL.USE_FLAG_TYPE AS RULE_NAME,                                               
              '' AS REMAINING_COUNT,                                                       
              '-1' AS REMAINING_TIME,                                                      
              '' AS RECOVE_TIME,                                                           
              '' AS VALID_RECIPE,                                                          
              '' AS VALID_IMTE,                                                            
              TL.RS_HOLD_TIME AS HOLD_TIME,                                                
              '' AS VALID_LOT,                                                             
              '' AS SPLIT_QTY                                                              
              FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRINHIBIT CT                                      
              INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_PATH_SETTING TL                           
              ON TL.TOOL_ID = CT.EQUIPMENTID                                               
              AND TL.USE_FLAG_TYPE = 'Userflag(2)' 
              WHERE CT.PASS_FLG_PRC='N'                                                    
              AND CT.prc_upt_user ='ECMS'                                                                                                                                              
           """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN2",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN2")


def getInhibitCase3ReworkData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_REWINFO(                                   
             PRODUCTID, ROUTEID, OPENO, EQUIPMENTID                                                                   
            )                                                                                
                SELECT A.PRODUCTID,A.ROUTEID,                                           
                       A.OPENO,A.EQUIPMENTID                                            
                       FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRINHIBIT A                                   
                       WHERE  1=1                                                            
                       AND SUBSTRING(A.ROUTEID,1,2)='YY'                                   
                       AND A.PASS_FLG_PRC = 'N'                                           
                       AND POSITION('P' IN A.EQUIPMENTID) = 1                                                                                                                                                   
           """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_REWINFO",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_REWINFO")


def getInhibitCase3NotReworkData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
           INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_NOTREWINFO(                                      
                PRODUCTID, OPENO, EQUIPMENTID                                                             
                )                                                                           
                 SELECT DISTINCT B.PRODUCTID,	                                            
                        B.OPENO,B.EQUIPMENTID 	                                            
                 FROM APS_TR_CSFRINHIBIT.APS_TR_CSFRINHIBIT B,{tempdb}APS_TMP_ETL_PIRUN_APS_WIP W                 
                 WHERE POSITION('P' IN B.EQUIPMENTID) = 1                 
                       AND SUBSTRING(B.ROUTEID,1,2)<>'YY'	                                    
                       AND B.PASS_FLG_PRC = 'Y'                                              
                       AND  B.PRODUCTID = W.PROD_ID AND B.OPENO = W.OPE_NO                                                                                                                                                                          
               """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_NOTREWINFO",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_NOTREWINFO")


def GetThirdRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
       INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN3(                      
         TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
         STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
         REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY                                                                   
        )                                                                                                                                              
         WITH CSFRINHIBIT AS(                                                            
              SELECT A.PRODUCTID,A.ROUTEID,                                            
                     A.OPENO,A.EQUIPMENTID                                             
                     FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_REWINFO A, {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_3CASE_NOTREWINFO B                    
                     WHERE 1=1                                                             
                     AND A.OPENO = B.OPENO                                             
                     AND A.PRODUCTID = B.PRODUCTID                                     
                     AND A.EQUIPMENTID = B.EQUIPMENTID                                     
         )                                                                             
         SELECT TL.TOOLG_ID,                                                             
             CT.EQUIPMENTID AS TOOL_ID,                                                    
             '' AS CH_ID,                                                            
             CT.PRODUCTID AS PROD_ID,                                                      
             CT.ROUTEID || '.%' AS PLAN_ID,                                            
             '' AS STEP_ID,                                                            
             '' AS RECIPE,                                                             
             '' AS PPID,                                                             
             --SUBSTR(CT.OPENO, 1, INSTR(CT.OPENO,'.',-1)-1) AS LAYER,                                                                      
             --SUBSTR(CT.OPENO, INSTR(CT.OPENO,'.',-1)+1) AS STAGE,       
              SUBSTRING(CT.OPENO, 1, POSITION('.' IN CT.OPENO)-1) AS LAYER,                                                                      
              SUBSTRING(CT.OPENO, POSITION('.' IN CT.OPENO) +1) AS STAGE,                   
             '' AS TECH,                                                             
             '' AS CUSTOMER,                                                             
             '' AS LOT_ID,                                                             
             '' AS RETICLE_ID,                                                             
             '' AS PRE_TOOL_ID,                                                            
             TL.USE_FLAG_TYPE AS RULE_NAME,                                                
             '' AS REMAINING_COUNT,                                                    
             '-1' AS REMAINING_TIME,                                                     
             '' AS RECOVE_TIME,                                                            
             '' AS VALID_RECIPE,                                                     
             '' AS VALID_IMTE,                                                             
             TL.RS_HOLD_TIME AS HOLD_TIME,                                                 
             '' AS VALID_LOT,                                                              
             '' AS SPLIT_QTY                                                               
             FROM CSFRINHIBIT CT                                                     
             INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_PATH_SETTING TL                            
             ON TL.TOOL_ID = CT.EQUIPMENTID                                                
             AND TL.USE_FLAG_TYPE ='Userflag(3)'                                                                                                                                                                       
                   """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN3",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN3")


def getCase4InhibitData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_CSFINFO(                                        
            PRODUCTID, ROUTEID, OPENO, EQUIPMENTID, PASS_FLG_PRC, UPDATE_TIME, CHIPBODY                                                         
          )                                                                          
           SELECT DISTINCT B.PRODUCTID,B.ROUTEID,                                     
                  B.OPENO,B.EQUIPMENTID,                                              
                  B.PASS_FLG_PRC,B.PRC_UPT_TIME AS UPDATE_TIME,                                       
                  -- SUBSTRING(B.PRODUCTID, 1, POSITION('.' IN B.PRODUCTID)-1) as CHIPBODY  
                  B.CHIPBODY as CHIPBODY                                                       
           FROM APS_TR_CSFRINHIBIT.APS_TR_CSFRINHIBIT B,{tempdb}APS_TMP_ETL_PIRUN_APS_WIP W                
           WHERE POSITION('P' IN B.EQUIPMENTID) = 1                             
                 AND W.PLAN_ID = B.ROUTEID                                     
                 -- AND SUBSTRING(B.PRODUCTID, 1, POSITION('.' IN B.PRODUCTID)-1) = W.CHIPBODY                                         
                 AND B.CHIPBODY = W.CHIPBODY
                 AND B.OPENO = W.OPE_NO                                                                                                                                                                                                                         
               """.format(current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_CSFINFO",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_CSFINFO")


def GetFourthRuleByCSBSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
           INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_FOUR_CSB(                                    
                PRODUCTID, ROUTEID, OPENO, EQUIPMENTID, PASS_FLG_PRC, CHIPBODY                                                                                         
               )                                                                                                                                                                    
                SELECT PRODUCTID,                                                                                                                                                           
                 ROUTEID,                                                                                       
                 OPENO,                                                                                         
                 EQUIPMENTID,                                                                                   
                 PASS_FLG_PRC,CHIPBODY                                                                          
                FROM (                                                                                                
                  SELECT B.PRODUCTID,                                                                           
                         B.ROUTEID,                                                                             
                         B.OPENO,                                                                               
                   B.EQUIPMENTID,                                                                         
                   B.PASS_FLG_PRC,                                                                        
                   CHIPBODY,                                                                              
                   ROW_NUMBER() OVER(PARTITION BY B.ROUTEID,B.OPENO,B.EQUIPMENTID,                        
                   CHIPBODY ORDER BY B.UPDATE_TIME DESC,B.PRODUCTID) AS RN,                                            
                   B.UPDATE_TIME                                                                          
                  FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_CSFINFO B                                            
                     ) R                                                                                              
                WHERE R.RN = 1 AND R.PASS_FLG_PRC = 'Y'                                                                                                                                                                                                                                                              
           """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_FOUR_CSB",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_FOUR_CSB")


def getCase4InhibitResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
            INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_RESULT(                                        
         PRODUCTID, ROUTEID, OPENO, EQUIPMENTID, PASS_FLG_PRC                                                             
        )                                                                          
         SELECT  A.PRODUCTID,A.ROUTEID,                                            
                A.OPENO,A.EQUIPMENTID,A.PASS_FLG_PRC                               
                FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_CSFINFO A, {tempdb}APS_TMP_ETL_PIRUN_FOUR_CSB B                    
                WHERE 1=1 AND A.PASS_FLG_PRC = 'N'                                 
                AND A.OPENO = B.OPENO                                              
                AND A.ROUTEID = B.ROUTEID                                          
                AND A.EQUIPMENTID = B.EQUIPMENTID                                  
                AND A.CHIPBODY = B.CHIPBODY                                                                                                                                                                                                      
               """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_RESULT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_RESULT")


def GetFourthRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
              INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN4(                                                                         
         TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
         STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
         REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY                                                                                                                      
        )                                                                                                                                                                                                 
         SELECT TL.TOOLG_ID,                                                                                                               
             CT.EQUIPMENTID AS TOOL_ID,                                                                                                    
             '' AS CH_ID,                                                                                                                  
             CT.PRODUCTID AS PROD_ID,                                                                                                      
             CT.ROUTEID || '.%' AS PLAN_ID,                                                                                                
             '' AS STEP_ID,                                                                                                                
             '' AS RECIPE,                                                                                                                 
             '' AS PPID,                                                                                                                   
           --  SUBSTR(CT.OPENO, 1, INSTR(CT.OPENO,'.',-1)-1) AS LAYER,                                                                       
            -- SUBSTR(CT.OPENO, INSTR(CT.OPENO,'.',-1)+1) AS STAGE,
               SUBSTRING(CT.OPENO, 1, POSITION('.' IN CT.OPENO)-1) AS LAYER,                                                                      
              SUBSTRING(CT.OPENO, POSITION('.' IN CT.OPENO) +1) AS STAGE,                                                                               
             '' AS TECH,                                                                                                                   
             '' AS CUSTOMER,                                                                                                               
             '' AS LOT_ID,                                                                                                                 
             '' AS RETICLE_ID,                                                                                                             
             '' AS PRE_TOOL_ID,                                                                                                            
             TL.USE_FLAG_TYPE AS RULE_NAME,                                                                                                
             '' AS REMAINING_COUNT,                                                                                                        
             '-1' AS REMAINING_TIME,                                                                                                       
             '' AS RECOVE_TIME,                                                                                                            
             '' AS VALID_RECIPE,                                                                                                           
             '' AS VALID_IMTE,                                                                                                             
             TL.RS_HOLD_TIME AS HOLD_TIME,                                                                                                 
             '' AS VALID_LOT,                                                            
             '' AS SPLIT_QTY                                                             
             FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_4CASE_RESULT CT                                                   
             INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_PATH_SETTING TL                            
             ON TL.TOOL_ID = CT.EQUIPMENTID                                                
             AND TL.USE_FLAG_TYPE ='Userflag(4)'                                                                                                                                                                                                                                           
                  """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN4",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN4")


def getCase5SourceData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
             INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_SOURCE(                                        
         PRODUCTID, ROUTEID, OPENO, EQUIPMENTID, PASS_FLG_PRC, SGS_FIRST_OP, RN                                                            
         )                                                                          
          SELECT T.PRODUCTID,T.ROUTEID,                                                                                  
           	    T.OPENO,T.EQUIPMENTID,T.PASS_FLG_PRC,                                                               
                 F.SGS_FIRST_OP,                                                                                         
           ROW_NUMBER() OVER(PARTITION BY T.PRODUCTID,T.ROUTEID,T.EQUIPMENTID,F.SGS_GROUP ORDER BY CAST(F.OPE_SEQ AS DECIMAL)) AS RN      
           FROM APS_TR_CSFRINHIBIT.APS_TR_CSFRINHIBIT T                                                            
           INNER JOIN APS_SYNC_FLOW.APS_SYNC_FLOW F                                                          
           ON SUBSTRING(F.MNPD_ID, 1, POSITION('.' IN F.MNPD_ID)-1) = T.ROUTEID                                               
           AND F.OPE_NO = T.OPENO                                                                                        
           AND F.SGS_FLAG='Y'                                                                                            
           AND F.PRODSPEC_ID = T.PRODUCTID                                                                               
          WHERE POSITION('P' IN T.EQUIPMENTID) = 1                                                                                                                                                                                                                                                                          
                   """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_SOURCE",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_SOURCE")


def getCase5ResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
          INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_RESULT(                                         
             PRODUCTID, ROUTEID, OPENO, EQUIPMENTID                                                             
             )                                                                           
              WITH CSA AS (                                                              
                 SELECT D.PRODUCTID,D.ROUTEID,                                           
                        D.OPENO,D.EQUIPMENTID,D.RN                                       
                        FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_SOURCE D                
                        WHERE (D.SGS_FIRST_OP <> 'Y' OR D.SGS_FIRST_OP IS NULL OR D.SGS_FIRST_OP = '')                                             
                        AND D.PASS_FLG_PRC = 'N'                                                                             
              ),                                                                                                             
              CSB AS (                                                                                                       
                  SELECT D.PRODUCTID,D.ROUTEID,                                                                              
                     D.OPENO,D.EQUIPMENTID,D.RN                                                                              
                     FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_SOURCE D                                                                                      
                     WHERE D.RN = 1                                                                                          
                     AND D.SGS_FIRST_OP = 'Y'                                                                                
                     AND D.PASS_FLG_PRC = 'Y'                                                                                
              )                                                                                                              
              SELECT  A.PRODUCTID,A.ROUTEID,                                                                            
                     A.OPENO,A.EQUIPMENTID                                                                              
                     FROM CSA A, CSB B                                                                                  
                     WHERE 1=1                                                                                             
                     AND A.ROUTEID = B.ROUTEID                                                                          
                     AND A.EQUIPMENTID = B.EQUIPMENTID                                                                  
                     AND A.PRODUCTID = B.PRODUCTID                                                                      
                     AND A.RN > B.RN                                                                                                                                                                                                                                                                                                                                                         
                       """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_RESULT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_RESULT")


def getCase5AddReworkResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_RESULT(                                         
            PRODUCTID, ROUTEID, OPENO, EQUIPMENTID                                                             
        )                                                                           
        WITH REW_FLOW AS (                                                              
            SELECT D.PROD_ID,D.OPE_NO,
                SUBSTRING(D.PLAN_ID, 1, POSITION('.' IN D.PLAN_ID)-1) AS PLAN_ID                                                                                
            FROM APS_ETL_FLOW.APS_ETL_FLOW D                
            WHERE D.SGS_FLAG = 'Y' 
            AND INSTR(D.PLAN_ID,'YY') = 1                                                                                                                        
        )                                                                                                             
        SELECT A.PRODUCTID,B.PLAN_ID,                                                                            
             A.OPENO,A.EQUIPMENTID                                                                              
        FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_RESULT A, REW_FLOW B                                                                                  
        WHERE 1=1                                                                                                                                                              
        AND A.PRODUCTID = B.PROD_ID
        AND A.OPENO = B.OPE_NO                                                                                                                                                                                                                                                                                                                                                                                                                           
        """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into getCase5AddReworkResultData",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_RESULT")


def GetFifthRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
               INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN5(                                                      
         TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
         STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
         REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY                                                                                                   
         )                                                                                                                                                                              
          SELECT TL.TOOLG_ID,                                                                                            
              CT.EQUIPMENTID AS TOOL_ID,                                                                                 
              '' AS CH_ID,                                                                                               
              CT.PRODUCTID AS PROD_ID,                                                                                   
              CT.ROUTEID || '.%' AS PLAN_ID,                                                                             
              '' AS STEP_ID,                                                                                             
              '' AS RECIPE,                                                                                              
              '' AS PPID,                                                                                                
             -- SUBSTR(CT.OPENO, 1, INSTR(CT.OPENO,'.',-1)-1) AS LAYER,                                                               
             -- SUBSTR(CT.OPENO, INSTR(CT.OPENO,'.',-1)+1) AS STAGE,  
               SUBSTRING(CT.OPENO, 1, POSITION('.' IN CT.OPENO)-1) AS LAYER,                                                                      
              SUBSTRING(CT.OPENO, POSITION('.' IN CT.OPENO) +1) AS STAGE,                                                      
              '' AS TECH,                                                                                                
              '' AS CUSTOMER,                                                                                            
              '' AS LOT_ID,                                                                                              
              '' AS RETICLE_ID,                                                                                          
              '' AS PRE_TOOL_ID,                                                                                         
              TL.USE_FLAG_TYPE AS RULE_NAME,                                                                             
              '' AS REMAINING_COUNT,                                                                                     
              '-1' AS REMAINING_TIME,                                                                                    
              '' AS RECOVE_TIME,                                                                                         
              '' AS VALID_RECIPE,                                                                                        
              '' AS VALID_IMTE,                                                                                          
              TL.RS_HOLD_TIME AS HOLD_TIME,                                                                              
              '' AS VALID_LOT,                                                               
              '' AS SPLIT_QTY                                                                
              FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRIHBT_5CASE_RESULT CT                          
              INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_PATH_SETTING TL                             
              ON TL.TOOL_ID = CT.EQUIPMENTID                                                 
              AND TL.USE_FLAG_TYPE ='Userflag(5)'                                                                                                                                                                                                                                                                                                                                                        
                           """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN5",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN5")


def GetSeventhRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
            INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN7(                     
        TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
         STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
         REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY                                                                           
       )                                                                                                                                              
        SELECT TL.TOOLG_ID,                                                            
            CT.EQUIPMENTID AS TOOL_ID,                                                 
            '' AS CH_ID,                                                               
            CT.PRODUCTID AS PROD_ID,                                                   
            CT.ROUTEID || '.%' AS PLAN_ID,                                             
            '' AS STEP_ID,                                                             
            '' AS RECIPE,                                                              
            '' AS PPID,                                                                
            --SUBSTR(CT.OPENO, 1, INSTR(CT.OPENO,'.',-1)-1) AS LAYER,                                                                    
            --SUBSTR(CT.OPENO, INSTR(CT.OPENO,'.',-1)+1) AS STAGE,         
                SUBSTRING(CT.OPENO, 1, POSITION('.' IN CT.OPENO)-1) AS LAYER,                                                                      
              SUBSTRING(CT.OPENO, POSITION('.' IN CT.OPENO) +1) AS STAGE,                    
            '' AS TECH,                                                                
            '' AS CUSTOMER,                                                            
            '' AS LOT_ID,                                                              
            '' AS RETICLE_ID,                                                          
            '' AS PRE_TOOL_ID,                                                         
            TL.USE_FLAG_TYPE AS RULE_NAME,                                             
            '' AS REMAINING_COUNT,                                                     
            '-1' AS REMAINING_TIME,                                                    
            '' AS RECOVE_TIME,                                                         
            '' AS VALID_RECIPE,                                                        
            '' AS VALID_IMTE,                                                          
            TL.RS_HOLD_TIME AS HOLD_TIME,                                              
            '' AS VALID_LOT,                                                           
            '' AS SPLIT_QTY                                                            
            FROM {tempdb}APS_TMP_ETL_PIRUN_CSFRINHIBIT CT                                    
            INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_PATH_SETTING TL                         
            ON TL.TOOL_ID = CT.EQUIPMENTID                                             
            AND TL.USE_FLAG_TYPE ='Userflag(7)'
            WHERE CT.PASS_FLG_PRC='Y'                                                  
            AND CT.PASS_FLG_MFG='N'                                                    
            AND CT.PRC_UPT_USER='EIWatchDog'                                           
        -- 控片产品的站点.NPR10 PASS_FLG_PRC='Y'  代表已开PATH
            AND NOT REGEXP_MATCHES(CT.OPENO,'NPR10')                                                                                                                                                                                                                                                                                                                                                                                             
        """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN7",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN7")


# 增加全区非炉管区膜厚写入PIRUN
def GetCoatRule1Sql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_RECIPE_ADD(
              TOOLG_ID, TOOL_ID, CH_ID, RULE_NAME, COUNT_UNIT, CURRENT_COUNT, LSL, USL, PROCESS_TIME,PARAMETER, ALL_CH_FLAG, CLEAN_MON_TIME, MODULE, CLEAN_GROUP, CLEAN_FIX_FLAG)                                                                                      
                 SELECT TL.TOOLG_ID,                                                   
                        TL.TOOL_ID,    
                        CASE WHEN E.CHAMBER_NO>' ' AND EXISTS (SELECT 1 FROM {tempdb}APS_TMP_ETL_PIRUN_TOOL N WHERE N.TOOL_ID = TL.TOOL_ID||'.'||E.CHAMBER_NO AND N.EQP_CHAMBER_FLAG='Chamber') THEN TL.TOOL_ID||'.'||E.CHAMBER_NO END AS CH_ID,                                                 
                        E.PM_TYPE AS RULE_NAME,                                        
                        E.SPEC_UNIT AS COUNT_UNIT,                                     
                        E.DC_ITEM_VALUE AS CURRENT_COUNT,                              
                        (CAST(E.PM_SPEC AS DECIMAL) - CAST(E.RANGE AS DECIMAL)) AS LSL,                                  
                        E.PM_SPEC AS USL,                                              
                        E.TARGET_TIME AS PROCESS_TIME, 
                        E.DC_ITEM_NAME,     
                        E.DOWN_FLAG, 
                        COALESCE(E.TEST_TIME,0) AS CLEAN_MON_TIME,   
                        TL.MODULE, 
                        E.PM_GROUP AS CLEAN_GROUP, 
                        CASE WHEN E.PM_LOCK_FLAG = 'Y' THEN '1' END AS CLEAN_FIX_FLAG                                
                 FROM APS_SYNC_TMS_EQP_PARAMETER.APS_SYNC_TMS_EQP_PARAMETER E,             
                      {tempdb}APS_TMP_ETL_PIRUN_TOOL TL                                   
                 WHERE  E.EQUIPMENT_NO = TL.TOOL_ID                                         
                       AND E.DC_ITEM_VALUE IS NOT NULL AND E.DC_ITEM_VALUE <> ''                                
                       AND E.SPEC_UNIT NOT IN ('Days','Months')                                                                                                                                                                                                                                                                                                                                                                                                                   
           """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_RECIPE_ADD",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_RECIPE_ADD")


# equipment_no、chamber_no、dc_item_name、spec_unit相同需要合并起来
# 需要合并成一条 rule_name 用逗号分隔。process_time 相加
def GetCoatRule1GroupSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_RECIPE_ADD_GROUP(
              TOOLG_ID, TOOL_ID, CH_ID, RULE_NAME, COUNT_UNIT, CURRENT_COUNT, LSL, USL, PROCESS_TIME,PARAMETER, ALL_CH_FLAG, CLEAN_MON_TIME, MODULE, CLEAN_GROUP, CLEAN_FIX_FLAG
              ) 
              WITH SOURCE_DATA AS (                                                                                     
                 SELECT TL.TOOLG_ID,                                                   
                        TL.TOOL_ID,    
                        TL.CH_ID,                                                 
                        (
                            SELECT STRING_AGG(distinct RULE_NAME,','  ORDER BY RULE_NAME)
                            FROM {tempdb}APS_TMP_ETL_PIRUN_RECIPE_ADD AD
                            WHERE AD.TOOL_ID = TL.TOOL_ID 
                            AND COALESCE(AD.CH_ID,'*') = COALESCE(TL.CH_ID,'*') 
                            AND COALESCE(AD.PARAMETER,'*') = COALESCE(TL.PARAMETER,'*') 
                            AND COALESCE(AD.COUNT_UNIT,'*') = COALESCE(TL.COUNT_UNIT,'*')
                        ) AS RULE_NAME,                                        
                        TL.COUNT_UNIT,                                     
                        TL.CURRENT_COUNT,                              
                        TL.LSL,                                  
                        TL.USL,                                              
                        TL.PROCESS_TIME, 
                        TL.PARAMETER,     
                        CASE WHEN TL.ALL_CH_FLAG = '1' AND TL.CLEAN_GROUP IS NOT NULL THEN NULL ELSE TL.ALL_CH_FLAG END AS ALL_CH_FLAG, 
                        TL.CLEAN_MON_TIME,   
                        TL.MODULE, 
                        CASE WHEN (TL.ALL_CH_FLAG IS NULL OR TL.ALL_CH_FLAG <> '1') AND TL.CH_ID IS NOT NULL 
                             THEN TL.CLEAN_GROUP || '.' || SUBSTRING(TL.CH_ID, POSITION('.' IN TL.CH_ID)+1)
                             ELSE TL.CLEAN_GROUP 
                             END AS CLEAN_GROUP , 
                        TL.CLEAN_FIX_FLAG                                
                 FROM {tempdb}APS_TMP_ETL_PIRUN_RECIPE_ADD TL  
              )
              SELECT MAX(TL.TOOLG_ID) AS TOOLG_ID,                                                   
                    TL.TOOL_ID,    
                    TL.CH_ID,                                                 
                    MAX(RULE_NAME) AS RULE_NAME,                                        
                    TL.COUNT_UNIT,                                     
                    MAX(TL.CURRENT_COUNT) AS CURRENT_COUNT,                              
                    MAX(TL.LSL) AS LSL,                                  
                    MAX(TL.USL) AS USL,                                              
                    SUM(CAST(TL.PROCESS_TIME AS DECIMAL)) AS PROCESS_TIME, 
                    TL.PARAMETER,     
                    MAX(TL.ALL_CH_FLAG) AS ALL_CH_FLAG, 
                    MAX(TL.CLEAN_MON_TIME) AS CLEAN_MON_TIME,   
                    MAX(TL.MODULE) AS MODULE, 
                    MAX(TL.CLEAN_GROUP) AS CLEAN_GROUP, 
                    MAX(TL.CLEAN_FIX_FLAG) AS CLEAN_FIX_FLAG  
              FROM SOURCE_DATA TL
              GROUP BY TOOL_ID,CH_ID,PARAMETER,COUNT_UNIT        

           """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_RECIPE_ADD_GROUP",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_RECIPE_ADD_GROUP")


def GetCoatRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */  INTO APS_ETL_PIRUN(
              PARENTID,TOOLG_ID, TOOL_ID, CH_ID, RULE_NAME, COUNT_UNIT, CURRENT_COUNT, LSL, USL, PROCESS_TIME,PARAMETER, ALL_CH_FLAG, CLEAN_MON_TIME, CLEAN_ACC_FLAG, CLEAN_GROUP, CLEAN_FIX_FLAG, UPDATE_TIME, PARTCODE)                                                          
                 SELECT DISTINCT '{uuid}' AS PARENTID,                             
                        TL.TOOLG_ID,                                                   
                        TL.TOOL_ID,    
                        TL.CH_ID,                                                 
                        TL.RULE_NAME,                                        
                        TL.COUNT_UNIT,                                     
                        TL.CURRENT_COUNT,                               
                        TL.LSL,                                  
                        TL.USL,                                             
                        TL.PROCESS_TIME, 
                        TL.PARAMETER,     
                        TL.ALL_CH_FLAG,
                        TL.CLEAN_MON_TIME, 
                        CASE WHEN S.MODULE IS NOT NULL AND S.MODULE <> '' THEN 1 END AS CLEAN_ACC_FLAG,  
                        TL.CLEAN_GROUP,  
                        TL.CLEAN_FIX_FLAG,                    
                        '{current_time}' AS UPDATE_TIME,                          
                        '' AS PARTCODE               
                 FROM {tempdb}APS_TMP_ETL_PIRUN_RECIPE_ADD_GROUP TL                                   
                 LEFT JOIN APS_SYNC_UMT_EQP_PM_DEFAULT_SETTING.APS_SYNC_UMT_EQP_PM_DEFAULT_SETTING S   
                 ON TL.MODULE = S.MODULE                                                
                 AND TL.TOOLG_ID LIKE REPLACE(S.EQP_G,'*','%')                          
                 AND TL.TOOL_ID LIKE REPLACE(S.EQP_ID,'*','%')                          
                 AND COALESCE(TL.CH_ID,'*') LIKE REPLACE(S.CHAMBER_NO,'*','%')               
                 AND TL.RULE_NAME LIKE REPLACE(S.PM_TYPE,'*','%')                       
                 AND S.ITEM_NAME ='TARGET_TYPE'                                                                                                                                                                                                                                                                                                                                                                                                                  
           """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetDfByRecipeAcceptSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """     
            INSERT  /*+ append */  INTO APS_ETL_PIRUN(
                        PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PPID, LAYER, STAGE,
                        RULE_NAME, COUNT_UNIT, CURRENT_COUNT, LSL, USL, PROCESS_TIME, PARAMETER, 
                        PRODG_ID, UPDATE_TIME, PARTCODE,WIP_ATTR3)  
            WITH SOURCE_DATA AS (                                                           
                 SELECT DISTINCT '{uuid}' AS PARENTID,                              
                        E.TOOLG_ID,                                                     
                        E.TOOL_ID, 
                        CASE WHEN P.CHAMBER_NO>' ' AND EXISTS (SELECT 1 FROM {tempdb}APS_TMP_ETL_PIRUN_TOOL N WHERE N.TOOL_ID = E.TOOL_ID||'.'||P.CHAMBER_NO AND N.EQP_CHAMBER_FLAG='Chamber') THEN E.TOOL_ID||'.'||P.CHAMBER_NO END AS CH_ID,                                                                                                     
                        CASE WHEN S.TYPE = 2 THEN  REPLACE(S.PRODG1,'*','%') END AS PROD_ID,                       
                        REPLACE(S.RECIPE_ID,'*','%') AS PPID,                                               
                        REPLACE(SUBSTRING(S.OPE_NO, 1, POSITION('.' IN S.OPE_NO)-1),'*','%') AS LAYER,                                                                      
                        REPLACE(SUBSTRING(S.OPE_NO, POSITION('.' IN S.OPE_NO) +1),'*','%') AS STAGE,       
                        P.PM_TYPE AS RULE_NAME,                                         
                        P.SPEC_UNIT AS COUNT_UNIT,                                      
                        P.DC_ITEM_VALUE AS CURRENT_COUNT,                               
                        S.LST,                                                          
                        S.UST,                                                          
                        '' AS PROCESS_TIME,  
                        S.ITEM_NAME, 
                        CASE WHEN S.TYPE = 1 THEN  REPLACE(S.PRODG1,'*','%') END AS PRODG_ID,                                           
                        '{current_time}' AS UPDATE_TIME,                           
                        '' AS PARTCODE                
                 FROM APS_SYNC_RTD_LIFE_TIME_CONTROL.APS_SYNC_RTD_LIFE_TIME_CONTROL S         
                 INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL E                               
                 ON E.TOOL_ID LIKE REPLACE(S.EQP_ID,'*','%') AND E.HOST_EQP_FLAG='Y'                                                
                 INNER JOIN APS_SYNC_TMS_EQP_PARAMETER.APS_SYNC_TMS_EQP_PARAMETER P        
                 ON P.EQUIPMENT_NO = E.TOOL_ID     
                 AND S.ITEM_NAME = P.DC_ITEM_NAME  
                 WHERE INSTR(S.EQP_ID,'W') <> 1  AND S.SPEC_TYPE = 'Accept'  
             )  
            SELECT PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PPID, LAYER, STAGE,
                   RULE_NAME, COUNT_UNIT, CURRENT_COUNT, LST, UST, PROCESS_TIME, ITEM_NAME, 
                   PRODG_ID, UPDATE_TIME, PARTCODE,
                   CASE WHEN CH_ID IS NULL or ch_id ='' THEN NULL else '0' END AS WIP_ATTR3
            FROM  SOURCE_DATA                                                                                                                                                                                                                                                                                                                                                                                                           
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetDfByRecipeRejectSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """     
            INSERT  /*+ append */  INTO APS_ETL_PIRUN(
                        PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PPID, LAYER, STAGE,
                        RULE_NAME, COUNT_UNIT, CURRENT_COUNT, LSL, USL, PROCESS_TIME, PARAMETER, 
                        PRODG_ID, UPDATE_TIME, PARTCODE, WIP_ATTR3)  
            WITH SOURCE_DATA AS (                                                           
                 SELECT '{uuid}' AS PARENTID,                              
                        E.TOOLG_ID,                                                     
                        E.TOOL_ID,                                                       
                        CASE WHEN P.CHAMBER_NO>' ' AND EXISTS (SELECT 1 FROM {tempdb}APS_TMP_ETL_PIRUN_TOOL N WHERE N.TOOL_ID = E.TOOL_ID||'.'||P.CHAMBER_NO AND N.EQP_CHAMBER_FLAG='Chamber') THEN E.TOOL_ID||'.'||P.CHAMBER_NO END AS CH_ID,                                                                                                                                                        
                        CASE WHEN S.TYPE = 2 THEN  REPLACE(S.PRODG1,'*','%') END AS PROD_ID,                       
                        REPLACE(S.RECIPE_ID,'*','%') AS PPID,                                               
                        REPLACE(SUBSTRING(S.OPE_NO, 1, POSITION('.' IN S.OPE_NO)-1),'*','%') AS LAYER,                                                                      
                        REPLACE(SUBSTRING(S.OPE_NO, POSITION('.' IN S.OPE_NO) +1),'*','%') AS STAGE,       
                        P.PM_TYPE AS RULE_NAME,                                         
                        P.SPEC_UNIT AS COUNT_UNIT,                                      
                        P.DC_ITEM_VALUE AS CURRENT_COUNT,                               
                        0 AS LST,                                                          
                        S.LST AS UST,                                                          
                        '' AS PROCESS_TIME,  
                        S.ITEM_NAME, 
                        CASE WHEN S.TYPE = 1 THEN  REPLACE(S.PRODG1,'*','%') END AS PRODG_ID,                                           
                        '{current_time}' AS UPDATE_TIME,                           
                        '' AS PARTCODE                
                 FROM APS_SYNC_RTD_LIFE_TIME_CONTROL.APS_SYNC_RTD_LIFE_TIME_CONTROL S         
                 INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL E                               
                 ON E.TOOL_ID LIKE REPLACE(S.EQP_ID,'*','%') AND E.HOST_EQP_FLAG='Y'                                                
                 INNER JOIN APS_SYNC_TMS_EQP_PARAMETER.APS_SYNC_TMS_EQP_PARAMETER P        
                 ON P.EQUIPMENT_NO = E.TOOL_ID     
                 AND S.ITEM_NAME = P.DC_ITEM_NAME  
                 WHERE INSTR(S.EQP_ID,'W') <> 1  AND S.SPEC_TYPE = 'Reject'
                 AND S.LST > 0  
                 UNION ALL
                 SELECT '{uuid}' AS PARENTID,                              
                        E.TOOLG_ID,                                                     
                        E.TOOL_ID,                                                       
                        CASE WHEN P.CHAMBER_NO>' ' AND EXISTS (SELECT 1 FROM {tempdb}APS_TMP_ETL_PIRUN_TOOL N WHERE N.TOOL_ID = E.TOOL_ID||'.'||P.CHAMBER_NO AND N.EQP_CHAMBER_FLAG='Chamber') THEN E.TOOL_ID||'.'||P.CHAMBER_NO END AS CH_ID,                                                                                                                                                        
                        CASE WHEN S.TYPE = 2 THEN  REPLACE(S.PRODG1,'*','%') END AS PROD_ID,                       
                        REPLACE(S.RECIPE_ID,'*','%') AS PPID,                                               
                        REPLACE(SUBSTRING(S.OPE_NO, 1, POSITION('.' IN S.OPE_NO)-1),'*','%') AS LAYER,                                                                      
                        REPLACE(SUBSTRING(S.OPE_NO, POSITION('.' IN S.OPE_NO) +1),'*','%') AS STAGE,       
                        P.PM_TYPE AS RULE_NAME,                                         
                        P.SPEC_UNIT AS COUNT_UNIT,                                      
                        P.DC_ITEM_VALUE AS CURRENT_COUNT,                               
                        S.UST AS LST,                                                          
                        cast(99999 as varchar) AS UST,                                                          
                        '' AS PROCESS_TIME,  
                        S.ITEM_NAME, 
                        CASE WHEN S.TYPE = 1 THEN  REPLACE(S.PRODG1,'*','%') END AS PRODG_ID,                                           
                        '{current_time}' AS UPDATE_TIME,                           
                        '' AS PARTCODE                
                 FROM APS_SYNC_RTD_LIFE_TIME_CONTROL.APS_SYNC_RTD_LIFE_TIME_CONTROL S         
                 INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL E                               
                 ON E.TOOL_ID LIKE REPLACE(S.EQP_ID,'*','%') AND E.HOST_EQP_FLAG='Y'                                                
                 INNER JOIN APS_SYNC_TMS_EQP_PARAMETER.APS_SYNC_TMS_EQP_PARAMETER P        
                 ON P.EQUIPMENT_NO = E.TOOL_ID     
                 AND S.ITEM_NAME = P.DC_ITEM_NAME  
                 WHERE INSTR(S.EQP_ID,'W') <> 1  AND S.SPEC_TYPE = 'Reject' 
             ) 
             SELECT PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PPID, LAYER, STAGE,
                   RULE_NAME, COUNT_UNIT, CURRENT_COUNT, LST, UST, PROCESS_TIME, ITEM_NAME, 
                   PRODG_ID, UPDATE_TIME, PARTCODE,
                   CASE WHEN CH_ID IS NULL or ch_id ='' THEN NULL else '0' END AS WIP_ATTR3
            FROM  SOURCE_DATA                                                                                                                                                                                                                                                                                                                                                                                                                               
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetWetByRecipeAcceptSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */  INTO APS_ETL_PIRUN(
                  PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PPID, LAYER, STAGE,
                  RULE_NAME, COUNT_UNIT, CURRENT_COUNT, LSL, USL, PROCESS_TIME, PARAMETER, PRODG_ID, UPDATE_TIME, PARTCODE)                                                               
         SELECT DISTINCT '{uuid}' AS PARENTID,                                                  
               I.TOOLG_ID,                                                                          
               D.EQP_ID,                                                                            
               '' AS CH_ID,                                                                         
               CASE WHEN S.TYPE = 2 THEN  REPLACE(S.PRODG1,'*','%') END AS PROD_ID,                                            
               '' AS PPID,                                                                          
               --DECODE(S.OPER_NO,'*','',SUBSTRING(S.OPER_NO, 1, POSITION('.' IN S.OPER_NO)-1)) AS LAYER,  
               (CASE WHEN S.OPE_NO = '*' THEN '' ELSE REPLACE(SUBSTRING(S.OPE_NO, 1, POSITION('.' IN S.OPE_NO)-1),'*','%') END) AS LAYER,
              -- DECODE(S.OPER_NO,'*','',SUBSTRING(S.OPER_NO, POSITION('.' IN S.OPER_NO)+1)) AS STAGE,     
                (CASE WHEN S.OPE_NO = '*' THEN '' ELSE REPLACE(SUBSTRING(S.OPE_NO, POSITION('.' IN S.OPE_NO)+1),'*','%') END) AS STAGE, 
               'Acid Control' AS RULE_NAME,                                                         
               CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN 'lifetime' ELSE 'Pieces' END AS COUNT_UNIT,   
               CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN ROUND(cast(D.VALUE as decimal)/60,3) ELSE cast(D.VALUE as decimal) END  AS CURRENT_COUNT, 
               CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN ROUND(cast(S.LST as decimal)/60,3) ELSE S.LST END AS LSL,           
               CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN ROUND(cast(S.UST as decimal)/60,3) ELSE S.UST END AS USL,           
               '' AS PROCESS_TIME,      
                S.ITEM_NAME, 
                CASE WHEN S.TYPE = 1 THEN  REPLACE(S.PRODG1,'*','%') END AS PRODG_ID,                                                           
               '{current_time}' AS UPDATE_TIME,                                               
                '' AS PARTCODE                                    
         FROM APS_SYNC_RTD_LIFE_TIME_CONTROL.APS_SYNC_RTD_LIFE_TIME_CONTROL S                           
         INNER JOIN APS_SYNC_WET_ACID_DATA.APS_SYNC_WET_ACID_DATA D
         ON D.EQP_ID LIKE REPLACE(S.EQP_ID,'*','%')                                      
         AND D.ITEM_NAME = S.ITEM_NAME  
         INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL I                                                   
         ON I.TOOL_ID = D.EQP_ID                                                                                                                                   
         WHERE S.SPEC_TYPE = 'Accept'  
         AND POSITION('W' IN S.EQP_ID) = 1                                                                                                                                                                                                                                                                                                                                                                                                                         
         """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetWetByRecipeRejectSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */  INTO APS_ETL_PIRUN(
                  PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PPID, LAYER, STAGE,
                   --modify by xiecheng for version up
                  RULE_NAME, COUNT_UNIT, CURRENT_COUNT, LSL, USL, PROCESS_TIME, PARAMETER, PRODG_ID,UPDATE_TIME, PARTCODE)                                                                  
        SELECT '{uuid}' AS PARENTID,                                                  
              I.TOOLG_ID,                                                                          
              D.EQP_ID,                                                                            
              '' AS CH_ID,                                                                         
              CASE WHEN S.TYPE = 2 THEN  REPLACE(S.PRODG1,'*','%') END AS PROD_ID,                                            
              '' AS PPID,                                                                           
               --DECODE(S.OPER_NO,'*','',SUBSTRING(S.OPER_NO, 1, POSITION('.' IN S.OPER_NO)-1)) AS LAYER,  
               (CASE WHEN S.OPE_NO = '*' THEN '' ELSE REPLACE(SUBSTRING(S.OPE_NO, 1, POSITION('.' IN S.OPE_NO)-1),'*','%') END) AS LAYER,
              -- DECODE(S.OPER_NO,'*','',SUBSTRING(S.OPER_NO, POSITION('.' IN S.OPER_NO)+1)) AS STAGE,     
                (CASE WHEN S.OPE_NO = '*' THEN '' ELSE REPLACE(SUBSTRING(S.OPE_NO, POSITION('.' IN S.OPE_NO)+1),'*','%') END) AS STAGE,     
              'Acid Control' AS RULE_NAME,                                                         
              CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN 'lifetime' ELSE 'Pieces' END AS COUNT_UNIT,     
              CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN ROUND(cast(D.VALUE as decimal)/60,3) ELSE cast(D.VALUE as decimal) END  AS CURRENT_COUNT,  
              0 AS LSL,                                                                            
              CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN ROUND(cast(S.LST as decimal)/60,3) ELSE S.LST END AS USL,         
              '' AS PROCESS_TIME,    
                --modify by xiecheng for version up     
              S.ITEM_NAME, 
              CASE WHEN S.TYPE = 1 THEN  REPLACE(S.PRODG1,'*','%') END AS PRODG_ID,                                                               
              '{current_time}' AS UPDATE_TIME,                                               
              '' AS PARTCODE                                    
        FROM APS_SYNC_RTD_LIFE_TIME_CONTROL.APS_SYNC_RTD_LIFE_TIME_CONTROL S                           
        INNER JOIN APS_SYNC_WET_ACID_DATA.APS_SYNC_WET_ACID_DATA D                                
        ON D.EQP_ID LIKE REPLACE(S.EQP_ID,'*','%')                            
        AND D.ITEM_NAME = S.ITEM_NAME 
        INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL I                                                   
        ON I.TOOL_ID = D.EQP_ID                                                                                                                                
        WHERE S.SPEC_TYPE = 'Reject'                                                                 
        AND S.LST > 0 
        AND POSITION('W' IN　S.EQP_ID) = 1                                                                              
        UNION ALL                                                                                  
        SELECT '{uuid}' AS PARENTID,                                                  
              I.TOOLG_ID,                                                                          
              D.EQP_ID,                                                                            
              '' AS CH_ID,                                                                         
              CASE WHEN S.TYPE = 2 THEN  REPLACE(S.PRODG1,'*','%') END AS PROD_ID,                                           
              '' AS PPID,                                                                          
               --DECODE(S.OPER_NO,'*','',SUBSTRING(S.OPER_NO, 1, POSITION('.' IN S.OPER_NO)-1)) AS LAYER,  
               (CASE WHEN S.OPE_NO = '*' THEN '' ELSE REPLACE(SUBSTRING(S.OPE_NO, 1, POSITION('.' IN S.OPE_NO)-1),'*','%') END) AS LAYER,
              -- DECODE(S.OPER_NO,'*','',SUBSTRING(S.OPER_NO, POSITION('.' IN S.OPER_NO)+1)) AS STAGE,     
                (CASE WHEN S.OPE_NO = '*' THEN '' ELSE REPLACE(SUBSTRING(S.OPE_NO, POSITION('.' IN S.OPE_NO)+1),'*','%') END) AS STAGE,
              'Acid Control' AS RULE_NAME,                                                         
              CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN 'lifetime' ELSE 'Pieces' END AS COUNT_UNIT,     
              CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN ROUND(cast(D.VALUE as decimal)/60,3) ELSE cast(D.VALUE as decimal) END  AS CURRENT_COUNT,  
              CASE WHEN SUBSTRING(S.ITEM_NAME,1,1) = 'K' THEN ROUND(cast(S.UST as decimal)/60,3) ELSE S.UST END AS LSL,         
              cast(99999 as varchar) AS USL,                                                                        
              '' AS PROCESS_TIME,    
               --modify by xiecheng for version up     
                S.ITEM_NAME,  
                CASE WHEN S.TYPE = 1 THEN  REPLACE(S.PRODG1,'*','%') END AS PRODG_ID,                                                              
              '{current_time}' AS UPDATE_TIME,                                               
               '' AS PARTCODE                                    
        FROM APS_SYNC_RTD_LIFE_TIME_CONTROL.APS_SYNC_RTD_LIFE_TIME_CONTROL S                                                          
        INNER JOIN APS_SYNC_WET_ACID_DATA.APS_SYNC_WET_ACID_DATA D                                
        ON D.EQP_ID LIKE REPLACE(S.EQP_ID,'*','%')                                 
        AND D.ITEM_NAME = S.ITEM_NAME 
        INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL I                                                   
        ON I.TOOL_ID = D.EQP_ID                                                                                                                              
        WHERE  S.SPEC_TYPE = 'Reject' 
        AND POSITION('W' IN S.EQP_ID) = 1                                                                                                                                                                                                                                                                                                                                                                                                                        
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetWaitTimeSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */  INTO APS_ETL_PIRUN(
                 PARENTID, TOOLG_ID, TOOL_ID, CH_ID, LAYER, STAGE,
                 RULE_NAME, HOLD_TIME, UPDATE_TIME, PARTCODE)                                                                
         SELECT DISTINCT '{uuid}' AS PARENTID,                                                                       
                S.EQP_G AS TOOLG_ID,                                                                             
                T.TOOL_ID,                                                                                       
                '' AS CH_ID,                                                                                     
                CASE WHEN S.OPE_NO='*' THEN ''                                                                   
                     WHEN S.OPE_NO LIKE '%*' THEN SUBSTRING(S.OPE_NO,1,2)                                           
                ELSE SUBSTRING(S.OPE_NO, 1, POSITION('.' IN S.OPE_NO)-1 ) END AS LAYER,                               
                CASE WHEN S.OPE_NO='*' THEN ''                                                                   
                     WHEN S.OPE_NO LIKE '%*' THEN ''                                                             
                ELSE SUBSTRING(S.OPE_NO, POSITION('.' IN S.OPE_NO)+1 ) END AS STAGE,                                  
                CASE WHEN S.MODULE='WET' THEN 'ChemicalChange' ELSE 'AccConstraint' END AS RULE_NAME,            
                S.WAIT_TIME AS HOLD_TIME,                                                                        
                '{current_time}' AS UPDATE_TIME,                                                            
                '' AS PARTCODE                                                 
         FROM APS_SYNC_EQP_STATUS_WAIT_TIME_SETTING.APS_SYNC_EQP_STATUS_WAIT_TIME_SETTING S, {tempdb}APS_TMP_ETL_PIRUN_TOOL T        
         WHERE S.MODULE IN ('DIFF','WET')                                                                                                                                                        
         AND T.TOOL_ID LIKE REPLACE(S.EQP_ID,'*','%')                                                            
         AND T.TOOLG_ID = S.EQP_G                                                                                
         AND T.MODULE IN ('DIFF','WET')                                                                                                                                                                                                                                                                                                                                                                                                                                
               """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetTestRunRecipeInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
            INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_RECIPE(
                 --modify by xiecheng for version up
                M_RECIPE,MODULE,TOOLID,EXPIRETIME,JOBGROUPID,JOBSPECID,LIFETIME,SUBNAME,EQP_G,STATE)                                                             
             SELECT *                                                                    
             FROM (                                                                      
                 SELECT                                                                  
                       SUBSTRING(T.DKEY,POSITION('_' IN T.DKEY)+1,LENGTH(T.DKEY)) AS M_RECIPE,
                       D.MODULE,    
                       T.TOOLID,                                                         
                      -- TO_CHAR(T.EXPIRETIME, 'YYYY-MM-DD HH24:MI:SS') AS EXPIRETIME, 
                       STRFTIME(T.EXPIRETIME, '%Y-%m-%d %H:%M:%S') AS EXPIRETIME,    
                       T.JOBGROUPID,                                                     
                       T.JOBSPECID,                                                      
                       T.LIFETIME,                                                       
                       T.SUBNAME,                                                        
                       D.TOOLG_ID,  
                       --modify by xiecheng for version up
                       T.STATE                                                       
                 FROM APS_SYNC_RDS_RECIPEINFO.APS_SYNC_RDS_RECIPEINFO T                  
                 INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL D                               
                 ON D.TOOL_ID = T.TOOLID AND D.HOST_EQP_FLAG = 'Y'                       
                 WHERE POSITION('F' IN T.TOOLID) = 1                                             
             )  WHERE  M_RECIPE IS NOT NULL AND M_RECIPE <> ''                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
                 """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_TEST_RUN_RECIPE",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_TEST_RUN_RECIPE")


def GetTestRunT21MedianInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
          INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_T21 (
               TOOLG_ID, TOOL_ID, PPID, PT, MODULE_ID)                                                             
             SELECT L.TOOLG_ID, L.TOOL_ID,                                    
                    L.PPID, L.PT, L.MODULE_ID                                          
             FROM APS_TMP_TYPE21_MEDIAN.APS_TMP_TYPE21_MEDIAN L                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           
                   """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_TEST_RUN_T21",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_TEST_RUN_T21")


def GetTestRunT22MedianInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_T22 (
                       TOOLG_ID, PPID, PT, MODULE_ID)                                                               
            SELECT L.TOOLG_ID,                                               
                   L.PPID, L.PT, L.MODULE_ID                                              
            FROM APS_TMP_TYPE22_MEDIAN.APS_TMP_TYPE22_MEDIAN L                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
            """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_TEST_RUN_T22",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_TEST_RUN_T22")


def GetTestRunT221MedianInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
           INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_T221(
                 TOOLG_ID, TOOL_ID, CH_CNT, PT, MODULE_ID)                                                               
                SELECT L.TOOLG_ID, L.TOOL_ID,                                   
                       L.CH_CNT, L.PT, L.MODULE_ID                                             
                FROM APS_TMP_TYPE221_MEDIAN.APS_TMP_TYPE221_MEDIAN L                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_TEST_RUN_T221",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_TEST_RUN_T221")


def GetRtdRunPStartSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
            INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_PS (
                 EQP_ID,OPE_NO,PRODSPEC_ID,CLAIM_TIME,OPE_CATEGORY,LOT_ID,CTRL_JOB)                                                                                                                            
                 SELECT S.EQP_ID,S.OPE_NO,S.PRODSPEC_ID,                                                                           
                     S.CLAIM_TIME,S.OPE_CATEGORY,                                                                      
                     S.LOT_ID,S.CTRL_JOB                                                                               
                 FROM APS_MID_FHOPEHS_VIEW.APS_MID_FHOPEHS_VIEW S                                                                                                 
                 WHERE S.OPE_CATEGORY ='OperationStart'                                                                               
                 AND (                                                                                                                
                  (STRFTIME(CURRENT_TIMESTAMP, '%H:%M:%S')  >= '07:30:00'                                                                     
                  AND S.CLAIM_TIME >= CONCAT(STRFTIME(CURRENT_TIMESTAMP, '%Y-%m-%d'), ' 07:30:00')       
                  AND S.CLAIM_TIME < CONCAT(STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day', '%Y-%m-%d'), ' 07:30:00'))      
                  OR                                                                                                               
                  (STRFTIME(CURRENT_TIMESTAMP, '%H:%M:%S') < '07:30:00'                                                                      
                  AND S.CLAIM_TIME >= CONCAT(STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '1 day', '%Y-%m-%d'), ' 07:30:00')     
                  AND S.CLAIM_TIME < CONCAT(STRFTIME(CURRENT_TIMESTAMP, '%Y-%m-%d'), ' 07:30:00'))          
         )                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_RTDRUN_PS",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_RTDRUN_PS")


def GetRtdRunPEndSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
            INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_PE (
                EQP_ID,OPE_NO,PRODSPEC_ID,CLAIM_TIME,OPE_CATEGORY,LOT_ID,CTRL_JOB )                                                                                                                                                
              SELECT S.EQP_ID,S.OPE_NO,S.PRODSPEC_ID,                                                                           
                    S.CLAIM_TIME,S.OPE_CATEGORY,                                                                      
                    S.LOT_ID,S.CTRL_JOB                                                                               
              FROM APS_MID_FHOPEHS_VIEW.APS_MID_FHOPEHS_VIEW S                                                                                                 
              WHERE S.OPE_CATEGORY ='OperationStartCancle'                                                                         
              AND (                                                                                                                
              (STRFTIME(CURRENT_TIMESTAMP, '%H:%M:%S')  >= '07:30:00'                                                                     
              AND S.CLAIM_TIME >= CONCAT(STRFTIME(CURRENT_TIMESTAMP, '%Y-%m-%d'), ' 07:30:00')       
              AND S.CLAIM_TIME < CONCAT(STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day', '%Y-%m-%d'), ' 07:30:00'))      
              OR                                                                                                                
              (STRFTIME(CURRENT_TIMESTAMP, '%H:%M:%S') < '07:30:00'                                                                      
              AND S.CLAIM_TIME >= CONCAT(STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '1 day', '%Y-%m-%d'), ' 07:30:00')     
              AND S.CLAIM_TIME < CONCAT(STRFTIME(CURRENT_TIMESTAMP, '%Y-%m-%d'), ' 07:30:00'))          
              )                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
          """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_RTDRUN_PE",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_RTDRUN_PE")


def GetRtdRunProcessStartSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_PSTART (
            EQP_ID,OPE_NO,PRODSPEC_ID,CLAIM_TIME,OPE_CATEGORY,LOT_ID,CTRL_JOB)                                                               
            SELECT D.EQP_ID,D.OPE_NO,D.PRODSPEC_ID,                                                 
                 D.CLAIM_TIME,D.OPE_CATEGORY,D.LOT_ID,D.CTRL_JOB                                  
            FROM {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_PS D                                                 
            WHERE NOT EXISTS (                                                                      
                 SELECT 1                                                                             
                 FROM {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_PE CD                                             
                 WHERE CD.CTRL_JOB = D.CTRL_JOB                                                       
                   AND CD.LOT_ID = D.LOT_ID                                                
            )                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              
          """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_RTDRUN_PSTART",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_RTDRUN_PSTART")


def GetRtdRunCountSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_COUNT (
         EQP_ID,LAYER,PD_ID,LOT_COUNT)                                                                
         SELECT L.EQP_ID,L.LAYER,                 
                     L.PD_ID || '%' AS PD_ID,     
                     L.LOT_COUNT                  
         FROM APS_SYNC_RTD_RUN_LOTBYEQP_COUNT.APS_SYNC_RTD_RUN_LOTBYEQP_COUNT L          
         WHERE L.LOT_COUNT > 0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             
             """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_RTDRUN_COUNT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_RTDRUN_COUNT")


def GetRtdRunLotCountSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
          INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_LOTCOUNT (EQP_ID,LAYER,PD_ID,RN)                                                              
         SELECT L.EQP_ID,L.LAYER,L.PD_ID,                          
                (cast(MAX(L.LOT_COUNT) as decimal) - cast(COUNT(*) as decimal)) AS RN                
         FROM {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_COUNT L                
         INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_PSTART P   
         ON P.EQP_ID = L.EQP_ID                                    
         AND P.OPE_NO = L.LAYER                                    
         AND P.PRODSPEC_ID LIKE L.PD_ID                            
         GROUP BY L.EQP_ID,L.LAYER,L.PD_ID                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_RTDRUN_LOTCOUNT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_RTDRUN_LOTCOUNT")


def GetRtdRunDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT  /*+ append */  INTO APS_ETL_PIRUN(
                PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, LAYER, STAGE, RULE_NAME, REMAINING_COUNT, UPDATE_TIME, PARTCODE)                                                            
         WITH RESULT_DATA AS (                                                         
             SELECT D.TOOLG_ID,                                                   
                    L.EQP_ID AS TOOL_ID,                                          
                    '' CH_ID,                                                     
                    L.PD_ID AS PROD_ID,                                           
                    SUBSTRING(L.LAYER, 1, POSITION('.' IN L.LAYER)-1) AS LAYER,         
                    SUBSTRING(L.LAYER, POSITION('.' IN L.LAYER)+1) AS STAGE,            
                    'RTD Lot Count Limit' AS RULE_NAME,                           
                    CASE WHEN C.EQP_ID IS NOT NULL AND C.EQP_ID <> '' THEN C.RN                      
                       ELSE L.LOT_COUNT END AS REMAINING_COUNT                    
             FROM {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_COUNT L                         
             INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL D                             
             ON D.TOOL_ID = L.EQP_ID                                               
             LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_RTDRUN_LOTCOUNT C                 
             ON C.EQP_ID = L.EQP_ID                                               
             AND C.LAYER = L.LAYER                                                
             AND C.PD_ID = L.PD_ID                                                
         )                                                                        
         SELECT DISTINCT '{uuid}' AS PARENTID,                       
                TOOLG_ID,                                                         
                TOOL_ID,                                                          
                CH_ID,                                                            
                PROD_ID,                                                          
                LAYER,                                                            
                STAGE,                                                            
                RULE_NAME,                                                        
                CASE WHEN REMAINING_COUNT < 0 THEN 0                              
                     ELSE REMAINING_COUNT END AS REMAINING_COUNT,                 
                '{current_time}' AS UPDATE_TIME,                             
                '' AS PARTCODE                  
         FROM RESULT_DATA D                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
                  """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetTestRunInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT  /*+ append */  INTO APS_ETL_PIRUN(
                PARENTID, TOOLG_ID, TOOL_ID, CH_ID, STEP_ID,
                PPID, RULE_NAME, REMAINING_TIME, RECOVER_TIME,
                VALID_TIME, JOB_ID, PROCESS_TIME, PICK_PILOT_MODE, UPDATE_TIME, PARTCODE)                                                                    
         WITH TEST_RUN_DATA AS(                                                                                    
              SELECT DISTINCT '{uuid}' AS PARENTID,  
                 T.MODULE,                                                 
                 T.EQP_G,                                                                                          
                 T.TOOLID,                                                                                         
                 '' AS CH_ID,                                                                                      
                 '' AS STEP_ID,                                                                                    
                 SUBSTRING(REPLACE(T.M_RECIPE,'.AA',''),POSITION('.' IN REPLACE(T.M_RECIPE,'.AA',''))+1) AS M_RECIPE,   
                 'TestRun' AS RULE_NAME,     
                 -- CASE WHEN (TO_DATE(T.EXPIRETIME,'YYYY-MM-DD HH24:MI:SS') - SYSDATE) <= 0 AND T.STATE = 1 
                --  THEN 999 ELSE ROUND((TO_DATE(T.EXPIRETIME,'YYYY-MM-DD HH24:MI:SS') - SYSDATE)*24,2) END AS REMAINING_TIME,   
                 CASE WHEN extract(epoch from(CAST(T.EXPIRETIME AS TIMESTAMP) - CAST(CURRENT_TIMESTAMP AS TIMESTAMP))) <= 0 AND T.STATE = 1  
                   THEN 999 ELSE  round((extract(epoch from(CAST(T.EXPIRETIME AS TIMESTAMP) - CAST(CURRENT_TIMESTAMP AS TIMESTAMP)))) / 3600.0, 2) END AS REMAINING_TIME,           
                 NULL AS RECOVER_TIME,                                                                             
                 T.LIFETIME AS VALID_TIME,                                                                         
                 T.JOBGROUPID || '(' || COALESCE(T.SUBNAME,'') || ')'AS JOB_ID,                                               
                 '{current_time}' AS UPDATE_TIME,                                                             
                 '' AS PARTCODE                                                  
              FROM {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_RECIPE T                                                       
         )                                                                                                         
         SELECT DISTINCT PARENTID,                                                                                 
          D.EQP_G,                                                                                           
          D.TOOLID,                                                                                          
          D.CH_ID,                                                                                           
          D.STEP_ID,                                                                                         
          D.M_RECIPE,                                                                                        
          D.RULE_NAME,                                                                                       
          D.REMAINING_TIME,                                                                                  
          D.RECOVER_TIME,                                                                                    
          D.VALID_TIME,                                                                                      
          D.JOB_ID,                                                                                          
          CASE WHEN T21.TOOL_ID IS NOT NULL AND T21.TOOL_ID <> '' THEN T21.PT ELSE COALESCE(COALESCE(T22.PT,T221.PT),0) END AS PROCESS_TIME, 
          '2' AS PICK_PILOT_MODE, 
          UPDATE_TIME,                                                                                        
          PARTCODE                                                                                            
         FROM TEST_RUN_DATA D                                                                                       
         LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_T21 T21                                                                
         ON T21.TOOLG_ID = D.EQP_G AND T21.TOOL_ID = D.TOOLID AND T21.PPID = D.M_RECIPE AND T21.MODULE_ID = D.MODULE                             
         LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_T22 T22                                                                
         ON T22.TOOLG_ID = D.EQP_G AND T22.PPID = D.M_RECIPE AND T22.MODULE_ID = D.MODULE                                                       
         LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_T221 T221                                                              
         ON T221.TOOLG_ID = D.EQP_G AND T221.TOOL_ID = D.TOOLID AND T221.CH_CNT = 1 AND T221.MODULE_ID = D.MODULE                                
         WHERE POSITION('-' IN SUBSTRING(D.M_RECIPE,-3,3)) =0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        
         """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetTestRunApcResultSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT INTO APS_ETL_PIRUN(                                                  
               PARENTID, TOOLG_ID, TOOL_ID, PPID, RULE_NAME, REMAINING_TIME,
               VALID_TIME, PROCESS_TIME, PICK_PILOT_MODE, UPDATE_TIME, PARTCODE                                                            
        )                                                                            
         WITH APC_DATA AS(                                                                                                                                                                 
              SELECT DISTINCT '{uuid}' AS PARENTID,                                                   
                     T.TOOLG_ID,                                                                                   
                     R.TOOL_ID,                                                                                    
                     R.PPID,                                                                                       
                     'APC' AS RULE_NAME, 
                     --CASE WHEN R.LAST_RUN_TIME IS NULL THEN -999 ELSE ROUND((R.LAST_RUN_TIME+R.LIFE_TIME/24 - SYSDATE)*24,2) END AS REMAINING_TIME,                                                                                                   
                     CASE WHEN R.LAST_RUN_TIME IS NULL OR R.LAST_RUN_TIME = '' THEN -999 
                     ELSE round((extract(epoch from(CAST(CAST(R.LAST_RUN_TIME AS TIMESTAMP) + INTERVAL (CAST(R.LIFE_TIME AS DECIMAL)) HOUR AS TIMESTAMP) - CAST(CURRENT_TIMESTAMP AS TIMESTAMP)))) / 3600.0, 2) END AS REMAINING_TIME,                                                                                         
                     R.LIFE_TIME AS VALID_TIME,                                                                    
                     '{current_time}' AS UPDATE_TIME,                                                         
                     ''  AS PARTCODE                                              
              FROM APS_ETL_APC_RESULT.APS_ETL_APC_RESULT R                                                   
              INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL T                                                                  
              ON R.TOOL_ID = T.TOOL_ID                                                                                                                                      
         )                                                                                                           
         SELECT DISTINCT PARENTID,                                                                                   
          D.TOOLG_ID,                                                                                              
          D.TOOL_ID,                                                                                                                                                                                    
          D.PPID,                                                                                                  
          D.RULE_NAME,                                                                                             
          D.REMAINING_TIME,                                                                                                                                                                        
          D.VALID_TIME,                                                                                                                                                                                 
          CASE WHEN T21.TOOL_ID IS NOT NULL AND T21.TOOL_ID <> '' THEN T21.PT ELSE COALESCE(COALESCE(T22.PT,T221.PT),0) END AS PROCESS_TIME, 
          '2' AS PICK_PILOT_MODE,      
          UPDATE_TIME,                                                                                             
          PARTCODE                                                                                                 
             FROM APC_DATA D                                                                                           
         LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_T21 T21                                                                  
         ON T21.TOOLG_ID = D.TOOLG_ID AND T21.TOOL_ID = D.TOOL_ID AND T21.PPID = D.PPID                               
         LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_T22 T22                                                                  
         ON T22.TOOLG_ID = D.TOOLG_ID AND T22.PPID = D.PPID                                                          
         LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_TEST_RUN_T221 T221                                                                
         ON T221.TOOLG_ID = D.TOOLG_ID AND T221.TOOL_ID = D.TOOL_ID AND T221.CH_CNT = 1                                   
         WHERE POSITION('-' IN SUBSTRING(D.PPID,-3,3)) =0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             
         """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetInsertSqlFromPiRun(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
          INSERT  /*+ append */  INTO APS_ETL_PIRUN(
               PARENTID,  TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
               STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
               REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY , 
               UPDATE_TIME, PARTCODE, PICK_PILOT_MODE)
          SELECT *  FROM ( 
          SELECT '{uuid}' AS PARENTID,  TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
               STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
               REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY ,'{current_time}' AS UPDATE_TIME, '' AS PARTCODE,
               PICK_PILOT_MODE
          FROM  {tempdb}APS_TMP_ETL_PIRUN 
          UNION ALL 
          SELECT '{uuid}' AS PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
               STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
               REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY , '{current_time}' AS UPDATE_TIME,'' AS PARTCODE,
               '0' AS PICK_PILOT_MODE
          FROM  {tempdb}APS_TMP_ETL_PIRUN1 
          UNION ALL 
          SELECT '{uuid}'  AS PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
               STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
               REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY , '{current_time}'  AS UPDATE_TIME, ''  AS PARTCODE,
               '0' AS PICK_PILOT_MODE
          FROM {tempdb}APS_TMP_ETL_PIRUN2
          UNION ALL 
          SELECT '{uuid}'  AS PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
               STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
               REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY ,  '{current_time}' AS UPDATE_TIME, '' AS PARTCODE,
               '0' AS PICK_PILOT_MODE
          FROM  {tempdb}APS_TMP_ETL_PIRUN3
          UNION ALL 
          SELECT '{uuid}'  AS PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
               STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
               REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY ,  '{current_time}'  AS UPDATE_TIME, '' AS PARTCODE,
               '0' AS PICK_PILOT_MODE
          FROM  {tempdb}APS_TMP_ETL_PIRUN4
          UNION ALL 
          SELECT '{uuid}'  AS PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
               STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
               REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY ,  '{current_time}' AS UPDATE_TIME, '' AS PARTCODE,
               '0' AS PICK_PILOT_MODE
          FROM {tempdb}APS_TMP_ETL_PIRUN5
          UNION ALL 
          SELECT '{uuid}'  AS PARENTID, TOOLG_ID, TOOL_ID, CH_ID, PROD_ID, PLAN_ID, STEP_ID,RECIPE, PPID,LAYER,
               STAGE, TECH, CUSTOMER, LOT_ID, RETICLE_ID, PRE_TOOL_ID, RULE_NAME, REMAINING_COUNT,
               REMAINING_TIME, RECOVER_TIME, VALID_RECIPE, VALID_TIME, HOLD_TIME, VALID_LOT, SPLIT_QTY , '{current_time}'  AS UPDATE_TIME, '' AS PARTCODE,
               '0' AS PICK_PILOT_MODE
          FROM {tempdb}APS_TMP_ETL_PIRUN7
          ) T WHERE SUBSTRING(T.PROD_ID,LENGTH(T.PROD_ID)) <> '_'                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          
          """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetLastPurgeDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """ 
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_PURGE_LAST(                          
            START_TIME, EQP_ID, RTN                                                       
         )                                                                    
          WITH PURGE_DATA AS (                                                                                        
             SELECT CAST(EVENT_TIME_ST AS TIMESTAMP) AS START_TIME,    
                    EQP_ID                                                                                            
             FROM APS_SYNC_OUT_DF_PURGE_COMBINE.APS_SYNC_OUT_DF_PURGE_COMBINE                                           
            -- WHERE PARTCODE = ' + lstCodes.get(27) + '                                                              
          )                                                                                                           
          SELECT  START_TIME,EQP_ID,                                                                                  
                  ROW_NUMBER() OVER(PARTITION BY EQP_ID ORDER BY START_TIME DESC) AS RTN                              
          FROM PURGE_DATA                                                                                             
          WHERE START_TIME > DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '7 day'     
   """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_PURGE_LAST",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_PURGE_LAST")


def GetPurgeEqpDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
            INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_PURGE_DATA(                         
               TOOLG_ID, EQUIPMENT_ID, VAL, REPORT_TIME, RTN, LAST_VAL                                                   
               )                                                                   
                WITH TOOL_DATA AS (                                                                                   
                     SELECT T.EQP_ID,                                                                                 
                            MAX(T.EQP_G) AS TOOLG_ID                                                                  
                     FROM APS_SYNC_ETL_TOOL.APS_SYNC_ETL_TOOL T                                                 
                     WHERE T.HOST_EQP_FLAG ='Y' 
                     --AND PARTCODE = ' + lstCodes.get(3) + '                              
                     GROUP BY EQP_ID                                                                                  
                ),
                EQP_ED_DATA AS (
                SELECT DISTINCT EQUIPMENT_ID,VALUE,
                       REPORT_TIME,ED_PARAMETER_ID
                FROM APS_SYNC_EQP_ED_DATA_REPORT.APS_SYNC_EQP_ED_DATA_REPORT
                WHERE REPORT_TIME > DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '7 day' AND ED_PARAMETER_ID = 'PGR0'  
                )                                                                                                     
                SELECT                                                                                                
                    D.TOOLG_ID,                                                                                       
                    EQUIPMENT_ID,                                                                                     
                    CAST(VALUE AS decimal) AS VAL,                                                                    
                    REPORT_TIME,                                                                                      
                    ROW_NUMBER() OVER(PARTITION BY EQUIPMENT_ID ORDER BY REPORT_TIME DESC) AS RTN,                     
                    LAG(CAST(VALUE AS decimal)) OVER (PARTITION BY EQUIPMENT_ID ORDER BY REPORT_TIME) AS LAST_VAL     
                FROM EQP_ED_DATA T                                            
                INNER JOIN TOOL_DATA D                                                                                
                ON D.EQP_ID = T.EQUIPMENT_ID                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_PURGE_DATA",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_PURGE_DATA")


def GetPurgeMaxBatchDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_PURGE_MAX_BATCH(                                             
             EQUIPMENT_ID, BATCH_SPEC, RTN2                                                                               
             )                                                                                            
             SELECT EQUIPMENT_ID,LAST_VAL AS BATCH_SPEC,                                                 
                    ROW_NUMBER() OVER(PARTITION BY EQUIPMENT_ID ORDER BY REPORT_TIME DESC) AS RTN2        
             FROM {tempdb}APS_TMP_ETL_PIRUN_PURGE_DATA                                                        
             WHERE RTN > 1                                                                               
             AND VAL < LAST_VAL                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_PURGE_MAX_BATCH",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_PURGE_MAX_BATCH")


def GetPurgeEqpBatchDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_PURGE_EQP_BATCH(                                          
             EQUIPMENT_ID, VAL, RTN                                                                           
             )                                                                                         
              SELECT EQUIPMENT_ID,                                                                     
                CAST(VALUE AS decimal) AS VAL,                                                    
                ROW_NUMBER() OVER(PARTITION BY EQUIPMENT_ID ORDER BY REPORT_TIME DESC) AS RTN      
              FROM APS_SYNC_EQP_ED_DATA_REPORT.APS_SYNC_EQP_ED_DATA_REPORT                                
              WHERE ED_PARAMETER_ID = 'PGS0' 
              --AND PARTCODE = ' + lstCodes.get(23) + '                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_PURGE_EQP_BATCH",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_PURGE_EQP_BATCH")


def GetPurgeDefBatchDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT INTO {tempdb}APS_TMP_ETL_PIRUN_PURGE_DEF_BATCH(                          
            EQP_G, EQP_ID, UNIT_DC_ITEM, UNIT_SPEC                                                           
            )                                                                         
             SELECT EQP_G,EQP_ID,UNIT_DC_ITEM,                                        
                    CAST(UNIT_SPEC AS decimal) AS UNIT_SPEC                           
             FROM APS_SYNC_UMT_EQP_UNIT_PLAN_SETTING.APS_SYNC_UMT_EQP_UNIT_PLAN_SETTING          
             WHERE UNIT_DC_ITEM='PGS0'                                                
             --AND PARTCODE = ' + lstCodes.get(24) + '                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_PIRUN_PURGE_DEF_BATCH",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_PIRUN_PURGE_DEF_BATCH")


def GetPurgeReultDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT INTO APS_ETL_PIRUN (                                       
             PARENTID,TOOLG_ID,TOOL_ID, RULE_NAME, COUNT_UNIT,VALID_COUNT,REMAINING_COUNT,
             PROCESS_TIME, UPDATE_TIME, PARTCODE                                                     
         )                                                                       
          SELECT DISTINCT '{uuid}' AS PARENTID,                                       
                 T1.TOOLG_ID,                                                                      
                 T1.EQUIPMENT_ID AS TOOL_ID,                                                       
                 'BatchPurge' AS RULE_NAME,                                                        
                 'BATCH' AS COUNT_UNIT,                                                            
                 COALESCE(T3.VAL,T2.BATCH_SPEC,T4.UNIT_SPEC) AS VALID_COUNT,      
                 CASE WHEN T1.REPORT_TIME >= COALESCE(T0.START_TIME, T1.REPORT_TIME) THEN COALESCE(T3.VAL,T2.BATCH_SPEC,T4.UNIT_SPEC) - T1.VAL 
                 ELSE (COALESCE(T3.VAL,T2.BATCH_SPEC,T4.UNIT_SPEC)) END AS REMAINING_COUNT,          
                 COALESCE(P11.PT,P12.PT) AS PROCESS_TIME,                                               
                '{current_time}' AS UPDATE_TIME,                                             
                  '' AS PARTCODE                                  
          FROM {tempdb}APS_TMP_ETL_PIRUN_PURGE_DATA T1                                                   
          LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_PURGE_MAX_BATCH T2 ON T2.RTN2 = 1 AND T1.EQUIPMENT_ID = T2.EQUIPMENT_ID                        
          LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_PURGE_EQP_BATCH T3 ON T3.RTN = 1 AND  T1.EQUIPMENT_ID = T3.EQUIPMENT_ID                        
          LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_PURGE_DEF_BATCH T4 ON T1.EQUIPMENT_ID = T4.EQP_ID 
          LEFT JOIN {tempdb}APS_TMP_ETL_PIRUN_PURGE_LAST T0 ON T1.EQUIPMENT_ID = T0.EQP_ID AND T0.RTN = 1                                             
          LEFT JOIN APS_MID_PURGE11.APS_MID_PURGE11 P11 ON T1.EQUIPMENT_ID = P11.TOOL_ID  
          LEFT JOIN APS_MID_PURGE12.APS_MID_PURGE12 P12 ON T1.TOOLG_ID = P12.TOOLG_ID     
          WHERE T1.RTN = 1                                                                         
          AND COALESCE( T3.VAL,T2.BATCH_SPEC,T4.UNIT_SPEC) IS NOT NULL                        
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetCuDummyDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT INTO APS_ETL_PIRUN (                                       
             PARENTID,TOOLG_ID,TOOL_ID, RULE_NAME, COUNT_UNIT,VALID_COUNT,REMAINING_COUNT,
             PROCESS_TIME, UPDATE_TIME, PARTCODE                                                     
         ) 
         WITH DUMMY_TIME_DATA AS (
            SELECT MAX(C.SYS_DATE) AS LAST_TIME,C.TOOLID AS EQP_ID
            FROM APS_SYNC_OCS_AUTODUMMY_COMPLETE.APS_SYNC_OCS_AUTODUMMY_COMPLETE C
            INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL T
            ON T.TOOL_ID = C.TOOLID AND T.HOST_EQP_FLAG = 'Y'
            AND T.TOOLG_ID = 'MA_AL(Cu)' AND CAST(C.SYS_DATE AS TIMESTAMP)>= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '10 day'
            GROUP BY C.TOOLID
         ),
         lAST_HIST_QTY AS (
            SELECT H.WAFER_QTY, H.EQP_ID,
                   ROW_NUMBER() OVER(PARTITION BY H.EQP_ID order BY H.CLAIM_TIME DESC) AS RNT
            FROM APS_TMP_LOTHISTORY_VIEW.APS_TMP_LOTHISTORY_VIEW H
            INNER JOIN DUMMY_TIME_DATA D 
            ON H.EQP_ID = D.EQP_ID
            WHERE CAST(H.CLAIM_TIME AS TIMESTAMP) > CAST(D.LAST_TIME AS TIMESTAMP)
         ),
         DUMMY_QTY_DATA AS (
            SELECT SUM(H.WAFER_QTY) AS QTY, H.EQP_ID
            FROM lAST_HIST_QTY H
            GROUP BY H.EQP_ID
         )                                                                
          SELECT DISTINCT '{uuid}' AS PARENTID,                                       
                 T.TOOLG_ID,                                                                      
                 T.TOOL_ID,                                                       
                 'RunDummy' AS RULE_NAME,                                                        
                 'WAFER' AS COUNT_UNIT,                                                            
                 '50' AS VALID_COUNT,
                 CASE WHEN D.EQP_ID IS NULL THEN 50 ELSE (
                    CASE WHEN 50 - CAST(D.QTY AS DECIMAL) <  0 THEN  50 - CAST(Q.WAFER_QTY AS DECIMAL(18,0)) 
                    ELSE 50 - CAST(D.QTY AS DECIMAL(18,0)) END
                 ) END AS REMAINING_COUNT,
                  M.PROCESS_TIME,          
                '{current_time}' AS UPDATE_TIME,                                             
                  '' AS PARTCODE                                  
          FROM {tempdb}APS_TMP_ETL_PIRUN_TOOL T                                                   
          LEFT JOIN DUMMY_QTY_DATA D 
          ON T.TOOL_ID = D.EQP_ID 
          LEFT JOIN APS_MID_IDLE_DUMMY_CU_MEDIAN.APS_MID_IDLE_DUMMY_CU_MEDIAN M
          ON M.TOOL_ID = T.TOOL_ID 
          LEFT JOIN lAST_HIST_QTY Q
          ON T.TOOL_ID = Q.EQP_ID AND RNT = 1                      
          WHERE T.HOST_EQP_FLAG = 'Y'  AND T.TOOLG_ID = 'MA_AL(Cu)'                                                                                                 
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetPadPMData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT INTO {tempdb}APS_TMP_PIRUN_PAD_DATA (                                       
             TOOLG_ID,LSL,PARAMETER,COUNT_UNIT,CH_ID,TOOL_ID                                                    
         ) 
         select toolg_id,lsl,parameter,count_unit,ch_id,tool_id 
         from aps_etl_pirun 
         where 1=1
         and count_unit in ('Hours', 'Pieces')
         and current_count is not null
         and clean_group = 'Pad'                                                                                                
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_PAD_DATA",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_PAD_DATA")


def GetPpidPTData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT INTO {tempdb}APS_TMP_PIRUN_PT_PER_WAFER (                                       
             TOOLG_ID, PARAMETER, PROCESS_TIME_PER_WAFER                                                    
         ) 
         WITH PPID_DATA AS(
            SELECT TOOLG_ID,PARAMETER,COAT_PER_WAFER 
            FROM APS_ETL_PPID_INFO.APS_ETL_PPID_INFO P
            WHERE EXISTS 
                (SELECT 1 FROM {tempdb}APS_TMP_PIRUN_PAD_DATA D WHERE D.TOOLG_ID = P.TOOLG_ID)
        ),
        TOOL_P_DATA AS (
            SELECT
                TOOLG_ID, 
                PARAMETER, 
                quantile_cont(cast(COAT_PER_WAFER as decimal(18,4)),0.5) OVER(PARTITION BY TOOLG_ID, PARAMETER ) AS PROCESS_TIME_PER_WAFER
                --PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY COAT_PER_WAFER) AS PROCESS_TIME_PER_WAFER
            FROM PPID_DATA
            WHERE PARAMETER IS NOT NULL
        ),
        TOOL_DATA AS (
            SELECT
                TOOLG_ID, 
                --PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY COAT_PER_WAFER) AS PROCESS_TIME_PER_WAFER
                quantile_cont(cast(COAT_PER_WAFER as decimal(18,4)),0.5) OVER(PARTITION BY TOOLG_ID ) AS PROCESS_TIME_PER_WAFER
            FROM PPID_DATA
        )
        SELECT
            TOOLG_ID, 
            PARAMETER, 
            MAX(PROCESS_TIME_PER_WAFER) AS PROCESS_TIME_PER_WAFER
        FROM TOOL_P_DATA
        GROUP BY TOOLG_ID, PARAMETER
        UNION ALL
        SELECT
            TOOLG_ID, 
            NULL AS PARAMETER,  
            MAX(PROCESS_TIME_PER_WAFER) AS PROCESS_TIME_PER_WAFER
        FROM TOOL_DATA
        GROUP BY TOOLG_ID                                                                                                
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_PT_PER_WAFER",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_PT_PER_WAFER")


def GetMinLslData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT INTO {tempdb}APS_TMP_PIRUN_MIN_LSL_DATA (                                       
             TOOL_ID, CH_ID, LSL_H, LSL_P                                                    
         ) 
         WITH pirun_result as (
            select 
                pp.TOOLG_ID,pp.LSL,pp.PARAMETER,pp.COUNT_UNIT,pp.CH_ID,pp.TOOL_ID, 
                coalesce(epi_1.process_time_per_wafer, epi_2.process_time_per_wafer) as process_time_per_wafer
            from {tempdb}APS_TMP_PIRUN_PAD_DATA pp
            left join {tempdb}APS_TMP_PIRUN_PT_PER_WAFER epi_1
            on pp.parameter = epi_1.parameter
            and pp.toolg_id = epi_1.toolg_id
            left join {tempdb}APS_TMP_PIRUN_PT_PER_WAFER epi_2
            on epi_2.parameter is null
            and pp.toolg_id = epi_2.toolg_id

        ),
        lsl_data as (
            select
                t.tool_id, t.ch_id,
                case -- Hours
                    when count_unit = 'Hours'  then cast(lsl as decimal)
                    when count_unit = 'Pieces' then cast(lsl as decimal) * cast(process_time_per_wafer as decimal)
                end as lsl_h, 
                case -- Pieces
                    when count_unit = 'Hours'  then cast(lsl as decimal) / cast(process_time_per_wafer as decimal)
                    when count_unit = 'Pieces' then cast(lsl as decimal)
                end as lsl_p
            from pirun_result t
        )
        select -- tool_id + ch_id
                tool_id, 
                ch_id, 
                min(lsl_h) as lsl_h, 
                min(lsl_p) as lsl_p
        from lsl_data
        where ch_id is not null
        group by tool_id, ch_id
        union all
        select -- tool_id
                tool_id, 
                null as ch_id, 
                min(lsl_h) as lsl_h, 
                min(lsl_p) as lsl_p
        from lsl_data
        group by tool_id                                                                                                
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_MIN_LSL_DATA",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_MIN_LSL_DATA")


def GetCoatFixBoundData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    # 先把PIRUN的结果存在在TMP表中
    sql = """      
             INSERT INTO {tempdb}APS_TMP_PIRUN_V1 (                                       
                 PARENTID,TOOL_ID,CH_ID,PROD_ID,PLAN_ID,STEP_ID,RECIPE,PPID,
                LAYER,STAGE,TECH,CUSTOMER,LOT_ID,RETICLE_ID,PRE_TOOL_ID,RULE_NAME,
                REMAINING_COUNT,REMAINING_TIME,RECOVER_TIME,VALID_RECIPE,TOOLG_ID,
                VALID_TIME,UPDATE_TIME,HOLD_TIME,PARTCODE,VALID_LOT,COUNT_UNIT,
                CURRENT_COUNT,LSL,USL,PROCESS_TIME,JOB_ID,SPLIT_QTY,PARAMETER,
                ALL_CH_FLAG,VALID_COUNT,PRODG_ID,CLEAN_ACC_FLAG,CLEAN_MON_TIME,
                CLEAN_GROUP,CLEAN_FIX_FLAG,PICK_PILOT_MODE,WIP_ATTR3,RLS_ATTR3                                                   
             ) 
             SELECT PARENTID,TOOL_ID,CH_ID,PROD_ID,PLAN_ID,STEP_ID,RECIPE,PPID,
                    LAYER,STAGE,TECH,CUSTOMER,LOT_ID,RETICLE_ID,PRE_TOOL_ID,RULE_NAME,
                    REMAINING_COUNT,REMAINING_TIME,RECOVER_TIME,VALID_RECIPE,TOOLG_ID,
                    VALID_TIME,UPDATE_TIME,HOLD_TIME,PARTCODE,VALID_LOT,COUNT_UNIT,
                    CURRENT_COUNT,LSL,USL,PROCESS_TIME,JOB_ID,SPLIT_QTY,PARAMETER,
                    ALL_CH_FLAG,VALID_COUNT,PRODG_ID,CLEAN_ACC_FLAG,CLEAN_MON_TIME,
                    CLEAN_GROUP,CLEAN_FIX_FLAG,PICK_PILOT_MODE,WIP_ATTR3,RLS_ATTR3,
             FROM APS_ETL_PIRUN                                                                                                
                  """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_V1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_V1")

    # 然后TRUNCATE PIRUN 的全部数据
    sql = """      
         TRUNCATE TABLE APS_ETL_PIRUN                                                                                              
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="TRUNCATE TABLE APS_ETL_PIRUN ",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")

    sql = """      
         INSERT INTO APS_ETL_PIRUN (                                       
                PARENTID,TOOL_ID,CH_ID,PROD_ID,PLAN_ID,STEP_ID,RECIPE,PPID,
                LAYER,STAGE,TECH,CUSTOMER,LOT_ID,RETICLE_ID,PRE_TOOL_ID,RULE_NAME,
                REMAINING_COUNT,REMAINING_TIME,RECOVER_TIME,VALID_RECIPE,TOOLG_ID,
                VALID_TIME,UPDATE_TIME,HOLD_TIME,PARTCODE,VALID_LOT,COUNT_UNIT,
                CURRENT_COUNT,LSL,USL,PROCESS_TIME,JOB_ID,SPLIT_QTY,PARAMETER,
                ALL_CH_FLAG,VALID_COUNT,PRODG_ID,CLEAN_ACC_FLAG,CLEAN_MON_TIME,
                CLEAN_GROUP,CLEAN_FIX_FLAG,PICK_PILOT_MODE,COAT_FIX_BOUND,WIP_ATTR3,
                RLS_ATTR3                                                  
         ) 
         with pirun_all_data as(
            select
                ep.*, 
                case -- 可以從lsl扣除的額度
                    when ep.current_count is null then null -- 必須是累積厚度規則
                    when ep.clean_group = 'Pad' then null -- 不用計算pad PM本身
                    when ep.count_unit = 'Hours'  then coalesce(pp_1.lsl_h, pp_2.lsl_h)
                    when ep.count_unit = 'Pieces' then coalesce(pp_1.lsl_p, pp_2.lsl_p)
                end as lsl_pp
            from {tempdb}APS_TMP_PIRUN_V1  ep
            left join {tempdb}APS_TMP_PIRUN_MIN_LSL_DATA pp_1
            on ep.ch_id = pp_1.ch_id
            and ep.tool_id = pp_1.tool_id
            left join {tempdb}APS_TMP_PIRUN_MIN_LSL_DATA pp_2
            on pp_2.ch_id is null
            and ep.tool_id = pp_2.tool_id
        )
        select
                PARENTID,TOOL_ID,CH_ID,PROD_ID,PLAN_ID,STEP_ID,RECIPE,PPID,
                LAYER,STAGE,TECH,CUSTOMER,LOT_ID,RETICLE_ID,PRE_TOOL_ID,RULE_NAME,
                REMAINING_COUNT,REMAINING_TIME,RECOVER_TIME,VALID_RECIPE,TOOLG_ID,
                VALID_TIME,UPDATE_TIME,HOLD_TIME,PARTCODE,VALID_LOT,COUNT_UNIT,
                CURRENT_COUNT,LSL,USL,PROCESS_TIME,JOB_ID,SPLIT_QTY,PARAMETER,
                ALL_CH_FLAG,VALID_COUNT,PRODG_ID,CLEAN_ACC_FLAG,CLEAN_MON_TIME,
                CLEAN_GROUP,CLEAN_FIX_FLAG,PICK_PILOT_MODE, 
                case when d.lsl_pp is null then null
                else (case when cast(d.lsl as decimal) - cast(d.lsl_pp as decimal) < 0 then 0.0 
                      else cast(d.lsl as decimal) - cast(d.lsl_pp as decimal) end)  end as COAT_FIX_BOUND,
                WIP_ATTR3,
                RLS_ATTR3
        from pirun_all_data d                                                                                                
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN LAST",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetRDSInhibitData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT INTO APS_ETL_PIRUN (                                       
                PARENTID,RULE_NAME,PICK_PILOT_MODE,TOOLG_ID,TOOL_ID,PPID,
                VALID_PPID,PROCESS_TIME,REMAINING_TIME,VALID_TIME,rls_attr3,UPDATE_TIME,PARTCODE                                                    
         ) 
         WITH RDS_INHIBIT_DATA AS (
                SELECT 'RDS_Inhibit' AS RULE_NAME,
                       '0' AS PICK_PILOT_MODE,
                       T.TOOLG_ID,
                       TOOLID AS TOOL_ID,
                       SUBSTRING( (SUBSTRING(DKEY, 1, LENGTH(DKEY)- POSITION('.' in reverse(DKEY)))),   
                                     LENGTH(( SUBSTRING(DKEY, 1, LENGTH(DKEY)- POSITION('.' in reverse(DKEY)))) ) 
                                     -POSITION('.' IN reverse(( SUBSTRING(DKEY, 1, LENGTH(DKEY)- POSITION('.' in reverse(DKEY)))) ) ) +2) AS PPID,
                       '6' AS PROCESS_TIME,
                       round((extract(epoch from(CAST(EXPIRETIME AS TIMESTAMP) 
                              - CAST(CURRENT_TIMESTAMP AS TIMESTAMP))))/3600.0, 2) AS REMAINING_TIME,
                       LIFETIME AS VALID_TIME,
                       EXPIRETIME,
                       LIFETIME,
                       LASTSTAMPTIME,
                       SPLIT_PART(DKEY, '_', 2) as rls_attr3
                FROM APS_SYNC_RDS_RECIPEINFO.APS_SYNC_RDS_RECIPEINFO V 
                INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL T 
                ON T.TOOL_ID = V.TOOLID AND T.HOST_EQP_FLAG='Y'
                WHERE V.MODULE ='ETCH'
         ),
         RESULT_DATA AS (
                SELECT RULE_NAME,PICK_PILOT_MODE,TOOLG_ID,TOOL_ID,PPID,rls_attr3,
                       PPID,PROCESS_TIME,REMAINING_TIME,VALID_TIME,
                       ROW_NUMBER() OVER(PARTITION BY TOOL_ID,PPID,rls_attr3 ORDER BY LASTSTAMPTIME DESC) AS RNT 
                FROM RDS_INHIBIT_DATA 
         )
         SELECT distinct '{uuid}' AS PARENTID, 
                RULE_NAME,PICK_PILOT_MODE,TOOLG_ID,TOOL_ID,PPID,
                PPID AS VALID_PPID,PROCESS_TIME,REMAINING_TIME,VALID_TIME,
                rls_attr3,
                '{current_time}' AS UPDATE_TIME,                                             
                '' AS PARTCODE
         FROM RESULT_DATA
         WHERE RNT = 1
         AND PPID IS NOT NULL 
         AND PPID <>''                                                                                             
          """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN GetRDSInhibitData",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetATPTaskQtimeData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT INTO {tempdb}APS_TMP_PIRUN_ATP_QTIME_TASK (                                       
            task_no                                                   
        ) 
        select distinct d.TASK_NO 
        from APS_SYNC_RTD_PIRUN_TASK_DEVICE.APS_SYNC_RTD_PIRUN_TASK_DEVICE d
        inner join aps_etl_qtime_spec.aps_etl_qtime_spec s 
        on INSTR(s.prod_id, d.P_DEVICE_NM||'.'||REPLACE(d.P_DEVICE_NM1,'*',''))= 1
        and s.ope_no_to = d.CONTROL_PROCESS
        and s.timelimit >= 48                                                                                              
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_ATP_QTIME_TASK",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_ATP_QTIME_TASK")


def GetATPCase1Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT INTO APS_ETL_PIRUN (                                       
            parentid,rule_name,toolg_id,tool_id,ch_id,rls_attr1,pick_pilot_mode,
            remaining_time,recover_time,remaining_count,update_time,partcode                                                    
        ) 
        select distinct '{uuid}' AS PARENTID,
               'ATP_PiRun' || '_' || C.MODULE || '(' || T.TASK_NO || ')' AS RULE_NAME,
               C.TOOLG_ID AS TOOLG_ID,
               T.EQP_ID AS TOOL_ID,
               CASE WHEN EXISTS (SELECT 1 FROM {tempdb}APS_TMP_ETL_PIRUN_TOOL P WHERE P.EQP_CHAMBER_FLAG='Chamber' AND P.TOOL_ID = T.EQP_ID||'.'||T.CHAMBER) 
                                        THEN T.EQP_ID||'.'||T.CHAMBER ELSE T.EQP_ID END AS CH_ID, 
               T.TASK_NO AS RLS_ATTR1,
               '1' AS pick_pilot_mode, 
               '-1' AS remaining_time, 
               '9999' AS recover_time,
               case when t.USE_TIME_TYPE in ('EXPIRE', 'PMRDS', 'ILNG') and n.task_no is not null then '2' end as remaining_count,
               '{current_time}' as update_time,
               '' as partocde
        from APS_SYNC_RTD_PIRUN_TASK.APS_SYNC_RTD_PIRUN_TASK t 
        INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL C
        ON C.tool_ID = T.EQP_ID 
        AND C.HOST_EQP_FLAG = 'Y'
        left join {tempdb}APS_TMP_PIRUN_ATP_QTIME_TASK n
        on n.task_no = t.TASK_NO
        where STATUS = 'CREATE' 
        and not exists (
            select TASK_NO 
            from aps_sync_PIRUN_TASK_SCHEDULE_LOT.aps_sync_PIRUN_TASK_SCHEDULE_LOT l
            where STATUS <> 4
            and l.TASK_NO = t.TASK_NO
        )                                                                                              
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into GetATPCase1Data",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetATPCase2LotData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT INTO APS_ETL_PIRUN (                                       
            parentid,rule_name,lot_id,toolg_id,tool_id,ch_id,rls_attr1,pick_pilot_mode,
            remaining_time,recover_time,remaining_count,update_time,partcode                                                    
        ) 
        select distinct '{uuid}' AS PARENTID,
               'ATP_PiRun' || '_' || C.MODULE || '(' || T.TASK_NO || ')' AS RULE_NAME,
               coalesce(t.lot_son, t.lot_mother) as lot_id,
               C.TOOLG_ID AS TOOLG_ID,
               T.EQP_ID AS TOOL_ID,
               CASE WHEN EXISTS (SELECT 1 FROM {tempdb}APS_TMP_ETL_PIRUN_TOOL P WHERE P.EQP_CHAMBER_FLAG='Chamber' AND P.TOOL_ID = T.EQP_ID||'.'||T.CHAMBER) THEN T.EQP_ID||'.'||T.CHAMBER ELSE T.EQP_ID END AS CH_ID, 
               T.TASK_NO AS RLS_ATTR1,
               '1' AS pick_pilot_mode, 
               '-1' AS remaining_time, 
               '9999' AS recover_time,
               null as remaining_count,
               '{current_time}' as update_time,
               '' partcode
        from APS_SYNC_RTD_PIRUN_TASK.APS_SYNC_RTD_PIRUN_TASK t 
        INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL C
        ON C.TOOL_ID = T.EQP_ID 
        AND C.HOST_EQP_FLAG = 'Y'
        where t.LOT_SON is not null
        or t.LOT_MOTHER is not null                                                                                             
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into GetATPCase2LotData",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def getReturnLineDATE(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT INTO APS_ETL_PIRUN(
                PARENTID,TOOLG_ID,TOOL_ID,PROD_ID,PRODG_ID,LAYER,STAGE,RULE_NAME,COUNT_UNIT,
                CURRENT_COUNT,LSL,USL,PROCESS_TIME,PARAMETER,PICK_PILOT_MODE,WIP_ATTR3,UPDATE_TIME,PARTCODE
                )                                                                    
         WITH MAX_EVENT_PER_EQP AS (
                SELECT 
                    AA.EQP_ID, 
                    MAX(AA.EVENTENDTMST) AS MAX_EVENTENDTMST,
                    MAX(T.EQP_G) AS TOOLG_ID,
                    MAX(AA.CONTROL_TYPE) AS CONTROL_TYPE,
                    MAX(RUN_COUNT) AS RUN_COUNT
                FROM APS_SYNC_RTD_PREFER_INHIBIT_LOT.APS_SYNC_RTD_PREFER_INHIBIT_LOT AA
                LEFT JOIN APS_SYNC_ETL_TOOL.APS_SYNC_ETL_TOOL T
                ON AA.EQP_ID = T.EQP_ID AND T.HOST_EQP_FLAG = 'Y'
                GROUP BY AA.EQP_ID
            )
            SELECT 
                '{uuid}' AS PARENTID,
                BB.TOOLG_ID AS TOOLG_ID,
                AA.EQP_ID AS TOOL_ID,
                REPLACE(AA.PRODSPEC_ID,'*','%') AS PROD_ID,
                REPLACE(AA.PRODG1,'*','%') AS PRODG_ID,
                CASE 
                WHEN AA.OPE_NO = '*' THEN '%'
                WHEN SUBSTRING(AA.OPE_NO,1,1) = '*' AND LEN(AA.OPE_NO) > 2 THEN '%' 
                ELSE SPLIT_PART(REPLACE(AA.OPE_NO,'*','%'),'.', 1) END AS LAYER,
                CASE 
                WHEN AA.OPE_NO = '*' THEN '%'
                WHEN SUBSTRING(AA.OPE_NO,1,1) = '*' AND LEN(AA.OPE_NO) > 2 THEN REPLACE(SUBSTRING(AA.OPE_NO,2,LEN(AA.OPE_NO)),'*','%')
                ELSE SPLIT_PART(REPLACE(AA.OPE_NO,'*','%'),'.', 2) END AS STAGE,
                'Prefer_return_line_inhibit' AS RULE_NAME,
                'LOT' AS COUNT_UNIT,
                AA.RUN_COUNT AS CURRENT_COUNT,                    
                CAST(AA.INHIBIT_COUNT AS VARCHAR) AS LSL, 
                NULL AS USL,                             
                NULL AS PROCESS_TIME,                    
                CONCAT('PRLI_',AA.CONTROL_TYPE) AS PARAMETER,
                '0' AS PICK_PILOT_MODE,
                '0' AS WIP_ATTR3,
                '{current_time}' AS UPDATE_TIME,
                '' AS PARTCODE
            FROM APS_SYNC_RTD_PREFER_INHIBIT_LOT.APS_SYNC_RTD_PREFER_INHIBIT_LOT AA
            INNER JOIN MAX_EVENT_PER_EQP BB 
            ON AA.EQP_ID = BB.EQP_ID 
            AND AA.EVENTENDTMST = BB.MAX_EVENTENDTMST
            UNION ALL
            SELECT 
                '{uuid}' AS PARENTID,
                BB.TOOLG_ID,
                BB.EQP_ID AS TOOL_ID,
                NULL AS PROD_ID,                         
                NULL AS PRODG_ID,                        
                NULL AS LAYER,                           
                NULL AS STAGE,                           
                'Prefer_return_line_inhibit' AS RULE_NAME,
                'LOT' AS COUNT_UNIT,
                BB.RUN_COUNT AS CURRENT_COUNT,
                '99999' AS LSL,                          
                NULL AS USL,                             
                '0.01' AS PROCESS_TIME,                  
                CONCAT('PRLI_',BB.CONTROL_TYPE) AS PARAMETER,
                '0' AS PICK_PILOT_MODE,
                '0' AS WIP_ATTR3,
                '{current_time}' AS UPDATE_TIME,
                '' AS PARTCODE
            FROM MAX_EVENT_PER_EQP BB                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              
         """.format(uuid=uuid, current_time=current_time)

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetATPCase2PreLotData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    # 先对预选LOT进行行转列
    sql = """      
        INSERT INTO {tempdb}APS_TMP_PIRUN_ATP_PRE_LOT (                                       
            task_no, lot_no                                                   
        ) 
        select TASK_NO,
          (
              select STRING_AGG(distinct LOT_NO,','  ORDER BY LOT_NO)
              from APS_SYNC_PIRUN_TASK_SCHEDULE_LOT.APS_SYNC_PIRUN_TASK_SCHEDULE_LOT t
              where t.task_no = l.TASK_NO
              and t.STATUS <> 4
          ) as LOT_NO 
        from APS_SYNC_PIRUN_TASK_SCHEDULE_LOT.APS_SYNC_PIRUN_TASK_SCHEDULE_LOT L
        WHERE L.STATUS <> 4                                                                                             
            """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_ATP_PRE_LOT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_ATP_PRE_LOT")

    sql = """      
        INSERT INTO APS_ETL_PIRUN (                                       
            parentid,rule_name,lot_id,toolg_id,tool_id,ch_id,rls_attr1,pick_pilot_mode,
            remaining_time,recover_time,remaining_count,update_time,partcode                                                   
        ) 
        select distinct '{uuid}' AS PARENTID,
               'ATP_PiRun' || '_' || C.MODULE || '(' || T.TASK_NO || ')' AS RULE_NAME,
               N.LOT_NO AS LOT_ID,
               C.TOOLG_ID AS TOOLG_ID,
               T.EQP_ID AS TOOL_ID,
               CASE WHEN EXISTS (SELECT 1 FROM {tempdb}APS_TMP_ETL_PIRUN_TOOL P WHERE P.EQP_CHAMBER_FLAG='Chamber' AND P.TOOL_ID = T.EQP_ID||'.'||T.CHAMBER) 
                    THEN T.EQP_ID||'.'||T.CHAMBER ELSE T.EQP_ID END AS CH_ID, 
               T.TASK_NO AS RLS_ATTR1,
               '1' AS pick_pilot_mode, 
               '-1' AS remaining_time, 
               '9999' AS recover_time,
               CASE WHEN T.STATUS not in ('CREATE','SPLIT','WAIT','FWAIT') THEN NULL 
                    when t.USE_TIME_TYPE in ('EXPIRE', 'PMRDS', 'ILNG') and n.task_no is not null then '2' 
                    end as remaining_count,
               '{current_time}' as update_time,
               '' as partcode
        from APS_SYNC_RTD_PIRUN_TASK.APS_SYNC_RTD_PIRUN_TASK t 
        INNER JOIN {tempdb}APS_TMP_ETL_PIRUN_TOOL C
        ON C.TOOL_ID = T.EQP_ID 
        AND C.HOST_EQP_FLAG = 'Y'
        LEFT JOIN {tempdb}APS_TMP_PIRUN_ATP_QTIME_TASK L
        ON L.TASK_NO = T.TASK_NO
        INNER join {tempdb}APS_TMP_PIRUN_ATP_PRE_LOT n
        on n.task_no = t.TASK_NO
        where t.LOT_SON is null
        and t.LOT_MOTHER is null                                                                                            
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into GetATPCase2PreLotData",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def GetPadCoatFixBoundData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    # 先对原先的数据进行编码
    sql = """      
        INSERT INTO {tempdb}APS_TMP_PIRUN_TT (                                       
            parentid,toolg_id,tool_id,ch_id,prod_id, plan_id,step_id,recipe,ppid,layer,stage,
             tech,customer,lot_id, reticle_id, pre_tool_id, rule_name,remaining_count,remaining_time,
             recover_time, valid_recipe, valid_time, hold_time, update_time, valid_lot,count_unit,current_count,
             lsl,usl,process_time,job_id,split_qty,parameter, prodg_id,all_ch_flag,valid_count,clean_acc_flag,
             clean_mon_time,clean_group,clean_fix_flag,PICK_PILOT_MODE,COAT_FIX_BOUND,valid_ppid, rls_attr1,
             partcode,wip_attr3,rls_attr3,rtn                                                 
        ) 
        select parentid,toolg_id,tool_id,ch_id,prod_id, plan_id,step_id,recipe,ppid,layer,stage,
               tech,customer,lot_id, reticle_id, pre_tool_id, rule_name,remaining_count,remaining_time,
               recover_time, valid_recipe, valid_time, hold_time, update_time, valid_lot,count_unit,current_count,
               lsl,usl,process_time,job_id,split_qty,parameter, prodg_id,all_ch_flag,valid_count,clean_acc_flag,
               clean_mon_time,clean_group,clean_fix_flag,PICK_PILOT_MODE,COAT_FIX_BOUND,valid_ppid, rls_attr1,
               partcode,wip_attr3,rls_attr3,
               row_number() over() as rtn 
        from APS_ETL_PIRUN                                                                                             
            """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_TT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_TT")

    # 把之前的ETL_PIRUN删除
    sql = """      
         TRUNCATE TABLE APS_ETL_PIRUN                                                                                              
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="TRUNCATE TABLE APS_ETL_PIRUN2",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")

    # 得到需要的pad数据
    sql = """      
    INSERT INTO {tempdb}APS_TMP_PIRUN_TT_PAD_PM (                                       
        toolg_id,parameter,tool_id,ch_id,clean_group,count_unit,lsl                                                 
    ) 
    select toolg_id,parameter,tool_id,ch_id,clean_group,count_unit,lsl 
    from {tempdb}APS_TMP_PIRUN_TT
    where count_unit in ('Hours', 'Pieces')
    and current_count is not null
    and (clean_group like 'Pad%' or clean_group = 'ChamberPM')                                                                                            
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_TT_PAD_PM",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_TT_PAD_PM")

    #  透過 etl_ppid_info 得到 每片的process_time
    sql = """      
        INSERT INTO {tempdb}APS_TMP_PIRUN_TT_PPID_INFO (                                       
            toolg_id,parameter,process_time_per_wafer                                                 
        ) 
        with ppid_info as(
            select toolg_id,parameter,COAT_PER_WAFER 
            from APS_ETL_PPID_INFO.APS_ETL_PPID_INFO
            where toolg_id in (select toolg_id from {tempdb}APS_TMP_PIRUN_TT_PAD_PM)
            and parameter in (select parameter from {tempdb}APS_TMP_PIRUN_TT where usl is not null and process_time is not null and process_time <> '' and count_unit='Hours')
        )
        select distinct 
            toolg_id, 
            parameter, 
            quantile_cont(cast(COAT_PER_WAFER as decimal(18,4)),0.5) OVER(PARTITION BY TOOLG_ID, PARAMETER )  as process_time_per_wafer
            --percentile_cont(0.5) WITHIN GROUP (ORDER BY coat_per_wafer) as process_time_per_wafer
        from ppid_info
        where parameter is not null
        --group by 1, 2
        union all
        select distinct
            toolg_id, 
            null as parameter, 
            quantile_cont(cast(COAT_PER_WAFER as decimal(18,4)),0.5) OVER(PARTITION BY TOOLG_ID ) as process_time_per_wafer
            --percentile_cont(0.5) WITHIN GROUP (ORDER BY coat_per_wafer) as process_time_per_wafer
        from ppid_info
        --group by 1                                                                                            
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_TT_PPID_INFO",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_TT_PPID_INFO")

    #  計算 PAD PM 從 0~下限 需run多少片
    sql = """      
        INSERT INTO {tempdb}APS_TMP_PIRUN_TT_PAD_PIECES (                                       
            tool_id,ch_id,clean_group,lsl_p                                                
        ) 
        WITH tt_pad_pieces_t1 as(
            select 
                pp.toolg_id,pp.tool_id,pp.ch_id,pp.clean_group,pp.count_unit,pp.lsl, 
                coalesce(epi_1.process_time_per_wafer, epi_2.process_time_per_wafer) as process_time_per_wafer
            from {tempdb}APS_TMP_PIRUN_TT_PAD_PM pp
            left join {tempdb}APS_TMP_PIRUN_TT_PPID_INFO epi_1
            on pp.parameter = epi_1.parameter
            and pp.toolg_id = epi_1.toolg_id
            left join {tempdb}APS_TMP_PIRUN_TT_PPID_INFO epi_2
            on epi_2.parameter is null
            and pp.toolg_id = epi_2.toolg_id
        ), 
        tt_pad_pieces_t2 as(
            select
            toolg_id,tool_id,ch_id,clean_group,count_unit,lsl, 
            case when count_unit = 'Hours'  then cast(cast(lsl as decimal) / cast(process_time_per_wafer as decimal) as decimal)
                 when count_unit = 'Pieces' then cast(lsl as decimal)
            end as lsl_p
            from tt_pad_pieces_t1 t1
        ), 
        tt_pad_pieces_t3 as(
            select
                t2.clean_group,
                t2.tool_id, 
                case
                    when count(distinct t2.ch_id) = 1 then max(t2.ch_id)
                end as ch_id, 
                min(lsl_p) as lsl_p
            from tt_pad_pieces_t2 t2
            group by 1, 2 
        )
        select t3.tool_id, t3.ch_id,
            max(t3.clean_group) as clean_group,
            max(t3.lsl_p) as lsl_p
        from tt_pad_pieces_t3 t3
        group by t3.tool_id, t3.ch_id                                                                                            
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_TT_PAD_PIECES",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_TT_PAD_PIECES")

    #  計算各規則每run一片會需要機上多少時間
    sql = """      
    INSERT INTO {tempdb}APS_TMP_PIRUN_TT_COAT_HOURS (                                       
        parameter,toolg_id,rtn,process_time_per_wafer                                                
    ) 
    with tt_coat_hours_t1 as(
        select
            ep.parameter,toolg_id,parameter,rtn
        from {tempdb}APS_TMP_PIRUN_TT ep
        where case when process_time='' then 0 else cast(process_time as double) end > 0
        and count_unit = 'Hours'
        and coalesce(clean_group, '') not like 'Pad%'
        and tool_id in (select tool_id from {tempdb}APS_TMP_PIRUN_TT_PAD_PM)
    )
    select
        pp.parameter,pp.toolg_id,pp.rtn, 
        coalesce(epi_1.process_time_per_wafer, epi_2.process_time_per_wafer) as process_time_per_wafer
    from tt_coat_hours_t1 as pp
    left join {tempdb}APS_TMP_PIRUN_TT_PPID_INFO epi_1
    on pp.parameter = epi_1.parameter
    and pp.toolg_id = epi_1.toolg_id
    left join {tempdb}APS_TMP_PIRUN_TT_PPID_INFO epi_2
    on epi_2.parameter is null
    and pp.toolg_id = epi_2.toolg_id                                                                                            
    """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PIRUN_TT_COAT_HOURS",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PIRUN_TT_COAT_HOURS")

    # 整合最终结果
    sql = """      
        INSERT INTO APS_ETL_PIRUN (                                       
            parentid,toolg_id,tool_id,ch_id,prod_id, plan_id,step_id,recipe,ppid,layer,stage,
            tech,customer,lot_id, reticle_id, pre_tool_id, rule_name,remaining_count,remaining_time,
            recover_time, valid_recipe, valid_time, hold_time, update_time, valid_lot,count_unit,current_count,
            lsl,usl,process_time,job_id,split_qty,parameter, prodg_id,all_ch_flag,valid_count,clean_acc_flag,
            clean_mon_time,clean_group,clean_fix_flag,PICK_PILOT_MODE,valid_ppid, rls_attr1,wip_attr3,RLS_ATTR3,coat_fix_bound,
            partcode                                              
        ) 
        with tt_coat_result_t1 as (
            select
                ep.*, 
                case -- 可以從lsl扣除的額度
                    when  ep.process_time ='' or ep.process_time is null  then null
                    when cast(ep.process_time as double) = 0 then null
                    when ep.current_count is null then null -- 必須是累積厚度規則
                    when ep.clean_group like 'Pad%' then null -- 不用計算pad PM本身
                    when coalesce(ep.clean_group, '') = 'ChamberPM' and coalesce(pp_1.clean_group, pp_2.clean_group) <> 'ChamberPM' then null -- ChamberPM只須跟ChamberPM join
                    when coalesce(ep.clean_group, '') <> 'ChamberPM' and coalesce(pp_1.clean_group, pp_2.clean_group) = 'ChamberPM' then null -- ChamberPM只須跟ChamberPM join
                    when ep.count_unit = 'Pieces' then coalesce(pp_1.lsl_p, pp_2.lsl_p) -- 如果是片數規則，則只要扣除 lsl_p 即可
                    when ep.count_unit = 'Hours'  then cast(coalesce(pp_1.lsl_p, pp_2.lsl_p) as decimal) * ch.process_time_per_wafer -- 如果是時間規則，則須再從lsl_p換算所需時間
                end as lsl_pp
            from {tempdb}APS_TMP_PIRUN_TT ep
            left join {tempdb}APS_TMP_PIRUN_TT_PAD_PIECES pp_1
            on ep.ch_id = pp_1.ch_id
            and ep.tool_id = pp_1.tool_id
            left join {tempdb}APS_TMP_PIRUN_TT_PAD_PIECES pp_2
            on pp_2.ch_id is null
            and ep.tool_id = pp_2.tool_id
            left join {tempdb}APS_TMP_PIRUN_TT_COAT_HOURS ch
            on ep.rtn = ch.rtn
        )
        select
            parentid,toolg_id,tool_id,ch_id,prod_id, plan_id,step_id,recipe,ppid,layer,stage,
            tech,customer,lot_id, reticle_id, pre_tool_id, rule_name,remaining_count,remaining_time,
            recover_time, valid_recipe, valid_time, hold_time, update_time, valid_lot,count_unit,current_count,
            lsl,usl,process_time,job_id,split_qty,parameter, prodg_id,all_ch_flag,valid_count,clean_acc_flag,
            clean_mon_time,clean_group,clean_fix_flag,PICK_PILOT_MODE,valid_ppid, rls_attr1,wip_attr3,RLS_ATTR3,
            case when t1.lsl_pp is null then null
                 else greatest(cast(t1.lsl as decimal) - cast(t1.lsl_pp as decimal), 0.0) end as COAT_FIX_BOUND, -- 如果小於0則取0
            partcode
        from tt_coat_result_t1 t1                                                                                            
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_PIRUN5",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_PIRUN")


def execute():
    ###############################################################
    ### 以下参数必须定义
    ### ETL_Proc_Name    : ETL 名称
    ### current_time     ：请直接拷贝
    ### current_time_short ：请直接拷贝
    ### uuid             ：请直接拷贝
    ### target_table     : 该ETL输出表名
    ### used_table_list  : 该ETL使用到的，参考到的表名(中间表不算)
    ### target_table_sql ： 该ETL输出表定义SQL
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_PIRUN_10M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_ETL_PIRUN"
    used_table_list = ['APS_TR_CSFRINHIBIT',
                       'APS_MID_FHOPEHS_VIEW',
                       'APS_MID_RSPILOT_RUN_HS_VIEW',
                       'APS_TMP_LOTHISTORY_VIEW',
                       'APS_SYNC_FLOW',
                       'APS_ETL_WIP',
                       'APS_ETL_APC_RESULT',
                       'APS_SYNC_SCHE_APC_PB',
                       'APS_SYNC_APC_SPLIT_WAFER',
                       'APS_SYNC_WIP',
                       'APS_SYNC_SCHE_APC_PA',
                       'APS_SYNC_ETL_TOOL',
                       'APS_SYNC_SCHE_APC_LOT_SPECIAL_VALUE',
                       'APS_SYNC_TMS_EQP_PARAMETER',
                       'APS_SYNC_RTD_LIFE_TIME_CONTROL',
                       'APS_SYNC_WET_ACID_DATA',
                       'APS_SYNC_EQP_STATUS_WAIT_TIME_SETTING',
                       'APS_SYNC_RDS_RECIPEINFO',
                       'APS_SYNC_RTD_RUN_LOTBYEQP_COUNT',
                       'APS_SYNC_RSPILOT_RUNING',
                       'APS_SYNC_UMT_PATH_CONVERT_SETTING',
                       'APS_TMP_TYPE22_MEDIAN',
                       'APS_TMP_TYPE21_MEDIAN',
                       'APS_TMP_TYPE221_MEDIAN',
                       'APS_SYNC_EQP_ED_DATA_REPORT',
                       'APS_SYNC_UMT_EQP_UNIT_PLAN_SETTING',
                       'APS_MID_PURGE11',
                       'APS_MID_PURGE12',
                       'APS_SYNC_OUT_DF_PURGE_COMBINE',
                       'APS_SYNC_UMT_EQP_PM_DEFAULT_SETTING',
                       'APS_SYNC_OCS_AUTODUMMY_COMPLETE',
                       'APS_MID_IDLE_DUMMY_CU_MEDIAN',
                       'APS_ETL_PPID_INFO',
                       'APS_ETL_FLOW',
                       'APS_SYNC_RTD_PIRUN_TASK',
                       'APS_SYNC_PIRUN_TASK_SCHEDULE_LOT',
                       'APS_SYNC_RTD_PIRUN_TASK_DEVICE',
                       'APS_ETL_QTIME_SPEC',
                       'APS_SYNC_RTD_PREFER_INHIBIT_LOT'
                       ]
    target_table_sql = """
        create table {}APS_ETL_PIRUN
        (
            parentid        VARCHAR(60) not null,
            toolg_id        VARCHAR(60) not null,
            tool_id         VARCHAR(60) not null,
            ch_id           VARCHAR(60),
            prod_id         VARCHAR(60),
            plan_id         VARCHAR(60),
            step_id         VARCHAR(60),
            recipe          VARCHAR(60),
            ppid            VARCHAR(60),
            layer           VARCHAR(60),
            stage           VARCHAR(60),
            tech            VARCHAR(60),
            customer        VARCHAR(60),
            lot_id          VARCHAR(60),
            reticle_id      VARCHAR(60),
            pre_tool_id     VARCHAR(60),
            rule_name       VARCHAR(60) not null,
            remaining_count VARCHAR(60),
            remaining_time  VARCHAR(60),
            recover_time    VARCHAR(60),
            valid_recipe    VARCHAR(60),
            valid_time      VARCHAR(60),
            update_time     VARCHAR(60) not null,
            hold_time       VARCHAR(60),
            partcode        VARCHAR(60) not null,
            valid_lot       VARCHAR(60),
            count_unit      VARCHAR(60),
            current_count   VARCHAR(60),
            lsl             VARCHAR(60),
            usl             VARCHAR(60),
            process_time    VARCHAR(60),
            job_id          VARCHAR(64),
            split_qty       VARCHAR(60),
            parameter       VARCHAR(64),
            all_ch_flag     VARCHAR(2),
            valid_count     DECIMAL,
            prodg_id        VARCHAR(64),
            clean_acc_flag  VARCHAR(1),
            clean_mon_time  DECIMAL,
            clean_group     VARCHAR(64),
            clean_fix_flag  VARCHAR(1),
            pick_pilot_mode VARCHAR(1),
            COAT_FIX_BOUND  DECIMAL,
            valid_ppid      VARCHAR(64),
            rls_attr1       VARCHAR(64),
            wip_attr3       VARCHAR(64),
            rls_attr3       VARCHAR(64)
        )
    """.format("")  # 注意:这里一定要这么写 [create {table_type} table 表名] => [create {table_type} table {}表名]
    target_db_file = my_duck.get_target_file_name(target_table, current_time_short)
    # -------------------------- 内存模式改成文件模式
    _temp_db_path = os.path.join(config.g_mem_speed_etl_output_path, target_table, "inprocess")
    if not os.path.exists(_temp_db_path):
        os.makedirs(_temp_db_path)

    temp_db_file = os.path.join(_temp_db_path, target_table + "_" + current_time_short + "_temp.db")
    # 处理中文件
    in_process_db_file = os.path.join(_temp_db_path, target_table + "_" + current_time_short + ".db")
    # 结果文件
    target_db_file = os.path.join(config.g_mem_etl_output_path, target_table,
                                  target_table + "_" + current_time_short + ".db")
    # --------------------------

    oracle_conn = None
    try:
        oracle_conn = my_oracle.oracle_get_connection()
        # 开始日志
        my_oracle.StartCleanUpAndLog(oracle_conn, ETL_Proc_Name, current_time)

        # -------------------------- 内存模式改成文件模式
        # 创建DuckDB
        duck_db_memory = my_duck.create_duckdb_in_file(_temp_db_path, in_process_db_file, target_table_sql)
        duck_db_memory.sql('SET threads TO 4')
        if not os.path.exists(os.path.join(config.g_mem_speed_etl_output_path, 'duck_temp', uuid)):
            os.makedirs(os.path.join(config.g_mem_speed_etl_output_path, 'duck_temp', uuid))
        duck_db_memory.execute(
            "SET temp_directory='{}'".format(os.path.join(config.g_mem_speed_etl_output_path, 'duck_temp', uuid)))

        if not config.g_debug_mode:
            create_temp_table(duck_db_memory)
        else:
            duck_db_temp = my_duck.create_duckdb_for_temp_table(_temp_db_path, temp_db_file)
            # 创建Temp表
            create_temp_table(duck_db_temp)
            duck_db_temp.commit()
            duck_db_temp.close()
            my_duck.attach_temp_db_write_able(duck_db_memory, "TEMPDB", temp_db_file)
        # --------------------------

        # Attach用到的表
        res_dict = my_duck.attach_used_table(oracle_conn, duck_db_memory, used_table_list)
        ################################################################################################################
        ## 以下为业务逻辑
        ################################################################################################################

        getetApsEtlWipSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetCsfrinhibitSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetApcPb1DataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetApcPa1DataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetEtlToolDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getPirunRunInfo(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getPirunHistData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getRecoverTimeByLotHisData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getRecoverTimeByMedianData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetPiRunSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetUserFlagIsActiveSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetFirstRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetSecondRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getInhibitCase3ReworkData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getInhibitCase3NotReworkData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetThirdRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getCase4InhibitData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetFourthRuleByCSBSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getCase4InhibitResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetFourthRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getCase5SourceData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getCase5ResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # 增加userFlag5的rework数据
        getCase5AddReworkResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetFifthRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetSeventhRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # 增加全区非炉管区膜厚写入PIRUN
        GetCoatRule1Sql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # equipment_no、chamber_no、dc_item_name、spec_unit相同需要合并起来
        GetCoatRule1GroupSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetCoatRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetDfByRecipeAcceptSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetDfByRecipeRejectSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetWetByRecipeAcceptSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetWetByRecipeRejectSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetWaitTimeSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetTestRunRecipeInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetTestRunT21MedianInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetTestRunT22MedianInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetTestRunT221MedianInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetRtdRunPStartSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetRtdRunPEndSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetRtdRunProcessStartSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetRtdRunCountSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetRtdRunLotCountSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetRtdRunDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetTestRunInfoSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetTestRunApcResultSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetInsertSqlFromPiRun(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        getReturnLineDATE(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        #  DF BATCH PURGE的相关逻辑
        # 最近一次的purge时间
        GetLastPurgeDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        #  ①抓取机台累积Batch记录
        GetPurgeEqpDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # ②计算过去7天累积最大的Batch数
        GetPurgeMaxBatchDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # ④抓取机台Batch设定值
        GetPurgeEqpBatchDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # ⑤抓取Batch设定防呆值
        GetPurgeDefBatchDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # ⑥最终的Purge值
        GetPurgeReultDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # 针对MA_AL(Cu) 没做50pcs要run一次dummy
        GetCuDummyDataSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        ################################################################################################################

        # 该逻辑放在最后面
        # 如果該機台有累積厚度規則pad PM(clean_group = 'Pad')，則同機台的其他累積厚度規則若累積厚度滿一定值也可一同處理
        # 保存累積厚度 Pad PM
        GetPadPMData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        # 透過 etl_ppid_info 得到 每片的process_time
        GetPpidPTData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        # 计算各機群可以扣除的量
        GetMinLslData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        # 获取COAT_FIX_BOUND信息
        GetCoatFixBoundData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        # ETCH  chamber 式機台 RUN  貨方式 是由 RDS/RMS 卡控時效性  RDS_Inhibit
        GetRDSInhibitData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        # 增加ATP_PIRUN功能
        # 获取有qtime剩余>=48小时的资讯
        GetATPTaskQtimeData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # 获取情景一 ATP_Pirun已開單，但是尚未挑選Pilot
        GetATPCase1Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # ATP_Pirun尚未結束，且已挑選精准Pilot
        GetATPCase2LotData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # ATP_Pirun尚未結束，预挑選Pilot
        GetATPCase2PreLotData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        # 修改算法，clean_group='Pad'改為模糊比對，並且加入clean_group='ChamberPM'邏輯
        GetPadCoatFixBoundData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        ################################################################################################################
        ## 写入log
        ################################################################################################################
        # 写版本号
        my_oracle.Monitor_HandlingVerControl(oracle_conn, uuid, target_table, target_db_file, current_time_short)

        ################################################################################################################
        ## 以上为业务逻辑
        ################################################################################################################
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            select_sql_in_duck = """select parentid,toolg_id,tool_id,
                                           CASE WHEN ch_id='' THEN NULL ELSE ch_id END AS ch_id,
                                           CASE WHEN prod_id='' THEN NULL ELSE prod_id END AS prod_id, 
                                           CASE WHEN plan_id='' THEN NULL ELSE plan_id END AS plan_id,
                                           CASE WHEN step_id='' THEN NULL ELSE step_id END AS step_id,
                                           CASE WHEN recipe='' THEN NULL ELSE recipe END AS recipe,
                                           CASE WHEN ppid='' THEN NULL ELSE ppid END AS ppid,
                                           CASE WHEN layer='' THEN NULL ELSE layer END AS layer,
                                           CASE WHEN stage='' THEN NULL ELSE stage END AS stage,
                                           CASE WHEN tech='' THEN NULL ELSE tech END AS tech,
                                           CASE WHEN customer='' THEN NULL ELSE customer END AS customer,
                                           CASE WHEN lot_id='' THEN NULL ELSE lot_id END AS lot_id, 
                                           CASE WHEN reticle_id='' THEN NULL ELSE reticle_id END AS reticle_id, 
                                           CASE WHEN pre_tool_id='' THEN NULL ELSE pre_tool_id END AS pre_tool_id, 
                                           rule_name,
                                           CASE WHEN remaining_count='' THEN NULL ELSE remaining_count END AS remaining_count,
                                           CASE WHEN remaining_time='' THEN NULL ELSE remaining_time END AS remaining_time,
                                           CASE WHEN recover_time='' THEN NULL ELSE recover_time END AS recover_time, 
                                           CASE WHEN valid_recipe='' THEN NULL ELSE valid_recipe END AS valid_recipe, 
                                           CASE WHEN valid_time='' THEN NULL ELSE valid_time END AS valid_time, 
                                           CASE WHEN hold_time='' THEN NULL ELSE hold_time END AS hold_time, 
                                           update_time, 
                                           CASE WHEN valid_lot='' THEN NULL ELSE valid_lot END AS valid_lot,
                                           CASE WHEN count_unit='' THEN NULL ELSE count_unit END AS count_unit,
                                           CASE WHEN current_count='' THEN NULL ELSE current_count END AS current_count,
                                           CASE WHEN lsl='' THEN NULL ELSE lsl END AS lsl,
                                           CASE WHEN usl='' THEN NULL ELSE usl END AS usl,
                                           CASE WHEN process_time='' THEN NULL ELSE process_time END AS process_time,
                                           CASE WHEN job_id='' THEN NULL ELSE job_id END AS job_id,
                                           CASE WHEN split_qty='' THEN NULL ELSE split_qty END AS split_qty,
                                           CASE WHEN parameter='' THEN NULL ELSE parameter END AS parameter,
                                           CASE WHEN prodg_id='' THEN NULL ELSE prodg_id END AS prodg_id,
                                           CASE WHEN all_ch_flag='' THEN NULL ELSE all_ch_flag END AS all_ch_flag,
                                           valid_count,
                                           CASE WHEN clean_acc_flag='' THEN NULL ELSE clean_acc_flag END AS clean_acc_flag, 
                                           clean_mon_time,
                                           CASE WHEN clean_group='' THEN NULL ELSE clean_group END AS clean_group, 
                                           CASE WHEN clean_fix_flag='' THEN NULL ELSE clean_fix_flag END AS clean_fix_flag,
                                           CASE WHEN PICK_PILOT_MODE='' OR PICK_PILOT_MODE IS NULL THEN '0' ELSE PICK_PILOT_MODE END AS PICK_PILOT_MODE,
                                           CASE WHEN COAT_FIX_BOUND IS NULL THEN NULL ELSE COAT_FIX_BOUND END AS COAT_FIX_BOUND,
                                           CASE WHEN valid_ppid ='' THEN NULL ELSE valid_ppid END AS valid_ppid,
                                           case when rls_attr1 = '' then null else rls_attr1 end as rls_attr1,
                                           case when wip_attr3 = '' then null else wip_attr3 end as wip_attr3,
                                           case when rls_attr3 = '' then null else rls_attr3 end as rls_attr3
                                           from APS_ETL_PIRUN
                                """
            postgres_table_define = """etl_pirun(parentid,toolg_id,tool_id,ch_id,prod_id, plan_id,step_id,recipe,ppid,layer,stage,
                                                 tech,customer,lot_id, reticle_id, pre_tool_id, rule_name,remaining_count,remaining_time,
                                                 recover_time, valid_recipe, valid_time, hold_time, update_time, valid_lot,count_unit,current_count,
                                                 lsl,usl,process_time,job_id,split_qty,parameter, prodg_id,all_ch_flag,valid_count,clean_acc_flag,
                                                 clean_mon_time,clean_group,clean_fix_flag,PICK_PILOT_MODE,COAT_FIX_BOUND,valid_ppid, rls_attr1,
                                                 wip_attr3,rls_attr3)
                                               """
            my_postgres.copy_duckdb_to_postgres(uuid=uuid,
                                                duckdb=duck_db_memory,
                                                table_name_in_duckdb=target_table,
                                                table_name_in_pg="etl_pirun",  # 要小写
                                                select_sql_in_duck=select_sql_in_duck,
                                                postgres_table_define=postgres_table_define,
                                                oracle_conn=oracle_conn,
                                                ETL_Proc_Name=ETL_Proc_Name)
        # 导出到目标文件中
        # target_db_file = my_duck.export_result_duck_file_and_close_duck_db_memory(duck_db_memory,
        #                                                                           target_table,
        #                                                                           target_table_sql.format("file_db."),
        #                                                                           current_time_short)
        target_db_file = my_duck.export_result_duck_file_and_close_duck_db_memory2(duck_db_memory,
                                                                                   in_process_db_file=in_process_db_file,
                                                                                   target_table=target_table,
                                                                                   current_time=current_time_short)
        # 写版本号
        my_oracle.HandlingVerControl(oracle_conn, uuid, target_table, target_db_file, current_time_short)
        # 加入PG执行结束的时间更新
        my_oracle.Update_PG_HandlingVerControl(oracle_conn, uuid, target_table, target_db_file, current_time_short)
        # 写完成日志
        my_oracle.EndCleanUpAndLog(oracle_conn, ETL_Proc_Name, current_time)
    except Exception as e:
        logging.exception("{ETL_Proc_Name} 處理出錯 : {e}".format(ETL_Proc_Name=ETL_Proc_Name, e=e))
        # 导出到目标文件中
        my_duck.export_result_duck_file_and_close_duck_db_memory2(duck_db_memory,
                                                                  in_process_db_file=in_process_db_file,
                                                                  target_table=target_table,
                                                                  current_time=current_time_short)
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_PIRUN_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
        # 删除TMP目录:LQN:2023/08/21
        if os.path.exists(os.path.join(config.g_mem_etl_output_path, 'duck_temp', uuid)):
            os.remove(os.path.join(config.g_mem_etl_output_path, 'duck_temp', uuid))
        if os.path.exists(temp_db_file) and not config.g_debug_mode:
            os.remove(temp_db_file)
        gc.collect()  # 内存释放


if __name__ == '__main__':
    # 单JOB测试用
    print("start")
    execute()
