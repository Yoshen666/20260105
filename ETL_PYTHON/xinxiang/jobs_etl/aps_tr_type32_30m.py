...
def create_temp_table(duck_db):
    ...
    '''
           ...
           '''
    sql = """
       create {table_type} table APS_ETL_TMP_TR_TYPE32_LOT_HIST
       (
            eqp_id     VARCHAR(64),
...
def GetType32FirstSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO APS_TR_TYPE32 (                                                
              GUID, MODULE_ID, TOOLG_ID, QTY, PROCESS_START, PROCESS_END, PARTKEY ,BATCH_FLAG, recipe_count                                                                                         
    )                                                                                                                                                                                                                 
     SELECT distinct '{uuid}' AS GUID,                                                                                       
            MODULE_ID,                                                                                                
            TOOLG_ID,                                                                                                                                                                                                                                                                                                    
            QTY,                                                                                                      
            PROCESS_START,                                                                                            
            ...
            BATCH_FLAG,
            recipe_count                                                                                                       
     FROM APS_TR_TYPE31.APS_TR_TYPE31                                     
    """.format(uuid=uuid, tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TR_TYPE32",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TR_TYPE32")


def GetType32OtherSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO APS_TR_TYPE32 (                                                
              GUID, MODULE_ID, TOOLG_ID, QTY, PROCESS_START, PROCESS_END, PARTKEY ,BATCH_FLAG,recipe_count                                                                                         
    )                                                                                                                                                                                                                   
     SELECT distinct '{uuid}' AS GUID,                                                                                       
            ...
            BATCH_FLAG,
            recipe_count                                                                                                       
     FROM APS_TR_TYPE31.APS_TR_TYPE31 T
     WHERE  1=1                                                                                                
     AND NOT EXISTS                                                                                            
     (                                                                                                         
         SELECT MODULE_ID FROM LAST_APS_TR_TYPE32.APS_TR_TYPE32 T32                                                               
         WHERE T32.MODULE_ID = T.MODULE_ID                                                                     
         AND T32.TOOLG_ID = T.TOOLG_ID                                                                         
         AND T32.QTY = T.QTY                                                                                   
         AND T32.PROCESS_START = T.PROCESS_START                                                               
         AND T32.PROCESS_END = T.PROCESS_END                                                                   
     )                                      
    """.format(uuid=uuid, tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_TR_BR.APS_TR_TYPE32_30M"
    ...
    try:
        ...
        file_name = my_file.get_last_db_file(conn=oracle_conn, table_name="APS_TR_TYPE32")
        # if 取不到
        if file_name is None:
            GetType32FirstSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        # else 取到的情况下
        # 手动 Attach APS_TR_TYPE31 AS LAST_APS_TR_TYPE31
        else:
            my_duck.attach_table(duck_db_memory=duck_db_memory, table_name="LAST_APS_TR_TYPE32",
                                 target_db_file=file_name)

            GetType32OtherSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
            last_table_name = 'LAST_APS_TR_TYPE32.APS_TR_TYPE32'
            # 判断attach前一个版本的数据是否为0，如果是0，则return
            rown = my_duck.get_row_count_in_duckdb(duck_db_memory, last_table_name)
            if rown == 0:
                logging.info("LAST_APS_TR_TYPE32 DATA IS NULL")
                return
            sql = """insert into APS_TR_TYPE32 select * from {last_table_name}
            ...
            my_duck.detach_table(duck_db=duck_db_memory, db_name="LAST_APS_TR_TYPE32")
        # 最后在判断最终表的数据是否为0，如果是0，则return
        rown = my_duck.get_row_count_in_duckdb(duck_db_memory, target_table)
        if rown == 0:
            logging.info("APS_TR_TYPE32 DATA IS NULL")
            return
        ################################################################################################################
        ## 以上为业务逻辑
        ################################################################################################################
        ...
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, 'XETL038')
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
