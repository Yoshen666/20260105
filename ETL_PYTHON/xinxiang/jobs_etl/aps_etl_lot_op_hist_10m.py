...
def create_temp_table(duck_db):
    ...
    sql = """
     create {table_type} table APS_TMP_ETL_LOT_OP_HIST_FHOPEHS
     (
        arrival   VARCHAR(64),
        ope_no  VARCHAR(64),
        lot_id    VARCHAR(64),
    ...
    sql = """
        create {table_type} table APS_TMP_ETL_LOT_OP_HIST_WIP
        (
           arrival   VARCHAR(64),
           ope_no  VARCHAR(64),
           lot_id    VARCHAR(64),
    ...
    sql = """
        ...
        (
            ...
            ph_recipe_id  VARCHAR(64),
            rtd_groupname VARCHAR(64),
            ARRIVAL_HF    VARCHAR(64)
        )
         """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_ETL_LOT_OP_HIST_FORMING_RESULT
        (
          lot_id        VARCHAR(64),
          mainpd_id     VARCHAR(64),
          batch_name    VARCHAR(64),
    ...
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_ETL_LOT_OP_HIST_BATCH_NAME
        (
          mainpd_id  VARCHAR(64),
          ope_no     VARCHAR(64),
          batch_name VARCHAR(64)
    ...
    duck_db.sql(sql)
    sql = """
       create {table_type} table APS_TMP_ETL_LOT_OP_HIST_BATCHFORMING_HS
       (
         lot_id        VARCHAR(60),
         ope_no        VARCHAR(60),
    ...
    sql = """
      create {table_type} table APS_TMP_ETL_LOT_OP_HIST_LOT_PRIORITY
      (
        lot_id        VARCHAR(64),
        group_id        VARCHAR(64),
...
def GetPoN1Sql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    sql = """
         INSERT INTO {tempdb}APS_TMP_ETL_LOT_OP_HIST_PO_N1(                           
          POS_PDID, MAINPD_ID, OPE_NO,BATCH_NAME                                                      
        )                                                                     
         SELECT N.POS_PDID,N.MAINPD_ID,N.OPE_NO,BATCH_NAME                                                                     
...
def GetWipArrivalTimeSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    # 获取V_FHOPEHS的信息
    sql = """
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_LOT_OP_HIST_WIP(                              
            ARRIVAL,OPE_NO,LOT_ID,MAINPD_ID
        )                                                                                           
         SELECT MAX(L.LAST_OPE_COMP_TIME) AS ARRIVAL,                                                                                                                  
                L.OPE_NO,                                                                         
                L.LOT_ID,
                L.MAINPD_ID                                                                            
         FROM APS_SYNC_WIP.APS_SYNC_WIP L                                         
         GROUP BY L.LOT_ID, L.OPE_NO,L.MAINPD_ID           
    """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_LOT_OP_HIST_WIP",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_LOT_OP_HIST_WIP")

def GetFhopehsTrackOutTimeSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    # 获取V_FHOPEHS的信息
    sql = """
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_LOT_OP_HIST_FHOPEHS(                              
            ARRIVAL,OPE_NO,LOT_ID,MAINPD_ID
        )                                                                                           
         SELECT MAX(L.CLAIM_TIME) AS ARRIVAL,                                                                                                                  
                L.OPE_NO,                                                                         
                L.LOT_ID,
                L.MAINPD_ID                                                                            
         FROM APS_MID_FHOPEHS_VIEW.APS_MID_FHOPEHS_VIEW L                                         
         WHERE L.OPE_CATEGORY='OperationComplete'                                                   
         AND L.CLAIM_TIME >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '10' DAY 
         GROUP BY L.LOT_ID, L.OPE_NO,L.MAINPD_ID           
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_LOT_OP_HIST_FHOPEHS",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_LOT_OP_HIST_FHOPEHS")
...
def GetViewLotHistorySql_PartOne(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    sql = """      
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_LOT_OP_HIST_PARTONE(                                          
                 LOT_ID,PRODSPEC_ID,MAINPD_ID,PRIORITY,
                 OPE_NO,WAFER_QTY, CTRL_JOB,EQP_ID,TOOLG_ID,
                 REAL_MODULE,LOTID,ARRIVAL_LD,ARRIVAL_AD,ARRIVAL_HF, RESERVE_TIME, TRACK_IN,
        ...
        SELECT   HS.LOT_ID,                                                                                     
           ...
           TOOLG_ID,                                                                                      
           REAL_MODULE,                                                                                   
           LD.LOT_ID AS LOTID,LD.ARRIVAL AS ARRIVAL_LD, AD.ARRIVAL AS ARRIVAL_AD, HF.ARRIVAL AS ARRIVAL_HF,                                                                              
           RS.RESERVE_TIME,                                                                               
           TRACK_IN,                                                                                      
           PROCESS_START,                                                                                 
           SCANNER_START,                                                                                 
        ...
        LEFT JOIN {tempdb}APS_TMP_ETL_LOT_OP_HIST_FHOPEHS HF
        ON HF.LOT_ID = HS.LOT_ID                                                                            
        AND HF.OPE_NO = HS.OPE_NO 
        AND HF.MAINPD_ID = HS.MAINPD_ID                                                                                           
          """.format(tempdb=my_duck.get_temp_table_mark())
...
def GetViewLotHistorySql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    sql = """      
         INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_LOT_OP_HIST(                                           
                ...
                TRACK_OUT, RETICLE_ID,TECH_ID,CUSTOMER_ID,LOT_TYPE,PH_RECIPE_ID, RTD_GROUPNAME,BATCH_NAME                                                                                       
        )                                                                                                         
         SELECT   HS.LOT_ID,                                                                                      
            HS.PRODSPEC_ID,                                                                                 
            HS.MAINPD_ID,                                                                                   
            ...
            (CASE WHEN HS.ARRIVAL_LD IS NOT NULL AND HS.ARRIVAL_LD <> '' THEN HS.ARRIVAL_LD 
                  WHEN HS.ARRIVAL_HF IS NOT NULL AND HS.ARRIVAL_HF <>'' AND CAST(HS.ARRIVAL_HF AS TIMESTAMP) < CAST(HS.TRACK_IN AS TIMESTAMP) THEN HS.ARRIVAL_HF                                         
         ...
         LEFT JOIN {tempdb}APS_TMP_ETL_LOT_OP_HIST_PO_N1 PN
         ON PN.OPE_NO = HS.OPE_NO                                                                           
         AND PN.MAINPD_ID = HS.MAINPD_ID                                                                    
...
def GetBatchFormingSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    # 为了获取状态信息BatchForming
    sql = """
         INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_LOT_OP_HIST_BATCHFORMING_HS                        
         ...
         FROM (                                                                                          
                  SELECT OH.LOT_ID,                                                                                  
                   ...
                   INNER JOIN {tempdb}APS_TMP_ETL_LOT_OP_HIST W ON OH.LOT_ID = W.LOT_ID                                            
                                                          AND OH.MAINPD_ID = W.MAINPD_ID                            
                                                          AND OH.PRODSPEC_ID = W.PRODSPEC_ID                        
                   WHERE  1=1                                                                                        
                   AND OH.CLAIM_TIME >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '5 day'                                                                        
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_LOT_OP_HIST_BATCHFORMING_HS",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_LOT_OP_HIST_BATCHFORMING_HS")

    # 先获取BatchName的信息
    sql = """
        INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_LOT_OP_HIST_BATCH_NAME                            
        ...
        AND EXISTS ( 
            SELECT 1 FROM {tempdb}APS_TMP_ETL_LOT_OP_HIST W           
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_LOT_OP_HIST_BATCH_NAME",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_LOT_OP_HIST_BATCH_NAME")

    # 获取封Batch的BatchName信息
    sql = """
        INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_LOT_OP_HIST_FORMING_RESULT                             
        ...
        SELECT DISTINCT H.LOT_ID,H.MAINPD_ID,                     
               ...
               H.CLAIM_USER_ID                                    
        FROM {tempdb}APS_TMP_ETL_LOT_OP_HIST_BATCHFORMING_HS H        
            INNER JOIN {tempdb}APS_TMP_ETL_LOT_OP_HIST_BATCH_NAME N       
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_LOT_OP_HIST_FORMING_RESULT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_LOT_OP_HIST_FORMING_RESULT")
...
def GetShrFlagData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_LOT_OP_HIST_LOT_PRIORITY
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_LOT_OP_HIST_LOT_PRIORITY",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_LOT_OP_HIST_LOT_PRIORITY")
...
def OtherSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    INSERT  /*+ append */  INTO APS_ETL_LOT_OP_HIST(                                                                                                     
     ...
     LOT_HIS AS (                                                                                                                                                
          ...
          MAX(HS.PH_RECIPE_ID) AS PH_RECIPE_ID,                                                                                                     
          MAX(HS.LOT_TYPE) AS LOT_TYPE,
          MAX(HS.BATCH_NAME) AS BATCH_NAME                                                                                                          
     ...
     HIS_DATA AS (                                                                                                                                               
         ...
         CASE WHEN PR.LOT_PRIORITY IS NULL THEN 500 ELSE PR.LOT_PRIORITY END AS PTY,                                                                                         
         CASE WHEN PR.SHR_FLAG IS NULL THEN -1 ELSE PR.SHR_FLAG END AS SHR_FLAG,                                                                                                                          
         ...
         -----优先保持跟etl_wip 一致                                                                                                                           
         CASE WHEN HWP.ARRIVAL IS NOT NULL AND HWP.ARRIVAL <> '' THEN HWP.ARRIVAL ELSE (
             ...
             SA.SPLIT_TIME ELSE LH.PREV_OP_COMP_TIME END )
         END AS ARRIVAL,                                  
         ...
         CASE WHEN RT.LOT_ID IS NOT NULL THEN RT.CLAIM_TIME END AS FORMBATCH_TIME                                                                                                                          
         FROM LOT_HIS LH                                                                                                                                         
         LEFT JOIN {tempdb}APS_TMP_ETL_LOT_OP_HIST_LOT_PRIORITY PR ON PR.LOT_ID = LH.LOT_ID                                                                                                    
         ...
         LEFT JOIN {tempdb}APS_TMP_ETL_LOT_OP_HIST_WIP HWP ON LH.LOT_ID = HWP.LOT_ID AND LH.OPE_NO = HWP.OPE_NO AND LH.MAINPD_ID = HWP.MAINPD_ID
         LEFT JOIN {tempdb}APS_TMP_ETL_LOT_OP_HIST_FORMING_RESULT RT
         ON RT.LOT_ID = LH.LOT_ID AND RT.MAINPD_ID = LH.MAINPD_ID  
         AND RT.BATCH_NAME = LH.BATCH_NAME                                                                           
     )                                                                                                                                                           
...
def execute():
    ...
    try:
        ...
        # arrvial 优先保持跟wip一致
        GetWipArrivalTimeSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetFhopehsTrackOutTimeSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        ...
        # 找到FromBatch信息
        GetBatchFormingSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # 三码逻辑
        ...
        if file_name is None:
            ...
            last_table_name = 'LAST_APS_ETL_LOT_OP_HIST.APS_ETL_LOT_OP_HIST'
            # 判断attach前一个版本的数据是否为0，如果是0，则return
            rown = my_duck.get_row_count_in_duckdb(duck_db_memory, last_table_name)
            if rown == 0:
                logging.info("LAST_APS_ETL_LOT_OP_HIST DATA IS NULL")
                return
            sql = """insert into APS_ETL_LOT_OP_HIST select * from {last_table_name}
                     WHERE CAST(TRACK_IN AS TIMESTAMP) >= DATE_TRUNC('second', CURRENT_TIMESTAMP)- INTERVAL '40 day'""".format(last_table_name=last_table_name)
        ...
        if rown == 0:
            logging.info("APS_ETL_LOT_OP_HIST DATA IS NULL")
            return
        ################################################################################################################
        ## 写入log
        ################################################################################################################
