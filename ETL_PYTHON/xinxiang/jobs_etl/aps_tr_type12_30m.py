...
def GetType12FirstSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO APS_TR_TYPE12 (                                
           GUID, TOOLG_ID, ARRIVAL, TRACK_IN,TRACK_OUT, PARTKEY, BANK_TIME
    )                                                                                         
     WITH TOOLG_DATA AS (                                                                     
     ...
     TYPE12 AS (                                                                                                                     
          SELECT                                                                                                                                                              
                ...
                -- CAST(LH.BANK_PERIOD_TIME*24 AS DECIMAL(18, 2)) AS BANK_TIME
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_TR_TYPE12",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TR_TYPE12")


def GetType12OtherSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO APS_TR_TYPE12 (                                
           GUID, TOOLG_ID, ARRIVAL, TRACK_IN,TRACK_OUT, PARTKEY, BANK_TIME
    )                                                                                         
     ...
     WHERE                                                                                                                           
            NOT EXISTS (                                                                                                                 
                SELECT 1 FROM LAST_APS_TR_TYPE12.APS_TR_TYPE12 T12                          
                WHERE  T12.TOOLG_ID = T.TOOLG_ID                                                                
                AND T12.TRACK_IN = T.OP_START_DATE_TIME                                                             
                AND T12.ARRIVAL = T.PREV_OP_COMP_TIME           
                AND T12.TRACK_OUT = T.CLAIM_TIME                                                          
        )                                                                                                                                       
    """.format(uuid=uuid)
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TR_TYPE12_30M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_TR_TYPE12"
    used_table_list = ['APS_SYNC_ETL_TOOL', 'APS_MID_FHOPEHS_VIEW', 'APS_TMP_LOTHISTORY_VIEW']
    target_table_sql = """
    ...
    try:
        ...
        ## 以下为业务逻辑
        ################################################################################################################
        # 取上一次的APS_TR_TYPE12版本
        file_name = my_file.get_last_db_file(conn=oracle_conn, table_name="APS_TR_TYPE12")
        # if 取不到
        if file_name is None:
            GetType12FirstSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        # else 取到的情况下
        # 手动 Attach APS_TR_TYPE12 AS LAST_APS_TR_TYPE12
        else:
            my_duck.attach_table(duck_db_memory=duck_db_memory, table_name="LAST_APS_TR_TYPE12",
                                 target_db_file=file_name)

            GetType12OtherSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
            last_table_name = 'LAST_APS_TR_TYPE12.APS_TR_TYPE12'
            # 判断attach前一个版本的数据是否为0，如果是0，则return
            rown = my_duck.get_row_count_in_duckdb(duck_db_memory, last_table_name)
            if rown == 0:
                logging.info("LAST_APS_TR_TYPE12 DATA IS NULL")
                return
            sql = """insert into APS_TR_TYPE12 select * from {last_table_name}
            ...
            my_duck.detach_table(duck_db=duck_db_memory, db_name="LAST_APS_TR_TYPE12")
        # 最后在判断最终表的数据是否为0，如果是0，则return
        rown = my_duck.get_row_count_in_duckdb(duck_db_memory, target_table)
        if rown == 0:
            logging.info("APS_TR_TYPE12 DATA IS NULL")
            return
        ################################################################################################################
        ## 以上为业务逻辑
        ################################################################################################################
        ...
        my_duck.export_result_duck_file_and_close_duck_db_memory2(duck_db_memory,
                                                                  ...
                                                                  current_time=current_time_short)
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, 'XETL034')
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
