...
def getSql(ETL_Proc_Name, oracle_conn, duck_db_memory, uuid, current_time):
    ...
    INSERT  /*+ append */  INTO APS_ETL_PRI_WIP (                                                                                             
           PARENTID,TOOLG_ID, LOT_ID, PRI_TYPE, UPDATE_TIME, PARTCODE                                                                       
    ) 
    WITH SOURCE_DATA AS (                                                                                                                                             
        SELECT  DISTINCT P.LOT_ID,  
                CASE WHEN N.LOT_ID IS NOT NULL AND T.DZ = 1 THEN 'FIRE '|| N.SHL_FireNumber ELSE D.LOT_PURPOSE END AS LOT_PURPOSE
        ...
        LEFT JOIN APS_SYNC_SHL_LOT_FIRE_NUMBER.APS_SYNC_SHL_LOT_FIRE_NUMBER N
        ON P.LOT_ID = N.LOT_ID  
        LEFT JOIN (WITH RECURSIVE cte(DZ) as ( select 1 as dz union all select dz + 1 from cte where dz < 2) select dz from cte ) T 
...
def execute():
    ...
    used_table_list = ['APS_SYNC_SHL_LOT_FIRE_NUMBER',
    ...
    try:
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_SGS_RLS_CODE_XX_ETL)  # TODO 改成你自己的CODE
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
