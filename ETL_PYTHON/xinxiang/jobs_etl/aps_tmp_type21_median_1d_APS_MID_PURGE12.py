...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO APS_MID_PURGE12(                                 
        PARENTID,  TOOLG_ID, PT, UPDATE_TIME, PARTCODE                                                                        
         ...
         PT_DATA AS (                                                                                     
             SELECT EQP_G,RN,                                                                      
                    --MEDIAN(CAST(PT AS DECIMAL)) OVER(PARTITION BY EQP_G ) AS MAX_50 
                    QUANTILE_CONT(CAST(PT AS DECIMAL),0.5) OVER(PARTITION BY EQP_G ) AS MAX_50                                          
             FROM SOURCE_DATA                                                                             
         ),                                                                                               
         RESULT_DATA AS (                                                                                 
             SELECT EQP_G,                                                                        
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_PURGE12",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_PURGE12")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_TYPE21_MEDIAN_1D_APS_MID_PURGE12"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_PURGE12"
    used_table_list = ['APS_SYNC_OUT_DF_PURGE_COMBINE', 'APS_SYNC_ETL_TOOL']
    target_table_sql = """
        create table {}APS_MID_PURGE12
        (
    ...
    try:
        ...
        my_duck.export_result_duck_file_and_close_duck_db_memory2(duck_db_memory,
                                                                  ...
                                                                  current_time=current_time_short)
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, "XETL057")
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
