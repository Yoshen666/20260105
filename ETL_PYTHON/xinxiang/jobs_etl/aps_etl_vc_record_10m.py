...
def GetRecordSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        ...
        PARENTID, LOT_ID, TOOLG_ID, TOOL_ID, REST_TIME, QTY,UPDATE_TIME, PARTCODE, VC_GROUP                                                  
         ...
         FROM {tempdb}APS_TMP_ETL_VC_RECORD_SGS  LW                                
         INNER JOIN {tempdb}APS_TMP_ETL_VC_RECORD_WIP W                             
                ON LW.LOT_ID = W.LOT_ID                
         GROUP BY LW.LOT_ID                                    
...
def GetWipSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
     INSERT /*+ APPEND */                                             
     INTO  {tempdb}APS_TMP_ETL_VC_RECORD_WIP                                
      (LOT_ID, 
       OPE_NO)                                                    
...
def GetSgsSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
       INSERT /*+ APPEND */                                            
         INTO  {tempdb}APS_TMP_ETL_VC_RECORD_SGS                               
              ( LOT_ID, 
                EQP_G,
                PHOTO_EQP_ID, 
                CURRENT_SGS_WEIGHT, 
...
def execute():
    ...
    try:
        ...
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            select_sql_in_duck =  """select PARENTID, LOT_ID, TOOLG_ID, TOOL_ID, REST_TIME, QTY, UPDATE_TIME, CASE WHEN VC_GROUP='' THEN NULL ELSE VC_GROUP END AS VC_GROUP from APS_ETL_VC_RECORD
            """
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_VC_RECORD_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
