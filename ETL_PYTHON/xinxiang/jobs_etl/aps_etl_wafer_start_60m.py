...
def GetWaferStartSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO APS_ETL_WAFER_START(                    
      PARENTID, MFG_DATE, LOT_ID, QTY, PROD_ID, UPDATE_TIME, PARTCODE                                                        
    )                                                                          
     SELECT '{uuid}' AS PARENTID,                                  
            -- TO_CHAR(I.START_DATE,'YYYY-MM-DD') AS MFG_DATE,
            STRFTIME(I.START_DATE, '%Y-%m-%d') AS MFG_DATE,                      
            I.KEY_NO AS LOT_ID,                                                 
            I.PCS_QTY AS QTY,                                                   
            I.PRODSPEC_ID AS PROD_ID,                                           
            '{current_time}' AS UPDATE_TIME,                              
            '' AS PARTCODE                   
     FROM APS_SYNC_WS_SCHEDULE_LOT_INFO.APS_SYNC_WS_SCHEDULE_LOT_INFO I                 
     -- WHERE I.PARTCODE ="+oldPartCode;
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_WAFER_START_60M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_ETL_WAFER_START"
    used_table_list = ['APS_SYNC_WS_SCHEDULE_LOT_INFO']
    target_table_sql = """
        create table {}APS_ETL_WAFER_START
        (
    ...
    try:
        ...
        ################################################################################################################
        # 获取WaferStart信息
        GetWaferStartSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_WAFER_START_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
