...
# 此規則只針對MA_AL(Cu)機群的機台：根據[V_OCS_AUTODUMMY_COMPLETE]selecttype in ('IDLE', 'PRDCHG')各機台的starttime~sys_date(單位：小時)，取中位数。
...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
          INSERT  /*+ append */  INTO APS_MID_IDLE_DUMMY_CU_MEDIAN(                             
        PARENTID, PROCESS_TIME, TOOL_ID, TOOLG_ID, UPDATE_TIME, PARTCODE                                                                     
       )                                                                                  
        WITH TOOL_DATA AS (                                                                
             SELECT EQP_ID,                                                                
                    MAX(EQP_G) AS TOOLG_ID                                                 
             FROM APS_SYNC_ETL_TOOL.APS_SYNC_ETL_TOOL T                              
             WHERE T.HOST_EQP_FLAG='Y' AND T.EQP_G='MA_AL(Cu)'         
             GROUP BY EQP_ID                                                               
        ),                                                                                 
        COMPLETE_DATA AS (                                                                      
             SELECT C.TOOLID,D.TOOLG_ID,                                                    
               ROUND((extract(epoch from(CAST(C.SYS_DATE AS TIMESTAMP) 
                     - (CAST(C.STARTTIME AS TIMESTAMP))))) / 3600.0, 3) AS PROCESS_TIME,               
             ROW_NUMBER() OVER (PARTITION BY TOOLID,TOOLG_ID  ORDER BY STARTTIME DESC) AS RN       
             FROM APS_SYNC_OCS_AUTODUMMY_COMPLETE.APS_SYNC_OCS_AUTODUMMY_COMPLETE C                
             INNER JOIN TOOL_DATA D                                                        
             ON D.EQP_ID = C.TOOLID                                                        
             WHERE C.SELECTTYPE IN ('IDLE','PRDCHG')         
             AND C.JOBSTATE='End'                                                                    
             AND C.SYS_DATE IS NOT NULL AND C.SYS_DATE <> '' AND C.STARTTIME IS NOT NULL AND C.STARTTIME <> ''      
             AND C.SYS_DATE >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '90 day'                                        
        ),                                                                                           
        SOURE_DATA AS (                                                                         
            SELECT DISTINCT  
                   QUANTILE_CONT(CAST(PROCESS_TIME AS DECIMAL),0.5) OVER(PARTITION BY TOOLID,TOOLG_ID ) AS PROCESS_TIME,                         
                   TOOLID,TOOLG_ID,RN                                                         
            FROM COMPLETE_DATA                                                             
        ),                                                                                 
        GROUP_DATA AS (                                                                         
            SELECT                                                                                   
                   ...
                   TOOLID,TOOLG_ID,MAX(RN) AS RN                                              
            FROM SOURE_DATA                                                                
            GROUP BY TOOLID,TOOLG_ID                                                       
        )                                                                                  
        SELECT '{uuid}',                                                   
               PROCESS_TIME,                                                               
               TOOLID,                                                                     
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_IDLE_DUMMY_CU_MEDIAN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_IDLE_DUMMY_CU_MEDIAN")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_TYPE11_MEDIAN_1D_APS_MID_TYPE3_CU_MEDIAN"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_IDLE_DUMMY_CU_MEDIAN"
    used_table_list = ['APS_SYNC_ETL_TOOL', 'APS_SYNC_OCS_AUTODUMMY_COMPLETE']
    target_table_sql = """
    create table {}APS_MID_IDLE_DUMMY_CU_MEDIAN
    (
        parentid     VARCHAR(64),
        toolg_id     VARCHAR(64),
        tool_id      VARCHAR(64),
    ...
    try:
        ...
        my_duck.export_result_duck_file_and_close_duck_db_memory2(duck_db_memory,
                                                                  ...
                                                                  current_time=current_time_short)
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, "XETL049")
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
