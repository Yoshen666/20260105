...
def create_temp_table(duck_db):
    ...
    sql = """
    create table APS_TMP_ETL_MASK_INFO_PRE_LOC
    (
       rtcl_id VARCHAR(64),
       loc_id  VARCHAR(64)
    )
    ...
    sql = """
    create table APS_TMP_ETL_MASK_INFO_ACTIVE
    (
    ...
    sql = """
    create table APS_TMP_ETL_MASK_INFO_V2
    (
      RTCL_ID VARCHAR(64),
      STATUS  VARCHAR(64)
    )
    ...
    sql = """
    create table APS_TMP_ETL_MASK_INFO_V1
    (
        parentid        VARCHAR(60),
...
def biz_method_02(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT /*+ append */
         INTO {tempdb}APS_TMP_ETL_MASK_INFO_V1
           (PARENTID,
            RETICLE_GROUP,
            RETICLE_ID,
            RETICLE_COUNT,
           ...
           SELECT '{uuid}' AS PARENTID,
                  ...
                  COALESCE((CASE
                        WHEN EXISTS (SELECT 1 FROM APS_SYNC_MCS_SCHED_LOCATION_INFO.APS_SYNC_MCS_SCHED_LOCATION_INFO LI
                                      WHERE LI.MASK_NOT_AVAILABLE = 'Y'
                                      AND MI.LOC_ID LIKE REPLACE(LI.TOOL_ID, '*', '_'))
                        THEN 'LOC-NOTAVAILABLE'
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_TMP_ETL_MASK_INFO_V1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_MASK_INFO_V1")
...
def getA3MaskData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_MASK_INFO_PRE_LOC(                              
        RTCL_ID,LOC_ID                                                                              
    )                                                                                             
    WITH SOURCE_DATA AS (
        SELECT TIMESTAMP,RTCL_ID,CASE WHEN LOC_ID='TMP_AUTO' THEN NULL ELSE LOC_ID END AS LOC_ID,
               ROW_NUMBER() OVER(PARTITION BY RTCL_ID ORDER BY TIMESTAMP DESC) AS RNT
        FROM aps_tmp_mask_history_view.aps_tmp_mask_history_view
    ),
    RESULT_DATA AS (
        SELECT  TIMESTAMP,LAG(LOC_ID IGNORE NULLS) OVER(PARTITION BY RTCL_ID ORDER BY TIMESTAMP) AS PRE_LOC_ID,* FROM SOURCE_DATA
    )
    SELECT  RTCL_ID,PRE_LOC_ID
    FROM RESULT_DATA
    WHERE RNT = 1                                                                                       
    """.format(current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_MASK_INFO_PRE_LOC",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_MASK_INFO_PRE_LOC")

    # 新对所有mask位置进行整合
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_MASK_INFO_ACTIVE(                              
        RETICLE_ID,POSITION                                                                              
    )                                                                                             
    SELECT RETICLE_ID,POSITION
    FROM {tempdb}APS_TMP_ETL_MASK_INFO_V1 V 
    WHERE V.reticle_status = 'ACTIVE' AND (V.position IS NULL OR V.position <> 'TMP_AUTO')
    UNION 
    SELECT V.RETICLE_ID,COALESCE(L.LOC_ID,V.POSITION) AS POSITION
    FROM {tempdb}APS_TMP_ETL_MASK_INFO_V1 V
    LEFT JOIN {tempdb}APS_TMP_ETL_MASK_INFO_PRE_LOC L
    ON V.RETICLE_ID = L.RTCL_ID 
    WHERE V.reticle_status = 'ACTIVE' AND V.position = 'TMP_AUTO'                                                                                   
        """.format(current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_MASK_INFO_ACTIVE",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_MASK_INFO_ACTIVE")
    # 找出active
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_MASK_INFO_V2(                              
        RTCL_ID,STATUS                                                                              
    )                                                                                             
    select DISTINCT V.reticle_id, 
           CASE WHEN SUBSTRING(I.LOCATION,2,1) <> '3' AND N.CORSS_FAB = 'N1A3' THEN 'A3-reserve' 
                WHEN SUBSTRING(I.LOCATION,2,1) = '3' AND (N.CORSS_FAB IS NULL OR N.CORSS_FAB NOT IN ('N1A1', 'N1A2')) THEN 'A3-hold'
                ELSE 'ACTIVE' END AS STATUS 
    from {tempdb}APS_TMP_ETL_MASK_INFO_ACTIVE V
    INNER JOIN APS_SYNC_MCS_SCHED_LOCATION_INFO.APS_SYNC_MCS_SCHED_LOCATION_INFO I
    ON V.position LIKE REPLACE(I.TOOL_ID,'*','_')
    LEFT JOIN APS_SYNC_MASK_CROSSFAB_INFO.APS_SYNC_MASK_CROSSFAB_INFO N
    ON N.MASK_ID = V.RETICLE_ID                                                                                     
    """.format(current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_MASK_INFO_V2",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_MASK_INFO_V2")

    # 合并
    sql = """
    INSERT INTO APS_ETL_MASK_INFO(                              
        PARENTID,RETICLE_GROUP,RETICLE_ID,RETICLE_COUNT,POSITION,
        POSITION_TIME,RETICLE_STATUS,INSPECTION_TIME,RUN_QTY,
        RUN_LBOUND,RUN_UBOUND,UPDATE_TIME,PARTCODE,LAST_POSITION                                                                             
    )                                                                                             
    select DISTINCT PARENTID,RETICLE_GROUP,V.RETICLE_ID,RETICLE_COUNT,POSITION,
            POSITION_TIME,
            COALESCE(N.STATUS, V.RETICLE_STATUS),
            INSPECTION_TIME,RUN_QTY,
            RUN_LBOUND,RUN_UBOUND,UPDATE_TIME,PARTCODE,LAST_POSITION 
    from {tempdb}APS_TMP_ETL_MASK_INFO_V1 V
    LEFT JOIN {tempdb}APS_TMP_ETL_MASK_INFO_V2 N
    ON N.RTCL_ID = V.RETICLE_ID                                                                                     
    """.format(current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_MASK_INFO_2",
                     sql=sql,
                     current_time=current_time,
...
def execute():
    ...
    try:
        ...
        # 需要把ACTIVE的状态数据，考量A3光罩的限制
        getA3MaskData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
