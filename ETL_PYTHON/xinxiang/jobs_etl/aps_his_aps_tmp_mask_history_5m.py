...
def execute():
    ...
    sync_day = 90
    query_sql = """SELECT TID, FUNC_ID, CASE WHEN MASK_SEQ_NO IS NULL THEN 0 ELSE MASK_SEQ_NO END AS MASK_SEQ_NO, RTCL_ID, MASK_GROUP, LOC_ID, STATUS, NEXT_LOC_ID, NEXT_STATUS, LOC_STATE, DOCUMENT_NO, PROPERTY, HR_CODE, SLOT_ID, REPLACE(REPLACE(COMM, chr(10), ''), chr(13), '')  AS COMM, "ACTION", USER_ID, "TIMESTAMP", SCRAP_CODE, PMOS_PROD_ID, POD_ID FROM V_ETL_MASK_HISTORY"""
    create_table_sql = """
    ...
    try:
        ...
        if last_file_name:
            ...
            if _create_time_date is not None:
                ...
                query_duck_sql = """
                    select * from {target_table} where 1=1 and {check_date_column} >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '3 day'
                ...
                select_insert_item = """
                            insert into {target_table_name} ({columns_str})             
                                ...
                                where 1=1
                                and  NOT EXISTS (
                                    SELECT HL.TID FROM last_one_hour_date_duck HL
                                where  
                                 1=1
                                 AND CASE WHEN HL.TID IS NULL  THEN '*' ELSE HL.TID END = CASE WHEN HS.TID IS NULL THEN '*' ELSE HS.TID END
