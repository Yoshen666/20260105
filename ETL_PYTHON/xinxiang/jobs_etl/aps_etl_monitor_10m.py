...
def create_temp_table(duck_db):
    ...
    sql = """
       create {table_type} table APS_TMP_ETL_MONITOR_RECIPE_SETUP
       (
            TOOL_ID VARCHAR(64), 
            PPID VARCHAR(64), 
...
def getRecipeSetupSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_MONITOR_RECIPE_SETUP(                                    
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_MONITOR_RECIPE_SETUP",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_MONITOR_RECIPE_SETUP")
...
def getSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        ...
        WITH GET_TOOLG AS(                                                                                                                                                                                                                                                                                   
             SELECT T.TOOL_ID,T.CH_ID,T.TOOLG_ID,MODULE_ID FROM APS_ETL_TOOL.APS_ETL_TOOL T                                                                                                                  
                 WHERE 
         ...
         MONITOR_PLAN_DATA AS (                                                                                                                                                     
           SELECT OP.MONTIOR_PLAN_NAME,                                                                                                                                             
             ...
             (SELECT  STRING_AGG(distinct COALESCE(CH_ID,MTOOL_ID),';' ORDER BY COALESCE(CH_ID,MTOOL_ID))
              FROM APS_SYNC_OCS_JOB_SPEC_GROUP_PO.APS_SYNC_OCS_JOB_SPEC_GROUP_PO OO
              LEFT JOIN GET_TOOLG T1 ON (INSTR(OO.MTOOL_ID || '.' || UCASE(OO.SUB_NAME), T1.CH_ID) = 1 )    
           ...
           ON (INSTR(OP.MTOOL_ID || '.' || UCASE(OP.SUB_NAME), T1.CH_ID) = 1 )                                                                                                                        
         ...
         SOURCE_DATA AS (                                                                                                                                                           
          SELECT   OP.JOB_SPEC_GROUP_ID,                                                                                                                                            
           ...
           LEFT JOIN GET_TOOLG T1 ON (INSTR(OP.MTOOL_ID || '.' || UCASE(OP.SUB_NAME), T1.CH_ID) = 1 )                                                                                                                             
           WHERE OP.MAINTOOLFLAG='true'                                                                                                                                             
           AND OP.PTOOL_ID = OP.MTOOL_ID                                                                                                                                            
           AND OP.MONTIOR_PLAN_NAME IS NULL                                                                                                                                         
           --AND OP.CREATE_TIME >= SYSDATE -3        
         ...
         SELECT                                                                                                                                                                     
              ...
              CASE WHEN CT.JOB_SPEC_GROUP_ID IS NOT NULL AND CT.JOB_SPEC_GROUP_ID <> '' AND CT.MEASURE_CT > 0                                                                                                      
                   ...
                   --THEN STRFTIME(DATE_TRUNC('second', CAST(M.CREATE_TIME AS TIMESTAMP) + INTERVAL (CAST(20 AS DECIMAL) - CAST(CT.MEASURE_CT AS DECIMAL)) HOUR),'%Y-%m-%d %H:%M:%S')
                   THEN date_add(CAST(M.CREATE_TIME AS TIMESTAMP),INTERVAL ((CAST(20 AS DECIMAL) - CAST(CT.MEASURE_CT AS DECIMAL))*3600) second)
...
def getWetPMSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
          ...
          WHERE P.PM_TYPE LIKE REPLACE(P.PM_TYPE,'*','%')                                                                                                                                  
           ...
           SELECT
                  ...
                  CASE WHEN I.EQP_ID IS NOT NULL THEN T.PM_TYPE ||'-'||STRFTIME( cast(I.START_TIME  as timestamp),'%Y%m%d')
                         WHEN p.EQP_ID IS NOT NULL THEN T.PM_TYPE ||'-'||STRFTIME( cast(P.BEGIN_TIME  as timestamp),'%Y%m%d')
                         ELSE T.PM_TYPE ||'-'||STRFTIME( cast(T.FORECAST_TIME  as date),'%Y%m%d') END AS JOB_ID,
                 CASE WHEN TRIM(I.EQP_ID) IS NOT NULL and TRIM(I.EQP_ID) <> '' THEN I.START_TIME  
                  ...
                  END AS START_DATE,
                 CASE WHEN I.EQP_ID IS NOT NULL AND I.EQP_ID <> '' THEN COALESCE(I.END_TIME,I.START_TIME)
    ...
    sql = """
           ...
           PM_DATA_F AS (
                ...
                ON E.EQP_ID = F.EQUIPMENT_NO
                WHERE 
                       STRFTIME(cast(F.FORECAST_TIME as DATE), '%Y-%m-%d') = STRFTIME(cast(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day' as DATE),'%Y-%m-%d')
                     AND CAST(replace(F.FORECAST_TIME,'/','-')||':00' AS TIMESTAMP) >  CAST(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day' AS TIMESTAMP)
           -- MFG要求过滤Track借机的数据(最新需求严格按照('*D PM')过滤)
                      AND ((SUBSTRING(F.EQUIPMENT_NO,1,1)='P' AND F.PM_TYPE LIKE ('%D PM'))  OR (SUBSTRING(F.EQUIPMENT_NO,1,1)<>'P'))
                      AND E.MODULE='WET'  
           ),
           ...
           PROP_TIME_DATA AS (
               SELECT EQP_ID, CASE WHEN LENGTH(P.BEGIN_TIMES) = 5 THEN    
               STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d') || ' ' || P.BEGIN_TIMES ||':00'   
              WHEN LENGTH(P.BEGIN_TIMES) = 4 AND position(':' in P.BEGIN_TIMES)=3 THEN                                            
                STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d') || ' ' || SUBSTRING(P.BEGIN_TIMES,1,3)||'0'||SUBSTRING(P.BEGIN_TIMES,4,1)||':00'  
             WHEN LENGTH(P.BEGIN_TIMES) = 4 AND position(':' IN P.BEGIN_TIMES)=2 THEN                                            
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d') || ' 0' || P.BEGIN_TIMES ||':00'                                            
             ELSE                                                                                                          
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d') || ' 00:00:00'                                                             
             END BEGIN_TIME,                                                                                               
             CASE WHEN LENGTH(P.END_TIMES) = 5 THEN                                                                        
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d') || ' ' || P.END_TIMES ||':00'                                               
             WHEN LENGTH(P.END_TIMES) = 4 AND position(':' IN P.END_TIMES)=3  THEN                                               
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d') || ' ' || SUBSTRING(P.END_TIMES,1,3)||'0'||SUBSTRING(P.END_TIMES,4,1)||':00'      
             WHEN LENGTH(P.END_TIMES) = 4 AND position(':' IN P.END_TIMES)=2  THEN                                         
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d') || ' 0' || P.END_TIMES ||':00'                                          
             ELSE                                                                                                    
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d') || ' 00:00:00'                                                         
             END END_TIME,PM_TYPE                                                                                                    
          FROM APS_SYNC_SCHE_PM_PROP_TIME.APS_SYNC_SCHE_PM_PROP_TIME P                                                       
          WHERE P.PM_TYPE LIKE REPLACE(P.PM_TYPE,'*','%')                                                                                                                                  
           ...
           BOOKING_INFO AS (
             ...
             WHERE S.E10_STATE='SDT' AND S.E10_CODE='4110'
             AND S.skip_flag ='N' 
                 AND STRFTIME( cast(S.START_TIME  as timestamp),'%Y-%m-%d') = STRFTIME(cast(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day' as timestamp),'%Y-%m-%d')
                 AND (S.END_TIME IS NULL OR S.END_TIME = '' OR S.END_TIME > DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day')
           )
           SELECT
                  '{uuid}'  AS PARENTID,
                  T.TOOLG_ID,
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_MONITOR_V2",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_MONITOR_V1")
    ...
    sql = """
           ...
           PM_DATA_F AS (
                ...
                ON E.EQP_ID = F.EQUIPMENT_NO
                WHERE 
                       STRFTIME(cast(F.FORECAST_TIME as DATE), '%Y-%m-%d') = STRFTIME(cast(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day' as DATE),'%Y-%m-%d')
                     AND CAST(replace(F.FORECAST_TIME,'/','-')||':00' AS TIMESTAMP) >  CAST(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day' AS TIMESTAMP)
           -- MFG要求过滤Track借机的数据(最新需求严格按照('*D PM')过滤)
                      AND ((SUBSTRING(F.EQUIPMENT_NO,1,1)='P' AND F.PM_TYPE LIKE ('%D PM'))  OR (SUBSTRING(F.EQUIPMENT_NO,1,1)<>'P'))
                      AND E.MODULE='WET'  
           ),
           ...
           PROP_TIME_DATA AS (
               SELECT EQP_ID, CASE WHEN LENGTH(P.BEGIN_TIMES) = 5 THEN    
               STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d') || ' ' || P.BEGIN_TIMES ||':00'   
              WHEN LENGTH(P.BEGIN_TIMES) = 4 AND position(':' in P.BEGIN_TIMES)=3 THEN                                            
                STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d') || ' ' || SUBSTRING(P.BEGIN_TIMES,1,3)||'0'||SUBSTRING(P.BEGIN_TIMES,4,1)||':00'  
             WHEN LENGTH(P.BEGIN_TIMES) = 4 AND position(':' IN P.BEGIN_TIMES)=2 THEN                                            
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d') || ' 0' || P.BEGIN_TIMES ||':00'                                            
             ELSE                                                                                                          
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d') || ' 00:00:00'                                                             
             END BEGIN_TIME,                                                                                               
             CASE WHEN LENGTH(P.END_TIMES) = 5 THEN                                                                        
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d') || ' ' || P.END_TIMES ||':00'                                               
             WHEN LENGTH(P.END_TIMES) = 4 AND position(':' IN P.END_TIMES)=3  THEN                                               
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d') || ' ' || SUBSTRING(P.END_TIMES,1,3)||'0'||SUBSTRING(P.END_TIMES,4,1)||':00'      
             WHEN LENGTH(P.END_TIMES) = 4 AND position(':' IN P.END_TIMES)=2  THEN                                         
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d') || ' 0' || P.END_TIMES ||':00'                                          
             ELSE                                                                                                    
                 STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d') || ' 00:00:00'                                                         
             END END_TIME,PM_TYPE                                                                                                    
          FROM APS_SYNC_SCHE_PM_PROP_TIME.APS_SYNC_SCHE_PM_PROP_TIME P                                                       
          WHERE P.PM_TYPE LIKE REPLACE(P.PM_TYPE,'*','%')                                                                                                                                  
           ...
           BOOKING_INFO AS (
             ...
             WHERE S.E10_STATE='SDT' AND S.E10_CODE='4110'
             AND S.skip_flag ='N' 
                 AND STRFTIME( cast(S.START_TIME  as timestamp),'%Y-%m-%d') = STRFTIME(cast(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day' as timestamp),'%Y-%m-%d')
                 AND (S.END_TIME IS NULL OR S.END_TIME = '' OR S.END_TIME > DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day')
           )
           SELECT
                  '{uuid}'  AS PARENTID,
                  T.TOOLG_ID,
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_MONITOR_V3",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_MONITOR_V1")
...
def getWetPMSql2(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    sql = """
          INSERT  /*+ append */  INTO APS_ETL_MONITOR(
               PARENTID, TOOLG_ID, TOOL_ID, CH_ID, JOB_ID, START_DATE, END_DATE,
               PROCESS_TIME, RETICLE_ID, RECIPE, JOB_STATUS,TRACK_IN, UPDATE_TIME,
               PARTCODE, FOUP_ID, JOB_TYPE, TOOL_CODE, FIXED_FLAG, FIXED_TIME,RECIPE_SETUP
           )
           WITH PM_DATA AS (
               SELECT PARENTID, V.TOOLG_ID, V.TOOL_ID, CH_ID, JOB_ID, 
                      CASE WHEN T.PROCESS_TIME IS NOT NULL AND START_DATE > cast(STRFTIME(cast(START_DATE as timestamp),'%Y-%m-%d') || ' 12:00:00' as timestamp) THEN NULL 
                           WHEN T.PROCESS_TIME IS NULL AND  START_DATE < cast(STRFTIME(cast(START_DATE as timestamp),'%Y-%m-%d') || ' 12:00:00' as timestamp) THEN NULL 
                               ELSE START_DATE END  START_DATE, 
                      CASE WHEN T.PROCESS_TIME IS NOT NULL AND END_DATE > cast(STRFTIME(cast(END_DATE as timestamp),'%Y-%m-%d') || ' 12:00:00' as timestamp) THEN  NULL 
                           WHEN T.PROCESS_TIME IS NULL AND END_DATE < cast(STRFTIME(cast(END_DATE as timestamp),'%Y-%m-%d') || ' 12:00:00' as timestamp) THEN  NULL
                      ...
                      ST.RTD_GROUPNAME AS RECIPE_SETUP
               ...
               LEFT JOIN {tempdb}APS_TMP_ETL_MONITOR_RECIPE_SETUP ST
               ON ST.TOOL_ID = V.TOOL_ID AND ST.PPID = V.RECIPE
          )
    ...
    sql = """
        ...
        -- MFG要求过滤Track借机的数据(最新需求严格按照('*D PM')过滤)
         ...
         SELECT                                                                                                                 
                ...
                CASE WHEN I.EQP_ID IS NOT NULL THEN STRFTIME(COALESCE(I.END_TIME,I.START_TIME), '%Y-%m-%d %H:%M:%S')                        
    ...
    sql = """
            INSERT  /*+ append */  INTO  APS_ETL_MONITOR (                                                                
             ...
             PM_DATA_F AS (                                                                                                         
                  ...
                  ON E.EQP_ID = F.EQUIPMENT_NO                                                                                      
                  WHERE 
                        STRFTIME(cast(F.FORECAST_TIME as date), '%Y-%m-%d') = STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d')     
                        AND CAST(replace(F.FORECAST_TIME,'/','-') || ':00' AS TIMESTAMP)  > CAST(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day' AS TIMESTAMP)                                             
            -- MFG要求过滤Track借机的数据(最新需求严格按照('*D PM')过滤)
                        AND ((SUBSTRING(F.EQUIPMENT_NO,1,1)='P' AND F.PM_TYPE LIKE ('%D PM'))  OR (SUBSTRING(F.EQUIPMENT_NO,1,1)<>'P'))                                                                                      
                        AND E.MODULE<>'WET'                                                                                             
             ...
             PROP_TIME_DATA1 AS (                                                                                                   
                SELECT P.EQP_G,P.MUDULE,P.EQP_ID,P.PM_TYPE,                                                                                   
                cast(STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day', '%Y-%m-%d') || ' ' || BEGIN_TIMES || ':00' as timestamp) AS BEGIN_TIME,                 
                DATE_TRUNC('second', cast(STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day', '%Y-%m-%d') || ' ' || END_TIMES || ':00' as timestamp) - INTERVAL (cast(P.INTERVAL_DAY as integer)*24) hour ) AS END_TIME                                                                               
                FROM APS_SYNC_SCHE_PM_PROP_TIME.APS_SYNC_SCHE_PM_PROP_TIME  P                                                                   
                WHERE 1=1                                                                                                                                                                                                            
                AND P.ITEM_TYPE='ACCEPT'                                                                                                      
             ),                                                                                                                     
             BOOKING_INFO AS (                                                                                                      
                ...
                WHERE S.E10_STATE='SDT' AND S.E10_CODE='4110' 
                AND S.skip_flag ='N'                                                                      
                AND STRFTIME(S.START_TIME,'%Y-%m-%d') = STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day','%Y-%m-%d')                                              
                AND (S.END_TIME IS NULL OR S.END_TIME > DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '1 day')                                                                                                                                            
             )                                                                                                                      
             SELECT                                                                                                                 
                     '{uuid}' AS PARENTID,                                                                              
                    T.TOOLG_ID,                                                                                                     
    ...
    sql = """
        INSERT  /*+ append */  INTO  APS_ETL_MONITOR (                                                                
         ...
         PM_DATA_F AS (                                                                                                         
              ...
              ON E.EQP_ID = F.EQUIPMENT_NO                                                                                      
              WHERE 
                    STRFTIME(cast(F.FORECAST_TIME as date), '%Y-%m-%d') = STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d')     
                    AND CAST(replace(F.FORECAST_TIME,'/','-') || ':00' AS TIMESTAMP)  > CAST(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day' AS TIMESTAMP)                                             
        -- MFG要求过滤Track借机的数据(最新需求严格按照('*D PM')过滤)
                    AND ((SUBSTRING(F.EQUIPMENT_NO,1,1)='P' AND F.PM_TYPE LIKE ('%D PM'))  OR (SUBSTRING(F.EQUIPMENT_NO,1,1)<>'P'))                                                                                      
                    AND E.MODULE<>'WET'                                                                                             
         ...
         PROP_TIME_DATA1 AS (                                                                                                   
            SELECT P.EQP_G,P.MUDULE,P.EQP_ID,P.PM_TYPE,                                                                                   
            cast(STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day', '%Y-%m-%d') || ' ' || BEGIN_TIMES || ':00' as timestamp) AS BEGIN_TIME,                 
            DATE_TRUNC('second', cast(STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day', '%Y-%m-%d') || ' ' || END_TIMES || ':00' as timestamp) - INTERVAL (cast(P.INTERVAL_DAY as integer)*24) hour ) AS END_TIME                                                                               
            FROM APS_SYNC_SCHE_PM_PROP_TIME.APS_SYNC_SCHE_PM_PROP_TIME  P                                                                   
            WHERE 1=1                                                                                                                                                                                                            
            AND P.ITEM_TYPE='ACCEPT'                                                                                                      
         ),                                                                                                                     
         BOOKING_INFO AS (                                                                                                      
            ...
            WHERE S.E10_STATE='SDT' AND S.E10_CODE='4110' 
            AND S.skip_flag ='N'                                                                      
            AND STRFTIME(S.START_TIME,'%Y-%m-%d') = STRFTIME(DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day','%Y-%m-%d')                                              
            AND (S.END_TIME IS NULL OR S.END_TIME > DATE_TRUNC('second', CURRENT_TIMESTAMP) + INTERVAL '2 day')                                                                                                                                            
         )                                                                                                                      
         SELECT                                                                                                                 
                 '{uuid}' AS PARENTID,                                                                              
                T.TOOLG_ID,                                                                                                     
    ...
    sql = """
       ...
       SELECT                                                                                                                       
             '{uuid}'  AS PARENTID,                                                                                 
              ...
              COALESCE(S.LOT_ID,S.PM_TYPE) AS JOB_ID,         
              ...
              '' AS RETICLE_ID,                                                                                                     
              '' AS RECIPE,                                                                                                         
              CASE WHEN S.skip_flag ='Y' AND s.force_flag='N' THEN 'Skip'
                   WHEN E.STATE_CODE='SDT' AND E.PREV_STATE='4110' THEN 'RUN' ELSE 'WAIT' END AS JOB_STATUS,                        
       ...
       AND (S.skip_flag ='Y' OR s.force_flag='Y' )                                                  
       ...
       FROM RESULT_DATA                                                                                 
       WHERE JOB_ID IS NOT NULL and  JOB_ID <> ''       GROUP BY PARENTID,TOOLG_ID,EQP_ID,CH_ID, JOB_ID,PARTCODE              
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
...
def getDfSourceMonitorData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        ...
        -- 针对过期时间：当天白天建的(7:30-19:30)，要在当班做完(7:30-次日7：30).如果是晚上建的(19:30-次日7：30)，则顺延一天（次日7：30- 第三日7：30）
         ...
         OR INSTR(OP.PTOOL_ID || '.' || UCASE(OP.SUB_NAME), T.CH_ID) = 1)
          """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
        INSERT INTO  {tempdb}APS_TMP_ETL_MONITOR_DF_PROCESS (
         ...
         SELECT T.TOOLG_ID,
                ...
                CASE WHEN CT.JOB_SPEC_GROUP_ID IS NOT NULL AND CT.JOB_SPEC_GROUP_ID <> '' AND CT.MEASURE_CT > 0 
                THEN date_add(CAST(T.END_DATE AS TIMESTAMP),INTERVAL (CAST(CT.MEASURE_CT AS DECIMAL)*3600*(-1)) second)                                                                    
    ...
    sql = """
          INSERT INTO APS_ETL_MONITOR (
           --modify by xiecheng for version up
               ...
               PROCESS_TIME, TRACK_IN, PROCESS_START, PARENTID, UPDATE_TIME, PARTCODE, RECIPE_SETUP
         )
          ...
          LEFT JOIN {tempdb}APS_TMP_ETL_MONITOR_RECIPE_SETUP ST
          ON ST.TOOL_ID = T.TOOL_ID AND ST.PPID = T.RECIPE
          """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
...
def execute():
    ...
    try:
        ...
        # 获取RECIPE_STEUP 资讯
        getRecipeSetupSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # 获取MONITOR QTY的资料信息
        ...
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            ...
            postgres_table_define = """etl_monitor(parentid,toolg_id,tool_id,ch_id,job_id, start_date,end_date,
                                                   process_time,reticle_id,recipe,job_status,track_in,update_time,
                                                    foup_id,job_type,tool_code,fixed_flag,fixed_time,process_start, recipe_setup)
                                         """
