...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO APS_ETL_HOLD_RLS(                             
         PARENTID, TOOLG_ID, LOT_ID, PROD_ID, LAYER, STAGE, HOLD_TIME,UPDATE_TIME, PARTCODE                                                                     
         )                                                                                   
          SELECT '{uuid}' AS PARENTID,                                          
                 CASE WHEN MAX(F.PRODSPEC_ID) IS NULL OR MAX(F.PRODSPEC_ID) = '' THEN '%' ELSE MAX(F.EQP_G) END AS TOOLG_ID,    
                 WD.LOT_ID,                                                                  
                 MAX(WD.PRODSPEC_ID) AS PROD_ID,                                             
                 SUBSTRING(WD.OPE_NO, 1, POSITION('.' IN WD.OPE_NO)-1) AS LAYER,                 
                 SUBSTRING(WD.OPE_NO, POSITION('.' IN WD.OPE_NO)+1) AS STAGE,                    
                 MAX(WD.RLS_HOUR) AS HOLD_TIME,                                              
                  '{current_time}' AS UPDATE_TIME,                                      
                  '' AS PARTCODE                          
          FROM APS_SYNC_UO_HOLDLOT_WAIT_DISPATCH.APS_SYNC_UO_HOLDLOT_WAIT_DISPATCH WD                   
          LEFT JOIN APS_SYNC_FLOW.APS_SYNC_FLOW F                                
          ON F.OPE_NO = WD.OPE_NO                                                            
          AND F.PRODSPEC_ID = WD.PRODSPEC_ID                                                                                                   
          GROUP BY WD.LOT_ID,WD.OPE_NO                                                                                                                                                                                                                  
    """.format(uuid=uuid, current_time=current_time)
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_HOLD_RLS",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_HOLD_RLS")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_HOLD_RLS_10M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_ETL_HOLD_RLS"
    used_table_list = ['APS_SYNC_UO_HOLDLOT_WAIT_DISPATCH',
                       'APS_SYNC_FLOW']
    target_table_sql = """
    create table {}APS_ETL_HOLD_RLS
    (
        ...
        PRIMARY KEY (PARENTID, TOOLG_ID, LOT_ID, PARTCODE)
    )
    ...
    try:
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_HOLD_RLS_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
