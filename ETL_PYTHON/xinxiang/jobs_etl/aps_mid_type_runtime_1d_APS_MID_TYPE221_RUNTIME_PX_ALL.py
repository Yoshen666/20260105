...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT /*+ append */ INTO APS_MID_TYPE221_RUNTIME_PX_ALL (              
        PARENTID,MODULE_ID, TOOLG_ID, TOOL_ID, CH_CNT, LESSTHAN20_FLAG,                                                                                                                                                                                                                                                            
     ...
     WITH TYPE21_LR_FIRST1 AS (                                                                                                                                                         
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT, O.QTY, O.TRACKER_PT                                                                                      
                   , ROUND((REGR_SLOPE(CAST(O.TRACKER_PT AS DOUBLE), CAST(O.QTY AS DOUBLE)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) * CAST(O.QTY AS DOUBLE) +
                            REGR_INTERCEPT(CAST(O.TRACKER_PT AS DOUBLE), CAST(O.QTY AS DOUBLE)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT)),6) AS PT_Y_TRACKER
                   , COUNT(1) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) AS CNT_OLD                                                                                  
                   , CASE WHEN COUNT(1) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) < 20                                                                              
                          THEN 'Y'                                                                                                                                                      
                          ELSE 'N'                                                                                                                                                      
                     END AS LESSTHAN20_FLAG                                                                                                                                             
                   , CASE WHEN MAX(O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) = MIN(O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT)      
                          THEN 'Y'                                                                                                                                                      
                          ELSE 'N'                                                                                                                                                      
                     END AS SAMEQTY_FLAG_ZWS                                                                                                                                            
                   ,ROUND(CAST(O.TRACKER_PT AS DOUBLE) / CAST(O.QTY AS DOUBLE), 5) AS ONE_WAFER_PT_TRACKER
             FROM (SELECT A.MODULE_ID, A.TOOLG_ID, A.TOOL_ID, A.CH_CNT,  cast(A.QTY as double)/2 as qty,                                                                                                               
      ...
      TYPE21_LR_FIRST_ZWS AS (                                                                                                                                                          
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT, O.SAMEQTY_FLAG_ZWS, O.LESSTHAN20_FLAG                                                                                   
                    -- ,AVG(O.ONE_WAFER_PT_TRACKER) AS ONE_WAFER_PT_TRACKER
                    ,AVG(CAST(O.ONE_WAFER_PT_TRACKER AS DOUBLE)) AS ONE_WAFER_PT_TRACKER
             FROM (                                                                                                                                                                     
                       SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT, O.ONE_WAFER_PT_TRACKER, O.SAMEQTY_FLAG_ZWS, O.LESSTHAN20_FLAG
                       FROM (                                                                                                                                                           
                               SELECT A.*                                                                                                                                               
                                    ,MEDIAN(RN) OVER(PARTITION BY A.MODULE_ID, A.TOOLG_ID, A.TOOL_ID, A.CH_CNT) MED                                                                       
                               FROM (                                                                                                                                                   
                                       SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT, O.SAMEQTY_FLAG_ZWS, O.LESSTHAN20_FLAG
                                             ,O.ONE_WAFER_PT_TRACKER                                                                                                                    
                                             ,ROW_NUMBER() OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT ORDER BY O.ONE_WAFER_PT_TRACKER) RN
                                       FROM TYPE21_LR_FIRST1 O                                                                                                                          
                      ) A                                                                                                                                                               
                            ) O                                                                                                                                                         
                       WHERE ABS(RN - MED) <= 0.5                                                                                                                                       
                   ) O                                                                                                                                                                  
            GROUP BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT, O.SAMEQTY_FLAG_ZWS, O.LESSTHAN20_FLAG                                                                                  
       ),                                                                                                                                                                               
       TYPE21_LR_FIRST_LESSTHAN20 AS (                                                                                                                                                  
             SELECT Z.MODULE_ID, Z.TOOLG_ID, Z.TOOL_ID, Z.CH_CNT, Z.LESSTHAN20_FLAG, Z.SAMEQTY_FLAG_ZWS, 'N' AS PA_PB_0_FLAG_ZWS,
       ...
       TYPE21_LR_FIRST_SAMEQTY AS (                                                                                                                                                     
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT, O.LESSTHAN20_FLAG, O.SAMEQTY_FLAG_ZWS , 'N' AS PA_PB_0_FLAG_ZWS,
             ...
             AND NOT EXISTS (SELECT 1                                                                                                                                                   
                   ...
                   AND O.CH_CNT = L20.CH_CNT)                                                                                                                                               
       ),                                                                                                                                                                               
       TYPE21_LR_FIRST2 AS (                                                                                                                                                            
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT, O.QTY, O.TRACKER_PT, O.PT_Y_TRACKER, O.CNT_OLD, O.LESSTHAN20_FLAG,
                    ...
                    -- SQRT((SUM(POWER(O.PT_Y_TRACKER - O.TRACKER_PT,2)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT)) / O.CNT_OLD)), 2) AS SE
                    SQRT((SUM(POWER(CAST(O.PT_Y_TRACKER AS DOUBLE) - CAST(O.TRACKER_PT AS DOUBLE),2)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT)) / CAST(O.CNT_OLD AS DOUBLE))), 2) AS SE                                                                                                                                                               
             FROM TYPE21_LR_FIRST1 O                                                                                                                                                    
             INNER JOIN TYPE21_LR_FIRST_ZWS Z ON O.MODULE_ID = Z.MODULE_ID AND O.TOOLG_ID = Z.TOOLG_ID AND O.TOOL_ID = Z.TOOL_ID AND O.CH_CNT = Z.CH_CNT
             WHERE NOT EXISTS (                                                                                                                                                         
                SELECT 1 FROM TYPE21_LR_FIRST_LESSTHAN20 L20 WHERE O.MODULE_ID = L20.MODULE_ID AND O.TOOLG_ID = L20.TOOLG_ID AND O.TOOL_ID = L20.TOOL_ID AND O.CH_CNT = L20.CH_CNT
             )                                                                                                                                                                          
             AND NOT EXISTS (                                                                                                                                                           
                SELECT 1 FROM TYPE21_LR_FIRST_SAMEQTY SQ WHERE O.MODULE_ID = SQ.MODULE_ID AND O.TOOLG_ID = SQ.TOOLG_ID AND O.TOOL_ID = SQ.TOOL_ID AND O.CH_CNT = SQ.CH_CNT
             )                                                                                                                                                                          
       ),                                                                                                                                                                               
       TYPE21_LR_FIRST3 AS(                                                                                                                                                             
               SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT, O.QTY, O.TRACKER_PT, O.PT_Y_TRACKER,                                                                                  
       ...
       TYPE21_LR_SECOND1 AS (                                                                                                                                                           
               SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT, O.QTY, O.TRACKER_PT, O.ONE_WAFER_PT_TRACKER                                                                           
               , CASE WHEN COUNT(1) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) < 20                                                                                  
                      THEN 'Y'                                                                                                                                                          
                      ELSE 'N'                                                                                                                                                          
                 END AS LESSTHAN20_FLAG                                                                                                                                                 
               , CASE WHEN MAX(O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) = MIN(O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT)
                      ...
                      ELSE 'N'                                                                                                                                                          
                 END AS SAMEQTY_FLAG_ZWS                                                                                                                                                
               -- , REGR_SLOPE(O.TRACKER_PT, O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) AS TRACKER_SLOPE_NEW
               , REGR_SLOPE(CAST(O.TRACKER_PT AS DOUBLE), CAST(O.QTY AS DOUBLE)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) AS TRACKER_SLOPE_NEW
               -- , REGR_INTERCEPT(O.TRACKER_PT, O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) AS TRACKER_INTER_NEW
               , REGR_INTERCEPT(CAST(O.TRACKER_PT AS DOUBLE), CAST(O.QTY AS DOUBLE)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.CH_CNT) AS TRACKER_INTER_NEW
       ...
       TYPE21_LR_SECOND_SAMEQTY AS (                                                                                                                                                    
             SELECT T.MODULE_ID, T.TOOLG_ID, T.TOOL_ID, T.CH_CNT, T.LESSTHAN20_FLAG, T.SAMEQTY_FLAG_ZWS , 'N' AS PA_PB_0_FLAG_ZWS,
             ...
             WHERE NOT EXISTS (SELECT 1                                                                                                                                                 
                   ...
                   AND T.CH_CNT = L20.CH_CNT)                                                                                                                                               
             AND T.SAMEQTY_FLAG_ZWS = 'Y'                                                                                                                                               
       ),                                                                                                                                                                               
       TYPE21_LR_SECOND2 AS (                                                                                                                                                           
              SELECT  N.MODULE_ID, N.TOOLG_ID, N.TOOL_ID, N.CH_CNT, N.QTY, N.TRACKER_PT, N.ONE_WAFER_PT_TRACKER                                                                           
               ...
               WHERE NOT EXISTS (                                                                                                                                                       
                 SELECT 1 FROM TYPE21_LR_SECOND_LESSTHAN20 L20 WHERE N.MODULE_ID = L20.MODULE_ID AND N.TOOLG_ID = L20.TOOLG_ID AND N.TOOL_ID = L20.TOOL_ID AND N.CH_CNT = L20.CH_CNT
               )                                                                                                                                                                        
               AND NOT EXISTS (                                                                                                                                                         
                 SELECT 1 FROM TYPE21_LR_SECOND_SAMEQTY SQ WHERE N.MODULE_ID = SQ.MODULE_ID AND N.TOOLG_ID = SQ.TOOLG_ID AND N.TOOL_ID = SQ.TOOL_ID AND N.CH_CNT = SQ.CH_CNT
               )                                                                                                                                                                        
       ),                                                                                                                                                                               
       TYPE21_LR_SECOND_PA_PB_0 AS (                                                                                                                                                    
               SELECT N.MODULE_ID, N.TOOLG_ID, N.TOOL_ID, N.CH_CNT, N.LESSTHAN20_FLAG, N.SAMEQTY_FLAG_ZWS, N.PA_PB_0_FLAG_ZWS,
       ...
       TYPE21_LR_SECOND3 AS (                                                                                                                                                           
            SELECT N.MODULE_ID, N.TOOLG_ID, N.TOOL_ID, N.CH_CNT, N.LESSTHAN20_FLAG,N.PA_PB_0_FLAG_ZWS, N.SAMEQTY_FLAG_ZWS,                                                                
            ...
            WHERE NOT EXISTS (SELECT 1                                                                                                                                                  
                   ...
                   AND N.CH_CNT = PAB.CH_CNT )                                                                                                                                              
       ),                                                                                                                                                                               
       TYPE21_LR_SECOND_RESULT AS (                                                                                                                                                     
             SELECT N3.MODULE_ID, N3.TOOLG_ID, N3.TOOL_ID, N3.CH_CNT, N3.LESSTHAN20_FLAG,                                                                                                 
             ...
             UNION                                                                                                                                                                      
             SELECT PAB.MODULE_ID, PAB.TOOLG_ID, PAB.TOOL_ID, PAB.CH_CNT, PAB.LESSTHAN20_FLAG,                                                                                            
                    PAB.SAMEQTY_FLAG_ZWS , PAB.PA_PB_0_FLAG_ZWS, PAB.ONE_WAFER_PT_TRACKER, PAB.TRACKER_SLOPE_NEW, PAB.TRACKER_INTER_NEW
             FROM TYPE21_LR_SECOND_PA_PB_0 PAB                                                                                                                                          
             UNION                                                                                                                                                                      
             SELECT OSQ.MODULE_ID, OSQ.TOOLG_ID, OSQ.TOOL_ID, OSQ.CH_CNT, OSQ.LESSTHAN20_FLAG,                                                                                            
             ...
             UNION                                                                                                                                                                      
             SELECT OL20.MODULE_ID, OL20.TOOLG_ID, OL20.TOOL_ID, OL20.CH_CNT, OL20.LESSTHAN20_FLAG,                                                                                       
             ...
             UNION                                                                                                                                                                      
             SELECT NSQ.MODULE_ID, NSQ.TOOLG_ID, NSQ.TOOL_ID, NSQ.CH_CNT, NSQ.LESSTHAN20_FLAG,                                                                                            
             ...
             UNION                                                                                                                                                                      
             SELECT NL20.MODULE_ID, NL20.TOOLG_ID, NL20.TOOL_ID, NL20.CH_CNT, NL20.LESSTHAN20_FLAG,                                                                                       
             ...
             GROUP BY MODULE_ID, TOOLG_ID, TOOL_ID, CH_CNT, LESSTHAN20_FLAG, SAMEQTY_FLAG_ZWS,                                                                                            
                      PA_PB_0_FLAG_ZWS, ONE_WAFER_PT_TRACKER, TRACKER_SLOPE_NEW, TRACKER_INTER_NEW                                                                                      
       )                                                                                                                                                                                                                                                                                                                                                              
       SELECT '{uuid}' AS PARENTID, 
              t.MODULE_ID, t.TOOLG_ID, t.TOOL_ID, 
              CASE WHEN CAST(T.CH_CNT AS DECIMAL(18,0)) = 0 THEN 1 ELSE T.CH_CNT END AS CH_CNT, 
              t.LESSTHAN20_FLAG,                                                                                                                                                                                                                                                            
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_TYPE221_RUNTIME_PX_ALL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_TYPE221_RUNTIME_PX_ALL")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_MID_TYPE_RUNTIME_1D_APS_MID_TYPE221_RUNTIME_PX_ALL"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_TYPE221_RUNTIME_PX_ALL"
    used_table_list = ['APS_TR_TYPE221']
    target_table_sql = """
    create table {}APS_MID_TYPE221_RUNTIME_PX_ALL
    (
        parentid varchar(64),
