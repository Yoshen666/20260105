...
from xinxiang.util import my_duck, my_oracle, my_date, cons_error_code, my_postgres, my_file, my_runner
...
def create_temp_table(duck_db):
    ...
    duck_db.sql(sql)
    sql = """
       create {table_type} table APS_TMP_ETL_FLOW_SUB_SET_BACKUPEQP
       (
           prod_id    VARCHAR(64),
           ope_no     VARCHAR(64),
           toolg_id   VARCHAR(64),
           ...
           backup_eqp VARCHAR(512),
           sche_toolg_id VARCHAR(64),
           OLD_TOOLG_ID  VARCHAR(64)
       )
       """.format(table_type=table_type)
    duck_db.sql(sql)
    sql = """
       create {table_type} table APS_TMP_ETL_FLOW_SUB_SET_EQPG1
       (
           prod_id    VARCHAR(64),
           ope_no     VARCHAR(64),
           toolg_id   VARCHAR(64),
    ...
    duck_db.sql(sql)
    sql = """
       create {table_type} table APS_TMP_ETL_FLOW_SUB_SET_EQPG2
       (
           prod_id    VARCHAR(64),
           ope_no     VARCHAR(64),
           toolg_id   VARCHAR(64),
    ...
    duck_db.sql(sql)
    sql = """
       create {table_type} table APS_TMP_ETL_FLOW_SUB_SET_EQPG3
       (
           prod_id    VARCHAR(64),
           ope_no     VARCHAR(64),
           toolg_id   VARCHAR(64),
    ...
    sql = """
           create {table_type} table APS_ETL_FLOW_PHOTO_TARC_FLAG
           (
                  prod_id       VARCHAR(64),
                  plan_id       VARCHAR(64),
                  ope_no       VARCHAR(64),
                  photo_tarc_flag  VARCHAR(64)
           )
           """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
          create {table_type} table APS_ETL_FLOW_NON_SGS_FLAG
          (
                 prod_id       VARCHAR(64),
                 plan_id       VARCHAR(64),
                 ope_no       VARCHAR(64),
                 non_sgs_flag  VARCHAR(64)
          )
          """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
         create {table_type} table APS_TMP_ETL_FLOW_V1
         (
            prod_id       VARCHAR(64),
            plan_no       VARCHAR(64),
            plan_id       VARCHAR(64),
    ...
    sql = """
         create {table_type} table APS_TMP_ETL_FLOW_V2
         (
            prod_id       VARCHAR(64),
            plan_no       VARCHAR(64),
            plan_id       VARCHAR(64),
    ...
    sql = """
         create {table_type} table APS_TMP_ETL_FLOW_V3
         (
            prod_id       VARCHAR(64),
            plan_no       VARCHAR(64),
            plan_id       VARCHAR(64),
    ...
    sql = """
        create {table_type} table APS_TMP_ETL_FLOW_V4
        (
           prod_id       VARCHAR(64),
           plan_no       VARCHAR(64),
           plan_id       VARCHAR(64),
    ...
    sql = """
        create {table_type} table APS_TMP_ETL_FLOW_V5
        (
           prod_id       VARCHAR(64),
           plan_no       VARCHAR(64),
           plan_id       VARCHAR(64),
    ...
    sql = """
        create {table_type} table APS_TMP_ETL_FLOW_V6
        (
           prod_id       VARCHAR(64),
           plan_no       VARCHAR(64),
           plan_id       VARCHAR(64),
...
def GetRecipeEqpLoadSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_FLOW_EQP_LOAD(                        
         EQP_ID, LCRECIPE_ID, PD_ID, PRODSPEC_ID, EQP_G, EQP_CATEGORY,SCHE_EQP_G                                                          
        ) 
        WITH SOURCE_DATA AS (
            SELECT L.EQP_ID,L.LCRECIPE_ID,                       
                    ...
                    T.EQP_CATEGORY,
                    T.SCHE_EQP_G,
                    ROW_NUMBER() OVER(PARTITION BY L.PRODSPEC_ID,T.EQP_G,L.PD_ID ORDER BY L.EQP_ID) AS RNT                                    
...
def GetFlowByBackUpEqpSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    INSERT INTO {tempdb}APS_TMP_ETL_FLOW_BACKUPEQP(                                             
                EQP_G, SCHE_EQP_G, PRODUCTID, OPENO, ROUTEID, RECIPE                                                                   
        )                                                                                                
         WITH BACK_EQPG AS (                                                                            
           SELECT  (CASE WHEN (SUBSTRING(SUBSTRING(N.OPE_NO, POSITION('.' IN N.OPE_NO)),3,1)='Q' AND            
             ...
             (CASE WHEN (SUBSTRING(SUBSTRING(N.OPE_NO, POSITION('.' IN N.OPE_NO)),3,1)='Q' AND            
             L.EQP_G IN ('PK_KrF','PH_ArF','PU_I-Line')) THEN 'None' ELSE L.SCHE_EQP_G END) AS SCHE_EQP_G,         
             L.PRODSPEC_ID,                                                                             
             N.OPE_NO,                                                                                  
             N.MAINPD_ID,                                                                               
             L.LCRECIPE_ID                                                                              
         ...
         RESULT_DATA AS (                                                                                              
             SELECT EQP_G,    
                    ...
                    MAINPD_ID,                                                                              
                    LCRECIPE_ID,
                    ROW_NUMBER() OVER(PARTITION BY EQP_G,PRODSPEC_ID,OPE_NO,MAINPD_ID ORDER BY EQP_G) AS RNT                                                       
             FROM BACK_EQPG CT                                                                              
         )  
         SELECT EQP_G, SCHE_EQP_G, PRODSPEC_ID, OPE_NO, MAINPD_ID, LCRECIPE_ID AS RECIPE
         FROM RESULT_DATA
         WHERE RNT = 1                                                                         
    """.format(tempdb=my_duck.get_temp_table_mark())
...
def GetSubAndRewFlowSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_FLOW_REAL_RWK_SUB(                              
             PROD_ID, PLAN_NO, PLAN_ID, STEP_ID,OPE_NO,EQP_ID,RECIPE,STEP_TYPE,TOOLG_ID,SCHE_TOOLG_ID, TOOLG_TYPE                                                                     
             ) 
         WITH SOURCE_DATA AS (                                                                                   
              SELECT BB.PRODSPEC_ID AS PROD_ID,                                                                                                            
                        '1' AS PLAN_NO,                                                                                                                      
                        AA.MAINPD_ID AS PLAN_ID,                                                                                                               
                        printf('.%09.4f',cast(CAST(AA.OPE_SEQ AS DECIMAL) as float)) AS STEP_ID,                                                                       
                        AA.OPE_NO,                                                                                                                            
                        BB.EQP_ID,                                                                                                               
                        BB.LCRECIPE_ID AS RECIPE,                                                                                                       
                        CASE WHEN SUBSTRING(AA.MAINPD_ID,1,1) = 'B' THEN 'I' ELSE 'R' END AS STEP_TYPE,  
                        BB.EQP_G AS TOOLG_ID, 
                        BB.SCHE_EQP_G AS SCHE_TOOLG_ID,                                                    
                        BB.EQP_CATEGORY AS TOOLG_TYPE,
                        ROW_NUMBER() OVER(PARTITION BY AA.MAINPD_ID,AA.OPE_NO,BB.PRODSPEC_ID,BB.EQP_G  ORDER BY BB.EQP_ID)  AS RNT                                                                                                               
          ...
          SELECT PROD_ID, PLAN_NO, PLAN_ID, STEP_ID,OPE_NO,EQP_ID,RECIPE,STEP_TYPE,TOOLG_ID,SCHE_TOOLG_ID, TOOLG_TYPE 
          FROM SOURCE_DATA
          WHERE RNT = 1                                                                                                                                             
        """.format(tempdb=my_duck.get_temp_table_mark())
...
def GetSubAndRewFlowBackUpEqpSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_FLOW_SUB_BACKUPEQP(                                   
             PROD_ID, OPE_NO, TOOLG_ID, SCHE_TOOLG_ID, PLAN_ID, BACKUP_EQP                                                                               
            )                                                                                             
             WITH SUB_REWORK_DATA AS (                                                                    
                 SELECT DISTINCT PROD_ID,OPE_NO,TOOLG_ID,SCHE_TOOLG_ID, PLAN_ID                                                                                       
                 FROM {tempdb}APS_TMP_ETL_FLOW_REAL_RWK_SUB                                              
             ),                                                                                           
             BACKUP_EQP_DATA AS (                                                                         
                  SELECT PROD_ID,OPE_NO,TOOLG_ID,SCHE_TOOLG_ID,PLAN_ID,     
...
def GetSubAndRewFlowBackUpEqpBySettingSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    #先把匹配到所以数据拉出来
    sql = """
            INSERT INTO {tempdb}APS_TMP_ETL_FLOW_SUB_SET_EQPG1(                                   
                 PROD_ID, OPE_NO, TOOLG_ID, SCHE_TOOLG_ID, PLAN_ID, BACKUP_EQP, EQP_G                                                                               
                )                                                                                             
                SELECT  B.PROD_ID,  B.OPE_NO, B.TOOLG_ID, B.SCHE_TOOLG_ID, B.PLAN_ID, B.BACKUP_EQP,E.EQP_G
                FROM {tempdb}APS_TMP_ETL_FLOW_SUB_BACKUPEQP B
                INNER JOIN APS_SYNC_UMT_SUB_FLOW_EQPG.APS_SYNC_UMT_SUB_FLOW_EQPG E
                ON INSTR(B.PROD_ID, CASE WHEN E.PRODSPEC_ID='*' THEN B.PROD_ID  ELSE REPLACE(E.PRODSPEC_ID,'*','') END)>0 
                AND INSTR(B.OPE_NO, CASE WHEN E.OPE_NO='*' THEN B.OPE_NO  ELSE REPLACE(E.OPE_NO,'*','') END)>0                                                                                  
        """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_SUB_SET_EQPG1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_SUB_SET_EQPG1")

    sql = """
            INSERT INTO {tempdb}APS_TMP_ETL_FLOW_SUB_SET_EQPG2(                                   
                 PROD_ID, OPE_NO, TOOLG_ID, SCHE_TOOLG_ID, PLAN_ID, BACKUP_EQP                                                                               
                )                                                                                             
                SELECT  DISTINCT B.PROD_ID,  B.OPE_NO, B.TOOLG_ID, B.SCHE_TOOLG_ID, B.PLAN_ID, B.BACKUP_EQP
                FROM {tempdb}APS_TMP_ETL_FLOW_SUB_BACKUPEQP B
                INNER JOIN APS_SYNC_UMT_SUB_FLOW_EQPG.APS_SYNC_UMT_SUB_FLOW_EQPG E
                ON INSTR(B.PROD_ID, CASE WHEN E.PRODSPEC_ID='*' THEN B.PROD_ID  ELSE REPLACE(E.PRODSPEC_ID,'*','') END)>0 
                AND INSTR(B.OPE_NO, CASE WHEN E.OPE_NO='*' THEN B.OPE_NO  ELSE REPLACE(E.OPE_NO,'*','') END)>0 
                AND E.EQP_G = B.TOOLG_ID                                                                                 
        """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_SUB_SET_EQPG2",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_SUB_SET_EQPG2")

    sql = """
       INSERT INTO {tempdb}APS_TMP_ETL_FLOW_SUB_SET_EQPG3(                                   
            PROD_ID, OPE_NO, TOOLG_ID, SCHE_TOOLG_ID, PLAN_ID, BACKUP_EQP,OLD_TOOLG_ID                                                                               
           )                                                                                             
           SELECT  DISTINCT B.PROD_ID,  B.OPE_NO, B.EQP_G AS TOOLG_ID, B.EQP_G AS SCHE_TOOLG_ID, B.PLAN_ID, 
                   CASE WHEN SUBSTRING(BACKUP_EQP,-1,1) =',' THEN BACKUP_EQP||TOOLG_ID 
                        WHEN BACKUP_EQP IS NULL THEN TOOLG_ID
                        ELSE BACKUP_EQP||','||TOOLG_ID END AS BACKUP_EQP,
                        B.TOOLG_ID AS OLD_TOOLG_ID
           FROM {tempdb}APS_TMP_ETL_FLOW_SUB_SET_EQPG1 B
           WHERE NOT EXISTS (
                SELECT 1
                FROM {tempdb}APS_TMP_ETL_FLOW_SUB_SET_EQPG2 E
                WHERE B.PROD_ID = E.PROD_ID
                AND B.PLAN_ID = E.PLAN_ID
                AND B.OPE_NO = E.OPE_NO
           )                                                                                
       """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_SUB_SET_EQPG3",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_SUB_SET_EQPG3")

    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_FLOW_SUB_SET_BACKUPEQP(                                   
             PROD_ID, OPE_NO, TOOLG_ID, SCHE_TOOLG_ID, PLAN_ID, BACKUP_EQP, OLD_TOOLG_ID                                                                               
            )                                                                                             
            SELECT  B.PROD_ID,  B.OPE_NO, B.TOOLG_ID, B.SCHE_TOOLG_ID, B.PLAN_ID, B.BACKUP_EQP, B.TOOLG_ID AS OLD_TOOLG_ID
            FROM {tempdb}APS_TMP_ETL_FLOW_SUB_SET_EQPG2 B
            UNION ALL 
            SELECT B.PROD_ID,  B.OPE_NO, B.TOOLG_ID, B.SCHE_TOOLG_ID, B.PLAN_ID, B.BACKUP_EQP, OLD_TOOLG_ID
            FROM {tempdb}APS_TMP_ETL_FLOW_SUB_SET_EQPG3 B
            UNION ALL 
            SELECT  B.PROD_ID,  B.OPE_NO, B.TOOLG_ID, B.SCHE_TOOLG_ID, B.PLAN_ID, B.BACKUP_EQP, B.TOOLG_ID AS OLD_TOOLG_ID
            FROM {tempdb}APS_TMP_ETL_FLOW_SUB_BACKUPEQP B
            WHERE NOT EXISTS (
                SELECT 1
                FROM  APS_SYNC_UMT_SUB_FLOW_EQPG.APS_SYNC_UMT_SUB_FLOW_EQPG E
                WHERE INSTR(B.PROD_ID, CASE WHEN E.PRODSPEC_ID='*' THEN B.PROD_ID  ELSE REPLACE(E.PRODSPEC_ID,'*','') END)>0 
                AND INSTR(B.OPE_NO, CASE WHEN E.OPE_NO='*' THEN B.OPE_NO  ELSE REPLACE(E.OPE_NO,'*','') END)>0 
            )                                                                                 
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_SUB_SET_BACKUPEQP",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_SUB_SET_BACKUPEQP")
...
def getMainFlowRSData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         INSERT  /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_MAIN_RSDATA(                                           
           RECIPE, PRODSPEC_ID, MNPD_ID, OPE_NO, EQP_G, SCHE_EQP_G, OPE_SEQ, CAPABILITY, SGS_FLAG, TOOLG_TYPE, SGS_GROUP, CT, PT                                                                              
          )                                                                                            
          WITH FLOW AS (                                                                                                                                     
               ...
               (CASE WHEN (SUBSTRING(SUBSTRING(CC.OPE_NO, position('.' in CC.OPE_NO)),3,1)='Q' AND                                                                   
               CC.EQP_G IN ('PK_KrF','PH_ArF','PU_I-Line')) THEN 'None' ELSE CC.SCHE_EQP_G END) AS SCHE_EQP_G,                                                          
               (CASE WHEN (SUBSTRING(SUBSTRING(CC.OPE_NO, position('.' in CC.OPE_NO)),3,1)='Q' AND                                                                   
            ...
            flow_ss as (
               SELECT   AA.LCRECIPE_ID AS RECIPE,CC.PRODSPEC_ID,CC.MNPD_ID,                                                                                 
             			CC.OPE_NO AS OPE_NO,CC.EQP_G, 
             			CC.SCHE_EQP_G, CC.OPE_SEQ,   
                    	CC.SGS_FLAG, CC.TOOLG_TYPE,                                                                                 
             		 	CC.SGS_GROUP,                                                                                                                
              			CC.INLINE_CT AS CT,                                                                                                                        
             			CC.INLINE_PROC_TIME AS PT,  
             			ROW_NUMBER() OVER(PARTITION BY CC.PRODSPEC_ID,CC.OPE_NO,CC.MNPD_ID,CC.OPE_SEQ ORDER BY OPE_SEQ) AS RNT          
               FROM FLOW CC                                                                                                                                    
            ...
            RESULE_DATA AS (
                SELECT  RECIPE,PRODSPEC_ID,MNPD_ID,                                                                                 
             			CC.OPE_NO,CC.EQP_G, 
             			CC.SCHE_EQP_G, CC.OPE_SEQ,   
                    	CC.SGS_FLAG, CC.TOOLG_TYPE,                                                                                 
             		 	CC.SGS_GROUP,                                                                                                                
             ...
             ss.OPE_NO, ss.EQP_G, SS.SCHE_EQP_G, ss.OPE_SEQ,                                                                                        
...
def getMainFlowRSByMaskData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        ...
        PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP, 
        STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG, SGS_GROUP, CT, PT, OPE_SEQ                                                                             
              ...
              COALESCE(FL.EQP_G,'None') AS TOOLG_ID, COALESCE(FL.SCHE_EQP_G,'None') AS SCHE_TOOLG_ID, 
              FL.RECIPE AS RECIPE, MI.MASK_GROUP AS RETICLE_GROUP,                                                        
    ...
    sql = """      
        INSERT  /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_MAIN_TOOL_RSDATA(                                            
             ...
             STEP_TYPE, TOOLG_TYPE,OPE_NO, CAPABILITY, SGS_FLAG, SGS_GROUP, CT, PT, OPE_SEQ                                                                              
                   ...
                   FL.LAYER,                                                                                      
                   FL.STAGE,                                                                                         
                   FL.TOOLG_ID, FL.SCHE_TOOLG_ID, FL.RECIPE, FL.RETICLE_GROUP,                                                         
...
def getRewDataByFlow(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_REWORK_RSDATA(                                            
             PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID,
             RECIPE, RETICLE_GROUP, STEP_TYPE, TOOLG_TYPE,
...
def getRewBackUpEqpData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
        INSERT  /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_REWORK_BACKUPEQP_RSDATA(                                            
            PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP, STEP_TYPE, TOOLG_TYPE,
           ...
           INNER JOIN {tempdb}APS_TMP_ETL_FLOW_SUB_SET_BACKUPEQP BA                                                                                         
           ON BA.PROD_ID = F.PROD_ID AND BA.OPE_NO = F.OPE_NO                                                                                           
           AND BA.OLD_TOOLG_ID = F.TOOLG_ID AND BA.PLAN_ID = F.PLAN_ID                                                                                                                                                                                                   
           """.format(tempdb=my_duck.get_temp_table_mark())
...
def GetFirstMainFlowSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """      
         ...
         SELECT FD.PROD_ID, FD.PLAN_NO, FD.PLAN_ID,                                                                                                          
              ...
              FD.LAYER,                                                                                                                                      
              FD.STAGE,                                                                                                                                      
              FD.TOOLG_ID, FD.SCHE_TOOLG_ID, FD.RECIPE, FD.RETICLE_GROUP,                                                                                                      
              ...
              CASE WHEN ZWS.PROD_ID IS NOT NULL AND CAST(ZWS.PROCESS_TIME AS DECIMAL) > 0 THEN ZWS.PROCESS_TIME
                   WHEN ZWST.OPE_NO IS NOT NULL AND CAST(ZWST.PROCESS_TIME AS DECIMAL) > 0 THEN ZWST.PROCESS_TIME
                   WHEN ZWSO.OPE_NO IS NOT NULL AND CAST(ZWSO.PROCESS_TIME AS DECIMAL) > 0 THEN ZWSO.PROCESS_TIME
                   WHEN ZWS2.TOOLG_ID IS NOT NULL AND CAST(ZWS2.PROCESS_TIME AS DECIMAL) > 0 THEN ZWS2.PROCESS_TIME
                   ELSE 0 END AS PROCESS_TIME,                                                                                                                                  
              CASE WHEN ZWS.PROD_ID IS NOT NULL AND CAST(ZWS.CYCLE_TIME AS DECIMAL) > 0 THEN ZWS.CYCLE_TIME
                   WHEN ZWST.OPE_NO IS NOT NULL AND CAST(ZWST.CYCLE_TIME AS DECIMAL) > 0 THEN ZWST.CYCLE_TIME
                   WHEN ZWSO.OPE_NO IS NOT NULL AND CAST(ZWSO.CYCLE_TIME AS DECIMAL) > 0 THEN ZWSO.CYCLE_TIME
                   WHEN ZWS2.TOOLG_ID IS NOT NULL AND CAST(ZWS2.CYCLE_TIME AS DECIMAL) > 0 THEN ZWS2.CYCLE_TIME
    ...
    sql = """
            INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_CHANGESEQ(                
                  ...
                  SELECT PROD_ID, PLAN_NO, PLAN_ID,                                                 
                         ...
                         LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID,  RECIPE,                                           
...
def GetAllChangeData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    # ALL_DATA
    sql = """
        INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_V1(                
                PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,
                STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG, PRODG_ID, PRODG_TECH, PROCESS_TIME, CYCLE_TIME,
                SGS_GROUP, OPE_SEQ, LAST_STEP, FIRST_STEP                                                                        
             )                                                                                  
        SELECT PROD_ID, PLAN_NO, PLAN_ID,
              COALESCE(STEP_ID,
                (
               ...
               LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,                                                                           
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_V1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_V1")

    # REPEAT_CHECK_DATA
    sql = """
        INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_V2(                
                PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,
                STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG, PRODG_ID, PRODG_TECH, PROCESS_TIME, CYCLE_TIME,
                SGS_GROUP, OPE_SEQ, LAST_STEP, FIRST_STEP                                                                        
             )                                                                                  
        ...
        FROM {tempdb}APS_TMP_ETL_FLOW_V1                                                                         
          """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_V2",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_V2")

    # ROW_NUMBER_DATA
    sql = """
        INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_V3(                
                PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,
                STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG, PRODG_ID, PRODG_TECH, PROCESS_TIME, CYCLE_TIME,
                SGS_GROUP, OPE_SEQ, N_NUM, LAST_STEP, FIRST_STEP                                                                        
             )                                                                                  
        SELECT PROD_ID, PLAN_NO, PLAN_ID,                                                                                                
                       STEP_ID,                                                                                                                  
                       LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,                                                                           
                       ...
                       LAST_STEP,FIRST_STEP                                                                                                      
        FROM {tempdb}APS_TMP_ETL_FLOW_V2                                                                          
          """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_V3",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_V3")

    # NULL_ROW_LESS
    sql = """
            INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_V4(                
                    PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,
                    STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG, PRODG_ID, PRODG_TECH, PROCESS_TIME, CYCLE_TIME,
                    SGS_GROUP, OPE_SEQ                                                                       
                 )                                                                                  
            SELECT PROD_ID, PLAN_NO, PLAN_ID,                                                                                                     
                               ...
                               STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG,                                                                      
                               PRODG_ID, PRODG_TECH, PROCESS_TIME, CYCLE_TIME, SGS_GROUP, OPE_SEQ                                                        
            FROM  {tempdb}APS_TMP_ETL_FLOW_V3                                                                                                          
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_V4",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_V4")

    # MIDDLE_ROW_THAN
    sql = """
        INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_V5(                
                PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,
                STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG, PRODG_ID, PRODG_TECH, PROCESS_TIME, CYCLE_TIME,
                SGS_GROUP, OPE_SEQ, LAST_STEP, FIRST_STEP, ROW_NUM, ROW_NUM1, N_COUNT                                                                       
             )                                                                                  
        SELECT A.PROD_ID, A.PLAN_NO, A.PLAN_ID,                                                                                         
            ...
            A.STEP_ID,                                                                                               
               ...
               A.N_NUM AS N_COUNT                                                                                                               
        FROM  {tempdb}APS_TMP_ETL_FLOW_V3  A                                                                                                                                                                                                         
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_V5",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_V5")

    # ROW_THEN_CHANGE
    sql = """
        INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_FLOW_V6(                
                PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,
                STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG, PRODG_ID, PRODG_TECH, PROCESS_TIME, CYCLE_TIME,
                SGS_GROUP, OPE_SEQ                                                                      
             )                                                                                  
               ...
               CASE WHEN (FIRST_STEP IS NULL OR FIRST_STEP = '' AND (LAST_STEP IS NOT NULL AND LAST_STEP <> '')) 
                    ...
                    ELSE 
                       -- ('.'|| LPAD(CAST( (CAST(FIRST_STEP as decimal) + ((cast(LAST_STEP as decimal) - cast(FIRST_STEP as decimal))/(cast(N_COUNT as decimal)+1)*cast(ROW_NUM as decimal)))  AS DECIMAL(4, 0)),4,'0') || '.0000'  )                                            
                        printf('.%09.4f',cast((CAST(FIRST_STEP as decimal(18,10)) + ((cast(LAST_STEP as decimal(18,10)) - cast(FIRST_STEP as decimal(18,10)))/(cast(N_COUNT as decimal)+1)*cast(ROW_NUM as decimal)))  as float))
                        END AS STEP_ID,
               LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,                                                                           
               STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG,                                                                 
               PRODG_ID, PRODG_TECH, PROCESS_TIME, CYCLE_TIME, SGS_GROUP, OPE_SEQ                                                        
        FROM  {tempdb}APS_TMP_ETL_FLOW_V5                                                                                                                              
        """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_FLOW_V6",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_V6")
    ...
    sql = """
        ...
        SELECT PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER,                                                                                    
               STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,                                                                                      
               STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY,                                                                                    
               SGS_FLAG, PRODG_ID, PRODG_TECH, PROCESS_TIME,                                                                                 
               CYCLE_TIME, SGS_GROUP                                                                                                         
         FROM {tempdb}APS_TMP_ETL_FLOW_V6                                                                                                                
         UNION ALL                                                                                                                           
         SELECT PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER,                                                                                   
               STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,                                                                                      
               STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY,                                                                                    
               SGS_FLAG, PRODG_ID, PRODG_TECH, PROCESS_TIME,                                                                                 
               CYCLE_TIME, SGS_GROUP                                                                                                         
        FROM {tempdb}APS_TMP_ETL_FLOW_V4                                                                               
          """.format(tempdb=my_duck.get_temp_table_mark())
...
def TruncateTable(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         TRUNCATE TABLE {tempdb}APS_TMP_ETL_FLOW                                       
        """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="TRUNCATE TABLE APS_TMP_ETL_FLOW",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW")

    sql = """
            TRUNCATE TABLE {tempdb}APS_TMP_ETL_FLOW_ANNOTATION_BRANCH                                       
           """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="TRUNCATE TABLE APS_TMP_ETL_FLOW_ANNOTATION_BRANCH",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_FLOW_ANNOTATION_BRANCH")

    sql = """
            TRUNCATE TABLE {tempdb}APS_ETL_FLOW_PHOTO_TARC_FLAG                                       
           """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="TRUNCATE TABLE APS_ETL_FLOW_PHOTO_TARC_FLAG",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_FLOW_PHOTO_TARC_FLAG")

    sql = """
            TRUNCATE TABLE {tempdb}APS_ETL_FLOW_NON_SGS_FLAG                                       
           """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="TRUNCATE TABLE APS_ETL_FLOW_NON_SGS_FLAG",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_FLOW_NON_SGS_FLAG")
    ...
    sql = """
         ...
         T2 AS (                                                                                                                                             
          ...
          SELECT  DISTINCT T0.PROD_ID, T0.PLAN_NO, T0.PLAN_ID,                                                                                                   
                  ...
                  T0.LAYER, T0.STAGE, T0.TOOLG_ID, T0.SCHE_TOOLG_ID,                                                                                                        
...
def JoinMedianSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
               INSERT INTO {tempdb}APS_ETL_FLOW_PHOTO_TARC_FLAG(
                  PROD_ID, PLAN_ID, OPE_NO, PHOTO_TARC_FLAG
                ) 
                WITH FLOW_DATA AS (                                               
                    SELECT F.PRODSPEC_ID,
                           F.MNPD_ID,
                           F.OPE_NO,  
                           F.PHOTO_TARC_FLAG,
                           ROW_NUMBER() OVER(PARTITION BY PRODSPEC_ID,MNPD_ID,OPE_NO ORDER BY PHOTO_TARC_FLAG DESC)  AS RNT
                    FROM APS_SYNC_FLOW.APS_SYNC_FLOW F  
                    WHERE F.PHOTO_TARC_FLAG IS NOT NULL AND F.PHOTO_TARC_FLAG <> ''
                )  
                SELECT PRODSPEC_ID,MNPD_ID,OPE_NO,PHOTO_TARC_FLAG
                FROM FLOW_DATA
                WHERE RNT = 1                                                                                            
                    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_FLOW_PHOTO_TARC_FLAG",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_FLOW_PHOTO_TARC_FLAG")

    sql = """
               INSERT INTO {tempdb}APS_ETL_FLOW_NON_SGS_FLAG(
                  PROD_ID, PLAN_ID, OPE_NO, NON_SGS_FLAG
                ) 
                WITH FLOW_DATA AS (                                               
                    SELECT F.PRODSPEC_ID,
                           F.MNPD_ID,
                           F.OPE_NO,  
                           F.NON_SGS_FLAG,
                           ROW_NUMBER() OVER(PARTITION BY PRODSPEC_ID,MNPD_ID,OPE_NO ORDER BY NON_SGS_FLAG DESC)  AS RNT
                    FROM APS_SYNC_FLOW.APS_SYNC_FLOW F  
                    WHERE F.NON_SGS_FLAG IS NOT NULL AND F.NON_SGS_FLAG <> ''
                )  
                SELECT PRODSPEC_ID,MNPD_ID,OPE_NO,NON_SGS_FLAG
                FROM FLOW_DATA
                WHERE RNT = 1                                                                                            
                    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_FLOW_NON_SGS_FLAG",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_FLOW_NON_SGS_FLAG")
    ...
    sql = """
           INSERT INTO APS_ETL_FLOW(
              PARENTID, PROD_ID, PLAN_NO, PLAN_ID, STEP_ID, LAYER, STAGE, TOOLG_ID, SCHE_TOOLG_ID, RECIPE, RETICLE_GROUP,
              STEP_TYPE, TOOLG_TYPE, OPE_NO, CAPABILITY, SGS_FLAG, PRODG_ID, PRODG_TECH, PROCESS_TIME,
              CYCLE_TIME, SGS_GROUP, MULTI_GROUP, MULTI_TARGET_FLAG,SKIP_FLAG, UPDATE_TIME, PARTCODE,
              MAIN_PLAN_ID,ENTRY_STEP_ID,RETN_STEP_ID,PHOTO_TARC_FLAG, NON_SGS_FLAG)                                                
            SELECT '{uuid}' AS PARENTID,                                                   
            M.PROD_ID, CAST(M.PLAN_NO AS INTEGER), M.PLAN_ID, M.STEP_ID, M.LAYER, M.STAGE, M.TOOLG_ID, M.SCHE_TOOLG_ID,                      
            ...
            LEFT JOIN {tempdb}APS_ETL_FLOW_PHOTO_TARC_FLAG TF
            ON M.OPE_NO = TF.OPE_NO AND M.PLAN_ID = TF.PLAN_ID AND M.PROD_ID = TF.PROD_ID
            LEFT JOIN {tempdb}APS_ETL_FLOW_NON_SGS_FLAG NSF
            ON M.OPE_NO = NSF.OPE_NO AND M.PLAN_ID = NSF.PLAN_ID AND M.PROD_ID = NSF.PROD_ID                                                                                                
                """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
...
def execute():
    ...
    try:
        ...
        # 获取SUB/REWORK的备援机台信息 获取设定的机群
        GetSubAndRewFlowBackUpEqpBySettingSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        # 获取RECIPE出来
        ...
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            select_sql_in_duck = """select  parentid, prodg_id, prodg_tech,prod_id,plan_no,plan_id, step_id,layer,stage,toolg_id,sche_toolg_id,recipe,reticle_group,step_type,toolg_type,process_time,
                                            ...
                                            case when PHOTO_TARC_FLAG = '' then null else PHOTO_TARC_FLAG end as PHOTO_TARC_FLAG,
                                            case when NON_SGS_FLAG ='' THEN NULL ELSE NON_SGS_FLAG END AS NON_SGS_FLAG
                                            from APS_ETL_FLOW
                                        """
            postgres_table_define = """etl_flow(parentid, prodg_id, prodg_tech,prod_id,plan_no,plan_id, step_id,layer,stage,toolg_id,sche_toolg_id,recipe,reticle_group,step_type,toolg_type,process_time,
                                                cycle_time,capability,sgs_flag,update_time,sgs_group,batch_name, batch_target_flag,multi_group,multi_target_flag, SKIP_FLAG,main_plan_id,
                                                entry_step_id,retn_step_id,PHOTO_TARC_FLAG,NON_SGS_FLAG)
                                    """
            my_oracle.StartCleanUpAndLog(oracle_conn, ETL_Proc_Name, current_time)
        ...
        # 如果出現主鍵衝突則需要重編step_id，如果還有報錯則直接報錯
        if "PRIMARY KEY or UNIQUE" in str(e):
            logging.info("-----------APS_ETL_FLOW主鍵衝突進入重編的代碼執行---------------")
            try:
                # Truncate 掉已賦值的數據
                TruncateTable(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
                # 獲取MainFlow的信息
                GetFirstMainFlowSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
                # 獲取RewFlow及SubFlow的信息
                GetFirstRwkToSubFlowSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
                # 記錄注記站點資訊（主要是BranchFlow）
                GetAnnotationBranchFlowData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
                # 關聯中位數信息並保存
                JoinMedianSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
                _eeeee = datetime.now()
                ...
                # 寫版本號
                my_oracle.Monitor_HandlingVerControl(oracle_conn, uuid, target_table, target_db_file,
                                                     current_time_short)
                ...
                if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
                    ...
                    postgres_table_define = """etl_flow(parentid, prodg_id, prodg_tech,prod_id,plan_no,plan_id, step_id,layer,stage,toolg_id,sche_toolg_id,recipe,reticle_group,step_type,toolg_type,process_time,
                                                                cycle_time,capability,sgs_flag,update_time,sgs_group,batch_name, batch_target_flag,multi_group,multi_target_flag, SKIP_FLAG,main_plan_id,entry_step_id,retn_step_id,
                                                                PHOTO_TARC_FLAG,NON_SGS_FLAG)
                                                    """
                    my_oracle.StartCleanUpAndLog(oracle_conn, ETL_Proc_Name, current_time)
                    my_postgres.copy_duckdb_to_postgres(uuid=uuid,
                                                        duckdb=duck_db_memory,
                                                        table_name_in_duckdb=target_table,
                                                        table_name_in_pg="etl_flow",  # 要小寫
                                                        select_sql_in_duck=select_sql_in_duck,
                                                        postgres_table_define=postgres_table_define,
                                                        oracle_conn=oracle_conn,
                                                        ETL_Proc_Name=ETL_Proc_Name)
                ...
                my_oracle.HandlingVerControl(oracle_conn, uuid, target_table, target_db_file, current_time_short)
                # 加入PG執行結束的時間更新
                my_oracle.Update_PG_HandlingVerControl(oracle_conn, uuid, target_table, target_db_file,
                                                       current_time_short)
                # 寫完成日志
                my_oracle.EndCleanUpAndLog(oracle_conn, ETL_Proc_Name, current_time)
                _eeeee = datetime.now()
