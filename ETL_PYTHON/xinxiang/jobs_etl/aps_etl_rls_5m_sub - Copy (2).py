import logging
import os
import sys
import time
import traceback

import duckdb

# sys.path.insert(0, sys.path[0])
# sys.path.insert(0, os.path.join(sys.path[0], ".."))
# sys.path.insert(0, os.path.join(sys.path[0], "..", '..'))
path = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.join(path, '..'))
sys.path.insert(0, os.path.join(path, '..', '..'))

from xinxiang import config
from xinxiang.util import my_oracle, cons_error_code, my_date, my_duck, my_log


def getRcpInhibit(
                    temp_process_db_file,
                    file_APS_TMP_ETL_RLS_EQPRCP_RESULT,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7):
    oracle_conn = my_oracle.oracle_get_connection()

    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_RLS_5M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()
    duck_db_temp = None
    try:
        time.sleep(1)
        # duck_db_temp = duckdb.connect(database=':memory:')
        time.sleep(1)
        target_table = "APS_ETL_RLS"
        # _temp_db_path = os.path.join(config.g_mem_speed_etl_output_path, target_table, "inprocess")
        # if not os.path.exists(_temp_db_path):
        #     os.makedirs(_temp_db_path)
        # # temp_db_file = os.path.join(_temp_db_path, target_table + "_" + current_time_short + "_sub_temp.db")
        duck_db_temp = duckdb.connect(temp_process_db_file)
        duck_db_temp.sql('SET threads TO 6')
        if not os.path.exists(os.path.join(config.g_mem_speed_etl_output_path, 'duck_temp', uuid)):
            os.makedirs(os.path.join(config.g_mem_speed_etl_output_path, 'duck_temp', uuid))
        duck_db_temp.execute(
            "SET temp_directory='{}'".format(os.path.join(config.g_mem_speed_etl_output_path, 'duck_temp', uuid)))

        # duck_db_temp.execute(
        #     "attach '{temp_process_db_file}' as dddd".format(temp_process_db_file=temp_process_db_file))
        sql = """ 
                          create table APS_TMP_ETL_RLS_APC_PILOT_RCP1
                          (
                               PROD_ID VARCHAR(64),
                               M_RECIPE VARCHAR(64),
                               EQP_ID VARCHAR(64),
                               SUB_LOT_TYPE VARCHAR(64),
                               SET_STATUS2 VARCHAR(64)
                          )
                              """
        duck_db_temp.sql(sql)

        sql = """
                create table APS_TMP_ETL_RLS_EQPRCP_RESULT as select * from '{path}'
                """.format(path=file_APS_TMP_ETL_RLS_EQPRCP_RESULT)
        duck_db_temp.sql(sql)
        sql = """
            create table APS_TMP_ETL_RLS_EQPRCP_INHIBIT1 as select * from '{path}'
            """.format(path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1)
        duck_db_temp.sql(sql)
        sql = """
            create table APS_TMP_ETL_RLS_EQPRCP_INHIBIT2 as select * from '{path}'
            """.format(path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2)
        duck_db_temp.sql(sql)
        sql = """
            create table APS_TMP_ETL_RLS_EQPRCP_INHIBIT4 as select * from '{path}'
            """.format(path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4)
        duck_db_temp.sql(sql)
        sql = """
            create table APS_TMP_ETL_RLS_EQPRCP_INHIBIT5 as select * from '{path}'
            """.format(path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5)
        duck_db_temp.sql(sql)
        sql = """
            create table APS_TMP_ETL_RLS_EQPRCP_INHIBIT6 as select * from '{path}'
            """.format(path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6)
        duck_db_temp.sql(sql)
        sql = """
            create table APS_TMP_ETL_RLS_EQPRCP_INHIBIT7 as select * from '{path}'
            """.format(path=file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7)
        duck_db_temp.sql(sql)

        sql = """
               INSERT INTO APS_TMP_ETL_RLS_APC_PILOT_RCP1 (                                                                                                                                  
                    PROD_ID,M_RECIPE,EQP_ID,SUB_LOT_TYPE,SET_STATUS2
                )    
                WITH  GROUP_APC_PILOT AS (
                    SELECT DISTINCT W.PROD_ID,W.M_RECIPE,W.EQP_ID,W.SUB_LOT_TYPE 
                    FROM APS_TMP_ETL_RLS_EQPRCP_RESULT W
                )                                                                                                                                                                             
                 SELECT  W.PROD_ID,W.M_RECIPE,W.EQP_ID,W.SUB_LOT_TYPE                                                                                                   
                      ,CASE WHEN CONCAT_WS('',MAX(ERI1.EQP_ID),MAX(ERI2.EQP_ID),MAX(ERI4.EQP_ID),MAX(ERI5.EQP_ID),MAX(ERI6.EQP_ID),MAX(ERI7.EQP_ID))<>''  THEN 'EqpRcpInhibit;' END AS SET_STATUS2                                                                                                                                                                                                                                                              
                 FROM GROUP_APC_PILOT W                                                                                                                                                       
                 LEFT JOIN APS_TMP_ETL_RLS_EQPRCP_INHIBIT1 ERI1 ON  ERI1.PRODUCT_ID = W.PROD_ID 
                                                             AND ERI1.EQP_ID = W.EQP_ID 
                                                             AND INSTR(W.M_RECIPE,ERI1.RECIPE_ID)>0                                             
                                                             --AND position(ERI1.RECIPE_ID in W.M_RECIPE)>0                                                                                                                                  
                 LEFT JOIN APS_TMP_ETL_RLS_EQPRCP_INHIBIT2 ERI2 ON ERI2.EQP_ID = W.EQP_ID   
                                                             AND INSTR(W.M_RECIPE,ERI2.RECIPE_ID)>0                                                 
                                                            -- AND position(ERI2.RECIPE_ID in W.M_RECIPE)>0                                      
                 LEFT JOIN APS_TMP_ETL_RLS_EQPRCP_INHIBIT4 ERI4 ON ERI4.SUB_LOT_TYPE = W.SUB_LOT_TYPE                                        
                                                             AND ERI4.EQP_ID = W.EQP_ID                                                  
                 LEFT JOIN APS_TMP_ETL_RLS_EQPRCP_INHIBIT5 ERI5 ON ERI5.SUB_LOT_TYPE = W.SUB_LOT_TYPE                                        
                                                             AND position(ERI5.RECIPE_ID in W.M_RECIPE)>0                                      
                 LEFT JOIN APS_TMP_ETL_RLS_EQPRCP_INHIBIT6 ERI6 ON ERI6.EQP_ID = W.EQP_ID  
                                                             AND ERI6.SUB_LOT_TYPE = W.SUB_LOT_TYPE                                                   
                                                             AND position(ERI6.RECIPE_ID in W.M_RECIPE)>0                                                                          
                 LEFT JOIN APS_TMP_ETL_RLS_EQPRCP_INHIBIT7 ERI7 ON  ERI7.PRODUCT_ID = (case when ERI7.PRODUCT_ID = '*' then '*' else W.PROD_ID end)                  
                                                             AND ERI7.EQP_ID = (case when ERI7.EQP_ID = '*' then '*' else W.EQP_ID end)                    
                                                             AND ERI7.SUB_LOT_TYPE =(case when ERI7.SUB_LOT_TYPE = '*' then '*' else W.SUB_LOT_TYPE end)   
                                                             AND position(ERI7.RECIPE_ID in (case when ERI7.RECIPE_ID = '*' then '*' else W.M_RECIPE end))>0 
                 GROUP BY W.PROD_ID,W.M_RECIPE,W.EQP_ID,W.SUB_LOT_TYPE 
                """.format()
        my_duck.exec_sql(oracle_conn=oracle_conn,
                         duck_db_memory=duck_db_temp,
                         ETL_Proc_Name="APS_ETL_BR.APS_ETL_RLS_5M",
                         methodName="Insert Into APS_TMP_ETL_RLS_APC_PILOT_RCP1",
                         sql=sql,
                         current_time=current_time,
                         update_table="APS_TMP_ETL_RLS_APC_PILOT_RCP1")
    except Exception as eee:
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, eee, "",
                                   cons_error_code.APS_ETL_RLS_CODE_XX_ETL)
        traceback.print_exc()
        logging.exception(traceback.print_exc())
        oracle_conn.close()
    finally:
        oracle_conn.close()
        duck_db_temp.close()


if __name__ == '__main__':
    print("start")
    my_log.init_log(os.path.join(config.g_log_path, "NextChipETL.APS_ETL_SUB.log"))  # 初期化Logger
    temp_process_db_file = sys.argv[1]
    file_APS_TMP_ETL_RLS_EQPRCP_RESULT = sys.argv[2]
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1 = sys.argv[3]
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2 = sys.argv[4]
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4 = sys.argv[5]
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5 = sys.argv[6]
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6 = sys.argv[7]
    file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7 = sys.argv[8]
    getRcpInhibit(temp_process_db_file,
                  file_APS_TMP_ETL_RLS_EQPRCP_RESULT,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT1,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT2,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT4,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT5,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT6,
                  file_APS_TMP_ETL_RLS_EQPRCP_INHIBIT7)
