...
def GetTypeFirstSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO APS_TMP_MASK_HISTORY_VIEW(                              
     ...
     WHERE HS.CLAIM_TIME >= DATE_TRUNC('day', CURRENT_TIMESTAMP) - INTERVAL '180 day'     
    """.format(current_time=current_time)
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_MASK_HISTORY_VIEW",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_MASK_HISTORY_VIEW")


def GetTypeOtherSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name, last_db_file):
    last_db_file = my_file.get_last_db_file(oracle_conn, "APS_TMP_MASK_HISTORY_VIEW")
    my_duck.attach_table(duck_db_memory, 'LAST_APS_TMP_MASK_HISTORY_VIEW', last_db_file)
    insert_sql = """
    INSERT  /*+ append */  INTO APS_TMP_MASK_HISTORY_VIEW(                              
        PARENTID,LOT_ID,OPE_NO,OPE_SEQ,PASS_COUNT,LINE,CLAIM_TIME,MAINPD_ID, 
        EQP_ID,MOVE_FLAG,REWORK_MOVE_FLAG,PURE_MOVE_FLAG,DUMMY_MOVE_FLAG, 
     ...
     SELECT gen_random_uuid() AS GUID                  
           ...
           ,CLAIM_TIME AS PARTKEY               
     FROM APS_TMP_LOTHISTORY_VIEW.APS_TMP_LOTHISTORY_VIEW HS                  
     WHERE HS.CLAIM_TIME >= date_add(CURRENT_TIMESTAMP, INTERVAL '-1 hour')                                               
     AND NOT EXISTS (                                                                                     
                SELECT HL.LOT_ID  FROM APS_TMP_LOTHISTORY_VIEW.APS_TMP_LOTHISTORY_VIEW HL                                             
                WHERE HL.PARTKEY > date_add(CURRENT_TIMESTAMP, INTERVAL '-1 hour')                                                
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_LOTHISTORY_VIEW",
                     sql=insert_sql,
                     current_time=current_time,
                     update_table="APS_TMP_LOTHISTORY_VIEW")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_LOTHISTORY_5M_APS_TMP_LOTHISTORY"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_TMP_LOTHISTORY"
    ...
    try:
        ...
        last_db_file = my_file.get_last_db_file(oracle_conn, "APS_TMP_LOTHISTORY")
