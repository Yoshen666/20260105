...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT /*+ append */ INTO APS_MID_TYPE21_RUNTIME_ALL (              
         PARENTID,MODULE_ID, TOOLG_ID, TOOL_ID, PPID, LESSTHAN20_FLAG,TRACKER_ZWS_FLAG,
     ...
     SELECT '{uuid}' AS PARENTID,                                                                                                                                                                         
             S.MODULE_ID, S.TOOLG_ID, S.TOOL_ID, S.PPID, S.LESSTHAN20_FLAG,                                                                                                                                                                                                                                                             
             '' AS TRACKER_ZWS_FLAG, 
             ...
             '' AS PARTCODE                                                                                                                                                                         
      FROM TYPE21_LR_FIRST_ZWS S          
    """.format(uuid=uuid, current_time=current_time)

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_MID_TYPE21_RUNTIME_ALL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_TYPE21_RUNTIME_ALL")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_MID_TYPE_RUNTIME_1D_APS_MID_TYPE21_RUNTIME_ALL"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_TYPE21_RUNTIME_ALL"
    used_table_list = ['APS_TR_TYPE21']
    target_table_sql = """
    create table {}APS_MID_TYPE21_RUNTIME_ALL
    (
      parentid             VARCHAR(64),
