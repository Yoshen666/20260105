...
def GetTypeFirstSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    INSERT  /*+ append */  INTO APS_TMP_MASK_HISTORY_VIEW (                              
           GUID, TID,FUNC_ID,MASK_SEQ_NO,RTCL_ID,MASK_GROUP,LOC_ID,
           STATUS,NEXT_LOC_ID,NEXT_STATUS,LOC_STATE,DOCUMENT_NO,
           PROPERTY,HR_CODE,SLOT_ID,COMM,ACTION,USER_ID,
           TIMESTAMP,SCRAP_CODE,PMOS_PROD_ID,POD_ID, UPDATE_TIME, PARTKEY
    )                                                                                                            
          SELECT gen_random_uuid() AS GUID                     
              ...
              ,POD_ID                                   
              ,'{current_time}'                  
              ,TIMESTAMP AS PARTKEY                   
          FROM APS_TMP_MASK_HISTORY_VIEW.APS_TMP_MASK_HISTORY_VIEW                       
          WHERE TIMESTAMP >= DATE_TRUNC('day', CURRENT_TIMESTAMP) - INTERVAL '90 day'        
...
def GetTypeOtherSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name, last_db_file):
    ...
    my_duck.attach_table(duck_db_memory, 'LAST_APS_TMP_MASK_HISTORY', last_db_file)
    insert_sql = """
    INSERT  /*+ append */  INTO APS_TMP_MASK_HISTORY_VIEW (                              
           GUID, TID,FUNC_ID,MASK_SEQ_NO,RTCL_ID,MASK_GROUP,LOC_ID,
           STATUS,NEXT_LOC_ID,NEXT_STATUS,LOC_STATE,DOCUMENT_NO,
     ...
     WHERE TIMESTAMP >= DATE_TRUNC('day', CURRENT_TIMESTAMP) - INTERVAL '1 day'                                                       
     AND NOT EXISTS (                                                                              
                SELECT HL.TID  FROM LAST_APS_TMP_MASK_HISTORY.APS_TMP_MASK_HISTORY_VIEW HL                                     
                WHERE HL.PARTKEY > DATE_TRUNC('day', CURRENT_TIMESTAMP) - INTERVAL '3 day'                                              
                AND HL.TID = HS.TID                                                                                
                AND HL.TIMESTAMP = HS.TIMESTAMP)                                                                                                   
    """.format(current_time=current_time)
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_MASK_HISTORY_5M_APS_TMP_MASK_HISTORY"
    ...
    target_table_sql = """
       ...
       (	
        ...
        MASK_SEQ_NO 	VARCHAR(64), 
        RTCL_ID 		VARCHAR(64), 
        MASK_GROUP 		VARCHAR(64), 
        LOC_ID 			VARCHAR(64) NOT NULL , 
        STATUS 			VARCHAR(64) NOT NULL , 
        NEXT_LOC_ID 	VARCHAR(64) NOT NULL , 
        ...
        SCRAP_CODE 		VARCHAR(64), 
        PMOS_PROD_ID 	VARCHAR(256), 
        POD_ID 			VARCHAR(64), 
