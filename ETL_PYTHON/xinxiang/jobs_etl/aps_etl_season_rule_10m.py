...
def create_temp_table(duck_db):
    ...
    sql = """
        create {table_type} table APS_TMP_ETL_SEASON_RULE_IDLE_SPEC
        (
            parentid               VARCHAR(64),
            toolg_id               VARCHAR(64),
            tool_id                VARCHAR(64),
            ...
            recipe_last_mode       VARCHAR(64),
            recipe_next            VARCHAR(255),
            recipe_next_mode       VARCHAR(64),
            idletime_from          VARCHAR(64),
            idletime_to            VARCHAR(64),
            run_limit              VARCHAR(64),
            run_qty                VARCHAR(64),
...
def GetSeasonRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_SEASON_RULE_IDLE_SPEC(                                                                  
            ...
            RUN_QTY, RESERVED_LP, ORG_SETUP_FLAG, UPDATE_TIME, PARTCODE                                                                                                     
        )                                                                                                                        
          ...
          SPEC_INFO_CHAMBER AS (                                                                                                          
             SELECT DISTINCT TL.EQP_G AS TOOLG_ID,                                                                                         
                    ...
                    NULL AS RUN_LIMIT,                                                                                               
                    NULL AS RUN_QTY,
                    CASE WHEN SP.LP_FLAG = 1 THEN 1 END AS LP_FLAG                                                                                                  
         ...
         SELECT '{uuid}' AS PARENTID,                                                                                                                                                            
                 ...
                 COALESCE(ZD33.PROCESS_TIME,0) AS PROCESS_TIME,                                                      
                 SP.LOT_TYPE,                                                                                                     
                 SP.RECIPE_LAST,                                                                                                  
                 SP.RECIPE_LAST_MODE,                                                                                             
                 SP.RECIPE_NEXT,                                                                                                  
                 ...
                 SP.RUN_QTY ,
                 SP.LP_FLAG,
                 CASE WHEN R.EQP_ID IS NOT NULL THEN '1' ELSE '0' END AS ORG_SETUP_FLAG,
          ...
          LEFT JOIN APS_SYNC_EQP_DUMMY_BY_CHAMBER_RULE.APS_SYNC_EQP_DUMMY_BY_CHAMBER_RULE R
          ON R.EQP_ID = SP.TOOL_ID AND SP.IDLETIME_FROM IS NOT NULL                                                                                           
    """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_SEASON_RULE_IDLE_SPEC",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_SEASON_RULE_IDLE_SPEC")

    # idle dummy新逻辑，宇清要求把其他没有列出来的chamber也需要加进来，方便uo做判断
    sql = """
       ...
       WITH SOURCE_DATA AS (
            SELECT PARENTID, TOOLG_ID, TOOL_ID, CH_ID, SEQ, PROCESS_TIME,
                    LOT_TYPE, RECIPE_LAST, RECIPE_LAST_MODE, RECIPE_NEXT,
                    RECIPE_NEXT_MODE, IDLETIME_FROM, IDLETIME_TO, RUN_LIMIT,
                    RUN_QTY, RESERVED_LP, ORG_SETUP_FLAG, UPDATE_TIME, PARTCODE 
            FROM {tempdb}APS_TMP_ETL_SEASON_RULE_IDLE_SPEC
            UNION ALL
            SELECT  S.PARENTID, S.TOOLG_ID, S.TOOL_ID, T.CH_ID, SEQ, PROCESS_TIME,
                    LOT_TYPE, RECIPE_LAST, RECIPE_LAST_MODE, RECIPE_NEXT,
                    RECIPE_NEXT_MODE, IDLETIME_FROM, IDLETIME_TO, RUN_LIMIT,
                    RUN_QTY, RESERVED_LP, '0' AS ORG_SETUP_FLAG, 
                    S.UPDATE_TIME, S.PARTCODE 
            FROM {tempdb}APS_TMP_ETL_SEASON_RULE_IDLE_SPEC S
            INNER JOIN APS_ETL_TOOL.APS_ETL_TOOL T
            ON S.TOOL_ID = T.TOOL_ID AND S.CH_ID <> T.CH_ID
            WHERE S.ORG_SETUP_FLAG = '1'
       )
       SELECT PARENTID, TOOLG_ID, TOOL_ID, CH_ID, SEQ, PROCESS_TIME,
            LOT_TYPE, RECIPE_LAST, RECIPE_LAST_MODE, RECIPE_NEXT,
            RECIPE_NEXT_MODE, IDLETIME_FROM, IDLETIME_TO, RUN_LIMIT,
            RUN_QTY, RESERVED_LP, 
            MAX(ORG_SETUP_FLAG) AS ORG_SETUP_FLAG, 
            UPDATE_TIME, PARTCODE
       FROM SOURCE_DATA D
       GROUP BY PARENTID, TOOLG_ID, TOOL_ID, CH_ID, SEQ, PROCESS_TIME,
                LOT_TYPE, RECIPE_LAST, RECIPE_LAST_MODE, RECIPE_NEXT,
                RECIPE_NEXT_MODE, IDLETIME_FROM, IDLETIME_TO, RUN_LIMIT,
                RUN_QTY, RESERVED_LP, UPDATE_TIME, PARTCODE 
       """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
...
def GetDummySeasonRuleSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
  ...
  SELECT DISTINCT  '{uuid}'  AS PARENTID,  
               ...
               CASE WHEN P.RUN_DUMMY <> 'Y' THEN CAST('0.0' AS DECIMAL)                                               
                    WHEN C.TOOLG_ID IS NOT NULL AND C.TOOLG_ID <> '' THEN CAST(COALESCE(C.PROCESS_TIME,0) AS DECIMAL)                                  
                    WHEN C2.TOOLG_ID IS NOT NULL AND C2.TOOLG_ID <> '' THEN CAST(COALESCE(C2.PROCESS_TIME,0) AS DECIMAL)
                    WHEN C3.RN = 1 THEN CAST(COALESCE(C3.PROCESS_TIME,0) AS DECIMAL)                                                                 
                    END AS PROCESS_TIME,                                                           
               P.PPID_LAST,                                                                        
               P.PPID_NEXT,                                                                        
               P.PPID_LAST_MODE,                                                                   
         ...
         LEFT JOIN APS_MID_IDLE_DUMMY34_MEDIAN.APS_MID_IDLE_DUMMY34_MEDIAN C                                                         
        ON C.TOOLG_ID = P.TOOLG_ID                                                                       
        AND C.PRODID = P.DMY_PRODID                                                                  
        AND C.CASEID = P.CASEID                                                                      
        LEFT JOIN APS_MID_IDLE_DUMMY35_MEDIAN.APS_MID_IDLE_DUMMY35_MEDIAN C2                                                       
        ON C2.TOOLG_ID = P.TOOLG_ID 
    ...
    sql = """
        INSERT  /*+ append */  INTO APS_ETL_SEASON_RULE(                                 
         ...
         SELECT DISTINCT  '{uuid}' AS PARENTID,                                                                                             
                ...
                CASE WHEN N.CONTROL_TYPE='1' THEN REPLACE(REPLACE(REPLACE(N.TO_ITEM,'*','%'),'!',''),'/',',') END AS PROD_ID_NEXT,                                       
                ...
                CASE WHEN N.CONTROL_TYPE='2' THEN REPLACE(REPLACE(REPLACE(REPLACE(N.TO_ITEM,'*','%'),'!',''),'/',','), '.','') END AS LOT_ID_NEXT,                                        
                ...
                CASE WHEN N.MIN_MOVE_PCS = '*' OR N.MIN_MOVE_PCS IS NULL OR N.MIN_MOVE_PCS = ' EQP' THEN NULL ELSE N.MIN_MOVE_PCS END AS CONSEC_CNT,                                                         
         ...
         AND CAST(COALESCE(N.NEED_TIME,0) AS DECIMAL) <> 999   		 
         AND 
    ...
    sql = """
        INSERT  /*+ append */  INTO APS_ETL_SEASON_RULE(                                 
         ...
         SELECT DISTINCT  '{uuid}' AS PARENTID,                                                                                             
                ...
                CASE WHEN N.CONTROL_OPE = '*' THEN '' ELSE 'IN' END AS STAGE_NEXT_MODE,                                                                 
                CASE WHEN N.MIN_MOVE_PCS = '*' OR N.MIN_MOVE_PCS IS NULL THEN NULL ELSE 'WAFER' END AS CONSEC_UNIT,                                                               
                CASE WHEN N.MIN_MOVE_PCS = '*' OR N.MIN_MOVE_PCS IS NULL THEN NULL OR N.MIN_MOVE_PCS = ' EQP' ELSE N.MIN_MOVE_PCS END AS CONSEC_CNT,                                                         
                CASE WHEN N.MIN_MOVE_PCS = '*' OR N.MIN_MOVE_PCS IS NULL THEN NULL ELSE 'LARGER' END AS CONSEC_CNT_MODE,                                                          
                '{current_time}' AS UPDATE_TIME,                                                                                                  
                '' AS PARTCODE                                                                                       
         FROM APS_SYNC_RTD_WET_SPECIAL_LOT_CONTROL.APS_SYNC_RTD_WET_SPECIAL_LOT_CONTROL N                                                                                                                                                                                                 
...
def execute():
    ...
    used_table_list = ['APS_SYNC_ETL_TOOL',
                       ...
                       'APS_MID_IDLE_DUMMY35_MEDIAN',
                       'APS_MID_IDLE_DUMMY34_MEDIAN',
                       'APS_SYNC_OCS_AUTODUMMY_COMPLETE',
                       'APS_SYNC_EQP_DUMMY_BY_CHAMBER_RULE',
                       'APS_ETL_TOOL'
                       ]
    target_table_sql = """      
