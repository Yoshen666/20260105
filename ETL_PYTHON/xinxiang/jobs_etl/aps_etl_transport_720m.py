...
# 找不到Location的Lot的数据为NA（正在传送中），要找设置默认时间的传送时间,所以所有都需要增加一条NA的数据
def getSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT /*+ append */
   INTO APS_ETL_TRANSPORT
     (PARENTID,
      LOCATION_FROM,
      LOCATION_TO,
      TRANSPORT_TIME,
     ...
     SELECT '{uuid}' AS PARENTID,
            TT.LOCATION_FROM,
            TT.LOCATION_TO,
            --ROUND(TT.TRANSPORT_TIME / 60 / 60, 3) AS TRANSPORT_TIME,
            CAST(CAST(TT.TRANSPORT_TIME AS DECIMAL) / 60 / 60 AS DECIMAL(18,3)) AS TRANSPORT_TIME,
            '' AS PPARTCODE,
            CAST('{current_time}' AS TIMESTAMP) AS UPDATE_TIME
       FROM APS_SYNC_MCS_SCHED_TRANSFER_TIME.APS_SYNC_MCS_SCHED_TRANSFER_TIME TT
     UNION ALL
     SELECT DISTINCT '{uuid}' AS PARENTID,
                     'NA' AS LOCATION_FROM,
                     T.LOCATION_TO,
                     --(SELECT ROUND(D.TRANSFER_TIME / 60 / 60, 3)
                     (SELECT CAST(CAST(D.TRANSFER_TIME AS DECIMAL) / 60 / 60 AS DECIMAL(18,3))
                        FROM APS_SYNC_TRANSFER_TIME_DEFALUTE.APS_SYNC_TRANSFER_TIME_DEFALUTE D) AS TRANSPORT_TIME,
                     '' AS PPARTCODE,
                     '{current_time}' AS UPDATE_TIME
       FROM APS_SYNC_MCS_SCHED_TRANSFER_TIME.APS_SYNC_MCS_SCHED_TRANSFER_TIME T                                                                                                  
    """.format(uuid=uuid, current_time=current_time)

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_TRANSPORT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_TRANSPORT")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_TRANSPORT_720M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_ETL_TRANSPORT"
    used_table_list = ['APS_SYNC_MCS_SCHED_TRANSFER_TIME',
                       'APS_SYNC_TRANSFER_TIME_DEFALUTE']
    target_table_sql = """      
    create table {}APS_ETL_TRANSPORT
    (
        parentid       VARCHAR(60) not null,
        location_from  VARCHAR(60) not null,
        location_to    VARCHAR(60) not null,
        transport_time DECIMAL not null,
        update_time    TIMESTAMP not null,
        partcode       VARCHAR(60) not null,
        PRIMARY KEY (PARENTID, LOCATION_FROM, LOCATION_TO, PARTCODE)
    )
    ...
    try:
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_TRANSPORT_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
