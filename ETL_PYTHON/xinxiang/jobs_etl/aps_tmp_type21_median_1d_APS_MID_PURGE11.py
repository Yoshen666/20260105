...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO APS_MID_PURGE11(                                 
        PARENTID,  TOOLG_ID, TOOL_ID, PT, UPDATE_TIME, PARTCODE                                                                         
        )                                                                                       
         WITH SOURCE_DATA AS (                                                                            
             SELECT T.EQP_G,C.EQP_ID,                                                                     
            -- ROUND(CEIL((TO_DATE(TO_CHAR(C.EVENT_TIME_END,'YYYY-MM-DD HH24:MI:SS'),'YYYY-MM-DD HH24:MI:SS')                                
            --  - TO_DATE(TO_CHAR(C.EVENT_TIME_ST,'YYYY-MM-DD HH24:MI:SS'),'YYYY-MM-DD HH24:MI:SS'))*24*60*60)/3600,3) AS PT,
               round((extract(epoch from(cast(C.EVENT_TIME_END as timestamp) - cast(C.EVENT_TIME_ST as timestamp)))) / 3600.0, 3)  AS PT,              
              ROW_NUMBER() OVER (PARTITION BY T.EQP_G,C.EQP_ID ORDER BY C.EVENT_TIME_END DESC) AS RN      
             ...
             WHERE EVENT_TIME_END IS NOT NULL AND EVENT_TIME_END <> ''                                                                                                    
         ),                                                                                               
         PT_DATA AS (                                                                                     
             SELECT EQP_G,EQP_ID,RN,                                                                      
                    --MEDIAN(CAST(PT AS DECIMAL)) OVER(PARTITION BY EQP_G,EQP_ID ) AS MAX_50
                    QUANTILE_CONT(CAST(PT AS DECIMAL),0.5) OVER(PARTITION BY EQP_G,EQP_ID ) AS MAX_50                                            
             FROM SOURCE_DATA                                                                             
         ),                                                                                               
         RESULT_DATA AS (                                                                                 
             SELECT EQP_G,EQP_ID,                                                                         
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_PURGE11",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_PURGE11")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_TYPE21_MEDIAN_1D_APS_MID_PURGE11"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_PURGE11"
    used_table_list = ['APS_SYNC_OUT_DF_PURGE_COMBINE', 'APS_SYNC_ETL_TOOL']
    target_table_sql = """
        create table {}APS_MID_PURGE11
        (
    ...
    try:
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_TMP_TYPE21_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
