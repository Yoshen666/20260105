...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.sync_APS_MID_PH_LOTHISTORY"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    source_table = "V_ETL_LOTHISTORY"
    target_table_name = "APS_MID_PH_LOTHISTORY_VIEW"
    ...
    create_table_sql = """
        CREATE TABLE APS_MID_PH_LOTHISTORY_VIEW (
            LOT_ID VARCHAR,
            OPE_NO VARCHAR,
            OPE_SEQ INTEGER,
            PASS_COUNT INTEGER,
    ...
    try:
        ...
        if last_file_name:
            ...
            if _create_time_date is not None:
                ...
                query_sql = query_sql \
                            + " where 1=1 and {check_date_column} >= TO_DATE('{create_time}', 'YYYY-MM-DD HH24:MI:SS') AND INSTR(EQP_ID,'P') = 1 AND OPE_CATEGORY = 'OperationComplete' " \
                ...
                query_duck_sql = """
                     SELECT LOT_ID, OPE_NO,CASE WHEN OPE_SEQ IS NULL THEN 0 ELSE OPE_SEQ END AS OPE_SEQ,CASE WHEN PASS_COUNT IS NULL THEN 0 ELSE PASS_COUNT END AS PASS_COUNT,LINE,CLAIM_TIME,MAINPD_ID,EQP_ID,MOVE_FLAG,REWORK_MOVE_FLAG,PURE_MOVE_FLAG,DUMMY_MOVE_FLAG,PROCESS_AREA,MODULE_ID,DETAIL_MODULE_ID,CASE WHEN PRIORITY IS NULL THEN 0 ELSE PRIORITY END AS PRIORITY,CASE WHEN PRIORITY_CLASS IS NULL THEN 0 ELSE PRIORITY_CLASS END AS PRIORITY_CLASS,SHL_FLAG,CASE WHEN CYCLE_TIME IS NULL THEN 0 ELSE CYCLE_TIME END AS CYCLE_TIME,PREV_OP_COMP_TIME,BANK_ID,PREV_BANK_ID,REWORK_CHILD_FLAG,PW_FLAG,PRODSPEC_ID,PHOTO_LAYER,SHIFT_DATE,SHIFT_HOUR,SHIFT,RECIPE_ID,RETICLE_ID,CASE WHEN WAFER_QTY IS NULL THEN 0 ELSE WAFER_QTY END AS WAFER_QTY,PRODGRP_ID,EQP_TYPE,CTRL_JOB,SUB_LOT_TYPE,OP_START_DATE_TIME,OPE_CATEGORY,LAYER,CHIP_BODY,SUFFIX,CLAIM_USER,REPLACE(REPLACE(claim_memo, chr(10), ''), chr(13), '')  AS claim_memo,LOT_OWNER_ID,NENG_FLAG,LOCATION_ID,PROCESS_START_TIME,CAST_ID,CASE WHEN BANK_PERIOD_TIME IS NULL THEN 0 ELSE BANK_PERIOD_TIME END AS BANK_PERIOD_TIME,CASE WHEN HOLD_PERIOD_TIME IS NULL THEN 0 ELSE HOLD_PERIOD_TIME END AS HOLD_PERIOD_TIME,BATCH_ID,LAST_MAIN_CTRL_JOB
                    from {target_table} where 1=1 and {check_date_column} >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '1 day'
                ...
                # 删除老数据
                delete_sql = """
                       delete from {target_table} where {check_date_column} < DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '300 day'
                       """.format(target_table=target_table_name,
                                  check_date_column=check_date_column,
                                  save_day=sync_day + 1)
            ...
            # 做全量
            query_sql = query_sql + """ where 1=1 and {check_date_column} >= SYSDATE - {sync_day} AND INSTR(EQP_ID,'P') = 1 AND OPE_CATEGORY = 'OperationComplete'""".format(
        ...
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, "XETL079")
