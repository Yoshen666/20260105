...
def create_temp_table(duck_db):
    ...
    duck_db.sql(sql)
    sql = """
        create {table_type} table APS_TMP_HANDLE_WIP_V1
        (
          parentid          VARCHAR(60) not null,
          lot_id            VARCHAR(60) not null,
          step_id           VARCHAR(60),
    ...
    sql = """
      create {table_type} table APS_TMP_ETL_WIP_ATRR1
      (
          lot_id        VARCHAR(64),
          ope_no        VARCHAR(64),
          prod_id       VARCHAR(64),
    ...
    sql = """
         create {table_type} table APS_TMP_ETL_WIP_PX_BATCH_ID
         (
             lot_id        VARCHAR(64),
             ope_no        VARCHAR(64),
             prod_id       VARCHAR(64),
    ...
    sql = """
         create {table_type} table APS_TMP_ETL_WIP_PLAT_FORM_DATA
         (
             prodg1        VARCHAR(64),
             plat_form     VARCHAR(64)
         )
    ...
    sql = """
         create {table_type} table APS_TMP_ETL_WIP_LOT_PRIORITY
         (
             LOT_ID         VARCHAR(64), 
             GROUP_ID       VARCHAR(64), 
    ...
    sql = """
         create {table_type} table APS_TMP_ETL_WIP_FLOW_DYB_RETURN
         (
            LOT_ID   VARCHAR(64), 
            OPENO   VARCHAR(64), 
    ...
    sql = """
        create {table_type} table APS_TMP_ETL_APC_PH_HIST
        (
           LOT_ID   VARCHAR(64), 
           OPE_NO   VARCHAR(64), 
           CLAIM_TIME    VARCHAR(64),
    ...
    sql = """
        create {table_type} table APS_TMP_ETL_WIP_QTIME_CAPACITY
        (
           PRODSPEC_ID   VARCHAR(64), 
           OPE_NO   VARCHAR(64), 
           CAPACITY_VALUE    VARCHAR(64) 
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
       create {table_type} table APS_TMP_ETL_WIP_CROSS_LOT
       (
          LOT_ID   VARCHAR(64), 
          TARGET_STEP_ID   VARCHAR(64), 
    ...
    sql = """
        create {table_type} table APS_TMP_IGNORE_WATER_LINE_LOT
        (
           LOT_ID   VARCHAR(64), 
           qtime_ignore_flag   VARCHAR(64) 
        )
    ...
    sql = """
        create {table_type} table APS_TMP_ETL_WIP_PILOT_EXCEPTION
        (
           LOT_ID   VARCHAR(64),
           TARGET_OPE_NO    VARCHAR(64),
           EXCEPT_PILOT_FLAG    VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)
