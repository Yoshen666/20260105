...
def GetLastFlowData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    sql = """
     INSERT INTO {tempdb}APS_TMP_ETL_WIP_LAST_FLOW
     (                                                                                            
...
def InsertWip2Temp(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    # 区分出PH的WIP和非PH的WIP信息
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_V
     (	    LOT_ID,                         
            PRODSPEC_ID,                    
            MAINPD_ID,                      
            OPE_NO,                         
     ...
     SELECT W.LOT_ID,                                                                      
           ...
           -- W.LOT_PROC_STATE,  
            CASE WHEN W.LOT_PROC_STATE = 'Waiting' AND W.CTRL_JOB_ID is not null AND W.CTRL_JOB_ID <>'' THEN 'JOBPREPARED'
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_OPER_START_HS                                               
    ...
    SELECT LOT_ID, OPE_NO, PH_RECIPE_ID, CLAIM_TIME, CTRL_JOB, EQP_ID, OPE_CATEGORY,PRODSPEC_ID,MAINPD_ID      
      FROM (SELECT OH.LOT_ID,                                                                                  
              ...
              FROM APS_MID_FHOPEHS_VIEW.APS_MID_FHOPEHS_VIEW OH                                                                                
              INNER JOIN {tempdb}APS_TMP_ETL_WIP_V W ON OH.LOT_ID = W.LOT_ID                                  
                                                     AND OH.OPE_NO = W.OPE_NO                                  
                                                     AND OH.MAINPD_ID = W.MAINPD_ID                            
                                                     AND OH.PRODSPEC_ID = W.PRODSPEC_ID                                    
             WHERE OH.CTRL_JOB is not null  AND OH.CTRL_JOB <> ''                                                                          
               -- AND OH.PARTKEY >= SYSDATE - 3
               AND OH.CLAIM_TIME >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '3 day'                                                                
               AND OH.OPE_CATEGORY IN ('OperationStart')
               order by OH.LOT_ID
               ) A                                                   
    WHERE A.RN = 1                                                                                             
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_OPER_RESERVE_HS                                               
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_OPER_BATCHFORMING_HS                        
    ...
    # 先获取BatchName的信息
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PO_N1_BATCH_NAME                            
     ...
     AND EXISTS ( 
         SELECT 1 FROM {tempdb}APS_TMP_ETL_WIP_V W           
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_FORMING_RESULT                             
     ...
     SELECT DISTINCT H.LOT_ID,H.MAINPD_ID,                     
            ...
            H.CLAIM_USER_ID                                    
     FROM {tempdb}APS_TMP_ETL_WIP_OPER_BATCHFORMING_HS H        
         INNER JOIN {tempdb}APS_TMP_ETL_WIP_PO_N1_BATCH_NAME N       
    ...
    sql = """
      INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_MORE_RETICLE                                                                                           
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_RECIPE_RETICLE
     (                                                                                                          
     ...
     WHERE RN = 1
     order by TOOLID, RECIPE_ID, RETICLE_ID
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_OPER_PROCESS_START_HS
     (LOT_ID, OPE_NO, RECIPE_ID, CLAIM_TIME, CTRL_JOB, EQP_ID, OPE_CATEGORY,PRODSPEC_ID,MAINPD_ID)          
     SELECT LOT_ID, OPE_NO, PH_RECIPE_ID, CLAIM_TIME, CTRL_JOB, EQP_ID, OPE_CATEGORY,PRODSPEC_ID,MAINPD_ID  
       FROM (SELECT OH.LOT_ID,                                                                              
                    OH.OPE_NO,                                                                              
              ...
              WHERE OH.CTRL_JOB is not null AND OH.CTRL_JOB <> ''                                                                      
                -- AND OH.PARTKEY >= SYSDATE - 3
                AND OH.CLAIM_TIME >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '3 day'                                                             
                AND OH.OPE_CATEGORY IN ('ProcessStart') 
                order by OH.LOT_ID) A                                                  
     WHERE A.RN = 1 
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    # 为了获取RUN状态的Reticle数据
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_OPEHS_RETICLE                                      
     ...
     -- FROM (SELECT OHR.LOT_ID,
        ...
        --           OHR.OPE_NO,
          ...
          --         OHR.CLAIM_TIME AS CLAIM_TIME,
            ...
            -- FROM APS_MID_FHOPEHS_RETICLE_VIEW.APS_MID_FHOPEHS_RETICLE_VIEW OHR                            
           --  WHERE 1 = 1                                                                          
             -- AND OHR.PARTKEY >=  SYSDATE - 5
           --  AND OHR.CLAIM_TIME >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '5 day'                                                        
     ...
     WITH GROUP_DATE AS (                                                                                                                         
            ...
            LASE_DATA AS (                                                                                                                               
                  ...
                  FROM GROUP_DATE order by LOT_ID                                                                                                                       
            )                                                                                                                                            
            SELECT A.LOT_ID, A.MAINPD_ID, A.OPE_NO, A.RETICLE_ID, A.CLAIM_TIME, A.OPE_CATEGORY                                                           
            FROM LASE_DATA A                                                                                                                             
            WHERE RN = 1  
             order by LOT_ID,MAINPD_ID, OPE_NO, OPE_CATEGORY, RETICLE_ID   
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PROCESS_MONITOR
    (LOT_ID, CR)                                        
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_LOT_PRIORITY
     (LOT_ID, GROUP_ID, LOT_PRIORITY, SHR_FLAG)                            
     SELECT P.LOT_ID, D.GROUP_ID, P.LOT_PRIORITY,
       CASE WHEN P.LOT_PRIORITY >= 100 AND P.LOT_PRIORITY <= 499 THEN -1
            WHEN D.GROUP_ID = 1 THEN 0
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_LOT_PRIORITY",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_LOT_PRIORITY")

    # V_LOT_LOCATION 数据insert 到 temp table
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_LOT_LOCATION                             
     (LOT_ID, FOUP_EQP_ID, FOUP_STATION_ID, FAB_PHASE)                                     
     SELECT A.LOT_ID, A.FOUP_EQP_ID, A.FOUP_STATION_ID, A.FAB_PHASE 
     FROM APS_SYNC_LOT_LOCATION.APS_SYNC_LOT_LOCATION A  
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_LOCATION_INFO             
    (TOOL_ID, LOCATION_ID)                                                     
    SELECT TOOL_ID, LOCATION 
    FROM APS_SYNC_MCS_SCHED_LOCATION_INFO.APS_SYNC_MCS_SCHED_LOCATION_INFO A                  
    ...
    sql = """
        INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_LOACTION_MANUAL             
        (TOOL_ID, LOCATION_ID)                                                     
        SELECT T.EQP_ID,                                                                  
               MAX(M.LOCATION) AS LOCATION_ID                                             
        ...
        AND INSTR(T.EQP_ID,REPLACE(M.TOOL_ID,'*','')) > 0                                 
        GROUP BY T.EQP_ID 
        """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_LOACTION_MANUAL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_LOACTION_MANUAL")

    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_FLOW_DYB_RETURN                                                                                       
     (                                                                                                                                                   
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_FLOW_DYB_RETURN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_FLOW_DYB_RETURN")

    # V_FLOW_DYB_RETURN 数据insert 到 temp table
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_DYB_RETURN_LOT                                                                                       
     (                                                                                                                                                   
       ...
       ,TRANS_STATE, PROCESS_START_TIME, WIP_CLAIM_TIME,CTRL_JOB_ID                                                                                               
     )                                                                                                                                                   
     SELECT W.LOT_ID, W.PRODSPEC_ID, W.OPE_NO, A.OPENO, COALESCE(A.DYB_MAINPD_ID, W.MAINPD_ID) AS DYB_MAINPD_ID
           ...
           ,W.TRANS_STATE, W.PROCESS_START_TIME,W.WIP_CLAIM_TIME,W.CTRL_JOB_ID                                                                                         
     FROM {tempdb}APS_TMP_ETL_WIP_V W                                                                                                                   
     LEFT JOIN {tempdb}APS_TMP_ETL_WIP_FLOW_DYB_RETURN A 
     ON A.LOT_ID = W.LOT_ID AND A.DYB_MAINPD_ID = W.MAINPD_ID                                                                                                                
     WHERE 1=1                                                                                                                                           
     AND POSITION('DYB' IN W.MAINPD_ID) = 1 
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PF_PO_N1                                                                
     (MAINPD_ID, POS_PDID, OPE_SEQ, OPE_NO, BATCH_NAME, BATCH_TARGET_FLAG,CTRL_JOB_ID)                                                                             
     SELECT N.MAINPD_ID, N.POS_PDID, N.OPE_SEQ, N.OPE_NO, N.BATCH_NAME,N.BATCH_TARGET_FLAG,L.CTRL_JOB_ID                             
     FROM APS_SYNC_PF_PO_N1.APS_SYNC_PF_PO_N1 N                                                                 
     INNER JOIN {tempdb}APS_TMP_ETL_WIP_DYB_RETURN_LOT L 
     ON N.OPE_NO = L.CURRENT_OPE_NO AND N.MAINPD_ID = L.DYB_MAINPD_ID
    ...
    sql = """
    INSERT INTO {tempdb}APS_TMP_DYB_FLOW_LOT_V1
      (
           ...
           WHITE_FLAG,BATCH_ID,TRANS_STATE,PROCESS_START_TIME,WIP_CLAIM_TIME, MULTI_GROUP,MULTI_TARGET_FLAG,CTRL_JOB_ID
      )
         SELECT  
               A.LOT_ID, 
               A.PRODSPEC_ID, 
               ...
               MAX(B.CTRL_JOB_ID)        AS CTRL_JOB_ID                                                 
         FROM {tempdb}APS_TMP_ETL_WIP_DYB_RETURN_LOT A                                                                                                 
         INNER JOIN {tempdb}APS_TMP_ETL_WIP_PF_PO_N1 B ON B.OPE_NO = A.CURRENT_OPE_NO AND B.MAINPD_ID = A.DYB_MAINPD_ID                                
    ...
    sql = """
     INSERT INTO {tempdb}APS_TMP_ETL_WIP_DYB_LOT                                                                                                          
     (                                                                                                                                               
        LOT_ID, PRODSPEC_ID, CURRENT_OPE_NO, OPENO, DYB_MAINPD_ID, RETURN_MAINPD_ID, RETURN_OPE_NO                                                   
       ...
       ,PROCESS_START_TIME, WIP_CLAIM_TIME, MULTI_GROUP,MULTI_TARGET_FLAG,CTRL_JOB_ID                                                                                                           
     )                                                                                                                                               
    WITH PRODG AS                                                                                                                                    
     (                                                                                                                                               
     ...
     SELECT D.LOT_ID, D.PRODSPEC_ID, D.CURRENT_OPE_NO, D.OPE_NO, D.DYB_MAINPD_ID, D.RETURN_MAINPD_ID, D.RETURN_OPE_NO                                
           ...
           ,CASE WHEN (SUBSTRING(SUBSTRING(d.current_ope_no, POSITION('.' IN d.current_ope_no)),3,1)='Q' AND d.EQP_G IN ('PK_KrF','PH_ArF','PU_I-Line'))
            ...
            D.CTRL_JOB_ID
     FROM {tempdb}APS_TMP_DYB_FLOW_LOT_V1 D                                                                                                             
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_UMT_CONTROL_LOT
     (	    
        TECH,                                                             
        PRODG1,                                                           
        PRODUCT_ID,                                                       
     ...
     AND (
            ...
            AND STRPTIME(STRFTIME(CURRENT_timestamp,'%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S') <= STRPTIME(CONCAT(STRFTIME(DATE_TRUNC('second', CAST(T.END_DATE AS DATE)) + INTERVAL '1 day', '%Y-%m-%d'),' 07:30:00'),'%Y-%m-%d %H:%M:%S') 
            )                    
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_WIP_LASTFLOW_V1
      ...
      FROM {tempdb}APS_TMP_ETL_WIP_V V                                                                
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_LAST_FLOW F ON                                                    
    ...
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_WIP_V5 
    (
        ...
        TRACKIN_TIME,PROCESS_START,TECH_ID,CUSTOMER_ID,PARENT_LOT_ID,CR,RECIPE,WHITE_FLAG,BATCH_ID,TRANS_STATE,WIP_CLAIM_TIME,MULTI_GROUP,
        MULTI_TARGET_FLAG,CTRL_JOB_ID
    )
       SELECT W.LOT_ID,                                                                                                           
           F.PRODG_ID,                                                                                                            
           F.PRODG_TECH,                                                                                                          
           ...
           W.CTRL_JOB_ID                                                                                     
      FROM {tempdb}APS_TMP_ETL_WIP_V W                                                                                           
      INNER JOIN {tempdb}APS_TMP_ETL_WIP_LASTFLOW_V1 F ON W.PRODSPEC_ID = F.PROD_ID                                                   
                               AND W.MAINPD_ID = F.PLAN_ID                                                                        
                               AND W.OPE_NO = F.OPE_NO                                                                            
                               AND F.STEP_TYPE = 'N'                                                                              
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PROCESS_MONITOR LM ON W.LOT_ID = LM.LOT_ID
      order by LOT_ID, PRODG_ID, PRODG_TECH, PRODSPEC_ID
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    (
     ...
     SELECT W.LOT_ID,                                                                                                            
            ...
            w.CTRL_JOB_ID                                                                               
     FROM {tempdb}APS_TMP_ETL_WIP_V W                                                                                           
     INNER JOIN {tempdb}APS_TMP_ETL_WIP_LASTFLOW_V1 RSF ON W.PRODSPEC_ID = RSF.PROD_ID                                                
    ...
    (
      ...
      SELECT W.LOT_ID,                                                                                                            
             ...
             W.CTRL_JOB_ID                                                                                    
      FROM {tempdb}APS_TMP_ETL_WIP_DYB_LOT W                                                                                        
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PROCESS_MONITOR LM ON W.LOT_ID = LM.LOT_ID
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    (
      ...
      SELECT W.LOT_ID,                                                                                                        
             ...
             W.CTRL_JOB_ID                                                                               
      FROM {tempdb}APS_TMP_ETL_WIP_DYB_LOT W                                                                                        
      INNER JOIN {tempdb}APS_TMP_ETL_WIP_LASTFLOW_V1 LF ON LF.PROD_ID = W.PRODSPEC_ID                                               
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP                                                                                    
     (                                                                                                                                
       ...
       BATCH_ID,FROMBATCH_TIME,FROMBATCH_TOOL, TRANS_STATE, FOUP_STATION_ID, MULTI_GROUP, MULTI_TARGET_FLAG, PTY_GROUP,conti_flag                           
     )                                                                                                                                             
        SELECT DISTINCT W.LOT_ID,                                                                                                     
               ...
               W.CASSETE_ID,                                                                                                          
               W.WAFER_QTY,                                                                                                           
               CASE WHEN S.LOT_PRIORITY IS NULL THEN 500 ELSE S.LOT_PRIORITY END AS PRIORITY_CLASS,                                                                                                      
               ...
               ----优先看非parallel_mode机台的规则
               CASE WHEN W.LOT_PROC_STATE<>'Processing' THEN NULL ELSE (
                    CASE WHEN PST.CTRL_JOB IS NOT NULL AND PST.CTRL_JOB <>'' THEN PST.new_process_start ELSE (
                        CASE WHEN (W.PROCESS_START IS NOT NULL and W.PROCESS_START <> '') THEN W.PROCESS_START ELSE
                            (CASE WHEN W.LOT_PROC_STATE='Processing' AND (W.TRACKIN_TIME IS NOT NULL and W.TRACKIN_TIME <> '') AND PS.CLAIM_TIME > W.TRACKIN_TIME THEN PS.CLAIM_TIME END) END    
                    ) END
               ) END AS PROCESS_START,   
             ...
             -- (CASE WHEN W.LOT_PROC_STATE='Processing' AND (W.TRACKIN_TIME IS NOT NULL and W.TRACKIN_TIME <> '') AND PS.CLAIM_TIME > W.TRACKIN_TIME THEN PS.CLAIM_TIME END) END AS PROCESS_START,  
               ...
               CASE WHEN S.SHR_FLAG IS NULL THEN -1 ELSE S.SHR_FLAG END AS SHR_FLAG,                                                                                                      
                ...
                CASE    WHEN (LI.LOCATION_ID IS NOT NULL and LI.LOCATION_ID <> '')      THEN LI.LOCATION_ID                                                    
                        WHEN (LMI.LOCATION_ID IS NOT NULL and LMI.LOCATION_ID <> '')      THEN LMI.LOCATION_ID
                ...
                CASE WHEN S.GROUP_ID IS NULL THEN 10 ELSE S.GROUP_ID END AS PTY_GROUP,
                CASE WHEN W.LOT_PROC_STATE<>'Processing' THEN NULL ELSE (
                    CASE WHEN PST.CTRL_JOB IS NOT NULL AND PST.CTRL_JOB <>'' THEN PST.conti_flag ELSE (
                        CASE WHEN (W.PROCESS_START IS NOT NULL and W.PROCESS_START <> '') THEN '0' ELSE
                            (CASE WHEN W.LOT_PROC_STATE='Processing' AND (W.TRACKIN_TIME IS NOT NULL and W.TRACKIN_TIME <> '') AND PS.CLAIM_TIME > W.TRACKIN_TIME THEN '0' END) END    
                    ) END
               ) END AS conti_flag                           
      FROM {tempdb}APS_TMP_ETL_WIP_V5 W                                                                                                 
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_LOT_PRIORITY S ON W.LOT_ID =  S.LOT_ID                                                                                                                           
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_CTL_JOB LJ    ON W.LOT_ID = LJ.LOT_ID AND W.OPE_NO = LJ.OPE_NO                                  
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_OPER_RESERVE_HS RH ON  LJ.LOT_ID = RH.LOT_ID                                                 
      ...
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_OPER_START_HS OS ON  LJ.LOT_ID = OS.LOT_ID                                                   
                                                         AND  LJ.OPE_NO = OS.OPE_NO                                                   
                                                         AND  LJ.MAINPD_ID = OS.MAINPD_ID                                             
                                                         AND  LJ.PRODSPEC_ID = OS.PRODSPEC_ID                                          
                                                         AND  LJ.CTRL_JOB = OS.CTRL_JOB
       --modify by xiecheng for version up                                                   
       LEFT JOIN {tempdb}APS_TMP_ETL_WIP_RECIPE_RETICLE RR  ON RH.EQP_ID = RR.CONVERT_TOOL_ID                                            
                                                        AND RH.RECIPE_ID_C = RR.RECIPE_ID_C                                                                                                  
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_OPEHS_RETICLE OHR ON W.LOT_ID = OHR.LOT_ID                                                      
                                                       AND W.MAINPD_ID = OHR.MAINPD_ID                                                
                                                       AND W.OPE_NO = OHR.OPE_NO                                                      
                                                       AND OS.CLAIM_TIME = OHR.CLAIM_TIME    
                                                         --modify by xiecheng for version up                                           
                                                       --AND OHR.OPE_CATEGORY = 'OperationStart'                                        
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_OPER_PROCESS_START_HS PS ON   LJ.LOT_ID = PS.LOT_ID                                               
                                                             AND  LJ.OPE_NO = PS.OPE_NO                                               
                                                             AND  LJ.MAINPD_ID = OS.MAINPD_ID                                         
                                                             AND  LJ.PRODSPEC_ID = OS.PRODSPEC_ID                                      
                                                             AND  LJ.CTRL_JOB = PS.CTRL_JOB  
      LEFT JOIN APS_MID_WIP_PROCESS_START.APS_MID_WIP_PROCESS_START PST ON W.CTRL_JOB_ID = PST.CTRL_JOB                                          
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_LOT_LOCATION L ON W.LOT_ID = L.LOT_ID                                                           
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_LOCATION_INFO LI ON L.FOUP_STATION_ID = LI.TOOL_ID
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_LOACTION_MANUAL LMI ON L.FOUP_STATION_ID = LMI.TOOL_ID  
       order by W.LOT_ID,                                                                                                     
               W.PRODG_ID,                                                                                                            
               W.PRODG_TECH,                                                                                                          
               W.PRODSPEC_ID,                                                                                                         
...
def InsertWipHoldCode2Temp(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_HC                                                                         
...
def InsertWipMergeHoldCode2Temp(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_MERGE_HC                                                                      
...
def getPreferToolData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
      INSERT INTO {tempdb}APS_TMP_ETL_WIP_PREFER_TOOL (                 
...
def getPreOHBData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
     INSERT INTO {tempdb}APS_TMP_ETL_WIP_PRE_OHB(                    
    EQP_ID,STK_ID                                                  
    )                                                               
     WITH STK_DATA AS (                                                                    
...
def getRetFlowData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT INTO {tempdb}APS_TMP_WIP_RET_FLOW(                
        ...
        LOT_ID, OPE_NO, PRODSPEC_ID, MAINPD_ID                                                   
         ...
         AND NOT EXISTS (SELECT 1 FROM APS_SYNC_CROSS_LOT_CSFRCROSSXFERLOTLIST.APS_SYNC_CROSS_LOT_CSFRCROSSXFERLOTLIST S
              WHERE S.LOT_ID = A.LOT_ID AND S.DIRECTION='Cross') 
         order by A.LOT_ID                                            
        """.format(tempdb=my_duck.get_temp_table_mark())
...
def getRetFlowStepData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
       INSERT INTO {tempdb}APS_TMP_WIP_RET_FLOW_STEP(                                                                   
       TOOLG_ID, RETN_MAINPD_ID, RETN_OPE_NO, RETN_STEP_ID,TARGET_OP_NO, REMAIN_QTIME,
        TARGET_STEP_ID, LOT_ID, OPE_NO, STEP_ID, PRODSPEC_ID, MAINPD_ID,MULTI_GROUP,
        MULTI_TARGET_FLAG                                                                                       
       )                                                                                                      
        SELECT CASE WHEN F.OPE_NO = D.RETN_OPE_NO THEN F.TOOLG_ID END TOOLG_ID,                              
            ...
            CASE WHEN F.OPE_NO = D.RETN_OPE_NO THEN F.MULTI_GROUP END MULTI_GROUP,
            CASE WHEN F.OPE_NO = D.RETN_OPE_NO THEN F.MULTI_TARGET_FLAG END MULTI_TARGET_FLAG                                                                  
        FROM {tempdb}APS_TMP_WIP_RET_FLOW D                                                               
...
def getRetFromStepData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
       INSERT INTO {tempdb}APS_TMP_WIP_RET_FROM_STEP(                                                                   
       TOOLG_ID, RETN_MAINPD_ID, RETN_OPE_NO, RETN_STEP_ID,TARGET_OP_NO, REMAIN_QTIME,
        TARGET_STEP_ID, LOT_ID, OPE_NO, STEP_ID, PRODSPEC_ID, MAINPD_ID, FLOW_STEP_ID,
        MULTI_GROUP, MULTI_TARGET_FLAG                                                                                      
       )                                                                                                      
        WITH REULT_DATA AS (                                                                                                                                                          
              SELECT MAX(TOOLG_ID) AS TOOLG_ID,                                                           
                     ...
                     MAX(MULTI_GROUP) AS MULTI_GROUP, 
                     MAX(MULTI_TARGET_FLAG) AS MULTI_TARGET_FLAG                                                       
              FROM {tempdb}APS_TMP_WIP_RET_FLOW_STEP                                                        
        ...
        FROM_STEP_DATA AS (                                                                                                
              SELECT D.TOOLG_ID,                                                                             
                     D.RETN_MAINPD_ID,D.RETN_OPE_NO,                                                   
                     D.RETN_STEP_ID,D.TARGET_OP_NO,                                                    
                     CASE WHEN D.TARGET_STEP_ID IS NOT NULL                                          
                           THEN D.REMAIN_QTIME END AS REMAIN_QTIME,                                         
                     D.TARGET_STEP_ID,                                                                          
                     D.LOT_ID,D.OPE_NO,D.STEP_ID,                                                                   
                     D.PRODSPEC_ID,D.MAINPD_ID,                                                              
                     F.STEP_ID AS FLOW_STEP_ID,                                                                  
                     ROW_NUMBER() OVER(PARTITION BY F.PROD_ID, F.PLAN_ID ORDER BY F.STEP_ID DESC) AS RNT,
                     D.MULTI_GROUP,
                     D.MULTI_TARGET_FLAG                                                                                  
              FROM REULT_DATA D                                                                                         
              INNER JOIN APS_ETL_FLOW.APS_ETL_FLOW F                                                                                                       
              ON F.PROD_ID = D.PRODSPEC_ID                                                                          
              AND F.PLAN_ID = D.MAINPD_ID                                                                            
              WHERE D.TOOLG_ID IS NOT NULL AND D.TOOLG_ID <> ''                                                                              
        )                                                                                                   
        SELECT TOOLG_ID,RETN_MAINPD_ID,RETN_OPE_NO,                                                         
                RETN_STEP_ID,TARGET_OP_NO,REMAIN_QTIME,                                                      
                TARGET_STEP_ID,LOT_ID,D.OPE_NO,D.STEP_ID,                                                    
                PRODSPEC_ID,D.MAINPD_ID,FLOW_STEP_ID,
                MULTI_GROUP,MULTI_TARGET_FLAG                                                         
        FROM  FROM_STEP_DATA D                                                                              
        WHERE D.RNT = 1                                                                                                               
        """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_WIP_RET_FROM_STEP",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_WIP_RET_FROM_STEP")


def getRetFlowResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
      INSERT INTO {tempdb}APS_TMP_WIP_RET_FLOW_RESULT(                                                                   
        TOOLG_ID, RETN_MAINPD_ID, RETN_OPE_NO, RETN_STEP_ID,TARGET_OP_NO, REMAIN_QTIME,
        TARGET_STEP_ID, LOT_ID, OPE_NO, STEP_ID, PRODSPEC_ID, MAINPD_ID, FLOW_STEP_ID,DZ                                                                                        
       )                                                                                                      
        SELECT TOOLG_ID,                                                                                                                                     
                RETN_MAINPD_ID,RETN_OPE_NO,                                                                
...
def InsertNormalFlowTargetWip2HandleTable(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT INTO /*+ append */ {tempdb}APS_TMP_ETL_WIP_V3 (
    ...
    FROMBATCH_TOOL,TRANS_STATE,FOUP_STATION_ID, PTY_GROUP,conti_flag)                                                                                                                         
          SELECT * FROM (                                                                                                                                               
              SELECT A.*  FROM (                                                                                                                                                    
                  SELECT W.LOT_ID                                                                                                                                       
                        ,W.OPE_SEQ                                                                                                                                      
                        ...
                        ,W.conti_flag                                    
                  FROM  {tempdb}APS_TMP_ETL_WIP W                                                                                                                         
                  LEFT JOIN {tempdb}APS_TMP_ETL_WIP_HC H ON W.LOT_ID = H.LOT_ID AND W.PRODSPEC_ID = H.PRODSPEC_ID AND W.PLAN_ID = H.PLAN_ID AND W.OPE_NO = H.OPE_NO                    
                  WHERE 1=1                                                                                                                                                         
    ...
    sql = """
     INSERT INTO {tempdb}APS_TMP_ETL_WIP_V4 (LOT_ID,STEP_ID,WIP_OPE_NO,TARGET_STEP_ID,TARGET_OPE_NO,TARGET_TOOLG_ID,PRODG_ID,PRODG_TECH,PRODSPEC_ID,PLAN_ID,TARGET_PLAN_ID,LOT_TYPE,
     ...
     FROMBATCH_TOOL,TRANS_STATE,FOUP_STATION_ID,MULTI_TARGET_FLAG,MULTI_GROUP,PTY_GROUP,conti_flag,RN)
         select LOT_ID,STEP_ID,WIP_OPE_NO,TARGET_STEP_ID,TARGET_OPE_NO,TARGET_TOOLG_ID,PRODG_ID,PRODG_TECH,PRODSPEC_ID,PLAN_ID,TARGET_PLAN_ID,LOT_TYPE,
     FOUP_ID,WAFER_QTY,PRIORITY,SHR_FLAG,LOT_STATUS,ARRIVAL,LAYER,STAGE,EQP_ID,RETICLE_ID,RECIPE,PPID,TRACKIN_TIME,PROCESS_START_TIME,TECH_ID,
     CUSTOMER_ID,LOCATION_ID,PARENT_LOT_ID,FLOW_RECIPE,CRITICAL_RATIO,HOLD_CODE,JOB_PREPARE,FAB_PHASE,WHITE_FLAG,BATCH_ID,FROMBATCH_TIME,
     FROMBATCH_TOOL,TRANS_STATE,FOUP_STATION_ID,MULTI_TARGET_FLAG,MULTI_GROUP,PTY_GROUP,conti_flag,RN from (
         SELECT cast(replace(A.TARGET_STEP_ID,'.','') as double) as aaaa, A.*                                                                                                                                                     
            ...
            FROM (                                                                                                                                                      
                     SELECT W.LOT_ID                                                                                                                                    
                           ...
                           ,W.PTY_GROUP
                           ,W.conti_flag                      
                     FROM {tempdb}APS_TMP_ETL_WIP_V3 W                                                                                    
                     INNER JOIN {tempdb}APS_TMP_ETL_WIP_LAST_FLOW TF ON  W.PRODSPEC_ID = TF.PROD_ID                                               
                     ...
                     GROUP BY W.LOT_ID                                                                                                
                           ...
                           ,TF.MULTI_TARGET_FLAG,TF.MULTI_GROUP,W.PTY_GROUP,W.conti_flag                                                                         
             ) A                                                                                                                      
            WHERE A.TARGET_STEP_ID > A.STEP_ID
            order by LOT_ID, aaaa
         )
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
    Insert /*+ append */ Into {tempdb}APS_TMP_HANDLE_WIP
     (
         ...
         MULTI_TARGET_FLAG, MULTI_GROUP, PTY_GROUP, conti_flag  
     )                              
     WITH TOOL_MODULE AS(                                       
          SELECT EQP_G                                          
     ...
     SELECT  DISTINCT '{uuid}' AS PARENTID , WIP.LOT_ID                                                                        
         ...
         ,WIP.PTY_GROUP
         ,wip.conti_flag                                                 
      FROM {tempdb}APS_TMP_ETL_WIP_V4 WIP                                                                                               
      LEFT JOIN {tempdb}APS_TMP_ETL_WIP_UMT_CONTROL_LOT UCL ON   WIP.TARGET_OPE_NO = UCL.OPE_NO                                         
     ...
     --车规产品等级
     --LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PREFER_TOOL PTT ON PTT.TECH = WIP.PRODG_TECH AND PTT.OPE_NO = WIP.TARGET_OPE_NO AND PTT.PRODUCT_ID=WIP.PRODSPEC_ID
     WHERE 1=1                                                                                                                        
     AND WIP.RN < 7                                                                                                                   
     ),                                                                                                                               
     ...
     SELECT  WIP.PARENTID , WIP.LOT_ID                                                                                                
         ...
         ,WIP.MULTI_TARGET_FLAG, WIP.MULTI_GROUP, WIP.PTY_GROUP, wip.conti_flag                                                                                      
     FROM TARGET_DATA WIP                                                                                                             
     WHERE 1=1                                                                                                                        
     AND WIP.RN < 4                                                                                                                   
     UNION ALL                                                                                                                        
...
def InsertOnHandeWip2HandleTable(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
     ...
     INSERT /*+ append */ INTO  {tempdb}APS_TMP_HANDLE_WIP                                                                                                      
       ...
       CRITICAL_RATIO, HOLD_CODE, PRE_TOOL_ID, CREATE_TIME, JOB_PREPARE, FAB_PHASE, WHITE_FLAG, BATCH_ID,FROMBATCH_TIME, FROMBATCH_TOOL, TRANS_STATE, FOUP_STATION_ID,
         ...
         SELECT DISTINCT '{uuid}'                                                                                                                    
          ...
          FROM  {tempdb}APS_TMP_ETL_WIP  W                                                                                                                        
          LEFT JOIN  {tempdb}APS_TMP_ETL_WIP_HC  H ON W.LOT_ID = H.LOT_ID AND W.PRODSPEC_ID = H.PRODSPEC_ID AND W.PLAN_ID = H.PLAN_ID AND W.OPE_NO = H.OPE_NO     
          LEFT JOIN  {tempdb}APS_TMP_ETL_WIP_UMT_CONTROL_LOT  UCL ON   W.TARGET_OPE_NO = UCL.OPE_NO                                                              
        ...
        -- 车规产品等级
      --  LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PREFER_TOOL PTT ON PTT.TECH = W.PRODG_TECH AND PTT.OPE_NO = W.TARGET_OPE_NO AND PTT.PRODUCT_ID=W.PRODSPEC_ID                                                        
    """.format(tempdb=my_duck.get_temp_table_mark(), uuid=uuid, current_time=current_time)
    ##############
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
    ...
    sql = """
        INSERT /*+ append */ INTO {tempdb}APS_TMP_HANDLE_WIP_V1  
           (PARENTID, LOT_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_TOOLG_ID, PRODG_ID, PRODG_TECH, PROD_ID, 
            PLAN_ID, TARGET_PLAN_ID, LOT_TYPE, FOUP_ID, QTY, PTY, SHR_FLAG, LOT_STATUS, ARRIVAL, LAYER, STAGE, TOOL_ID, 
            RETICLE_ID, RECIPE, PPID, TRACK_IN, PROCESS_START, TECH_ID, CUSTOMER, LOCATION, PARENT_LOT, FLOW_RECIPE, 
            CRITICAL_RATIO, HOLD_CODE, PRE_TOOL_ID, CREATE_TIME, JOB_PREPARE, FAB_PHASE, WHITE_FLAG, BATCH_ID,FROMBATCH_TIME, FROMBATCH_TOOL, TRANS_STATE, FOUP_STATION_ID,
            MULTI_TARGET_FLAG, MULTI_GROUP, PTY_GROUP, conti_flag)                                                         
            SELECT PARENTID, LOT_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_TOOLG_ID, PRODG_ID, PRODG_TECH, PROD_ID, 
                    PLAN_ID, TARGET_PLAN_ID, LOT_TYPE, FOUP_ID, QTY, PTY, SHR_FLAG, LOT_STATUS, ARRIVAL, LAYER, STAGE, TOOL_ID, 
                    RETICLE_ID, RECIPE, PPID, TRACK_IN, PROCESS_START, TECH_ID, CUSTOMER, LOCATION, PARENT_LOT, FLOW_RECIPE, 
                    CRITICAL_RATIO, HOLD_CODE, PRE_TOOL_ID, CREATE_TIME, JOB_PREPARE, FAB_PHASE, WHITE_FLAG, BATCH_ID,FROMBATCH_TIME, FROMBATCH_TOOL, TRANS_STATE, FOUP_STATION_ID,
                    MULTI_TARGET_FLAG, MULTI_GROUP, PTY_GROUP, conti_flag                                          
            FROM {tempdb}APS_TMP_HANDLE_WIP S                                                                                               
             """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_HANDLE_WIP_V1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_HANDLE_WIP_V1")

    # 然后删掉之前的APS_TMP_ETL_RLS_WIP
    sql = """      
         TRUNCATE TABLE {tempdb}APS_TMP_HANDLE_WIP                                                                                              
              """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="TRUNCATE TABLE APS_TMP_HANDLE_WIP",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_HANDLE_WIP")

    sql = """
         INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_CROSS_LOT  
             ...
             INNER JOIN {tempdb}APS_TMP_HANDLE_WIP_V1 W               
             ...
             WHERE S.DIRECTION='Cross'                                                                                  
      """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_CROSS_LOT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_CROSS_LOT")
     ...
     INSERT /*+ append */ INTO {tempdb}APS_TMP_HANDLE_WIP  
        (PARENTID, LOT_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_TOOLG_ID, PRODG_ID, PRODG_TECH, PROD_ID, 
         ...
         SELECT PARENTID, LOT_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_TOOLG_ID, PRODG_ID, PRODG_TECH, PROD_ID, 
                 ...
                 CRITICAL_RATIO, HOLD_CODE, PRE_TOOL_ID, CREATE_TIME, JOB_PREPARE, FAB_PHASE, WHITE_FLAG, BATCH_ID,FROMBATCH_TIME, FROMBATCH_TOOL, TRANS_STATE, FOUP_STATION_ID,
                 MULTI_TARGET_FLAG, MULTI_GROUP, PTY_GROUP, conti_flag                                          
         FROM {tempdb}APS_TMP_HANDLE_WIP_V1 S             
         WHERE NOT EXISTS (
            SELECT 1 
            FROM {tempdb}APS_TMP_ETL_WIP_CROSS_LOT L
            WHERE L.LOT_ID = S.LOT_ID AND L.TARGET_STEP_ID <= S.TARGET_STEP_ID
...
def InsertBackUpWip2HandleTable(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
      INSERT /*+ append */ INTO {tempdb}APS_HANDLE_WIP
     (
        PARENTID, LOT_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_TOOLG_ID, PRODG_ID, PRODG_TECH, PROD_ID,
        PLAN_ID, TARGET_PLAN_ID, LOT_TYPE, FOUP_ID, QTY, PTY, SHR_FLAG, LOT_STATUS, ARRIVAL, LAYER, STAGE, TOOL_ID,
        RETICLE_ID, RECIPE, PPID, TRACK_IN, PROCESS_START, TECH_ID, CUSTOMER, LOCATION, PARENT_LOT, FLOW_RECIPE,
      ...
      SELECT W.PARENTID                                                                                                                                   
            ...
            ,W.JOB_PREPARE                                                                                                                                
            ,W.FAB_PHASE, W.WHITE_FLAG, W.BATCH_ID, W.FROMBATCH_TIME, W.FROMBATCH_TOOL, W.TRANS_STATE,W.FOUP_STATION_ID                                   
            ,W.MULTI_TARGET_FLAG, W.MULTI_GROUP, W.PTY_GROUP, W.conti_flag                                                                                                           
       FROM ALL_DATA W                                                                                                                                    
       WHERE W.RN1 = 1   
    """.format(tempdb=my_duck.get_temp_table_mark())
...
def InsertWipPreEqp2Temp(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    WITH RECURSIVE rec(PROD_ID, PLAN_ID, OPE_NO, TARGET_PLAN_ID, TARGET_OPE_NO, CURRENT_LOT, levela) AS (
        SELECT W.PROD_ID         AS PROD_ID,
               ...
               A.LOT_ID          AS CHILD_LOT,
               A.PARENT_LOT_ID   AS PARENT_LOT 
               FROM {tempdb}APS_TMP_ETL_WIP_V A 
               LEFT JOIN {tempdb}APS_HANDLE_WIP W ON A.LOT_ID = W.LOT_ID AND (W.PARENT_LOT IS NOT NULL and W.PARENT_LOT <> '')
               WHERE 1=1
        UNION ALL
    ...
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_SPLIT_LOT
    (PROD_ID, PLAN_ID, OPE_NO, TARGET_PLAN_ID, TARGET_OPE_NO, CURRENT_LOT, SEQ, CHILD_LOT, PARENT_LOT) 
    SELECT DISTINCT                                                                                     
    ...
    FROM rec 
    where rec.PROD_ID IS NOT NULL AND rec.PROD_ID <> ''
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    #  前二层 也同理。
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_APC_PA_V1 (PRODSPEC_ID,PR_OP_NO,OP_NO_1,OP_NO_2) 
     SELECT DISTINCT                                                                                                                                      
         A.PRODSPEC_ID                                                                                                                                    
        ,A.PR_OP_NO                                                                                                                                       
        ,A.OP_NO_1                                                                                                                                        
    ...
    # 把黄光的历史进行合并
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_APC_PH_HIST (LOT_ID,CLAIM_TIME,OPE_NO,EQP_ID ) 
         SELECT LOT_ID,CLAIM_TIME,OPE_NO,EQP_ID                                                                                                                                       
         FROM APS_MID_PH_LOTHISTORY.APS_MID_PH_LOTHISTORY LH1 
         where  LH1.PRODSPEC_ID is not null and LH1.PRODSPEC_ID <> ''                                                                           
         AND LH1.MAINPD_ID is not null  and LH1.MAINPD_ID <> '' 
         UNION ALL
         SELECT LOT_ID,CLAIM_TIME,OPE_NO,EQP_ID                                                                                                                                       
         FROM APS_MID_PH_LOTHISTORY_A3.APS_MID_PH_LOTHISTORY_A3 LH1 
         where  LH1.PRODSPEC_ID is not null and LH1.PRODSPEC_ID <> ''                                                                           
         AND LH1.MAINPD_ID is not null  and LH1.MAINPD_ID <> ''   
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_APC_PH_HIST",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_APC_PH_HIST")

    sql = """
     INSERT INTO {tempdb}APS_TMP_ETL_WIP_PREEQP_V1 (CURRENT_LOT,PROD_ID,PLAN_ID,OPE_NO,TARGET_PLAN_ID,TARGET_OPE_NO,OP_NO_1,PRE_EQP_1_HIS)
       SELECT B.LOT_ID                                                                                                                             	      
            , B.PROD_ID                                                                                                                                   
            , B.PLAN_ID                                                                                                                                   
            , B.OPE_NO                                                                                                                                    
       ...
       FROM (                                                                                                                                             
              SELECT A.LOT_ID                                                                                                                             
                ...
                FROM (SELECT  W.LOT_ID                                                                                                                    
                            ...
                            , LH1.CLAIM_TIME                                                                                                              
                      FROM {tempdb}APS_HANDLE_WIP W                                                                                                       
                      LEFT JOIN {tempdb}APS_TMP_ETL_APC_PA_V1 A ON W.PROD_ID = A.PRODSPEC_ID AND W.TARGET_OPE_NO = A.PR_OP_NO                               
                      LEFT JOIN {tempdb}APS_TMP_ETL_APC_PH_HIST LH1 ON W.LOT_ID = LH1.LOT_ID                                                              
                                                    AND LH1.OPE_NO = A.OP_NO_1                                                                                                                                                
                      WHERE NOT EXISTS (SELECT 1 FROM {tempdb}APS_TMP_ETL_WIP_SPLIT_LOT S WHERE W.LOT_ID = S.CURRENT_LOT
                      order by W.LOT_ID)                                        
                ) A                                                                                                                                       
         ) B                                                                                                                                              
         WHERE B.RN = 1 
    ...
    sql = """
     INSERT INTO {tempdb}APS_TMP_ETL_WIP_PREEQP_V1 (CURRENT_LOT,PROD_ID,PLAN_ID,OPE_NO,TARGET_PLAN_ID,TARGET_OPE_NO,OP_NO_1,PRE_EQP_1_HIS)
       ...
       FROM (                                                                                                                                           
          ...
          , LH1.EQP_ID                                                                                                                        
          , LH1.CLAIM_TIME                                                                                                                    
                    , LAST_VALUE(LH1.EQP_ID IGNORE NULLS) OVER(PARTITION BY A.CURRENT_LOT, A.TARGET_OPE_NO ORDER BY A.SEQ DESC,CAST(LH1.CLAIM_TIME AS TIMESTAMP) 
                                 ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS PRE_TOOL                            
                    , ROW_NUMBER() OVER(PARTITION BY A.CHILD_LOT, A.TARGET_OPE_NO,A.CURRENT_LOT ORDER BY LH1.CLAIM_TIME,A.SEQ ) AS RN                                       
               FROM {tempdb}APS_TMP_ETL_WIP_SPLIT_LOT A                                                                                                        
               LEFT JOIN {tempdb}APS_TMP_ETL_APC_PA_V1 B ON A.PROD_ID = B.PRODSPEC_ID AND A.TARGET_OPE_NO = B.PR_OP_NO                                    
               LEFT JOIN {tempdb}APS_TMP_ETL_APC_PH_HIST LH1 ON LH1.LOT_ID = A.CHILD_LOT                                                                   
         ...
         INSERT INTO {tempdb}APS_TMP_ETL_WIP_PREEQP_V1 (CURRENT_LOT,PROD_ID,PLAN_ID,OPE_NO,TARGET_PLAN_ID,TARGET_OPE_NO,OP_NO_1,PRE_EQP_1_HIS)
           ...
           FROM (                                                                                                                                           
              ...
              , LH1.CLAIM_TIME                                                                                                                    
                        , LAST_VALUE(LH1.EQP_ID IGNORE NULLS) OVER(PARTITION BY A.CURRENT_LOT, A.TARGET_OPE_NO ORDER BY A.SEQ DESC,CAST(LH1.CLAIM_TIME AS TIMESTAMP) 
                                     ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS PRE_TOOL                            
                        , ROW_NUMBER() OVER(PARTITION BY A.PARENT_LOT, A.TARGET_OPE_NO,A.CURRENT_LOT ORDER BY LH1.CLAIM_TIME,A.SEQ ) AS RN                                       
                   FROM {tempdb}APS_TMP_ETL_WIP_SPLIT_LOT A                                                                                                        
                   LEFT JOIN {tempdb}APS_TMP_ETL_APC_PA_V1 B ON A.PROD_ID = B.PRODSPEC_ID AND A.TARGET_OPE_NO = B.PR_OP_NO                                    
                   LEFT JOIN {tempdb}APS_TMP_ETL_APC_PH_HIST LH1 ON LH1.LOT_ID = A.PARENT_LOT                                                                   
                                                 AND LH1.OPE_NO = B.OP_NO_1                                                                                 
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PRE_EQP1                                                                                                       
    (LOT_ID, PROD_ID, PLAN_ID, OPE_NO, TARGET_PLAN_ID, TARGET_OPE_NO, OPE_NO_1, PRE_EQP, EQP_ID_1)                                                           
    SELECT DISTINCT E1.CURRENT_LOT                                                                                                                                    
         , E1.PROD_ID                                                                                                                                        
         , E1.PLAN_ID                                                                                                                                        
         , E1.OPE_NO                                                                                                                                         
         , E1.TARGET_PLAN_ID                                                                                                                                 
         , E1.TARGET_OPE_NO                                                                                                                                  
         , MAX(E1.OP_NO_1) AS OP_NO_1                                                                                                                                         
         , MAX(E1.PRE_EQP_1_HIS) AS PRE_EQP                                                                                                                       
         , MAX(E1.PRE_EQP_1_HIS) AS PRE_EQP_1_HIS                                                                                                                                  
    FROM  {tempdb}APS_TMP_ETL_WIP_PREEQP_V1 E1
    group by                
         E1.CURRENT_LOT                                                                                                                                    
         , E1.PROD_ID                                                                                                                                        
         , E1.PLAN_ID                                                                                                                                        
    ...
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_WIP_PREEQP_V2 (CURRENT_LOT,PROD_ID,PLAN_ID,OPE_NO,TARGET_PLAN_ID,TARGET_OPE_NO,OP_NO_2,PRE_EQP_2_HIS) 
       SELECT B.LOT_ID                                                                                                                             	      
            , B.PROD_ID                                                                                                                                   
            , B.PLAN_ID                                                                                                                                   
            , B.OPE_NO                                                                                                                                    
      ...
      FROM (                                                                                                                                           
          ...
          , LH1.CLAIM_TIME                                                                                                                    
                   , LAST_VALUE(LH1.EQP_ID IGNORE NULLS) OVER(PARTITION BY A.CURRENT_LOT, A.TARGET_OPE_NO ORDER BY A.SEQ DESC                          
                                 ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS PRE_TOOL                            
                   , ROW_NUMBER() OVER(PARTITION BY A.CHILD_LOT, A.TARGET_OPE_NO ORDER BY LH1.CLAIM_TIME,A.SEQ ) AS RN                                       
              FROM {tempdb}APS_TMP_ETL_WIP_SPLIT_LOT A                                                                                                        
              LEFT JOIN {tempdb}APS_TMP_ETL_APC_PA_V1 B ON A.PROD_ID = B.PRODSPEC_ID AND A.TARGET_OPE_NO = B.PR_OP_NO                                    
              LEFT JOIN {tempdb}APS_TMP_ETL_APC_PH_HIST LH1 ON LH1.LOT_ID = A.CHILD_LOT                                                                 
        ...
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_PREEQP_V2 (CURRENT_LOT,PROD_ID,PLAN_ID,OPE_NO,TARGET_PLAN_ID,TARGET_OPE_NO,OP_NO_2,PRE_EQP_2_HIS) 
          ...
          FROM (                                                                                                                                           
              ...
              , LH1.CLAIM_TIME                                                                                                                    
                       , LAST_VALUE(LH1.EQP_ID IGNORE NULLS) OVER(PARTITION BY A.CURRENT_LOT, A.TARGET_OPE_NO ORDER BY A.SEQ DESC                          
                                     ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS PRE_TOOL                            
                       , ROW_NUMBER() OVER(PARTITION BY A.PARENT_LOT, A.TARGET_OPE_NO ORDER BY LH1.CLAIM_TIME,A.SEQ ) AS RN                                       
                  FROM {tempdb}APS_TMP_ETL_WIP_SPLIT_LOT A                                                                                                        
                  LEFT JOIN {tempdb}APS_TMP_ETL_APC_PA_V1 B ON A.PROD_ID = B.PRODSPEC_ID AND A.TARGET_OPE_NO = B.PR_OP_NO                                    
                  LEFT JOIN {tempdb}APS_TMP_ETL_APC_PH_HIST LH1 ON LH1.LOT_ID = A.PARENT_LOT                                                                 
                                                AND LH1.OPE_NO = B.OP_NO_2                                                                                                                                                               
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PRE_EQP2                                                                                                         
     ...
     SELECT DISTINCT E1.CURRENT_LOT                                                                                                                                    
          ...
          , E1.TARGET_PLAN_ID                                                                                                                                 
          , E1.TARGET_OPE_NO                                                                                                                                  
          , MAX(E1.OP_NO_2) AS OP_NO_2                                                                                                                                        
          , MAX(E1.PRE_EQP_2_HIS) AS PRE_EQP                                                                                                                       
          , MAX(E1.PRE_EQP_2_HIS) AS PRE_EQP_2_HIS                                                                                                                                  
     FROM  {tempdb}APS_TMP_ETL_WIP_PREEQP_V2 E1
     group by E1.CURRENT_LOT                                                                                                                                    
          , E1.PROD_ID                                                                                                                                        
          , E1.PLAN_ID                                                                                                                                        
          , E1.OPE_NO                                                                                                                                         
          , E1.TARGET_PLAN_ID                                                                                                                                 
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PREEQP                                                                                                         
     ...
     SELECT E1.LOT_ID, E1.PROD_ID, E1.PLAN_ID, E1.OPE_NO, E1.TARGET_PLAN_ID, E1.TARGET_OPE_NO                                                                   
            ,CASE WHEN (E1.PRE_EQP IS NULL OR E1.PRE_EQP = '') 
                  ...
                  THEN E1.PRE_EQP || (CASE WHEN (E2.PRE_EQP IS NOT NULL and E2.PRE_EQP <> '') THEN ';' || E2.PRE_EQP ELSE '' END)                                    
             END AS PRE_EQP                                                                                                                                     
            ,E1.OPE_NO_1, E2.OPE_NO_2                                                                                                                           
     FROM {tempdb}APS_TMP_ETL_WIP_PRE_EQP1 E1                                                                                                                          
     LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PRE_EQP2 E2 ON E1.LOT_ID = E2.LOT_ID                                                                                            
...
def InsertParentToolFlag2TempTable(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PARENT_TOOL_FLAG_PART3
     ...
     order by FROM_OPE_NO, FROM_LOT, TO_LOT
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PARENT_TOOL_FLAG_PART1
     ...
     FROM {tempdb}APS_TMP_ETL_WIP_PARENT_TOOL_FLAG_PART3 H                                                                                      
     INNER JOIN {tempdb}APS_HANDLE_WIP W ON H.FROM_OPE_NO = W.TARGET_OPE_NO 
     AND W.LOT_ID = H.FROM_LOT  
     UNION ALL
     SELECT W.PARENTID, W.LOT_ID, W.PROD_ID, W.PLAN_ID, W.STEP_ID, W.OPE_NO,                                                            
     W.TARGET_STEP_ID, W.TARGET_OPE_NO, W.TARGET_PLAN_ID, 'Y' AS PARENT_TOOL_FLAG                                                       
     FROM {tempdb}APS_TMP_ETL_WIP_PARENT_TOOL_FLAG_PART3 H                                                                                         
     INNER JOIN {tempdb}APS_HANDLE_WIP W ON H.FROM_OPE_NO = W.TARGET_OPE_NO
     AND W.LOT_ID = H.TO_LOT   
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PARENT_TOOL_FLAG_PART2
     (PARENTID, LOT_ID, PROD_ID, PLAN_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_PLAN_ID, PARENT_TOOL_FLAG)
     SELECT W.PARENTID, W.LOT_ID, W.PROD_ID, W.PLAN_ID, W.STEP_ID, W.OPE_NO,                                                            
     W.TARGET_STEP_ID, W.TARGET_OPE_NO, W.TARGET_PLAN_ID, 'Y' AS PARENT_TOOL_FLAG                                                       
     FROM APS_SYNC_RSSPLIT_INFO.APS_SYNC_RSSPLIT_INFO S                                                                               
     INNER JOIN {tempdb}APS_HANDLE_WIP W ON (S.LOT_MOTHER = W.LOT_ID OR S.LOT_SON = W.LOT_ID)                                         
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_PARENT_TOOL_FLAG                                                                             
     (PARENTID, LOT_ID, PROD_ID, PLAN_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_PLAN_ID, PARENT_TOOL_FLAG)
     SELECT W.PARENTID, W.LOT_ID, W.PROD_ID, W.PLAN_ID, W.STEP_ID, W.OPE_NO,                                                            
     W.TARGET_STEP_ID, W.TARGET_OPE_NO, W.TARGET_PLAN_ID, W.PARENT_TOOL_FLAG                                                            
     FROM {tempdb}APS_TMP_ETL_WIP_PARENT_TOOL_FLAG_PART1 W                                                                                          
     UNION                                                                                                                              
     SELECT W.PARENTID, W.LOT_ID, W.PROD_ID, W.PLAN_ID, W.STEP_ID, W.OPE_NO,                                                            
     W.TARGET_STEP_ID, W.TARGET_OPE_NO, W.TARGET_PLAN_ID, W.PARENT_TOOL_FLAG                                                            
     FROM {tempdb}APS_TMP_ETL_WIP_PARENT_TOOL_FLAG_PART2 W  
    """.format(tempdb=my_duck.get_temp_table_mark())
...
def logFlowWip(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_HANDLE_WIP_LOG
    SELECT * FROM {tempdb}APS_HANDLE_WIP_REPEAT
    """.format(tempdb=my_duck.get_temp_table_mark())
...
def GetRetFlowWipData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT INTO {tempdb}APS_ETL_WIP_HANDLE_RET_FLOW(                                                                  
         PARENTID,LOT_ID,STEP_ID,OPE_NO,TARGET_STEP_ID,TARGET_OPE_NO 
                    ...
                    ,REMAIN_QTIME, QTIME_STEP_ID_TO, PTY_GROUP, conti_flag                                                                                       
         )                                                                                                  
          SELECT W.PARENTID                                                                                                                                    
                 ,W.LOT_ID                                                                                                       
                 ,W.STEP_ID                                                                                                      
                 ...
                 ,W.FAB_PHASE                                                                                                    
                 ,W.TARGET_PROD_ID                                                                                               
                 ,CASE WHEN R.LOT_ID IS NOT NULL and R.LOT_ID <> '' THEN COALESCE(R.FLOW_STEP_ID,W.STEP_ID) ELSE COALESCE(S.FLOW_STEP_ID,W.FROM_STEP_ID) END AS FROM_STEP_ID
                 ,CASE WHEN R.LOT_ID IS NOT NULL and R.LOT_ID <> '' THEN R.RETN_STEP_ID ELSE COALESCE(S.RETN_STEP_ID,W.TO_STEP_ID) END AS TO_STEP_ID                         
                 ,W.WHITE_FLAG                                                                                                   
                 ,W.BATCH_ID                                                                                                     
                 ,W.FROMBATCH_TIME                                                                                               
                 ,W.FROMBATCH_TOOL                                                                                               
                 ,W.TRANS_STATE,W.FOUP_STATION_ID                                                                                
                 ,CASE WHEN R.DZ = 2 THEN S.MULTI_GROUP ELSE W.MULTI_GROUP END MULTI_GROUP
                 ,CASE WHEN R.DZ = 2 THEN S.MULTI_TARGET_FLAG ELSE W.MULTI_TARGET_FLAG END MULTI_TARGET_FLAG                                                                               
                 ,COALESCE(S.REMAIN_QTIME, R.REMAIN_QTIME) AS REMAIN_QTIME                                                                                               
                 ,COALESCE(R.TARGET_STEP_ID,S.TARGET_STEP_ID) AS QTIME_STEP_ID_TO
                 ,W.PTY_GROUP
                 ,W.conti_flag                                                                           
          FROM {tempdb}APS_HANDLE_WIP_REPEAT W                                                                                                 
          LEFT JOIN {tempdb}APS_TMP_WIP_RET_FLOW_RESULT R                                                                                
          ...
          AND CASE WHEN position('DBY' in R.MAINPD_ID) = 0 AND W.TARGET_OPE_NO = R.OPE_NO THEN 1                                       
                   ...
                   ELSE 0 END = 1 
          LEFT JOIN {tempdb}APS_TMP_WIP_RET_FROM_STEP S 
          ON  W.LOT_ID = S.LOT_ID
          order by W.LOT_ID  ,W.STEP_ID   ,W.OPE_NO                      
    """.format(tempdb=my_duck.get_temp_table_mark())
...
def InsertFlowChange(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}TMP_FLOW_ORDER_CHANGE                               
    ...
    # 2023-03-23 将转flow拆分
    sql = """
    INSERT /*+ append */ INTO {tempdb}TMP_APS_HANDLE_WIP
     (
     ...
     (WITH FROM_DATA1 AS                                                                                                                       
      (SELECT A.*,                                                                                                                             
         ...
         FROM {tempdb}APS_HANDLE_WIP A                                                                                                                 
         LEFT JOIN {tempdb}TMP_FLOW_ORDER_CHANGE B                                                                                           
           ON A.LOT_ID = B.OBJECT_ID                                                                                                           
          AND A.PLAN_ID = B.MAINPD_ID                                                                                                          
          AND A.OPE_NO = B.OPE_NO                                                                                                              
         LEFT JOIN {tempdb}APS_TMP_ETL_WIP_LAST_FLOW C                                                                                                 
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}TMP_APS_HANDLE_WIP_V
    (
        PARENTID,LOT_ID,STEP_ID,OPE_NO,TARGET_STEP_ID,TARGET_OPE_NO
        ,TARGET_TOOLG_ID,PRODG_ID,PRODG_TECH,PROD_ID,PLAN_ID
        ,TARGET_PLAN_ID,LOT_TYPE,FOUP_ID,QTY,PTY,SHR_FLAG,LOT_STATUS
        ...
        ,FAB_PHASE,TARGET_PROD_ID,FROM_STEP_ID,TO_STEP_ID,WHITE_FLAG
        ,BATCH_ID,FROMBATCH_TIME,FROMBATCH_TOOL,TRANS_STATE,FOUP_STATION_ID,MULTI_GROUP,MULTI_TARGET_FLAG
        ,PTY_GROUP,conti_flag FROM {tempdb}APS_HANDLE_WIP
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_HANDLE_WIP_REPEAT
     (
        PARENTID,LOT_ID,STEP_ID,OPE_NO,TARGET_STEP_ID,TARGET_OPE_NO
        ,TARGET_TOOLG_ID,PRODG_ID,PRODG_TECH,PROD_ID,PLAN_ID
        ,TARGET_PLAN_ID,LOT_TYPE,FOUP_ID,QTY,PTY,SHR_FLAG,LOT_STATUS
     ...
     select distinct A.parentid,                                                          
          ...
          ,A.PTY_GROUP,A.conti_flag                             
     from {tempdb}TMP_APS_HANDLE_WIP_V A                                                          
     LEFT JOIN {tempdb}TMP_APS_HANDLE_WIP B                                                       
     ON A.LOT_ID = B.LOT_ID                                                               
     ...
     order by A.lot_id,A.step_id,    A.ope_no
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    sql = """
    TRUNCATE TABLE {tempdb}APS_HANDLE_WIP
    """.format(tempdb=my_duck.get_temp_table_mark())
    ...
    INSERT /*+ append */ INTO {tempdb}APS_HANDLE_WIP                       
     (
      ...
      WITH ALL_DATA AS (                                                                                                                                  
            SELECT cast(replace(W.TARGET_STEP_ID,'.','') as double) as aaaa,W.*                                                                                                                                    
            ,ROW_NUMBER() OVER(PARTITION BY W.LOT_ID, W.TARGET_PLAN_ID, W.TARGET_STEP_ID ORDER BY w.from_step_id,W.QTIME_STEP_ID_TO DESC) AS RN1
            FROM  {tempdb}APS_ETL_WIP_HANDLE_RET_FLOW  W      
            order by aaaa                                                                                                        
      )                                                                                                                                                   
      SELECT	W.PARENTID                                        
        ,W.LOT_ID                                                 
        ...
        ,W.REMAIN_QTIME,W.QTIME_STEP_ID_TO
        ,W.PTY_GROUP,W.conti_flag                          
       FROM ALL_DATA W                                            
       WHERE W.RN1 = 1   
       order by W.PARENTID                                        
        ,W.LOT_ID                                                 
...
def getFutureHoldByMeasureData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_WIP_FUTURE_HOLD_MS (                  
     ...
     WITH F_DATE AS(                                                                                   
         ...
         WHERE F.HOLD_TYPE IN ('FutureHold','MergeHold')                                              
          ...
          AND NOT EXISTS (SELECT 1 FROM APS_SYNC_CROSS_LOT_CSFRCROSSXFERLOTLIST.APS_SYNC_CROSS_LOT_CSFRCROSSXFERLOTLIST S
              WHERE S.LOT_ID = W.LOT_ID AND S.DIRECTION='Cross')                                                    
     )                                                                                               
...
def getFutureHoldByMeasureResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
      INSERT INTO {tempdb}APS_TMP_ETL_WIP_FUTURE_HOLD_MR(                                         
         ...
         WITH SOURCE_DATA AS (                                                                                                         
              SELECT DISTINCT D.LOT_ID,F.STEP_ID,                                                  
                     ...
                     D.HOLD_TYPE                                                                   
              FROM {tempdb}APS_TMP_ETL_WIP_FUTURE_HOLD_MS D                                                
              INNER JOIN {tempdb}APS_TMP_ETL_WIP_LAST_FLOW F                                                   
              ON F.PLAN_ID = D.MAINPD_ID                                                           
              AND F.PROD_ID = D.PRODSPEC_ID                                                        
              AND F.OPE_NO = D.OPE_NO                                                              
              INNER JOIN {tempdb}APS_TMP_ETL_WIP_LAST_FLOW F1                                                  
              ON F1.PLAN_ID = D.MAINPD_ID                                                          
              AND F1.PROD_ID = D.PRODSPEC_ID                                                       
              AND F1.OPE_NO = D.WIP_OPE_NO                                                         
         ),                                                                                        
         RESULT_DATA AS (                                                                          
             SELECT cast(replace(D.STEP_ID,'.','') as double) as aaaa,
                    D.LOT_ID,                                                                      
                    D.STEP_ID,                                                                     
                    D.WIP_STEP_ID,                                                                 
                    D.HOLD_TYPE,                                                                   
...
def getVirtQtimeSpecSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
       INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_WIP_CONSTRAINT_V1 (
...
def getAllQtimeSpecSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT  /*+ append */  INTO {tempdb}APS_TMP_ETL_WIP_CONSTRAINT_V2 ( 
               PRODSPEC_ID,FLOW_KEY, FROM_OPE_NO, FROM_OPE_SEQ, TO_OPE_NO,TO_OPE_SEQ,QTIME_LIMIT)  
         ...
         FROM {tempdb}APS_TMP_ETL_WIP_CONSTRAINT_V1 D                                                             
         WHERE D.FROM_OPE_SEQ is not null and D.FROM_OPE_SEQ <> '' AND D.TO_OPE_SEQ is not null  and D.TO_OPE_SEQ <> ''                                  
...
def getPilotExceptionData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_PILOT_EXCEPTION(                                                  
            LOT_ID, TARGET_OPE_NO, EXCEPT_PILOT_FLAG                                                                      
        )                                                                                        
        SELECT DISTINCT W.LOT_ID, W.TARGET_OPE_NO,
               CASE
                   WHEN PE.lot IS NOT NULL AND (
                       (PE.layer_step = '*' AND W.TARGET_OPE_NO LIKE '__.P____') OR
                       (PE.layer_step <> '*' AND PE.layer_step = W.TARGET_OPE_NO)
                   ) THEN 1
                   ELSE 0
               END AS EXCEPT_PILOT_FLAG
        FROM (
            SELECT LOT_ID, TARGET_OPE_NO FROM {tempdb}APS_TMP_ETL_WIP_FUTRUE_HOLD
            UNION
            SELECT LOT_ID, TARGET_OPE_NO FROM {tempdb}APS_TMP_ETL_WIP_ANNOTATION_ALL
        ) W
        LEFT JOIN APS_SYNC_RSPILOT_EXCEPTION.APS_SYNC_RSPILOT_EXCEPTION PE ON W.LOT_ID = PE.lot                                                              
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_PILOT_EXCEPTION",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_PILOT_EXCEPTION")
...
def InsertWipResultTable(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_WIP_CONSTRAINT_V1 (
      PRODSPEC_ID,FLOW_KEY,FROM_OPE_NO,FROM_OPE_SEQ,TO_OPE_NO,TO_OPE_SEQ,QTIME_LIMIT
    )
        SELECT QF.PRODSPEC_ID
               ...
               ----目前只参考内控qtime和fvlot_qtime的qtime spec,不参考该表
               ,9999 as QTIME_LIMIT
         FROM APS_SYNC_QTIME_CONSTRAINT.APS_SYNC_QTIME_CONSTRAINT QF
         LEFT JOIN {tempdb}APS_TMP_ETL_WIP_LAST_FLOW FF ON QF.PRODSPEC_ID = FF.PROD_ID
                                               AND QF.FLOW_KEY = FF.PLAN_ID
                                               AND QF.FROM_OPE_NO = FF.OPE_NO
         LEFT JOIN {tempdb}APS_TMP_ETL_WIP_LAST_FLOW TF ON QF.PRODSPEC_ID = TF.PROD_ID
    ...
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_QTIME_FLOW
     (  PRODSPEC_ID                                                              
       ,FLOW_KEY                                                                 
       ,FROM_OPE_NO                                                              
       ,FROM_OPE_SEQ                                                             
     ...
     SELECT  CD.PRODSPEC_ID                                                      
             ...
             ,CD.QTIME_LIMIT                                                     
     FROM {tempdb}APS_TMP_ETL_WIP_CONSTRAINT_V2 CD                                              
     LEFT JOIN {tempdb}APS_TMP_ETL_WIP_LAST_FLOW CF ON CD.PRODSPEC_ID = CF.PROD_ID           
    ...
    # 插入Assign batch id数据
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_ASSIGN_ABTCH
    ...
    # 拆分两两join 查找QTime
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_WIP_QTIME_RESULT1(                  
     ...
     SELECT  W.LOT_ID                                                                                                                                         
            ...
            ,QF.FLOW_KEY                                                                                                                                   
     FROM {tempdb}APS_HANDLE_WIP W                                                                                                                                 
     INNER JOIN {tempdb}APS_TMP_ETL_WIP_QTIME_FLOW QF ON W.PROD_ID = QF.PRODSPEC_ID AND W.PLAN_ID = QF.FLOW_KEY                                              
    ...
    # 拆分两两join 查找QTime
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_WIP_QTIME_RESULT2 (                  
       ...
       ,OPER_NO,OPER_SEQ,QTIME_LIMIT,TRIGGER_TIME,PRODSPEC_ID
       ,FLOW_KEY,QTIME_FULL
    )                                                                
     SELECT  W.LOT_ID                                                                                                                                         
            ,W.STEP_ID                                                                                                                                        
            ...
            ,W.OPER_NO                                                                                                                        
            ,W.OPER_SEQ
            ,case when cast(LQ.QTIME_SPEC as double) > cast(W.QTIME_LIMIT as double) then w.qtime_limit else LQ.QTIME_SPEC end AS QTIME_LIMIT                                                                                                                                   
            ,LQ.TRIGGER_TIME                                                                                                                                  
            ,W.PRODSPEC_ID                                                                                                                                   
            ,W.FLOW_KEY
            ,case when cast(LQ.QTIME_SPEC as double) > cast(W.QTIME_LIMIT as double) then
                    date_add(CAST(LQ.TRIGGER_TIME AS TIMESTAMP),INTERVAL (CAST(w.qtime_limit AS DECIMAL)*3600) second) else LQ.TARGET_TIME end AS QTIME_FULL                                                                                                                                     
     FROM {tempdb}APS_TMP_ETL_WIP_QTIME_RESULT1 W                                                                                                                
    ...
    # 拆分两两join 查找QTime
    sql = """
    INSERT INTO {tempdb}APS_TMP_ETL_WIP_QTIME_RESULT3 (                  
     ...
     ,MIN_QTIME_LIMIT,TRIGGER_TIME,QTIME_FULL
    )                                                               
     SELECT  W.LOT_ID                                                                                                                                         
            ,W.STEP_ID                                                                                                                                        
            ,W.OPE_NO                                                                                                                                         
            ...
            ,W.TRIGGER_TIME                                                                                                                               
            ,W.QTIME_FULL
      FROM {tempdb}APS_TMP_ETL_WIP_QTIME_RESULT2 W                                                                                                                                 
      INNER JOIN {tempdb}APS_TMP_ETL_WIP_LAST_FLOW F ON W.PRODSPEC_ID = F.PROD_ID                                                                                      
    ...
    # remain_qtime考量capacity係數
    sql = """
       INSERT INTO {tempdb}APS_TMP_ETL_WIP_QTIME_CAPACITY (                  
            PRODSPEC_ID, OPE_NO, CAPACITY_VALUE
       )                                                               
       select DISTINCT F.PRODSPEC_ID,F.OPE_NO,COALESCE(SETTING_VALUE, DEFAULT_VAL, 1) AS CAPACITY_VALUE
       from APS_SYNC_RTD_CAPACITY_SETTING.APS_SYNC_RTD_CAPACITY_SETTING S
       INNER JOIN APS_SYNC_FLOW.APS_SYNC_FLOW F 
       ON S.SCHE_EQP_G  = F.SCHE_EQP_G     
       """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_QTIME_CAPACITY",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_QTIME_CAPACITY")

    sql = """
    INSERT /*+ append */ INTO {tempdb}APS_TMP_ETL_WIP_RESULT                                                                                                                      
     ( 
      --modify by xiecheng for version up
         PARENTID, LOT_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_TOOLG_ID, PRODG_ID, PRODG_TECH, PROD_ID,
        PLAN_ID, TARGET_PLAN_ID, LOT_TYPE, FOUP_ID, QTY, PTY, SHR_FLAG, LOT_STATUS, REMAIN_QTIME, ORG_REMAIN_QTIME, QTIME_STEP_ID_TO,
        ...
        LOCATION, FACTORY, PARENT_LOT, CRITICAL_RATIO, HOLD_CODE, PRE_TOOL_ID, PARENT_TOOL_FLAG, UPDATE_TIME, PARTCODE, 
            TARGET_PROD_ID, FROM_STEP_ID,TO_STEP_ID, WHITE_FLAG, BATCH_ID, ASSIGN_MULTI_ID,FROMBATCH_TIME, ASSIGN_TOOL_ID, TRANS_STATE, 
               FOUP_STATION_ID, INHIBIT_PASS_FLAG, PREOHB_TOOL_ID,QTIME_FULL, PTY_GROUP, conti_flag
     )                                                                                                                                               
        WITH QTIME_RESULT AS (                                                                                                                                                   
            ...
            -- AS REMAIN_QTIME      
             ...
             CASE WHEN TARGET_OPE_NO <> FROM_OPE_NO 
                  THEN  ROUND(CAST(Q.QTIME_LIMIT AS DECIMAL) - round((extract(epoch from(STRPTIME('{current_time}', '%Y-%m-%d %H:%M:%S') - STRPTIME(Q.TRIGGER_TIME, '%Y-%m-%d %H:%M:%S')))) / 3600.0, 2) - 
                        (SUM(CAST(Q.TARGET_OPER_CT AS DECIMAL))
                        OVER(PARTITION BY Q.LOT_ID, Q.CUR_OPE_NO, Q.TARGET_OPE_NO, Q.TARGET_PLAN_ID ORDER BY Q.OPER_SEQ DESC 
                        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) - CAST(Q.TARGET_OPER_CT AS DECIMAL)),2) 
             END AS ORG_REMAIN_QTIME,
             CASE WHEN TARGET_OPE_NO <> FROM_OPE_NO 
                  THEN  ROUND(ROUND(CAST(Q.QTIME_LIMIT AS DECIMAL)  - 
                        (SUM(CAST(Q.TARGET_OPER_CT AS DECIMAL))
                        OVER(PARTITION BY Q.LOT_ID, Q.CUR_OPE_NO, Q.TARGET_OPE_NO, Q.TARGET_PLAN_ID ORDER BY Q.OPER_SEQ DESC 
                        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) - CAST(Q.TARGET_OPER_CT AS DECIMAL)), 2) * CAPACITY_VALUE, 2) 
                        - round((extract(epoch from(STRPTIME('{current_time}', '%Y-%m-%d %H:%M:%S') - STRPTIME(Q.TRIGGER_TIME, '%Y-%m-%d %H:%M:%S')))) / 3600.0, 2)
             END AS REMAIN_QTIME                                                                                                                                                                 
        FROM (                                                                                                                                                              
               SELECT Q.* 
                      ,COALESCE(CAST(C.CAPACITY_VALUE AS DOUBLE), 1) AS CAPACITY_VALUE                                                                                                                                               
                     ,ROW_NUMBER()                                                                                                                                          
                ...
                FROM {tempdb}APS_TMP_ETL_WIP_QTIME_RESULT3 Q   
                LEFT JOIN {tempdb}APS_TMP_ETL_WIP_QTIME_CAPACITY C
                ON C.PRODSPEC_ID = Q.PROD_ID AND C.OPE_NO = Q.TO_OPE_NO                                                                                                                                                            
              ) Q                                                                                                                                                                           
              WHERE Q.RN = 1                                                                                                                                                                
        ),                                                                                                                                                                                  
     -- 对LotStatus做特殊处理：针对MergeHold的处理
      MERGEHOLD_DETAIL_DATE AS (                                                                                    
         ...
         FROM {tempdb}APS_TMP_ETL_WIP_MERGE_HC H                                                            
         INNER JOIN {tempdb}APS_HANDLE_WIP W                                                                                
      ...
      -- 对LotStatus做特殊处理：针对MergeHold的处理
           ...
           --  THEN 'MergeHold' ELSE W.LOT_STATUS END )END)END AS LOT_STATUS  
            ...
            -- ,COALESCE(Q.REMAIN_QTIME, 999) AS REMAIN_QTIME     
              ...
              ,COALESCE(W.REMAIN_QTIME, COALESCE(Q.ORG_REMAIN_QTIME, 999)) AS ORG_REMAIN_QTIME                                                                                                        
            -- ,Q.TO_OPE_SEQ AS QTIME_STEP_ID_TO   
             ...
             --modify by xiecheng for version up                                                                                                                                          
             --,W.RETICLE_ID  
              ,CASE WHEN (W.TRACK_IN IS NOT NULL and W.TRACK_IN <> '') AND W.STEP_ID = W.TARGET_STEP_ID 
            ...
            -- 如果LOCATION在运输中，则先看前层机台的位置
             ...
             ,CASE WHEN (W.LOCATION IS NULL OR W.LOCATION='')AND (E.PRE_EQP IS NOT NULL and E.PRE_EQP <>'') THEN COALESCE((SELECT VLI.LOCATION_ID FROM  {tempdb}APS_TMP_ETL_WIP_LOCATION_INFO VLI                 
             ...
             ,W.PARENT_LOT                                                                                                                                                  
             ,W.CRITICAL_RATIO                                                                                                                                      
             ,CASE WHEN LENGTH(W.HOLD_CODE) > 512 THEN SUBSTRING(W.HOLD_CODE,1,512) ELSE W.HOLD_CODE END AS HOLD_CODE                                                  
             ,E.PRE_EQP                                                                                                                                             
             ,T.PARENT_TOOL_FLAG                                                                                                                                    
            ,'{current_time}'                                                                                                                                 
            ,''
             ...
             ,W.TO_STEP_ID                                                                                                                                          
             ,W.WHITE_FLAG                                                                                                                                          
             --- 判断只有当frombatch_time 有值的情况下才放值
             ,CASE WHEN BRT.CLAIM_TIME IS NOT NULL AND BRT.CLAIM_TIME <>'' THEN W.BATCH_ID END AS BATCH_ID                                                                                                                                            
             ,ABT.ASSIGN_MULTI_ID                                                                                                                                   
       ...
       LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PREEQP E ON W.LOT_ID = E.LOT_ID                                                                                                      
       ...
       LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PARENT_TOOL_FLAG T ON W.PARENTID = T.PARENTID                                                                                          
       ...
       LEFT JOIN {tempdb}APS_TMP_ETL_WIP_ASSIGN_ABTCH ABT ON ABT.LOT_ID = W.LOT_ID                                                                                      
                                                      AND ABT.OPE_NO = W.TARGET_OPE_NO                                                                              
       LEFT JOIN {tempdb}APS_TMP_ETL_WIP_FORMING_RESULT BRT ON BRT.LOT_ID = W.LOT_ID                                                                                      
                                                      AND BRT.BATCH_NAME = W.MULTI_GROUP                                                                              
        LEFT JOIN {tempdb}APS_TMP_ETL_WIP_MORE_RETICLE MRTN  ON MRTN.PROD_ID = W.PROD_ID                                               
                                                     AND MRTN.OPE_NO = W.TARGET_OPE_NO                                             
        LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PRE_OHB POTT ON POTT.STK_ID = W.FOUP_STATION_ID   
    """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    ...
    # 增加PX_BOND凑管逻辑
    insertWipAtrr1Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    # 加入Batch_id
    insertPxBatchIdData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    # 增加量测站点FutrueHold的数据进去
    insertWipResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)


def insertWipAtrr1Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_ATRR1(                                                   
             LOT_ID, OPE_NO, PROD_ID, WIP_ATTR1                                                                     
        )                                                                                        
        select W.LOT_ID,F.OPE_NO,F.PRODSPEC_ID,'Device' AS WIP_ATTR1 
        from APS_SYNC_BONDING_OPE_NO.APS_SYNC_BONDING_OPE_NO F
        INNER JOIN aps_sync_mes_bond_prod_parts.aps_sync_mes_bond_prod_parts P
        ON F.PRODSPEC_ID = P.PRODSPEC_ID
        INNER JOIN APS_SYNC_WIP.APS_SYNC_WIP W
        ON W.MAINPD_ID = F.MAINPD_ID AND W.PRODSPEC_ID = F.PRODSPEC_ID
        UNION
        select W.LOT_ID,F.OPE_NO,F.PRODSPEC_ID,'Carrier' AS WIP_ATTR1 
        from APS_SYNC_BONDING_OPE_NO.APS_SYNC_BONDING_OPE_NO F
        INNER JOIN aps_sync_mes_bond_prod_parts.aps_sync_mes_bond_prod_parts P
        ON F.PRODSPEC_ID = P.PARTS_ID
        INNER JOIN APS_SYNC_WIP.APS_SYNC_WIP W
        ON W.MAINPD_ID = F.MAINPD_ID AND W.PRODSPEC_ID = F.PRODSPEC_ID                                                                
        """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_ATRR1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_ATRR1")


def insertPxBatchIdData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_PX_BATCH_ID(                                                   
             LOT_ID, OPE_NO, PROD_ID, BATCH_ID                                                                     
        )                                                                                        
        SELECT DISTINCT H.LOT_ID,H.OPE_NO,H.PROD_ID,M.BOND_GRP_ID
        FROM APS_SYNC_MES_FSBONDGRP_MAP.APS_SYNC_MES_FSBONDGRP_MAP M
        INNER JOIN {tempdb}APS_TMP_ETL_WIP_ATRR1 H 
        ON ((H.LOT_ID = M.BASE_LOT_ID AND H.WIP_ATTR1 = 'Device')
        OR (H.LOT_ID = M.PLAN_TOP_LOT_ID AND H.WIP_ATTR1 = 'Carrier'))                                                                
        """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_PX_BATCH_ID",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_PX_BATCH_ID")


def insertWipResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    # 抓取
    getPlatFormData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    getIgnorQTData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_FUTRUE_HOLD(                                                   
            PARENTID, LOT_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_TOOLG_ID, PRODG_ID, PRODG_TECH, PROD_ID,
            PLAN_ID, TARGET_PLAN_ID, LOT_TYPE, FOUP_ID, QTY, PTY, SHR_FLAG, LOT_STATUS, REMAIN_QTIME, QTIME_STEP_ID_TO,
            ARRIVAL, LAYER, STAGE, TOOL_ID, RETICLE_ID, RECIPE, PPID, JOB_PREPARE, TRACK_IN, PROCESS_START, TECH_ID, CUSTOMER,
            LOCATION, FACTORY, PARENT_LOT, CRITICAL_RATIO, HOLD_CODE, PRE_TOOL_ID, PARENT_TOOL_FLAG, UPDATE_TIME, PARTCODE, 
            TARGET_PROD_ID, FROM_STEP_ID,TO_STEP_ID, WHITE_FLAG, BATCH_ID, ASSIGN_MULTI_ID,FROMBATCH_TIME, ASSIGN_TOOL_ID, TRANS_STATE, 
            FOUP_STATION_ID, INHIBIT_PASS_FLAG, PREOHB_TOOL_ID,WIP_ATTR1,QTIME_FULL, PLAT_FORM, PTY_GROUP ,conti_flag, qtime_ignore_flag                                                                      
        )                                                                                        
         SELECT R.PARENTID,                                                                     
                R.LOT_ID,                                                                       
                R.STEP_ID,                                                                      
                ...
                R.TO_STEP_ID,                                                                   
                R.WHITE_FLAG,                                                                   
                CASE WHEN BD.LOT_ID IS NOT NULL THEN BD.BATCH_ID ELSE R.BATCH_ID END AS BATCH_ID,                                                                     
                R.ASSIGN_MULTI_ID,                                                              
                ...
                R.PTY_GROUP,
                R.conti_flag,
                IGD.qtime_ignore_flag                                                                                                                                              
         FROM {tempdb}APS_TMP_ETL_WIP_RESULT R                                                      
         LEFT JOIN {tempdb}APS_TMP_ETL_WIP_FUTURE_HOLD_MR M                                             
         ON R.LOT_ID = M.LOT_ID  
         LEFT JOIN {tempdb}APS_TMP_ETL_WIP_ATRR1 A
         ON A.LOT_ID = R.LOT_ID AND A.PROD_ID = R.PROD_ID
         AND A.OPE_NO = R.TARGET_OPE_NO  
         LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PX_BATCH_ID BD
         ON BD.LOT_ID = R.LOT_ID AND BD.PROD_ID = R.PROD_ID
         AND BD.OPE_NO = R.TARGET_OPE_NO 
         LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PLAT_FORM_DATA FD
         ON FD.PRODG1 = R.PRODG_ID                    
         LEFT JOIN {tempdb}APS_TMP_IGNORE_WATER_LINE_LOT IGD
         ON IGD.LOT_ID = R.LOT_ID                                      
        """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_FUTRUE_HOLD",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_FUTRUE_HOLD")

    # 加入注记站点的站点数据
    getAnnotationData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    getAnnotationFlowData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    getAnnotationMAXStepData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    getAnnotationResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    getAnnotationQtimeData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    getAnnotationAllData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    # 合并所有代码
    apsEtlWipData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

    getPilotExceptionData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)


def getIgnorQTData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_IGNORE_WATER_LINE_LOT(                                                   
            LOT_ID, qtime_ignore_flag                                                                      
        )                                                                                        
        SELECT DISTINCT LOT_ID, IGNORE_QTIME_FLAG 
        FROM APS_SYNC_IGNORE_WATER_LINE_LOT.APS_SYNC_IGNORE_WATER_LINE_LOT P
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_IGNORE_WATER_LINE_LOT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_IGNORE_WATER_LINE_LOT")


def getPlatFormData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_PLAT_FORM_DATA(                                                   
            PRODG1, PLAT_FORM                                                                      
        )                                                                                        
        SELECT  P.PRODG1,MAX(P.PLAT_FORM) AS PLAT_FORM 
        FROM APS_SYNC_PRODUCT_PLATFORM.APS_SYNC_PRODUCT_PLATFORM P
        GROUP BY PRODG1                                                              
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_PLAT_FORM_DATA",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_PLAT_FORM_DATA")


def getAnnotationData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_ANNOTATION_DATA(                                                   
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_ANNOTATION_DATA",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_ANNOTATION_DATA")


def getAnnotationFlowData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_ANNOTATION_FLOW(                                                   
            LOT_ID, MAINPD_ID, OPE_NO, ANNOTATION_FLAG, PRODSPEC_ID, STEP_ID                                                                       
        )                                                                                        
         SELECT B.LOT_ID,B.MAINPD_ID,                                                 
                B.OPE_NO,B.ANNOTATION_FLAG,                                           
                B.PRODSPEC_ID,F.STEP_ID                                               
         FROM {tempdb}APS_TMP_ETL_WIP_ANNOTATION_DATA B                                              
         INNER JOIN APS_ETL_FLOW.APS_ETL_FLOW F                           
         ON 1=1                        
         AND F.PROD_ID = B.PRODSPEC_ID                                                
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_ANNOTATION_FLOW",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_ANNOTATION_FLOW")


def getAnnotationMAXStepData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_ANNOTATION_MAX_STEP(                                                   
            TARGET_PLAN_ID, PROD_ID, LOT_ID, MAX_STEP_ID, MIN_STEP_ID                                                                       
        )                                                                                        
        SELECT TARGET_PLAN_ID,PROD_ID,W.LOT_ID,                            
               MAX(W.TARGET_STEP_ID) AS MAX_STEP_ID,                           
               MIN(W.TARGET_STEP_ID) AS MIN_STEP_ID                            
        FROM {tempdb}APS_TMP_ETL_WIP_FUTRUE_HOLD W                                    
        INNER JOIN {tempdb}APS_TMP_ETL_WIP_ANNOTATION_FLOW S                          
        ON S.MAINPD_ID = W.TARGET_PLAN_ID                                  
        AND S.PRODSPEC_ID = W.PROD_ID                                      
        AND S.LOT_ID = W.LOT_ID                                            
        GROUP BY W.TARGET_PLAN_ID,W.PROD_ID,W.LOT_ID,W.STEP_ID                                                         
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_ANNOTATION_MAX_STEP",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_ANNOTATION_MAX_STEP")


def getAnnotationResultData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_ANNOTATION_RESULT(                                                   
            LOT_ID, PROD_ID, PLAN_ID, STEP_ID, ENTRY_STEP_ID, RETN_STEP_ID, MAIN_PLAN_ID, 
            OPE_NO, TOOLG_ID                                                                      
        )                                                                                        
         WITH BRANCH_DATA AS (                                                             
               SELECT W.LOT_ID,W.TARGET_PLAN_ID,W.MIN_STEP_ID,W.MAX_STEP_ID,W.PROD_ID          
               FROM {tempdb}APS_TMP_ETL_WIP_ANNOTATION_MAX_STEP W                                          
               INNER JOIN {tempdb}APS_TMP_ETL_WIP_ANNOTATION_FLOW S                                       
               ON S.MAINPD_ID = W.TARGET_PLAN_ID                                               
               AND S.PRODSPEC_ID = W.PROD_ID                                                   
               AND S.LOT_ID = W.LOT_ID                                                         
         ...
         SELECT  D.LOT_ID,F.PROD_ID,F.PLAN_ID,F.STEP_ID,                                    
         ...
         AND F.TOOLG_TYPE = 'P' AND F.MAIN_PLAN_ID IS NOT NULL  AND F.MAIN_PLAN_ID  <> ''                                                          
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_ANNOTATION_RESULT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_ANNOTATION_RESULT")


def getAnnotationQtimeData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_ANNOTATION_QTIME(                                                   
            LOT_ID, PROD_ID, PLAN_ID, STEP_ID, ENTRY_STEP_ID, RETN_STEP_ID, 
            MAIN_PLAN_ID, OPE_NO, TOOLG_ID,REMAIN_QTIME,ORG_REMAIN_QTIME,QTIME_FULL                                                                    
        )                                                                                        
        SELECT DISTINCT F.LOT_ID,F.PROD_ID,F.PLAN_ID,F.STEP_ID,                              
                 F.ENTRY_STEP_ID,F.RETN_STEP_ID,F.MAIN_PLAN_ID,                                       
                 F.OPE_NO,F.TOOLG_ID,                                                                 
                 COALESCE(D.REMAIN_QTIME,999) AS REMAIN_QTIME,
                 COALESCE(D.ORG_REMAIN_QTIME,999) AS ORG_REMAIN_QTIME,
                 D.QTIME_FULL                                              
        FROM {tempdb}APS_TMP_ETL_WIP_ANNOTATION_RESULT F                                                
        LEFT JOIN {tempdb}APS_TMP_ETL_WIP_FUTRUE_HOLD D                                                 
        ON F.MAIN_PLAN_ID = D.TARGET_PLAN_ID                                                 
        AND F.PROD_ID = D.PROD_ID                                                            
        AND F.RETN_STEP_ID = D.TARGET_STEP_ID                                                
        AND F.LOT_ID = D.LOT_ID                                                          
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_ANNOTATION_QTIME",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_ANNOTATION_QTIME")


def getAnnotationAllData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO {tempdb}APS_TMP_ETL_WIP_ANNOTATION_ALL(                                                   
            PARENTID, LOT_ID, STEP_ID, OPE_NO, TARGET_STEP_ID, TARGET_OPE_NO, TARGET_TOOLG_ID, PRODG_ID, PRODG_TECH, PROD_ID,
            PLAN_ID, TARGET_PLAN_ID, LOT_TYPE, FOUP_ID, QTY, PTY, SHR_FLAG, LOT_STATUS, REMAIN_QTIME, ORG_REMAIN_QTIME,QTIME_STEP_ID_TO, 
            ARRIVAL, LAYER, STAGE, TOOL_ID, RETICLE_ID, RECIPE, PPID, JOB_PREPARE, TRACK_IN, PROCESS_START, TECH_ID, CUSTOMER,  
            LOCATION, FACTORY, PARENT_LOT, CRITICAL_RATIO, HOLD_CODE, PRE_TOOL_ID, PARENT_TOOL_FLAG, UPDATE_TIME, PARTCODE, 
            TARGET_PROD_ID, FROM_STEP_ID,TO_STEP_ID, WHITE_FLAG, BATCH_ID, ASSIGN_MULTI_ID,FROMBATCH_TIME, ASSIGN_TOOL_ID, TRANS_STATE, 
            FOUP_STATION_ID, INHIBIT_PASS_FLAG, PREOHB_TOOL_ID, SWITCH_FLAG, ENTRY_STEP_ID, RETN_STEP_ID, MAIN_PLAN_ID,WIP_ATTR1,
            QTIME_FULL, PLAT_FORM, PTY_GROUP, conti_flag ,  qtime_ignore_flag                                                            
        )                                                                                        
         WITH WIP_DATA AS (                                                                                     
             SELECT R.*,                                                                                        
                 ROW_NUMBER() OVER(PARTITION BY R.LOT_ID,R.STEP_ID ORDER BY R.TARGET_STEP_ID DESC) AS RTN            
             FROM {tempdb}APS_TMP_ETL_WIP_ANNOTATION_QTIME Q                                                               
             INNER JOIN {tempdb}APS_TMP_ETL_WIP_FUTRUE_HOLD R                                                              
             ON Q.LOT_ID = R.LOT_ID                                                                             
         )                                                                                                      
         SELECT R.PARENTID,                                                                                     
                 R.LOT_ID,                                                                                      
                 R.STEP_ID,                                                                                     
                 R.OPE_NO,                                                                                      
                 Q.STEP_ID AS TARGET_STEP_ID,                                                                   
                 Q.OPE_NO AS TARGET_OPE_NO,                                                                     
                 Q.TOOLG_ID AS TARGET_TOOLG_ID,                                                                 
                 R.PRODG_ID,                                                                                    
                 R.PRODG_TECH,                                                                                  
                 R.PROD_ID,                                                                                     
                 R.PLAN_ID,                                                                                     
                 Q.PLAN_ID AS TARGET_PLAN_ID,                                                                   
                 R.LOT_TYPE,                                                                                    
                 R.FOUP_ID,                                                                                     
                 R.QTY, PTY,                                                                                    
                 R.SHR_FLAG,                                                                                    
                 ...
                 R.PREOHB_TOOL_ID,                                                                              
                 '1' AS SWITCH_FLAG,                                                                            
                 Q.ENTRY_STEP_ID,Q.RETN_STEP_ID,Q.MAIN_PLAN_ID,
                 R.WIP_ATTR1,
                 Q.QTIME_FULL,
                 R.PLAT_FORM,
                 R.PTY_GROUP,
                 ...
                 IGD.qtime_ignore_flag                                              
         FROM {tempdb}APS_TMP_ETL_WIP_ANNOTATION_QTIME Q                                                                   
         INNER JOIN WIP_DATA R                                                                                  
         ON R.LOT_ID = Q.LOT_ID                                                                                 
         AND RTN = 1         
         LEFT JOIN {tempdb}APS_TMP_IGNORE_WATER_LINE_LOT IGD
         ON IGD.LOT_ID = R.LOT_ID                                                                                                                                                       
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_ETL_WIP_ANNOTATION_ALL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_ETL_WIP_ANNOTATION_ALL")


def apsEtlWipData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT INTO APS_ETL_WIP(                                                   
            ...
            LOCATION, FACTORY, PARENT_LOT, CRITICAL_RATIO, HOLD_CODE, PRE_TOOL_ID, PARENT_TOOL_FLAG, UPDATE_TIME, PARTCODE,
            TARGET_PROD_ID, FROM_STEP_ID,TO_STEP_ID, WHITE_FLAG, BATCH_ID, ASSIGN_MULTI_ID,FROMBATCH_TIME, ASSIGN_TOOL_ID, TRANS_STATE, 
            FOUP_STATION_ID, INHIBIT_PASS_FLAG, PREOHB_TOOL_ID, SWITCH_FLAG, WIP_ATTR1, QTIME_FULL, WIP_ATTR2, PTY_GROUP, conti_flag , qtime_ignore_flag ,EXCEPT_PILOT_FLAG                                                           
        )                                                                                        
        SELECT DISTINCT R.PARENTID,                                                                    
                R.LOT_ID,                                                                      
                R.STEP_ID,                                                                     
                R.OPE_NO,                                                                      
                R.TARGET_STEP_ID,                                                              
                ...
                R.TRACK_IN,                                                                    
                R.PROCESS_START,                                                               
                CASE WHEN R.TECH_ID='' THEN NULL ELSE R.TECH_ID END AS TECH_ID,                                                                     
                R.CUSTOMER,                                                                    
                R.LOCATION,                                                                    
                R.FACTORY,                                                                     
                CASE WHEN R.PARENT_LOT='' THEN NULL ELSE R.PARENT_LOT END AS PARENT_LOT,                                                                  
                R.CRITICAL_RATIO,                                                              
                R.HOLD_CODE,                                                                   
                R.PRE_TOOL_ID,                                                                 
                R.PARENT_TOOL_FLAG,                                                            
                ...
                R.FROM_STEP_ID,                                                                
                R.TO_STEP_ID,                                                                  
                CASE WHEN R.WHITE_FLAG='' THEN NULL ELSE R.WHITE_FLAG END AS WHITE_FLAG,                                                                  
                CASE WHEN R.BATCH_ID='' THEN NULL ELSE R.BATCH_ID END AS BATCH_ID,                                                                         
                R.ASSIGN_MULTI_ID,                                                             
                R.FROMBATCH_TIME,                                                              
                R.ASSIGN_TOOL_ID,                                                              
                R.TRANS_STATE,                                                                 
                ...
                R.INHIBIT_PASS_FLAG,                                                           
                R.PREOHB_TOOL_ID,                                                              
                CASE WHEN EXISTS (SELECT 1 FROM {tempdb}APS_TMP_ETL_WIP_ANNOTATION_ALL B WHERE             
                     B.LOT_ID = R.LOT_ID AND B.MAIN_PLAN_ID = R.TARGET_PLAN_ID)                 
                        THEN '1' END AS SWITCH_FLAG,
                R.WIP_ATTR1,
                CASE WHEN R.REMAIN_QTIME <> 999 THEN R.QTIME_FULL END AS QTIME_FULL,
                R.PLAT_FORM,
                R.PTY_GROUP,
                R.conti_flag,   
                R.qtime_ignore_flag,
                COALESCE(PE.EXCEPT_PILOT_FLAG, 0) AS EXCEPT_PILOT_FLAG                
         FROM {tempdb}APS_TMP_ETL_WIP_FUTRUE_HOLD R  
         LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PILOT_EXCEPTION PE
         ON PE.LOT_ID = R.LOT_ID AND PE.TARGET_OPE_NO = R.TARGET_OPE_NO         
         WHERE NOT EXISTS (                                                                    
            SELECT 1 FROM {tempdb}APS_TMP_ETL_WIP_ANNOTATION_ALL A                                        
            WHERE A.LOT_ID = R.LOT_ID AND A.MAIN_PLAN_ID = R.TARGET_PLAN_ID                    
            AND R.TARGET_STEP_ID > A.ENTRY_STEP_ID AND R.TARGET_STEP_ID < A.RETN_STEP_ID       
         )                                                                                     
         UNION ALL                                                                             
         SELECT DISTINCT R.PARENTID,                                                                    
                R.LOT_ID,                                                                      
                ...
                CASE WHEN R.BATCH_ID='' THEN NULL ELSE R.BATCH_ID END AS BATCH_ID,                                                                    
                R.ASSIGN_MULTI_ID,                                                             
                CASE WHEN R.FROMBATCH_TIME='' THEN NULL ELSE R.FROMBATCH_TIME END AS FROMBATCH_TIME,                                                              
                R.ASSIGN_TOOL_ID,                                                              
                R.TRANS_STATE,                                                                 
                R.FOUP_STATION_ID,                                                             
                R.INHIBIT_PASS_FLAG,                                                           
                ...
                R.qtime_ignore_flag,
                COALESCE(PE.EXCEPT_PILOT_FLAG, 0) AS EXCEPT_PILOT_FLAG                   
         FROM {tempdb}APS_TMP_ETL_WIP_ANNOTATION_ALL R                   
         LEFT JOIN {tempdb}APS_TMP_ETL_WIP_PILOT_EXCEPTION PE
         ON PE.LOT_ID = R.LOT_ID AND PE.TARGET_OPE_NO = R.TARGET_OPE_NO           
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
...
def SummaryOnhandAndTragetWip(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    # 找到对应的From_Step_id
    getRetFromStepData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
    # 针对特殊Flow Lot回归站点需求
