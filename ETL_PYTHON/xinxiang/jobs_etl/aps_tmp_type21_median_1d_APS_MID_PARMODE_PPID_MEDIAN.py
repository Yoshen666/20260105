...
def create_temp_table(duck_db):
    ...
    '''
    sql = """
        create {table_type} table APS_TMP_PARMODE_PPID_MEDIAN_TOOL
        (
          tool_id  VARCHAR(64),
          MINOR_CH    VARCHAR(64)
        )
    ...
    sql = """
       create {table_type} table APS_TMP_PARMODE_PPID_MEDIAN_CH_COUNT
       (
         CTRL_JOB  VARCHAR(64),
         CH_COUNT    VARCHAR(64)
       )
...
def GetMinorChFlagData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_PARMODE_PPID_MEDIAN_TOOL (                                
           TOOL_ID, MINOR_CH
        )                                                                                      
        SELECT TOOL_ID, 
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_TMP_PARMODE_PPID_MEDIAN_TOOL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PARMODE_PPID_MEDIAN_TOOL")

def GetChCountData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO {tempdb}APS_TMP_PARMODE_PPID_MEDIAN_CH_COUNT (                                
           CTRL_JOB, CH_COUNT
        )                                                                                      
        SELECT CTRL_JOB, 
               COUNT(*) AS CH_COUNT 
        ...
        AND NOT EXISTS (SELECT 1 FROM {tempdb}APS_TMP_PARMODE_PPID_MEDIAN_TOOL L 
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_PARMODE_PPID_MEDIAN_CH_COUNT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_PARMODE_PPID_MEDIAN_CH_COUNT")

def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO APS_MID_PARMODE_PPID_MEDIAN (                                
       PARENTID, PPID, PT, UPDATE_TIME, PARTCODE
    )                                                                                      
    WITH TYPE21 AS (                                                                      
          ...
          FROM APS_TR_TYPE21_PARMODE.APS_TR_TYPE21_PARMODE T  
          INNER JOIN {tempdb}APS_TMP_PARMODE_PPID_MEDIAN_CH_COUNT C
    ...
    NUM_DATA AS (
        SELECT ROUND((25/CAST(QTY AS DECIMAL))*PT*COUNT, 3) AS PT,
               ...
               PPID,QTY,
               COUNT,
               ROW_NUMBER() OVER(PARTITION BY PPID ORDER BY PROCESS_END DESC) AS RN
        FROM TYPE21
    ),
    COUNT_DATA AS (
        SELECT PT,
    ...
    PT_DATA AS (                                                                          
         SELECT 
              QUANTILE_CONT(CAST(PT AS DECIMAL),0.5) OVER(PARTITION BY PPID ) AS MAX_50
             ,PPID, RN                                            
         FROM COUNT_DATA                                                                  
    ),                                                                                    
    RESULT_DATA AS (                                                                      
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_MID_PARMODE_PPID_MEDIAN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_PARMODE_PPID_MEDIAN")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_TYPE21_MEDIAN_1D_APS_MID_PARMODE_PPID_MEDIAN"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_PARMODE_PPID_MEDIAN"
    used_table_list = ['APS_TR_TYPE21_PARMODE', 'APS_ETL_TOOL']
    target_table_sql = """
        create table {}APS_MID_PARMODE_PPID_MEDIAN
        (
