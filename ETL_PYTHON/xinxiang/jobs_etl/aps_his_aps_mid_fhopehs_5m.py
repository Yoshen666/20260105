...
def execute():
    ...
    sync_day = 25
    # 第一次用sqluld 不能换行
    query_sql = """SELECT LOT_ID, LOT_TYPE, SUB_LOT_TYPE, CAST_ID, CAST_CATEGORY, MAINPD_ID, OPE_NO, PD_ID, CASE WHEN OPE_PASS_COUNT IS NULL THEN 0 ELSE OPE_PASS_COUNT END AS OPE_PASS_COUNT, PD_NAME, HOLD_STATE, CLAIM_TIME, CASE WHEN CLAIM_SHOP_DATE IS NULL THEN 0 ELSE CLAIM_SHOP_DATE END AS CLAIM_SHOP_DATE, CLAIM_USER_ID, MOVE_TYPE, OPE_CATEGORY, PROD_TYPE, TEST_TYPE, MFG_LAYER, CASE WHEN EXT_PRIORITY IS NULL THEN 0 ELSE EXT_PRIORITY END AS EXT_PRIORITY, CASE WHEN PRIORITY_CLASS IS NULL THEN 0 ELSE PRIORITY_CLASS END AS PRIORITY_CLASS, PREV_PRODSPEC_ID, PRODSPEC_ID, PRODGRP_ID, TECH_ID, CUSTOMER_ID, CUSTPROD_ID, ORDER_NO, STAGE_ID, STAGEGRP_ID, PHOTO_LAYER, LOCATION_ID, AREA_ID, EQP_ID, REPLACE(REPLACE(EQP_NAME, chr(10), ''), chr(13), '')  AS EQP_NAME, OPE_MODE, LC_RECIPE_ID, RECIPE_ID, PH_RECIPE_ID, CASE WHEN RETICLE_COUNT IS NULL THEN 0 ELSE RETICLE_COUNT END AS RETICLE_COUNT, CASE WHEN FIXTURE_COUNT IS NULL THEN 0 ELSE FIXTURE_COUNT END AS FIXTURE_COUNT, CASE WHEN RPARM_COUNT IS NULL THEN 0 ELSE RPARM_COUNT END AS RPARM_COUNT, CASE WHEN INIT_HOLD_FLAG IS NULL THEN 0 ELSE INIT_HOLD_FLAG END AS INIT_HOLD_FLAG, CASE WHEN LAST_HLDREL_FLAG IS NULL THEN 0 ELSE LAST_HLDREL_FLAG END AS LAST_HLDREL_FLAG, HOLD_TIME, CASE WHEN HOLD_SHOP_DATE IS NULL THEN 0 ELSE HOLD_SHOP_DATE END AS HOLD_SHOP_DATE, HOLD_USER_ID, HOLD_TYPE, HOLD_REASON_CODE, HOLD_REASON_DESC, REASON_CODE, REASON_DESCRIPTION, BANK_ID, PREV_BANK_ID, PREV_MAINPD_ID, PREV_OPE_NO, PREV_PD_ID, PREV_PD_NAME, CASE WHEN PREV_PASS_COUNT IS NULL THEN 0 ELSE PREV_PASS_COUNT END AS PREV_PASS_COUNT, PREV_STAGE_ID, PREV_STAGEGRP_ID, PREV_PHOTO_LAYER, FLOWBATCH_ID, CTRL_JOB, CASE WHEN REWORK_COUNT IS NULL THEN 0 ELSE REWORK_COUNT END AS REWORK_COUNT, CASE WHEN ORG_WAFER_QTY IS NULL THEN 0 ELSE ORG_WAFER_QTY END AS ORG_WAFER_QTY, CASE WHEN CUR_WAFER_QTY IS NULL THEN 0 ELSE CUR_WAFER_QTY END AS CUR_WAFER_QTY, CASE WHEN PROD_WAFER_QTY IS NULL THEN 0 ELSE PROD_WAFER_QTY END AS PROD_WAFER_QTY, CASE WHEN CNTL_WAFER_QTY IS NULL THEN 0 ELSE CNTL_WAFER_QTY END AS CNTL_WAFER_QTY, CASE WHEN CLAIM_PROD_QTY IS NULL THEN 0 ELSE CLAIM_PROD_QTY END AS CLAIM_PROD_QTY, CASE WHEN CLAIM_CNTL_QTY IS NULL THEN 0 ELSE CLAIM_CNTL_QTY END AS CLAIM_CNTL_QTY, CASE WHEN TOTAL_GOOD_UNIT IS NULL THEN 0 ELSE TOTAL_GOOD_UNIT END AS TOTAL_GOOD_UNIT, CASE WHEN TOTAL_FAIL_UNIT IS NULL THEN 0 ELSE TOTAL_FAIL_UNIT END AS TOTAL_FAIL_UNIT, LOT_OWNER_ID, PLAN_END_TIME, WFRHS_TIME, CASE WHEN CRITERIA_FLAG IS NULL THEN 0 ELSE CRITERIA_FLAG END AS CRITERIA_FLAG, REPLACE(REPLACE(CLAIM_MEMO, chr(10), ''), chr(13), '')  AS CLAIM_MEMO, STORE_TIME, ORIG_LOT_ID, GET_FROM, FAB_ID, EVENT_CREATE_TIME, HOLD_OPE_NO, HOLD_REASON_OPE_NO, ORIGINAL_FAB_ID, DESTINATION_FAB_ID, RELATED_LOT_ID, BOND_GRP_ID, EQPMONJOB_ID, PD_TYPE, PREV_PD_TYPE FROM V_FHOPEHS"""
    create_table_sql = """
    ...
    try:
        ...
        if last_file_name:
            ...
            if _create_time_date is not None:
                ...
                query_duck_sql = """
                    select LOT_ID, LOT_TYPE, SUB_LOT_TYPE, CAST_ID, CAST_CATEGORY, MAINPD_ID, OPE_NO, PD_ID, 
                           CASE WHEN OPE_PASS_COUNT IS NULL THEN 0 ELSE OPE_PASS_COUNT END AS OPE_PASS_COUNT, 
                           PD_NAME, HOLD_STATE, CLAIM_TIME, 
                           CASE WHEN CLAIM_SHOP_DATE IS NULL THEN 0 ELSE CLAIM_SHOP_DATE END AS CLAIM_SHOP_DATE, CLAIM_USER_ID, MOVE_TYPE, OPE_CATEGORY, PROD_TYPE, TEST_TYPE, MFG_LAYER, 
                           CASE WHEN EXT_PRIORITY IS NULL THEN 0 ELSE EXT_PRIORITY END AS EXT_PRIORITY, 
                           CASE WHEN PRIORITY_CLASS IS NULL THEN 0 ELSE PRIORITY_CLASS END AS PRIORITY_CLASS, PREV_PRODSPEC_ID, PRODSPEC_ID, PRODGRP_ID, TECH_ID, CUSTOMER_ID, CUSTPROD_ID, ORDER_NO, STAGE_ID, STAGEGRP_ID, PHOTO_LAYER, LOCATION_ID, AREA_ID, EQP_ID, REPLACE(REPLACE(EQP_NAME, chr(10), ''), chr(13), '')  AS EQP_NAME, OPE_MODE, LC_RECIPE_ID, RECIPE_ID, PH_RECIPE_ID, 
                           CASE WHEN RETICLE_COUNT IS NULL THEN 0 ELSE RETICLE_COUNT END AS RETICLE_COUNT, 
                           CASE WHEN FIXTURE_COUNT IS NULL THEN 0 ELSE FIXTURE_COUNT END AS FIXTURE_COUNT, 
                           CASE WHEN RPARM_COUNT IS NULL THEN 0 ELSE RPARM_COUNT END AS RPARM_COUNT, 
                           CASE WHEN INIT_HOLD_FLAG IS NULL THEN 0 ELSE INIT_HOLD_FLAG END AS INIT_HOLD_FLAG, 
                           CASE WHEN LAST_HLDREL_FLAG IS NULL THEN 0 ELSE LAST_HLDREL_FLAG END AS LAST_HLDREL_FLAG, 
                           HOLD_TIME, 
                           CASE WHEN HOLD_SHOP_DATE IS NULL THEN 0 ELSE HOLD_SHOP_DATE END AS HOLD_SHOP_DATE, HOLD_USER_ID, HOLD_TYPE, HOLD_REASON_CODE, HOLD_REASON_DESC, REASON_CODE, REASON_DESCRIPTION, BANK_ID, PREV_BANK_ID, PREV_MAINPD_ID, PREV_OPE_NO, PREV_PD_ID, PREV_PD_NAME, 
                           CASE WHEN PREV_PASS_COUNT IS NULL THEN 0 ELSE PREV_PASS_COUNT END AS PREV_PASS_COUNT, PREV_STAGE_ID, PREV_STAGEGRP_ID, PREV_PHOTO_LAYER, FLOWBATCH_ID, CTRL_JOB, 
                           CASE WHEN REWORK_COUNT IS NULL THEN 0 ELSE REWORK_COUNT END AS REWORK_COUNT, 
                           CASE WHEN ORG_WAFER_QTY IS NULL THEN 0 ELSE ORG_WAFER_QTY END AS ORG_WAFER_QTY, 
                           CASE WHEN CUR_WAFER_QTY IS NULL THEN 0 ELSE CUR_WAFER_QTY END AS CUR_WAFER_QTY, 
                           CASE WHEN PROD_WAFER_QTY IS NULL THEN 0 ELSE PROD_WAFER_QTY END AS PROD_WAFER_QTY, 
                           CASE WHEN CNTL_WAFER_QTY IS NULL THEN 0 ELSE CNTL_WAFER_QTY END AS CNTL_WAFER_QTY, 
                           CASE WHEN CLAIM_PROD_QTY IS NULL THEN 0 ELSE CLAIM_PROD_QTY END AS CLAIM_PROD_QTY, 
                           CASE WHEN CLAIM_CNTL_QTY IS NULL THEN 0 ELSE CLAIM_CNTL_QTY END AS CLAIM_CNTL_QTY, 
                           CASE WHEN TOTAL_GOOD_UNIT IS NULL THEN 0 ELSE TOTAL_GOOD_UNIT END AS TOTAL_GOOD_UNIT, 
                           CASE WHEN TOTAL_FAIL_UNIT IS NULL THEN 0 ELSE TOTAL_FAIL_UNIT END AS TOTAL_FAIL_UNIT, LOT_OWNER_ID, PLAN_END_TIME, WFRHS_TIME, 
                           CASE WHEN CRITERIA_FLAG IS NULL THEN 0 ELSE CRITERIA_FLAG END AS CRITERIA_FLAG, REPLACE(REPLACE(CLAIM_MEMO, chr(10), ''), chr(13), '')  AS CLAIM_MEMO, STORE_TIME, ORIG_LOT_ID, GET_FROM, FAB_ID, EVENT_CREATE_TIME, HOLD_OPE_NO, HOLD_REASON_OPE_NO, ORIGINAL_FAB_ID, DESTINATION_FAB_ID, RELATED_LOT_ID, BOND_GRP_ID, EQPMONJOB_ID, PD_TYPE, PREV_PD_TYPE 
                    from {target_table} 
                    where 1=1 and {check_date_column} >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '1 day'
                ...
                # 删除老数据
                delete_sql = """
                       delete from {target_table} where {check_date_column} < DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '25 day'
                       """.format(target_table=target_table_name,
                                  check_date_column=check_date_column,
                                  save_day=sync_day + 1)
