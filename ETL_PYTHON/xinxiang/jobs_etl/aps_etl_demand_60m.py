...
def create_temp_table(duck_db):
    ...
    if not config.g_debug_mode:
        table_type = 'TEMP'
    sql = """
    create {table_type} table APS_TMP_ETL_DEMAND_RTD
    (
    ...
    duck_db.sql(sql)
    sql = """
    create {table_type} table APS_TMP_ETL_DEMAND_TECH
    (
      parentid         VARCHAR(60) not null,
      toolg_id         VARCHAR(60),
      tool_id          VARCHAR(60),
    ...
    duck_db.sql(sql)
    sql = """
    create {table_type} table APS_TMP_ETL_DEMAND_PRODG
    (
      ope_no        VARCHAR(60),
      prodg_id      VARCHAR(60),
      eqp_id        VARCHAR(60),
    ...
    duck_db.sql(sql)
    sql = """
    create {table_type} table APS_TMP_ETL_DEMAND_PRODTECH
    (
      ope_no        VARCHAR(60),
      prodg_id      VARCHAR(60),
      eqp_id        VARCHAR(60),
...
def execute():
    ...
    used_table_list = ['APS_TMP_LOTHISTORY_VIEW',
                       'APS_SYNC_PRODUCT',
    ...
    try:
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_ETL_DEMAND_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
