...
def process_main_business_logic(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    """
    主要业务逻辑处理函数
    TODO: 根据具体需求修改此函数
    """
    sql = """
    INSERT INTO APS_ETL_RETICLE_TRANSFER_COUNT_LIMIT
    (PARENTID, TOOLG_ID, TARGET_RANGE_FROM, TARGET_RANGE_TO, RETICLE_LOAD_LIMIT, RETICLE_LOAD_LIMIT_CROSS_FAB, UPDATE_TIME)
    SELECT 
        '{uuid}' AS PARENTID,
        TOOLG_ID,
        TARGET_RANGE_FROM,
        TARGET_RANGE_TO,
        RETICLE_LOAD_LIMIT,
        RETICLE_LOAD_LIMIT_CROSS_FAB,
        '{current_time}' AS UPDATE_TIME
    FROM APS_SYNC_RETICLE_TRANSFER_COUNT_LIMIT.APS_SYNC_RETICLE_TRANSFER_COUNT_LIMIT
    """.format(uuid=uuid, current_time=current_time)

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_RETICLE_TRANSFER_COUNT_LIMIT",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_RETICLE_TRANSFER_COUNT_LIMIT")
...
def execute():
    ...
    # TODO: 修改以下参数
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_RETICLE_TRANSFER_COUNT_LIMIT_5M"  # 修改ETL名称
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    # TODO: 修改目标表名
    target_table = "APS_ETL_RETICLE_TRANSFER_COUNT_LIMIT"

    # TODO: 修改来源表列表 (通常是APS_SYNC_xxxx)
    used_table_list = ['APS_SYNC_RETICLE_TRANSFER_COUNT_LIMIT']

    # TODO: 修改目标表结构定义
    target_table_sql = """
        create table {}APS_ETL_RETICLE_TRANSFER_COUNT_LIMIT
        (
          PARENTID                          VARCHAR(64) not null,
          TOOLG_ID                           VARCHAR(64),
          TARGET_RANGE_FROM                 VARCHAR(64),
          TARGET_RANGE_TO                   VARCHAR(64),
          RETICLE_LOAD_LIMIT                VARCHAR(64),
          RETICLE_LOAD_LIMIT_CROSS_FAB      VARCHAR(64),
    ...
    try:
        ...
        process_main_business_logic(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        ...
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            ...
            select_sql_in_duck = """select parentid,
                                     ...
                                     from APS_ETL_RETICLE_TRANSFER_COUNT_LIMIT
                                 """

            postgres_table_define = """etl_mask_config(parentid,TOOLG_ID,TARGET_RANGE_FROM,TARGET_RANGE_TO,RETICLE_LOAD_LIMIT,RETICLE_LOAD_LIMIT_CROSS_FAB,update_time,ver_timekey)
                                    """

            my_postgres.copy_duckdb_to_postgres(uuid=uuid,
                                                duckdb=duck_db_memory,
                                                table_name_in_duckdb=target_table,
                                                table_name_in_pg="etl_mask_config",  # 要小写
        ...
        if oracle_conn is not None:
            ...
            my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                       cons_error_code.APS_ETL_RETICLE_TRANSFER_COUNT_LIMIT_CODE_XX_ETL)
        raise e
