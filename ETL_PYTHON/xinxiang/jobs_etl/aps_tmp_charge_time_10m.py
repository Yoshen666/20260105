...
# ① 针对DIFF区域
# ② tool_id,ppid 取中位数
def GetChargeTimeSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO APS_TMP_CHARGE_TIME(                              
         PARENTID, TOOL_ID, PPID, CHARGE_TIME, UPDATE_TIME, PARTCODE                                                                
         )                                                                                    
          WITH SOU_DATA AS (                                                                 
              SELECT A.TOOL_ID, 
                     A.PPID, 
                    QUANTILE_CONT(CAST(CHARGE_TIME AS DECIMAL),0.5) OVER(PARTITION BY TOOL_ID,PPID ) AS CHARGE_TIME                                                            
                                                                                 
              FROM APS_TR_CHARGE_TIME.APS_TR_CHARGE_TIME A                                                      
          )                                                                                  
          SELECT DISTINCT '{uuid}' AS PARENTID,                                          
                 TOOL_ID,PPID,CHARGE_TIME,                                                        
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_TMP_CHARGE_TIME",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_CHARGE_TIME")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_CHARGE_TIME_10M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_TMP_CHARGE_TIME"
    used_table_list = ['APS_TR_CHARGE_TIME']
    target_table_sql = """
    create table {}APS_TMP_CHARGE_TIME
    (
        parentid    VARCHAR(64),
        tool_id     VARCHAR(64),
        ppid        VARCHAR(64),
    ...
    try:
        ...
        GetChargeTimeSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        ...
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                   cons_error_code.APS_TMP_CHARGE_TIME_CODE_XX_ETL)
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
