...
def InsertPreferToolData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    """
    sql = """
    INSERT INTO APS_ETL_LOT_SITUATION
        (PARENTID, LOT_ID, OPE_NO, LAYER, STAGE, EQP_ID, RESERVE_TIME, OP_START_TIME, PROC_START_TIME, PROC_END_TIME, OP_END_TIME, FORM_BATCH_TIME, CTRL_JOB, FLOWBATCH_ID, UPDATE_TIME)
    WITH ranked_data AS (
        SELECT 
            '{uuid}' AS PARENTID,
            LOT_ID,
            ...
            ROW_NUMBER() OVER (
                PARTITION BY LOT_ID, OPE_NO 
                ORDER BY 
                    COALESCE(PROC_END_TIME, '1900-01-01'::timestamp) DESC,
                    COALESCE(PROC_START_TIME, '1900-01-01'::timestamp) DESC
            ) as rn
        FROM APS_SYNC_LOT_SITUATION.APS_SYNC_LOT_SITUATION
    )
    SELECT 
        PARENTID,
        LOT_ID,
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_ETL_LOT_SITUATION",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_LOT_SITUATION")
...
def execute():
    ...
    # TODO: 修改以下参数
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_LOT_SITUATION_5M"  # 修改ETL名称
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    # TODO: 修改目标表名
    target_table = "APS_ETL_LOT_SITUATION"

    # TODO: 修改来源表列表 (通常是APS_SYNC_xxxx)
    used_table_list = ['APS_SYNC_LOT_SITUATION']

    # TODO: 修改目标表结构定义
    target_table_sql = """
        create table {}APS_ETL_LOT_SITUATION
        (
            ...
            OP_START_TIME    VARCHAR(64),
            PROC_START_TIME    VARCHAR(64),
            PROC_END_TIME    VARCHAR(64),
            OP_END_TIME    VARCHAR(64),
            FORM_BATCH_TIME    VARCHAR(64),
            CTRL_JOB    VARCHAR(64),
    ...
    try:
        ...
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            ...
            select_sql_in_duck = """select distinct parentid,
                                           LOT_ID,	
                                           OPE_NO,
                                           LAYER,
                                           STAGE,
                                           EQP_ID AS tool_id,  -- 改這行，讓 EQP_ID 對應 tool_id
                                           RESERVE_TIME,	
                                           OP_START_TIME AS track_in,  -- 如果需要對應 track_in
                                           PROC_START_TIME AS process_start,  -- 對應 process_start
                                           PROC_END_TIME AS process_end,  -- 對應 process_end  
                                           case when OP_END_TIME is null then update_time else op_end_time end AS track_out,  -- 對應 track_out
                                           FORM_BATCH_TIME AS formbatch_time,
                                           CTRL_JOB,	
                                           FLOWBATCH_ID AS batch_id,  -- 對應 batch_id
                                     ...
                                     from APS_ETL_LOT_SITUATION
                                 """

            postgres_table_define = """ETL_LOT_SITUATION(parentid, lot_id, ope_no, layer, stage, tool_id, job_prepare, track_in, process_start, process_end, track_out, formbatch_time, CTRL_JOB, batch_id, update_time, ver_timekey)
                                    """

            my_postgres.copy_duckdb_to_postgres(uuid=uuid,
                                                duckdb=duck_db_memory,
                                                table_name_in_duckdb=target_table,
                                                table_name_in_pg="etl_lot_situation",  # 要小写
        ...
        if oracle_conn is not None:
            ...
            my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file,
                                       cons_error_code.APS_ETL_LOT_SITUATION_CODE_XX_ETL)
        raise e
