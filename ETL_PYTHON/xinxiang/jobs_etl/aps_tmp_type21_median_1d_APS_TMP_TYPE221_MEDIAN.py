...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO APS_TMP_TYPE221_MEDIAN (                                
         PARENTID,  MODULE_ID, TOOLG_ID, TOOL_ID, CH_CNT, PT, UPDATE_TIME, PARTCODE              
    )                                                                                      
     WITH TYPE221 AS (                                                                     
       SELECT 
             -- ROUND(CEIL((TO_DATE(OPERATION_COMPLETE,'YYYY-MM-DD HH24:MI:SS') - TO_DATE(PROCESS_START,'YYYY-MM-DD HH24:MI:SS'))*24*60*60)/3600,3) AS PT,    
             -- TODO 暂时这样,后续再测试  
             round((extract(epoch from(CAST(OPERATION_COMPLETE AS TIMESTAMP) - (CAST(PROCESS_START AS TIMESTAMP))))) / 3600.0, 3) AS PT,    
             T.MODULE_ID, T.TOOLG_ID, T.TOOL_ID, T.CH_CNT,                                 
             ROW_NUMBER() OVER (PARTITION BY T.MODULE_ID,T.TOOLG_ID,T.TOOL_ID,CH_CNT ORDER BY T.PROCESS_END DESC) AS RN     
       FROM APS_TR_TYPE221.APS_TR_TYPE221 T                                                               
       WHERE (T.MODULE_ID IN ('DIFF','WET') OR T.TOOLG_ID = 'N2 purge')                                                 
       AND T.OPERATION_COMPLETE IS NOT NULL AND T.OPERATION_COMPLETE <> ''                                                
     ),                                                                                    
     PT_DATA AS (                                                                          
     SELECT 
          -- PERCENTILE_CONT(0.5) WITHIN GROUP(ORDER BY PT) OVER(PARTITION BY MODULE_ID,TOOLG_ID,TOOL_ID,CH_CNT) AS MAX_50
          --MEDIAN(CAST(PT AS DECIMAL)) OVER(PARTITION BY MODULE_ID,TOOLG_ID,TOOL_ID,CH_CNT ) AS MAX_50
          QUANTILE_CONT(CAST(PT AS DECIMAL),0.5) OVER(PARTITION BY MODULE_ID,TOOLG_ID,TOOL_ID,CH_CNT ) AS MAX_50                        
         ,MODULE_ID,TOOLG_ID, TOOL_ID, CH_CNT, RN                                          
     FROM TYPE221                                                                        
     ),                                                                                    
     RESULT_DATA AS (                                                                      
        SELECT MODULE_ID,TOOLG_ID, TOOL_ID, CH_CNT                                         
                ,MAX(MAX_50) AS PT,MAX(RN) AS RN                                           
        FROM PT_DATA                                                                       
        GROUP BY MODULE_ID,TOOLG_ID, TOOL_ID, CH_CNT                                       
     )                                                                                     
     SELECT '{uuid}',                                                    
            MODULE_ID,TOOLG_ID, TOOL_ID, CH_CNT,                                           
            PT,                                                                            
           '{current_time}',                                                         
            ''                                           
     FROM RESULT_DATA                                                                      
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_TMP_TYPE221_MEDIAN",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_TYPE221_MEDIAN")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_TYPE21_MEDIAN_1D_APS_TMP_TYPE221_MEDIAN"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_TMP_TYPE221_MEDIAN"
    used_table_list = ['APS_TR_TYPE221']
    target_table_sql = """
        create table APS_TMP_TYPE221_MEDIAN
        (
    ...
    try:
        ...
        my_duck.export_result_duck_file_and_close_duck_db_memory2(duck_db_memory,
                                                                  ...
                                                                  current_time=current_time_short)
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, "XETL060")
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
