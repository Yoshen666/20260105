...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.sync_APS_MID_FHOPEHS_RETICLE"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    source_table = "V_FHOPEHS_RETICLE"
    target_table_name = "APS_MID_FHOPEHS_RETICLE_VIEW"
    check_date_column = "CLAIM_TIME"
    sync_day = 12
    query_sql = """SELECT LOT_ID, MAINPD_ID, OPE_NO, CASE WHEN OPE_PASS_COUNT IS NULL THEN 0 ELSE OPE_PASS_COUNT END AS OPE_PASS_COUNT, CLAIM_TIME, OPE_CATEGORY, RETICLE_ID, ORIG_LOT_ID FROM V_FHOPEHS_RETICLE"""
    create_table_sql = """
    ...
    try:
        ...
        if last_file_name:
            ...
            if _create_time_date is not None:
                ...
                query_duck_sql = """
                    select LOT_ID, MAINPD_ID, OPE_NO, CASE WHEN OPE_PASS_COUNT IS NULL THEN 0 ELSE OPE_PASS_COUNT END AS OPE_PASS_COUNT, CLAIM_TIME, OPE_CATEGORY, RETICLE_ID, ORIG_LOT_ID
                    from {target_table} where 1=1 and {check_date_column} >= DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '3 day'
                ...
                select_insert_item = """
                    ...
                    select {columns_str}  FROM last_one_hour_date T
                    where 1=1
                    and  NOT EXISTS (
                        SELECT HL.LOT_ID FROM last_one_hour_date_duck HL
                        where 
                          1=1
                         AND CASE WHEN T.LOT_ID IS NULL OR T.LOT_ID = '' THEN '*' ELSE T.LOT_ID END = CASE WHEN HL.LOT_ID IS NULL OR HL.LOT_ID = '' THEN '*' ELSE HL.LOT_ID END                 
                         AND CASE WHEN T.MAINPD_ID IS NULL OR T.MAINPD_ID = '' THEN '*' ELSE T.MAINPD_ID END = CASE WHEN HL.MAINPD_ID IS NULL OR HL.MAINPD_ID = '' THEN '*' ELSE HL.MAINPD_ID END
                         AND CASE WHEN T.OPE_NO IS NULL OR T.OPE_NO = '' THEN '*' ELSE T.OPE_NO END = CASE WHEN HL.OPE_NO IS NULL OR HL.OPE_NO = '' THEN '*' ELSE HL.OPE_NO END
                         AND CASE WHEN T.OPE_CATEGORY IS NULL OR T.OPE_CATEGORY = '' THEN '*' ELSE T.OPE_CATEGORY END = CASE WHEN HL.OPE_CATEGORY IS NULL OR HL.OPE_CATEGORY = '' THEN '*' ELSE HL.OPE_CATEGORY END
                         AND CASE WHEN T.RETICLE_ID IS NULL OR T.RETICLE_ID = '' THEN '*' ELSE T.RETICLE_ID END = CASE WHEN HL.RETICLE_ID IS NULL OR HL.RETICLE_ID = '' THEN '*' ELSE HL.RETICLE_ID END                                    
                         AND CASE WHEN T.ORIG_LOT_ID IS NULL OR T.ORIG_LOT_ID = '' THEN '*' ELSE T.ORIG_LOT_ID END = CASE WHEN HL.ORIG_LOT_ID IS NULL OR HL.ORIG_LOT_ID = '' THEN '*' ELSE HL.ORIG_LOT_ID END                      
                         AND CASE WHEN T.OPE_PASS_COUNT IS NULL  THEN CAST(-1 AS INTEGER) ELSE CAST(T.OPE_PASS_COUNT AS INTEGER) END = CASE WHEN HL.OPE_PASS_COUNT IS NULL  THEN CAST(-1 AS INTEGER) ELSE CAST(HL.OPE_PASS_COUNT AS INTEGER) END 
                         AND CAST(T.CLAIM_TIME AS TIMESTAMP) = CAST(HL.CLAIM_TIME AS TIMESTAMP)
                ...
                # 删除老数据
                delete_sql = """
                       delete from {target_table} where {check_date_column} < DATE_TRUNC('second', CURRENT_TIMESTAMP) - INTERVAL '12 day'
                       """.format(target_table=target_table_name,
                                  check_date_column=check_date_column,
                                  save_day=sync_day + 1)
        ...
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, "XETL073")
