...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT /*+ append */ INTO APS_MID_TYPE221_RUNTIME_ALL (              
             PARENTID,MODULE_ID, TOOLG_ID, TOOL_ID, CH_CNT, LESSTHAN20_FLAG,TRACKER_ZWS_FLAG,
             ONE_WAFER_PT_TRACKER, TRACKER_SLOPE_NEW, TRACKER_INTER_NEW,UPDATE_TIME,PARTCODE                                                             
    )                                                                          
      WITH TYPE221_LR_FIRST1 AS (                                                                                                                                                                                                                                                                     
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.QTY, O.CH_CNT, O.TRACKER_PT                                                                                                                                    
                   , ROUND((REGR_SLOPE(CAST(O.TRACKER_PT AS DECIMAL), CAST(O.QTY AS DECIMAL)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID,O.CH_CNT) * CAST(O.QTY AS DECIMAL) +                                                                                    
                            REGR_INTERCEPT(CAST(O.TRACKER_PT AS DECIMAL), CAST(O.QTY AS DECIMAL)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID,O.CH_CNT)),6) AS PT_Y_TRACKER                                                                      
             ...
             FROM (SELECT A.MODULE_ID, A.TOOLG_ID, A.TOOL_ID,A.CH_CNT, A.QTY,                                                                                                                                           
      ...
      TYPE221_LR_FIRST_ZWS AS (                                                                                                                                                                                
       ...
       SELECT '{uuid}' AS PARENTID,                                                                                                                                                                         
             S.MODULE_ID, S.TOOLG_ID, S.TOOL_ID,
             CASE WHEN CAST(S.CH_CNT AS DECIMAL(18,0)) = 0 THEN 1 ELSE S.CH_CNT END AS CH_CNT, 
             S.LESSTHAN20_FLAG,                                                                                                                                                                                                                                                             
             '' AS TRACKER_ZWS_FLAG,                                                                                                                                                                                                                                                                                                                                                                                                              
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_TYPE221_RUNTIME_ALL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_TYPE221_RUNTIME_ALL")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_MID_TYPE_RUNTIME_1D_APS_MID_TYPE221_RUNTIME_ALL"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_TYPE221_RUNTIME_ALL"
    used_table_list = ['APS_TR_TYPE221']
    target_table_sql = """
        create table {}APS_MID_TYPE221_RUNTIME_ALL
        (
          parentid             VARCHAR(64),
    ...
    try:
        ...
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, "XETL070")
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
