...
def create_temp_table(duck_db):
    ...
    '''
    sql = """
        create {table_type} table APS_TMP_SIZE_CONTROL_V1
        (
            parentid VARCHAR(64), 
            ...
            partcode VARCHAR(64),
            RLS_ATTR2 VARCHAR(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)

    sql = """
        create {table_type} table APS_TMP_SIZE_CONTROL_V2
        (
            parentid VARCHAR(64), 
            rule_name VARCHAR(64), 
            toolg_id VARCHAR(64), 
    ...
    sql = """
        create {table_type} table APS_TMP_SIZE_CONTROL_V3
        (
            parentid VARCHAR(64), 
            rule_name VARCHAR(64), 
            toolg_id VARCHAR(64), 
    ...
    sql = """
        create {table_type} table APS_TMP_SIZE_CONTROL_SETTING
        (
            id1 varchar(64),
            eqp_id varchar(64),
            ...
            require_flag varchar(64),
            purpose  varchar(64)
        )
        """.format(table_type=table_type)
    duck_db.sql(sql)

def GetWetBatchSizeSettingData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT  /*+ append */  INTO {tempdb}APS_TMP_SIZE_CONTROL_SETTING(               
        id1,eqp_id,prodspec_id,ope_no,batch_size,memo,
        update_time,update_user,require_flag,purpose                                                     
    )                                                                     
    WITH SETTING_DATA AS (                                                                                                                                                                          
        SELECT id1,eqp_id,prodspec_id,ope_no,batch_size,memo,
        update_time,update_user,require_flag,purpose,
        row_number() over(partition by eqp_id,prodspec_id,ope_no order by purpose desc) as rnt                                                                                              
        FROM APS_SYNC_RTD_WET_BATCH_SIZE_SETTING_NEW.APS_SYNC_RTD_WET_BATCH_SIZE_SETTING_NEW T                                                                                                                                                                                                             
    )  
    select id1,eqp_id,prodspec_id,ope_no,batch_size,memo,
            update_time,update_user,require_flag,purpose
    from SETTING_DATA
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_TMP_SIZE_CONTROL_SETTING",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_SIZE_CONTROL_SETTING")

def GetWetBatchV1Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT  /*+ append */  INTO {tempdb}APS_TMP_SIZE_CONTROL_V1(               
             PARENTID, RULE_NAME, TOOLG_ID, TOOL_ID, PROD_ID, LAYER,
             STAGE, MIN_FOUP_SIZE, MAX_FOUP_SIZE,HARD_FLAG, RLS_ATTR2, UPDATE_TIME, PARTCODE                                                    
         )                                                                     
           WITH TOOL_DATA AS (                                                                                                                                                                          
                SELECT EQP_ID,                                                                                             
                       EQP_G                                                                                               
           ...
           SELECT DISTINCT '{uuid}' AS PARENTID,                                                                                   
                 ...
                 case when purpose='Quality' then '1' else '0' end AS HARD_FLAG,
                 case when purpose='Quality' then '0' else '1' end AS rls_attr2,                                                                                         
                 '{current_time}' AS UPDATE_TIME,                                                            
                 '' AS PARTCODE                                                 
           FROM {tempdb}APS_TMP_SIZE_CONTROL_SETTING T                                                                             
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_SIZE_CONTROL_V1",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_SIZE_CONTROL_V1")

def GetWetBatchV2Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
             INSERT  /*+ append */  INTO {tempdb}APS_TMP_SIZE_CONTROL_V2(               
                 PARENTID, RULE_NAME, TOOLG_ID, TOOL_ID, PROD_ID, LAYER,
                 STAGE, MIN_FOUP_SIZE, MAX_FOUP_SIZE,HARD_FLAG,RLS_ATTR2, UPDATE_TIME, PARTCODE                                                    
             )                                                                     
             WITH LC_DATA AS(
                SELECT LCRECIPE_ID,MNTR_PRODSPEC_ID 
                FROM APS_SYNC_MES_FRLRCP.APS_SYNC_MES_FRLRCP P
                WHERE  EXISTS (SELECT 1 FROM APS_SYNC_WIP.APS_SYNC_WIP W 
                     WHERE W.WAFER_QTY >= 25 AND P.MNTR_PRODSPEC_ID = W.PRODSPEC_ID)
            ),
            CASE1_DATA AS (
                SELECT DISTINCT PROD_ID, OPE_NO 
                FROM APS_ETL_FLOW.APS_ETL_FLOW F
                WHERE  EXISTS (SELECT 1 FROM LC_DATA L WHERE L.LCRECIPE_ID = F.RECIPE)
            )
            SELECT S.PARENTID, S.RULE_NAME, S.TOOLG_ID, S.TOOL_ID, 
                   D.PROD_ID, SUBSTRING(D.OPE_NO, 1, POSITION('.' IN D.OPE_NO)-1) AS LAYER,
                   SUBSTR(D.OPE_NO, POSITION('.' IN D.OPE_NO)+1) AS STAGE, 
                   S.MIN_FOUP_SIZE, S.MAX_FOUP_SIZE,
                   '0' AS HARD_FLAG, 
                   '1' AS RLS_ATTR2,
                   S.UPDATE_TIME, S.PARTCODE
            FROM {tempdb}APS_TMP_SIZE_CONTROL_V1 S
            INNER JOIN CASE1_DATA D
            ON D.PROD_ID LIKE S.PROD_ID
            AND D.OPE_NO LIKE S.LAYER || '.' || S.STAGE  
            WHERE S.HARD_FLAG = 1                                                                                                                                                                                                                                                                    
            """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_SIZE_CONTROL_V2",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_SIZE_CONTROL_V2")

def GetWetBatchV3Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT  /*+ append */  INTO {tempdb}APS_TMP_SIZE_CONTROL_V3(               
             PARENTID, RULE_NAME, TOOLG_ID, TOOL_ID, PROD_ID, LAYER,
             STAGE, MIN_FOUP_SIZE, MAX_FOUP_SIZE,HARD_FLAG, RLS_ATTR2,UPDATE_TIME, PARTCODE                                                    
         )                                                                     
        WITH CASE2_DATA AS (
          SELECT MON_PROD_ID AS PROD_ID 
          FROM APS_SYNC_CSMONWFRCFG.APS_SYNC_CSMONWFRCFG
          WHERE WAFER_COUNT > 0
        )
        SELECT S.PARENTID, S.RULE_NAME, S.TOOLG_ID, S.TOOL_ID, 
               D.PROD_ID, S.LAYER, S.STAGE, 
               S.MIN_FOUP_SIZE, S.MAX_FOUP_SIZE,
               '0' AS HARD_FLAG, 
               '1' AS RLS_ATTR2,
               S.UPDATE_TIME, S.PARTCODE
        FROM {tempdb}APS_TMP_SIZE_CONTROL_V1 S
        INNER JOIN CASE2_DATA D
        ON D.PROD_ID LIKE S.PROD_ID  
        WHERE S.HARD_FLAG = 1                                                                                                                                                                                                                                                                    
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_TMP_SIZE_CONTROL_V3",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_SIZE_CONTROL_V3")

def GetWetBatchAllData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
         INSERT  /*+ append */  INTO APS_ETL_SIZE_CONTROL(               
             PARENTID, RULE_NAME, TOOLG_ID, TOOL_ID, PROD_ID, LAYER,
             STAGE, MIN_FOUP_SIZE, MAX_FOUP_SIZE,HARD_FLAG, RLS_ATTR2, UPDATE_TIME, PARTCODE                                                    
         )                                                                     
         WITH WET_RESULT_DATA AS (
              SELECT PARENTID, RULE_NAME, TOOLG_ID, TOOL_ID, PROD_ID, LAYER,
                     STAGE, MIN_FOUP_SIZE, MAX_FOUP_SIZE,HARD_FLAG, RLS_ATTR2, UPDATE_TIME, PARTCODE
              FROM {tempdb}APS_TMP_SIZE_CONTROL_V1
              UNION ALL
              SELECT PARENTID, RULE_NAME, TOOLG_ID, TOOL_ID, PROD_ID, LAYER,
                     STAGE, MIN_FOUP_SIZE, MAX_FOUP_SIZE,HARD_FLAG, RLS_ATTR2, UPDATE_TIME, PARTCODE
              FROM {tempdb}APS_TMP_SIZE_CONTROL_V2
              UNION ALL
              SELECT PARENTID, RULE_NAME, TOOLG_ID, TOOL_ID, PROD_ID, LAYER,
                     STAGE, MIN_FOUP_SIZE, MAX_FOUP_SIZE,HARD_FLAG, RLS_ATTR2, UPDATE_TIME, PARTCODE
              FROM {tempdb}APS_TMP_SIZE_CONTROL_V3
         )
         SELECT PARENTID, RULE_NAME, TOOLG_ID, TOOL_ID, PROD_ID, LAYER,
               STAGE, MIN_FOUP_SIZE, MAX_FOUP_SIZE,
               MIN(HARD_FLAG) AS HARD_FLAG, 
               MAX(RLS_ATTR2) AS RLS_ATTR2,
               UPDATE_TIME, PARTCODE
         FROM WET_RESULT_DATA S
         GROUP BY PARENTID, RULE_NAME, TOOLG_ID, TOOL_ID, PROD_ID, LAYER,
               STAGE, MIN_FOUP_SIZE, MAX_FOUP_SIZE, UPDATE_TIME, PARTCODE                                                                                                                                                                                                                                                                      
        """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_SIZE_CONTROL WET",
                     sql=sql,
                     current_time=current_time,
...
def execute():
    ...
    used_table_list = ['APS_SYNC_UMT_RUN_RULE_BATCH_SIZE',
                       'APS_SYNC_ETL_TOOL',
                       'APS_SYNC_RTD_WET_BATCH_SIZE_SETTING_NEW',
    ...
    try:
        ...
        # 获取RTD_WET_BATCH_SIZE_SETTING_NEW过滤后的信息
        GetWetBatchSizeSettingData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)

        # 酸槽单双批逻辑
        GetWetBatchV1Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetWetBatchV2Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetWetBatchV3Data(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        GetWetBatchAllData(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name)
        ...
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            select_sql_in_duck = """select  parentid,
                                            ...
                                            CASE WHEN RLS_ATTR2 ='' THEN NULL ELSE RLS_ATTR2 END AS RLS_ATTR2,
                                            update_time,
            ...
            postgres_table_define = """etl_size_control(parentid,rule_name,toolg_id,tool_id,prodg_tech,prodg_id,prod_id,layer,stage,lot_type,foup_con,
                                                        min_batch_size,max_batch_size,min_foup_size,max_foup_size,hard_flag,RLS_ATTR2,update_time,other_min_batch_size )
                                           """
