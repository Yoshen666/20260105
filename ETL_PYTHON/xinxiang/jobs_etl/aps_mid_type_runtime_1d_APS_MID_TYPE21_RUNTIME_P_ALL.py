...
def create_temp_table(duck_db):
    ...
    '''
    sql = """
    create {table_type} table APS_MID_TYPE21_RUNTIME_P_TS
    (
    ...
    duck_db.sql(sql)
    sql = '''
    create {table_type} table APS_MID_TYPE21_RUNTIME_P_ETC
    (
      module_id            VARCHAR(64),
      toolg_id             VARCHAR(64),
      tool_id              VARCHAR(64),
...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    # 先计算Tracker和Scanner的 线性回归
    sql = """
     INSERT /*+ append */ INTO  {tempdb}APS_MID_TYPE21_RUNTIME_P_TS
      WITH TYPE21_LR_FIRST1 AS (                                                                                                                                                         
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID, O.QTY, O.TRACKER_PT, O.SCANNER_PT                                                                                       
                   -- , ROUND((REGR_SLOPE(O.TRACKER_PT, O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID) * O.QTY +
                   ...
                   --         REGR_INTERCEPT(O.TRACKER_PT, O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID)),6) AS PT_Y_TRACKER
                   ...
                   -- , ROUND((REGR_SLOPE(O.SCANNER_PT, O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID) * O.QTY +
                   , ROUND((REGR_SLOPE(CAST(O.SCANNER_PT AS DOUBLE), CAST(O.QTY AS DOUBLE)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID) * CAST(O.QTY AS DOUBLE) +
                   --         REGR_INTERCEPT(O.SCANNER_PT, O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID)),6) AS PT_Y_SCANNER
                   REGR_INTERCEPT(CAST(O.SCANNER_PT AS DOUBLE), CAST(O.QTY AS DOUBLE)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID)),6) AS PT_Y_SCANNER
             ...
             FROM (SELECT A.MODULE_ID, A.TOOLG_ID, A.TOOL_ID, A.PPID, A.QTY,                                                                                                            
      ...
      TYPE21_LR_FIRST_ZWS AS (                                                                                                                                                          
       ...
       TYPE21_LR_FIRST_ZWS_SCANNER AS (                                                                                                                                                 
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID, O.SAMEQTY_FLAG_ZWS, O.LESSTHAN20_FLAG                                                                                   
                    -- ,AVG(O.ONE_WAFER_PT_SCANNER) AS ONE_WAFER_PT_SCANNER
                    ,AVG(CAST(O.ONE_WAFER_PT_SCANNER AS DOUBLE)) AS ONE_WAFER_PT_SCANNER                                                                                                                
             FROM (                                                                                                                                                                     
                       SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID, O.ONE_WAFER_PT_SCANNER, O.SAMEQTY_FLAG_ZWS, O.LESSTHAN20_FLAG
                       FROM (                                                                                                                                                           
                               SELECT A.*                                                                                                                                               
                                    ,MEDIAN(RN) OVER(PARTITION BY A.MODULE_ID, A.TOOLG_ID, A.TOOL_ID, A.PPID) MED                                                                       
                               FROM (                                                                                                                                                   
                                       SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID, O.SAMEQTY_FLAG_ZWS, O.LESSTHAN20_FLAG
                                             ...
                                             ,ROW_NUMBER() OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID ORDER BY O.ONE_WAFER_PT_SCANNER) RN
                                       FROM TYPE21_LR_FIRST1 O                                                                                                                          
                      ) A                                                                                                                                                               
                            ) O                                                                                                                                                         
                       WHERE ABS(RN - MED) <= 0.5                                                                                                                                       
       ...
       TYPE21_LR_FIRST_LESSTHAN20_SCANNER AS (                                                                                                                                          
             SELECT Z.MODULE_ID, Z.TOOLG_ID, Z.TOOL_ID, Z.PPID, Z.LESSTHAN20_FLAG, Z.SAMEQTY_FLAG_ZWS, 'N' AS PA_PB_0_FLAG_ZWS,
                    Z.ONE_WAFER_PT_SCANNER, 0 AS SCANNER_SLOPE_NEW, 0 AS SCANNER_INTER_NEW                                                                                              
                                                                                                                                                                                        
             FROM TYPE21_LR_FIRST_ZWS_SCANNER Z                                                                                                                                         
             WHERE Z.LESSTHAN20_FLAG = 'Y'                                                                                                                                              
       ),                                                                                                                                                                               
       TYPE21_LR_FIRST_SAMEQTY_SCANNER AS (                                                                                                                                             
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID, O.LESSTHAN20_FLAG, O.SAMEQTY_FLAG_ZWS , 'N' AS PA_PB_0_FLAG_ZWS,
                    O.ONE_WAFER_PT_SCANNER,  0 AS SCANNER_SLOPE_NEW, 0 AS SCANNER_INTER_NEW                                                                                             
             FROM TYPE21_LR_FIRST_ZWS_SCANNER O                                                                                                                                         
             WHERE O.SAMEQTY_FLAG_ZWS = 'Y'                                                                                                                                             
             AND NOT EXISTS (SELECT 1                                                                                                                                                   
                     FROM TYPE21_LR_FIRST_LESSTHAN20_SCANNER L20                                                                                                                        
                   WHERE O.MODULE_ID = L20.MODULE_ID                                                                                                                                    
                   AND O.TOOLG_ID = L20.TOOLG_ID                                                                                                                                        
                   AND O.TOOL_ID = L20.TOOL_ID                                                                                                                                          
                   AND O.PPID = L20.PPID)                                                                                                                                               
       ),                                                                                                                                                                               
       TYPE21_LR_FIRST2_SCANNER AS (                                                                                                                                                    
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID, O.QTY, O.SCANNER_PT, O.PT_Y_SCANNER, O.CNT_OLD, O.LESSTHAN20_FLAG,
                    ...
                    -- SQRT((SUM(POWER(O.PT_Y_SCANNER - O.SCANNER_PT,2)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID)) / O.CNT_OLD)
                    SQRT((SUM(POWER(CAST(O.PT_Y_SCANNER AS DOUBLE) - CAST(O.SCANNER_PT AS DOUBLE),2)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID)) / CAST(O.CNT_OLD AS DOUBLE))
                    ), 2)                                                                                                                                                               
                    AS SE                                                                                                                                                               
             FROM TYPE21_LR_FIRST1 O                                                                                                                                                    
             INNER JOIN TYPE21_LR_FIRST_ZWS_SCANNER Z ON O.MODULE_ID = Z.MODULE_ID AND O.TOOLG_ID = Z.TOOLG_ID AND O.TOOL_ID = Z.TOOL_ID AND O.PPID = Z.PPID
             WHERE NOT EXISTS (                                                                                                                                                         
               SELECT 1 FROM TYPE21_LR_FIRST_LESSTHAN20_SCANNER L20 WHERE O.MODULE_ID = L20.MODULE_ID AND O.TOOLG_ID = L20.TOOLG_ID AND O.TOOL_ID = L20.TOOL_ID AND O.PPID = L20.PPID
             )                                                                                                                                                                          
             AND NOT EXISTS (                                                                                                                                                           
               SELECT 1 FROM TYPE21_LR_FIRST_SAMEQTY_SCANNER SQ WHERE O.MODULE_ID = SQ.MODULE_ID AND O.TOOLG_ID = SQ.TOOLG_ID AND O.TOOL_ID = SQ.TOOL_ID AND O.PPID = SQ.PPID
             )                                                                                                                                                                          
       ),                                                                                                                                                                               
       TYPE21_LR_FIRST3_SCANNER AS(                                                                                                                                                     
               SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID, O.QTY, O.SCANNER_PT, O.PT_Y_SCANNER,                                                                                  
                      O.CNT_OLD, O.SE, O.ONE_WAFER_PT_SCANNER,                                                                                                                          
                      O.LESSTHAN20_FLAG, O.SAMEQTY_FLAG_ZWS                                                                                                                             
                FROM TYPE21_LR_FIRST2_SCANNER O                                                                                                                                         
                WHERE O.SE <= 1.96                                                                                                                                                      
       ),                                                                                                                                                                               
       TYPE21_LR_SECOND1_SCANNER AS (                                                                                                                                                   
               SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID, O.QTY, O.SCANNER_PT, O.ONE_WAFER_PT_SCANNER                                                                           
               , CASE WHEN COUNT(1) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID) < 20                                                                                  
                      THEN 'Y'                                                                                                                                                          
                      ELSE 'N'                                                                                                                                                          
                 END AS LESSTHAN20_FLAG                                                                                                                                                 
               , CASE WHEN MAX(O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID) = MIN(O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID)
                      ...
                      ELSE 'N'                                                                                                                                                          
                 END AS SAMEQTY_FLAG_ZWS                                                                                                                                                
               -- , REGR_SLOPE(O.SCANNER_PT, O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID) AS SCANNER_SLOPE_NEW
               , REGR_SLOPE(CAST(O.SCANNER_PT AS DOUBLE), CAST(O.QTY AS DOUBLE)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID) AS SCANNER_SLOPE_NEW
               -- , REGR_INTERCEPT(O.SCANNER_PT, O.QTY) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID) AS SCANNER_INTER_NEW
               , REGR_INTERCEPT(CAST(O.SCANNER_PT AS DOUBLE), CAST(O.QTY AS DOUBLE)) OVER(PARTITION BY O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID) AS SCANNER_INTER_NEW
               FROM TYPE21_LR_FIRST3_SCANNER O                                                                                                                                          
       ),                                                                                                                                                                               
       TYPE21_LR_SECOND_LESSTHAN20_SCANNER AS (                                                                                                                                         
             SELECT O.MODULE_ID, O.TOOLG_ID, O.TOOL_ID, O.PPID, O.LESSTHAN20_FLAG, O.SAMEQTY_FLAG_ZWS, 'N' AS PA_PB_0_FLAG_ZWS,
                    O.ONE_WAFER_PT_SCANNER, 0 AS SCANNER_SLOPE_NEW, 0 AS SCANNER_INTER_NEW                                                                                              
             FROM TYPE21_LR_SECOND1_SCANNER O                                                                                                                                           
             WHERE O.LESSTHAN20_FLAG = 'Y'                                                                                                                                              
       ),                                                                                                                                                                               
       TYPE21_LR_SECOND_SAMEQTY_SCANNER AS (                                                                                                                                            
             SELECT T.MODULE_ID, T.TOOLG_ID, T.TOOL_ID, T.PPID, T.LESSTHAN20_FLAG, T.SAMEQTY_FLAG_ZWS , 'N' AS PA_PB_0_FLAG_ZWS,
                    T.ONE_WAFER_PT_SCANNER,  0 AS SCANNER_SLOPE_NEW, 0 AS SCANNER_INTER_NEW                                                                                             
             FROM TYPE21_LR_SECOND1_SCANNER T                                                                                                                                           
             WHERE NOT EXISTS (SELECT 1                                                                                                                                                 
                   FROM TYPE21_LR_SECOND_LESSTHAN20_SCANNER L20                                                                                                                         
                   WHERE T.MODULE_ID = L20.MODULE_ID                                                                                                                                    
                   AND T.TOOLG_ID = L20.TOOLG_ID                                                                                                                                        
                   AND T.TOOL_ID = L20.TOOL_ID                                                                                                                                          
                   AND T.PPID = L20.PPID)                                                                                                                                               
       ...
       TYPE21_LR_SECOND2_SCANNER AS (                                                                                                                                                   
              SELECT  N.MODULE_ID, N.TOOLG_ID, N.TOOL_ID, N.PPID, N.QTY, N.SCANNER_PT, N.ONE_WAFER_PT_SCANNER                                                                           
                     ,N.LESSTHAN20_FLAG, N.SAMEQTY_FLAG_ZWS                                                                                                                             
                     ...
                     ,CASE WHEN N.SCANNER_SLOPE_NEW < 0 OR N.SCANNER_SLOPE_NEW + N.SCANNER_INTER_NEW < 0                                                                                
                           ...
                           ELSE 'N'                                                                                                                                                     
                      END AS PA_PB_0_FLAG_ZWS                                                                                                                                           
               FROM TYPE21_LR_SECOND1_SCANNER N                                                                                                                                         
               WHERE NOT EXISTS (                                                                                                                                                       
                SELECT 1 FROM TYPE21_LR_SECOND_LESSTHAN20_SCANNER L20 WHERE N.MODULE_ID = L20.MODULE_ID AND N.TOOLG_ID = L20.TOOLG_ID AND N.TOOL_ID = L20.TOOL_ID AND N.PPID = L20.PPID 
               )                                                                                                                                                                        
               AND NOT EXISTS (                                                                                                                                                         
                SELECT 1 FROM TYPE21_LR_SECOND_SAMEQTY_SCANNER SQ WHERE N.MODULE_ID = SQ.MODULE_ID AND N.TOOLG_ID = SQ.TOOLG_ID AND N.TOOL_ID = SQ.TOOL_ID AND N.PPID = SQ.PPID         
               )                                                                                                                                                                        
       ),                                                                                                                                                                               
       TYPE21_LR_SECOND_PA_PB_0_SCANNER AS (                                                                                                                                            
               SELECT N.MODULE_ID, N.TOOLG_ID, N.TOOL_ID, N.PPID, N.LESSTHAN20_FLAG, N.SAMEQTY_FLAG_ZWS, N.PA_PB_0_FLAG_ZWS,
                      N.ONE_WAFER_PT_SCANNER, 0 AS SCANNER_SLOPE_NEW, 0 AS SCANNER_INTER_NEW                                                                                            
               FROM TYPE21_LR_SECOND2_SCANNER N                                                                                                                                         
               WHERE N.PA_PB_0_FLAG_ZWS = 'Y'                                                                                                                                           
       ),                                                                                                                                                                               
       TYPE21_LR_SECOND3_SCANNER AS (                                                                                                                                                   
            SELECT N.MODULE_ID, N.TOOLG_ID, N.TOOL_ID, N.PPID, N.LESSTHAN20_FLAG,N.PA_PB_0_FLAG_ZWS, N.SAMEQTY_FLAG_ZWS,                                                                
                   N.ONE_WAFER_PT_SCANNER, N.SCANNER_SLOPE_NEW, N.SCANNER_INTER_NEW                                                                                                     
            FROM  TYPE21_LR_SECOND2_SCANNER N                                                                                                                                           
            WHERE NOT EXISTS (SELECT 1                                                                                                                                                  
                   FROM TYPE21_LR_SECOND_PA_PB_0_SCANNER PAB                                                                                                                            
                   WHERE N.MODULE_ID = PAB.MODULE_ID                                                                                                                                    
                   AND N.TOOLG_ID = PAB.TOOLG_ID                                                                                                                                        
                   AND N.TOOL_ID = PAB.TOOL_ID                                                                                                                                          
                   AND N.PPID = PAB.PPID )                                                                                                                                              
       ),                                                                                                                                                                               
       TYPE21_LR_SECOND_RESULT_SCANNER AS (                                                                                                                                             
             SELECT N3.MODULE_ID, N3.TOOLG_ID, N3.TOOL_ID, N3.PPID, N3.LESSTHAN20_FLAG,                                                                                                 
                   N3.SAMEQTY_FLAG_ZWS , N3.PA_PB_0_FLAG_ZWS, N3.ONE_WAFER_PT_SCANNER, N3.SCANNER_SLOPE_NEW, N3.SCANNER_INTER_NEW
             FROM TYPE21_LR_SECOND3_SCANNER N3                                                                                                                                          
             UNION                                                                                                                                                                      
             SELECT PAB.MODULE_ID, PAB.TOOLG_ID, PAB.TOOL_ID, PAB.PPID, PAB.LESSTHAN20_FLAG,                                                                                            
       ...
       FROM TYPE21_LR_SECOND_RESULT_SCANNER S                                                                                                                                           
       INNER JOIN TYPE21_LR_SECOND_RESULT T ON S.MODULE_ID = T.MODULE_ID AND S.TOOLG_ID = T.TOOLG_ID AND S.TOOL_ID = T.TOOL_ID AND S.PPID = T.PPID
    """.format(tempdb=my_duck.get_temp_table_mark())

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_MID_TYPE21_RUNTIME_P_TS",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_TYPE21_RUNTIME_P_TS")

    # Photo 其他机群不需要算线性回归(PT, NPT 均为 0)
    sql = """
     INSERT /*+ append */ INTO {tempdb}APS_MID_TYPE21_RUNTIME_P_ETC
     SELECT A.MODULE_ID, A.TOOLG_ID, A.TOOL_ID, A.PPID
          , '' AS LESSTHAN20_FLAG
          , '' AS SCANNER_ZWS_FLAG                                                
          , '' AS TRACKER_ZWS_FLAG                                                
          , 'ETC' AS PHOTO_TYPE                                                   
     ...
     AND A.MODULE_ID = 'PHOTO'                                                    
     AND NOT EXISTS (SELECT 1 FROM {tempdb}APS_MID_TYPE21_RUNTIME_P_TS TS                     
                         WHERE A.MODULE_ID = TS.MODULE_ID                         
                         AND A.TOOLG_ID = TS.TOOLG_ID                             
                         AND A.TOOL_ID = TS.TOOL_ID                               
                         AND A.PPID = TS.PPID                                     
                       )                                                          
     GROUP BY A.MODULE_ID, A.TOOLG_ID, A.TOOL_ID, A.PPID                          
    """.format(tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_MID_TYPE21_RUNTIME_P_ETC",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_TYPE21_RUNTIME_P_ETC")

    # Tracker和Scanner的线性回归结果和 Photo 其他机群的线性回归结果merge
    sql = """
     INSERT /*+ append */ INTO APS_MID_TYPE21_RUNTIME_P_ALL
     SELECT '{uuid}' AS PARENTID,                                                  
            TSR.MODULE_ID, TSR.TOOLG_ID, TSR.TOOL_ID, TSR.PPID, TSR.LESSTHAN20_FLAG,                           
            ...
            '{current_time}' AS UPDATE_TIME,                                     
             '' AS PARTCODE                                 
     FROM  {tempdb}APS_MID_TYPE21_RUNTIME_P_TS  TSR                                             
     UNION ALL                                                                                                                                                                     
     SELECT '{uuid}' AS PARENTID,                                                   
            ER.MODULE_ID, ER.TOOLG_ID, ER.TOOL_ID, ER.PPID, ER.LESSTHAN20_FLAG,                          
            ...
            '{current_time}'  AS UPDATE_TIME,                                     
            '' AS PARTCODE                                 
     FROM  {tempdb}APS_MID_TYPE21_RUNTIME_P_ETC  ER                                               
    """.format(uuid=uuid, current_time=current_time, tempdb=my_duck.get_temp_table_mark())
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_MID_TYPE21_RUNTIME_P_ALL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_TYPE21_RUNTIME_P_ALL")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_MID_TYPE_RUNTIME_1D_APS_MID_TYPE21_RUNTIME_P_ALL"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_TYPE21_RUNTIME_P_ALL"
    used_table_list = ['APS_TR_TYPE21']
    target_table_sql = """
        create table {}APS_MID_TYPE21_RUNTIME_P_ALL
        (
          parentid             VARCHAR(64),
          module_id            VARCHAR(64),
          toolg_id             VARCHAR(64),
    ...
    try:
        ...
        my_duck.export_result_duck_file_and_close_duck_db_memory2(duck_db_memory,
                                                                  ...
                                                                  current_time=current_time_short)
        # 写警告日志
        my_oracle.SaveAlarmLogData(oracle_conn, ETL_Proc_Name, e, target_db_file, "XETL065")
        raise e
    finally:
        oracle_conn.commit()
        oracle_conn.close()
