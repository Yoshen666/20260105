...
def GetChargeTimeSql(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
        INSERT  /*+ append */  INTO APS_TMP_CHARGE_TIME_TOOL(                              
            PARENTID, TOOL_ID, CHARGE_TIME, UPDATE_TIME, PARTCODE                                                                
         )                                                                                    
          WITH SOU_DATA AS (                                                                 
              SELECT A.TOOL_ID, 
                    QUANTILE_CONT(CAST(CHARGE_TIME AS DECIMAL),0.5) OVER(PARTITION BY TOOL_ID ) AS CHARGE_TIME                                                            
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_TMP_CHARGE_TIME_TOOL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_TMP_CHARGE_TIME_TOOL")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_TMP_CHARGE_TIME_TOOL_10M"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_TMP_CHARGE_TIME_TOOL"
    used_table_list = ['APS_TR_CHARGE_TIME']
    target_table_sql = """
    create table {}APS_TMP_CHARGE_TIME_TOOL
    (
        parentid    VARCHAR(64),
        tool_id     VARCHAR(64),
