...
def biz_method_01(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    sql = """
    INSERT /*+ append */ INTO APS_MID_TYPE22_RUNTIME_PX_ALL (              
        PARENTID,MODULE_ID, TOOLG_ID, PPID, LESSTHAN20_FLAG,                                                                                                                                                                                                                                                            
     ...
     WITH TYPE21_LR_FIRST1 AS (                                                                                                                                                         
             SELECT O.MODULE_ID, O.TOOLG_ID, O.PPID, O.QTY, O.TRACKER_PT                                                                                      
             ...
             FROM (SELECT A.MODULE_ID, A.TOOLG_ID, A.PPID,  cast(A.QTY as double)/2 as qty,                                                                                                               
      ...
      TYPE21_LR_FIRST_ZWS AS (                                                                                                                                                          
       ...
       TYPE21_LR_FIRST2 AS (                                                                                                                                                            
             ...
             FROM TYPE21_LR_FIRST1 O                                                                                                                                                    
             INNER JOIN TYPE21_LR_FIRST_ZWS Z ON O.MODULE_ID = Z.MODULE_ID AND O.TOOLG_ID = Z.TOOLG_ID  AND O.PPID = Z.PPID
             WHERE NOT EXISTS (                                                                                                                                                         
                SELECT 1 FROM TYPE21_LR_FIRST_LESSTHAN20 L20 WHERE O.MODULE_ID = L20.MODULE_ID AND O.TOOLG_ID = L20.TOOLG_ID  AND O.PPID = L20.PPID
             )                                                                                                                                                                          
             AND NOT EXISTS (                                                                                                                                                           
                SELECT 1 FROM TYPE21_LR_FIRST_SAMEQTY SQ WHERE O.MODULE_ID = SQ.MODULE_ID AND O.TOOLG_ID = SQ.TOOLG_ID AND O.PPID = SQ.PPID
             )                                                                                                                                                                          
       ),                                                                                                                                                                               
       TYPE21_LR_FIRST3 AS(                                                                                                                                                             
       ...
       TYPE21_LR_SECOND2 AS (                                                                                                                                                           
              SELECT  N.MODULE_ID, N.TOOLG_ID, N.PPID, N.QTY, N.TRACKER_PT, N.ONE_WAFER_PT_TRACKER                                                                           
               ...
               WHERE NOT EXISTS (                                                                                                                                                       
                 SELECT 1 FROM TYPE21_LR_SECOND_LESSTHAN20 L20 WHERE N.MODULE_ID = L20.MODULE_ID AND N.TOOLG_ID = L20.TOOLG_ID AND N.PPID = L20.PPID
               )                                                                                                                                                                        
               AND NOT EXISTS (                                                                                                                                                         
                 SELECT 1 FROM TYPE21_LR_SECOND_SAMEQTY SQ WHERE N.MODULE_ID = SQ.MODULE_ID AND N.TOOLG_ID = SQ.TOOLG_ID AND N.PPID = SQ.PPID
               )                                                                                                                                                                        
       ),                                                                                                                                                                               
       ...
       SELECT '{uuid}' AS PARENTID, 
              t.MODULE_ID, t.TOOLG_ID, t.PPID, t.LESSTHAN20_FLAG,                                                                                                                                                                                                                                                            
    ...
    my_duck.exec_sql(oracle_conn=oracle_conn,
                     ...
                     methodName="Insert Into APS_MID_TYPE22_RUNTIME_PX_ALL",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_MID_TYPE22_RUNTIME_PX_ALL")
...
def execute():
    ...
    ###############################################################
    ETL_Proc_Name = "APS_ETL_BR.APS_MID_TYPE_RUNTIME_1D_APS_MID_TYPE22_RUNTIME_PX_ALL"
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    target_table = "APS_MID_TYPE22_RUNTIME_PX_ALL"
    used_table_list = ['APS_TR_TYPE22']
    target_table_sql = """
    create table {}APS_MID_TYPE22_RUNTIME_PX_ALL
    (
        parentid varchar(64),
