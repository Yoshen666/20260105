...
def process_main_business_logic(duck_db_memory, uuid, current_time, oracle_conn, ETL_Proc_Name):
    ...
    """
    sql = """
    INSERT INTO APS_ETL_LOT_NOPATH_WHITE_LIST
    (PARENTID, LOT_ID, OPE_NO, LAYER, STAGE, UPDATE_TIME)
    SELECT 
        '{uuid}' AS PARENTID,
        T1.LOT_ID,
        T1.OPE_NO,
        SPLIT_PART(T1.OPE_NO, '.', 1) AS LAYER,
        SPLIT_PART(T1.OPE_NO, '.', 2) AS STAGE,
        '{current_time}' AS UPDATE_TIME
    FROM APS_SYNC_LOT_NOPATH_WHITE_LIST.APS_SYNC_LOT_NOPATH_WHITE_LIST T1
    """.format(uuid=uuid, current_time=current_time)

    my_duck.exec_sql(oracle_conn=oracle_conn,
                     duck_db_memory=duck_db_memory,
                     ETL_Proc_Name=ETL_Proc_Name,
                     methodName="Insert Into APS_ETL_LOT_NOPATH_WHITE_LIST",
                     sql=sql,
                     current_time=current_time,
                     update_table="APS_ETL_LOT_NOPATH_WHITE_LIST")
...
def execute():
    ...
    # TODO: 修改以下参数
    ETL_Proc_Name = "APS_ETL_BR.APS_ETL_LOT_NOPATH_WHITE_LIST_5M"  # 修改ETL名称
    current_time = my_date.date_time_second_str()
    current_time_short = my_date.date_time_second_short_str()
    uuid = my_oracle.UUID()

    # TODO: 修改目标表名
    target_table = "APS_ETL_LOT_NOPATH_WHITE_LIST"

    # TODO: 修改来源表列表 (通常是APS_SYNC_xxxx)
    used_table_list = ['APS_SYNC_LOT_NOPATH_WHITE_LIST']

    # TODO: 修改目标表结构定义
    target_table_sql = """
        create table {}APS_ETL_LOT_NOPATH_WHITE_LIST
        (
    ...
    try:
        ...
        if config.g_copy_to_pg and my_runner.judge_main_server(oracle_conn):
            ...
            select_sql_in_duck = """select parentid,
                                     ...
                                     from APS_ETL_LOT_NOPATH_WHITE_LIST
                                 """

            postgres_table_define = """etl_white_list(parentid,LOT_ID,OPE_NO,LAYER,STAGE,update_time,ver_timekey)
                                    """

            my_postgres.copy_duckdb_to_postgres(uuid=uuid,
                                                duckdb=duck_db_memory,
                                                table_name_in_duckdb=target_table,
                                                table_name_in_pg="etl_white_list",  # 要小写
