...
def HandlingVerControl_backup(conn, Uuid, TableName, FileName, StartTime,host_name):
    # 测试模式的时候，向本地数据库记录
...
def duck_db_to_oracle(table_name,csv_delimiter):
    ...
    duck_db = None
    current_time_short = my_date.date_time_second_short_str()
    tmpName = "WRITE_BACK_TO_ORACLE"
    try:
        dbcursor = oracle_conn.cursor()
        if not my_runner.judge_main_server(oracle_conn):
            if oracle_conn:
                logging.info("当前服务器不是主服务器,系统取消oracle回写功能")
            return

        # 从oracle数据库获取对应etl job的duckdb文件绝对路径
        # 2023-12-04 將my_file模板的get_last_db_file方法替換成my_oracle.get_last_create_file方法配合ha框架
        target_file_name = my_oracle.get_last_create_file(oracle_conn, table_name)
        duck_db = duckdb.connect(database=':memory:')
        # attach duckdb文件到内存中
        sql = "ATTACH '{file_name}' AS {table_name} (READ_ONLY) ".format(file_name=target_file_name,
                                                                         table_name=table_name)
        duck_db.sql(sql)
        ...
        if result is not None:
            ...
            # 数据为0 也需要写入版本，方便监控
            parentid = my_oracle.UUID()
            service_names = my_oracle.GetAllServiceName(conn=oracle_conn)
            for service_name in service_names:
                HandlingVerControl_backup(conn=oracle_conn, Uuid=parentid,
                                          TableName='write_back_to_oracle_' + table_name,
                                          FileName=target_file_name, StartTime=current_time_short,
                                          host_name=service_name[0])
            return
        ver_sql = f"""
        select * from aps_etl_ver_control tt where tt.id = '{parentid}' and tt.table_name = '{table_name}' 
        """
        dbcursor.execute(ver_sql)
        ...
        if part_name:
            ...
            if result is not None:
                ...
                raise Exception(f"oracle数据库不存在表名{table_name},duckdb回写oracle数据库失败！")


            data_columns_sql = select_columns.replace("PARTCODE", "'"+part_code +"' as PARTCODE")
            if table_name=="APS_ETL_RLS":
                data_columns_sql = data_columns_sql.replace("RETICLE_ID", "case when reticle_id =' ' then 'N/A' ELSE reticle_id end RETICLE_ID")
                data_columns_sql = data_columns_sql.replace("TOOL_ID","case when TOOL_ID =' ' then 'N/A' ELSE TOOL_ID end TOOL_ID")
                data_columns_sql = data_columns_sql.replace("RECIPE,","case when RECIPE =' ' then 'N/A' ELSE RECIPE end RECIPE,")
                data_columns_sql = data_columns_sql.replace("PPID", "case when PPID =' ' then 'N/A' ELSE PPID end PPID")
                data_columns_sql = data_columns_sql.replace("LOT_ID","case when LOT_ID =' ' then 'N/A' ELSE LOT_ID end LOT_ID")
                data_columns_sql = data_columns_sql.replace("TARGET_STEP_ID","case when TARGET_STEP_ID =' ' then 'N/A' ELSE TARGET_STEP_ID end TARGET_STEP_ID")
                data_columns_sql = data_columns_sql.replace("TARGET_PLAN_ID","case when TARGET_PLAN_ID =' ' then 'N/A' ELSE TARGET_PLAN_ID end TARGET_PLAN_ID")
            ...
            import_result = my_cmder.create_sqlldr_ctlfile(table_name,csv_delimiter,select_columns,csv_file_name,'')
        ...
        # 在oracle数据库中回写版本信息
        if import_result == 0 or import_result == 2:
            HandlingVerControlOracle(conn=oracle_conn, Uuid=parentid, TableName=table_name, Partcode=part_code)
            # HandlingVerControl(conn=oracle_conn, Uuid=parentid, TableName='write_back_to_oracle_'+ table_name,FileName=target_file_name,StartTime=current_time_short)
            service_names = my_oracle.GetAllServiceName(conn=oracle_conn)
            for service_name in service_names:
                HandlingVerControl_backup(conn=oracle_conn, Uuid=parentid, TableName='write_back_to_oracle_' + table_name,
                               FileName=target_file_name, StartTime=current_time_short,host_name=service_name[0])

        logging.info("""Write back success:{table_name}""".format(table_name=table_name))
    except Exception as e:
        logging.error("""Table_Name: {table_name};   {e}""".format(table_name=table_name,e=e))
        print("""Table_Name: {table_name};  {e}""".format(table_name=table_name,e=e))
        msg = tmpName + "异常： " + table_name + " " + str(e)
        # 写警告日志
...
def execute_flow():
    ...
    duck_db_to_oracle(table_name=table_name, csv_delimiter='^~>')
...
def execute_rls():
    ...
    msg = """
               导出Oracle完成，用时 {exec_time} 秒.
            """.format(table_name_in_duckdb=table_name, exec_time=exec_time)
    print(msg)
