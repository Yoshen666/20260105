...
def execute():
    ...
    ETL_Proc_Name = "COPY_LOTHISTORY_FOR_BACKUP"
    try:
        # 每小时58min执行
        ...
        if not my_runner.judge_backup_server_status(oracle_conn, config.other_ip):
            ...
            table_name = "APS_MID_OCS_JOB_GROUP_PO_STATE_H_VIEW"
            backup_folder = "\\\{other_ip}\workspace\SrcData".format(other_ip=config.other_ip)
            ...
            if file_full_path is not None and file_full_path != '':
                ...
                my_oracle.SaveAlarmLogData(conn=oracle_conn, ETLProcName="ETLProcCopyOcsJobStateH", Exception=e, file_name=None,
                                           alarm_code=cons_error_code.APS_COPY_BACKUP_CODE_XX_ETL)
                # 写完成日志
                my_oracle.EndCleanUpAndLog(oracle_conn, ETL_Proc_Name, current_time)
    except Exception as copyException:
        logging.error("COPY_OCS_JOB_STATE_H_FOR_BACKUP 异常：" + str(copyException))
        my_oracle.SaveAlarmLogData(conn=oracle_conn, ETLProcName="ETLProcCopyOcsJobStateH", Exception=copyException, file_name=None,
                                   alarm_code=cons_error_code.APS_COPY_BACKUP_CODE_XX_ETL)
    finally:
        if oracle_conn:
            oracle_conn.close()
