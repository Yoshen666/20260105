...
def execute():
    ...
    ETL_Proc_Name = "COPY_ETL_FOR_BACKUP"
    try:
        ...
        etl_backup_folder = "\\\{other_ip}\workspace\ETLOutput".format(other_ip=config.other_ip)
        sync_backup_folder = "\\\{other_ip}\workspace\SrcData".format(other_ip=config.other_ip)
        ...
        logging.error("COPY_ETL_FOR_BACKUP 异常："+ str(copyException))
