...
def copy_duckdb_to_postgres(uuid, duckdb,
    ...
    try:
        ...
        if duck_row > 0:
            ...
            # 单独获取PG服务器的地址位置，并把文件copy过去
            target_path_file = my_oracle.GetPgBackUpHostNameData(oracle_conn)

            copy_cmder = "xcopy {source} {target} /Y".format(source=csv_file_name, target=target_path_file)
            logging.info(copy_cmder)
            my_cmder.exec(copy_cmder)

            _end_time = time.time()
            exec_time = _end_time - _start_time
            msg = """xcopy > {target_path} 用时 {exec_time} 秒.""".format(exec_time=exec_time, target_path=target_path_file)
            logging.info(msg)
            print(msg)
            _start_time = time.time()
...
def copy_to_yth_func(export_to_pg_prod_test, postgres_table_define, csv_file_name, csv_delimiter, table_name_in_pg, current_time, uuid):
    ...
    try:
        ...
        msg = tmpName + "异常： " + table_name_in_pg + " " + str(e)
