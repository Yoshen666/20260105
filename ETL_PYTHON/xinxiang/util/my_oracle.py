...
def override_HandlingVerControl(conn, Uuid, TableName, FileName, StartTime):
    # 测试模式的时候，向本地数据库记录
    if config.g_debug_mode:
        conn = oracle_get_connection_local()

    dbcursor = None
    host_name = socket.gethostbyaddr(config.other_ip)[0].split(".")[0].upper()
    try:
...
def Monitor_HandlingVerControl(conn, Uuid, TableName, FileName, StartTime):
    # 测试模式的时候，向本地数据库记录
    if config.g_debug_mode:
        conn = oracle_get_connection_local()
    ...
    try:
        timeStr = my_date.date_time_second_short_str()
        sql = """
        INSERT INTO APS_ETL_MONITOR_DUCK
        ( ID, MODULE_ID, TOOLG_ID, TABLE_NAME, UPDATE_TIME, UPDATE_USER, FILE_NAME, START_TIME, SERVER_NAME, ETL_END_TIME)
        VALUES('{}', '' , '', '{}', '{}', '{}', '{}', '{}', '{}','{}') 
        """.format(
            Uuid,
            TableName,
...
def Update_ETL_HandlingVerControl(conn, Uuid, TableName, FileName, StartTime):
    # 测试模式的时候，向本地数据库记录
    if config.g_debug_mode:
        conn = oracle_get_connection_local()
    ...
    try:
        timeStr = my_date.date_time_second_short_str()
        sql = """
        UPDATE aps_etl_ver_control_duck 
        SET ETL_END_TIME = '{}'
        WHERE ID = '{}'
        """.format(
            timeStr,
        ...
        dbcursor.execute(sql)
        sql = """
        UPDATE APS_ETL_SYNC_CONTROL_DUCK 
        SET ETL_END_TIME = '{}'
        WHERE ID = '{}'
        """.format(
            timeStr,
...
def Update_PG_HandlingVerControl(conn, Uuid, TableName, FileName, StartTime):
    # 测试模式的时候，向本地数据库记录
    if config.g_debug_mode:
        conn = oracle_get_connection_local()
...
def StartCleanUpAndLogUuid(conn, ETLProcName, ETLStartTime, UuidName, EndMessage="OK"):
    # 测试模式的时候，向本地数据库记录
    if config.g_debug_mode:
        conn = oracle_get_connection_local()
    ...
    current_time = my_date.date_time_second_str()
    sql = """
        INSERT INTO ETL_EXEC_LOG (ETL_NAME, START_TIME, END_TIME, DURATION, REC_COUNT, END_MESSAGE, EVENT_NAME, partkey, PARENTID) 
        VALUES('{ETLProcName}', TIMESTAMP '{ETLStartTime}', TIMESTAMP '{ETLStartTime}' , 0,  -1, '{EndMessage}', 'PSTART', TO_DATE('{current_time}','YYYY-MM-DD HH24:MI:SS'),'{UuidName}' )
        """.format(
        ETLProcName=ETLProcName,
        ETLStartTime=ETLStartTime,
        EndMessage=EndMessage,
...
def EndCleanUpAndLogUuid(conn, ETLProcName, ETLStartTime, UuidName, EndMessage="OK"):
    # 测试模式的时候，向本地数据库记录
    if config.g_debug_mode:
        conn = oracle_get_connection_local()
    ...
    sql = """
        INSERT INTO ETL_EXEC_LOG (ETL_NAME, START_TIME, END_TIME, DURATION, REC_COUNT, END_MESSAGE, EVENT_NAME, partkey, PARENTID) 
        VALUES('{ETLProcName}', TIMESTAMP '{ETLStartTime}', TIMESTAMP '{_end_str}' , {dur},  -1, '{EndMessage}', 'PEND', TO_DATE('{_end_str}','YYYY-MM-DD HH24:MI:SS'),'{UuidName}' )
        """.format(
        ETLProcName=ETLProcName,
        ETLStartTime=ETLStartTime,
        _end_str=_end_str,
        ...
        sql = """
                insert into alarm_send_log
                                   ...
                                   select s.code,
                                          s.module,
                                          s.level_id,
                                          s.msg_cn || ' {ETLProcName} ' || ' {exception}',
                                          '0',
                                          sysdate,
                                          'SYNC ETL',
                                          '{job_msg}',
                ...
                """.format(job_msg=job_msg, alarm_code=alarm_code, exception=exception,ETLProcName=ETLProcName)
    else:
        sql = """
                insert into alarm_send_log
                                   (code,
...
def SaveAlarmLogData(conn, ETLProcName, Exception, file_name, alarm_code):
    ...
    exception = str(Exception)
    # print(my_runner.judge_main_server(conn))
    if not my_runner.judge_main_server(conn):
        sql = """
                insert into alarm_send_log
                                   (code,
...
def GetAllServiceName(conn):
    sql = """
            select srv_name from APS_ETL_DETL_SERVICE
          """
    dbcursor = None
    try:
        dbcursor = conn.cursor()
...
def DropOldPartition(conn, TableName, interval):
    sql = """
         SELECT COUNT(*) AS ROW_NUM                         
         FROM USER_OBJECTS A                                
         WHERE 1=1                                          
         AND A.OBJECT_NAME = '{tableName}'            
         AND A.OBJECT_TYPE = 'TABLE PARTITION'              
         AND A.CREATED > SYSDATE - '{interval}' 
    """.format(tableName=TableName, interval=interval)

    sql2 = """
         SELECT A.OBJECT_NAME, A.SUBOBJECT_NAME, A.CREATED  
         FROM USER_OBJECTS A                                
         WHERE 1=1                                          
         AND A.OBJECT_NAME = '{tableName}'             
         AND A.OBJECT_TYPE = 'TABLE PARTITION'              
    ...
    try:
        ...
        result = dbcursor.fetchall()
        for data_r in result:
            print(data_r)
            sql="""
                ALTER TABLE {tableName} DROP PARTITION ({SUBOBJECT_NAME}) UPDATE INDEXES 
            """.format(tableName=data_r[0],SUBOBJECT_NAME=data_r[1])
            dbcursor.execute(sql)
    except Exception as e:
        log = TableName + ": " +str(e)
        print(log)
        ...
        if dbcursor:
            dbcursor.close()
        logging.info(TableName + " DROP PARTITION IS SUCCESS")


# 获取所有备库需要copy的数据
def get_all_copy_file(conn):
    if config.g_debug_mode:
        conn = oracle_get_connection_local()
    ...
    WITH LOG_DATA AS (
        SELECT D.*,
               ROW_NUMBER() OVER(PARTITION BY TABLE_NAME ORDER BY UPDATE_TIME DESC ) AS RNT
        FROM aps_etl_ver_control_duck D
        WHERE D.SERVER_NAME='{server_name}'
        AND D.UPDATE_TIME > TO_CHAR(SYSDATE - 1,'YYYYMMDDHH24MISS')
        AND D.UPDATE_USER='CIMP'
        AND INSTR(TABLE_NAME,'write_back_to_oracle') = 0
        AND TABLE_NAME <> 'CSFRINHIBIT'
        AND INSTR(TABLE_NAME,'_VIEW') =0
        AND TABLE_NAME<>'APS_MID_PH_LOTHISTORY'
         AND TABLE_NAME<>'APS_MID_PH_LOTHISTORY_A3'
        AND NOT EXISTS(
            SELECT 1 
            FROM aps_etl_ver_control_duck K
            WHERE K.SERVER_NAME<>'{server_name}'
            AND K.TABLE_NAME = D.TABLE_NAME
            AND K.UPDATE_USER='CIMP'
            AND K.FILE_NAME = D.FILE_NAME
            AND K.UPDATE_TIME > TO_CHAR(SYSDATE - 2,'YYYYMMDDHH24MISS')
        )
    )
...
def sendTempAlarm(oracle_con, alarmMsg, alarm_code):

    cursor = None
    try:
        if config.g_debug_mode:
            oracle_con = oracle_get_connection_local()

        hostname = socket.gethostname()
        hostAddress = socket.gethostbyname(hostname)
        msgCode = alarmMsg + " HostName : " + hostname + ", HostAddress : " + hostAddress
            ...
            sql = """
                    insert into alarm_send_log
                                       ...
                                       select s.code,
                                         ...
                                         from set_alarm_code_mapping s
                                        where s.code =  '{alarm_code}'
                    """.format(job_msg=msgCode, alarm_code=alarm_code, msg=alarmMsg)
        else:
            sql = """
                    insert into alarm_send_log
                                       (code,
        ...
        # sql_cmd = " INSERT INTO ALARM_SEND_LOG(CODE, MODULE, LEVEL_ID, MSG, SEND_ALARM, DISPATCH_TIME, JOB, JOB_MSG) VALUES ('XETL0004','XX_ETL','L2', '%s', 1, TO_DATE('%s','YYYYMMDD HH24MISS'), '%s', '%s') " % (
        # alarmMsg,DETL_END_TIME, tempName, msgCode)
...
def TEST_HandlingVerControl(conn, Uuid, TableName, FileName, StartTime):
    # 测试模式的时候，向本地数据库记录
    if config.g_debug_mode:
        conn = oracle_get_connection_local()

    dbcursor = None
    host_name = socket.gethostbyaddr(config.test_ip)[0].split(".")[0].upper()
    try:
        timeStr = my_date.date_time_second_short_str()
        sql = """
                INSERT INTO aps_etl_ver_control_duck
...
# 获取所有备库需要copy的数据
def get_all_last_copy_file(conn):
    if config.g_debug_mode:
        conn = oracle_get_connection_local()
...
def GetPgBackUpHostNameData(conn):
    try:
        sqlQuery = " select C.CURRENT_HOST_SERVER  from V_HA_CLUSTER C "
        print(sqlQuery)
        with conn.cursor() as cursor:
            cursor.execute(sqlQuery)
            result = cursor.fetchone()
            # 显示完整路径类似：'\\\\A2SCHDBP02\\ETLOutput'
            return '\\\\' + result[0] + '\\ETLOutput' if result else 0
    except Exception as e:
        traceback.print_exc()
        logging.error(traceback.format_exc())